...
// memfd_create shim (works even on older bionic headers)
#ifndef MFD_CLOEXEC
#define MFD_CLOEXEC 0x0001U
#endif
static inline int stealth_memfd_create(const char* name, unsigned int flags) {
...
static const char* emu_file_blacklist[] = {
    // Emulator configs
    "init.vbox86.rc", "init.vbox64.rc", "init.pscloud.rc", "init.dundi.rc",
    "init.rockchip.rc", "init.rk30board.rc", "init.hi3660.rc",
    "init.androidVM_x86.rc", "init.android_x86.rc",
    "fstab.vbox86", "fstab.vbox64",
    "ueventd.vbox64.rc",
    // Emulator libraries
    "libKaopuSdk.so", "libc_malloc_debug_qemu.so", "libc_malloc_debug_qemu.so-arm",
    "libldutils.so", "libdroidbox-ril.so", "libnoxd.so",
    "libhotx612.so", "libhotx711.so",
    // Emulator properties
    "ttVM-prop", "microvirt-prop", "XCPlayer-prop", "xiaopiVM-prop",
    "androVM-prop", "nemuVM-prop", "nox-prop", "droid4x-prop", "x86.prop",
    // Emulator applications
    "LauncherEx.apk", "com.android.ld.appstore",
    "com.ldmnq.launcher3", "com.bluestacks.appmart",
    // Emulator devices
    "dev/awd", "goldfish_sync", "goldfish",
    "virtpipe", "virtpipe-common", "virtpipe-common-syzsaow", "virtpipe-render",
    "virtpipe-sec", "dev/wanbai",
    "genyd",
    // QEMU
    "qemu", "qemu_pipe", "qemu_trace",
    NULL
};

static const char* emu_maps_blacklist[] = {
    "libhoudini", "libndk_translation", "libnb_houdini",
    "libhotx612", "libhotx711",
    "libc_malloc_debug_qemu",
    "libKaopuSdk", "libldutils", "libdroidbox", "libnoxd",
    "qemu", "vbox", "goldfish", "virtpipe",
    "gralloc.vbox86", "gralloc.ranchu",
    "noxd", "noxspeedup",
    NULL
};

static const char* emu_prop_blacklist[] = {
    "vbox", "qemu", "goldfish", "nox", "genymotion", "andy",
    "bluestacks", "bst.", "ttVM", "microvirt", "droid4x",
    "init.svc.vbox86", "init.svc.vbox", "init.svc.qemu",
    "ro.kernel.qemu", "ro.hardware.virtual",
    NULL
};

static const char* emu_readdir_blacklist[] = {
    "goldfish", "virtpipe", "qemu", "awd", "vbox",
    NULL
};

static bool is_emu_path(const char* path) {
    if (!path) return false;
    for (int i = 0; emu_file_blacklist[i]; i++) {
        if (strstr(path, emu_file_blacklist[i])) return true;
    }
    return false;
}

static bool is_emu_maps_line(const char* line) {
    if (!line) return false;
    for (int i = 0; emu_maps_blacklist[i]; i++) {
        if (strstr(line, emu_maps_blacklist[i])) return true;
    }
    return false;
}

static bool is_emu_prop(const char* name) {
    if (!name) return false;
    for (int i = 0; emu_prop_blacklist[i]; i++) {
        if (strstr(name, emu_prop_blacklist[i])) return true;
    }
    return false;
}

static bool is_emu_dirent(const char* name) {
    if (!name) return false;
    for (int i = 0; emu_readdir_blacklist[i]; i++) {
        if (strstr(name, emu_readdir_blacklist[i])) return true;
    }
    return false;
}

static std::mutex fake_maps_mutex;
static char fake_maps_path[256] = {0};
static std::string cached_pkg_name;

static void ensure_pkg_name() {
    if (cached_pkg_name.empty()) {
        char cmdline[256] = {0};
        ...
        if (fp) {
            ...
            if (bytes > 0) {
                ...
                cached_pkg_name = name;
            }
        }
        if (cached_pkg_name.empty())
            cached_pkg_name = "com.pubg.imobile";
    }
}

static void generate_fake_maps() {
    std::lock_guard<std::mutex> lock(fake_maps_mutex);
    ensure_pkg_name();

    FILE* real = fopen("/proc/self/maps", "r");
    if (!real) { LOGE(HIDE_STR("[FakeMaps] Failed to open /proc/self/maps")); return; }

    snprintf(fake_maps_path, sizeof(fake_maps_path),"/data/data/%s/.fmaps", cached_pkg_name.c_str());
             

    FILE* fake = fopen(fake_maps_path, "w");
    if (!fake) { LOGE(HIDE_STR("[FakeMaps] Failed to create %s"), fake_maps_path); fclose(real); return; }
    ...
    while (fgets(line, sizeof(line), real)) {
        total++;
        if (is_emu_maps_line(line)) {
            filtered++;
            if (first_run) {
                LOGI(HIDE_STR("[FakeMaps] FILTERED: %s"), line);
            }
        } else {
            fputs(line, fake);
        }
    ...
    fclose(fake);
    if (first_run) {
        LOGI(HIDE_STR("[FakeMaps] Done: %d total lines, %d filtered, saved to %s"), total, filtered, fake_maps_path);
        first_run = false;
    }
}


int (*o_system_property_get)(const char* name, char* value);
int hk_system_property_get(const char* name, char* value) {
    int ret = o_system_property_get(name, value);
    if (is_emu_prop(name)) {
        //LOGI(HIDE_STR("[FakeProp] Blocked prop by name: %s = %s"), name, value);
        value[0] = '\0';
        return 0;
    }
    if (ret > 0 && value) {
        if (strstr(value, "vbox") || strstr(value, "goldfish") ||
            strstr(value, "qemu") || strstr(value, "nox") ||
            strstr(value, "genymotion") || strstr(value, "andy")) {
            //LOGI(HIDE_STR("[FakeProp] Blocked prop by value: %s = %s"), name, value);
            value[0] = '\0';
            return 0;
        }
    }
...
static size_t anogs_size_cache = 0;
static bool is_from_anogs_safe(void* ret) {
    uintptr_t addr = (uintptr_t)ret;
    return libanogsheader && anogs_size_cache &&
           addr >= libanogsheader && addr < (libanogsheader + anogs_size_cache);
}

// Track fds that point to our fake maps file so we can spoof fstat
static std::mutex maps_fd_mutex;
static std::unordered_set<int> fake_maps_fds;

static void track_fake_fd(int fd) {
    std::lock_guard<std::mutex> lock(maps_fd_mutex);
    fake_maps_fds.insert(fd);
}

static void untrack_fake_fd(int fd) {
    std::lock_guard<std::mutex> lock(maps_fd_mutex);
    fake_maps_fds.erase(fd);
}

static bool is_fake_maps_fd(int fd) {
    std::lock_guard<std::mutex> lock(maps_fd_mutex);
    return fake_maps_fds.count(fd) > 0;
}

// ---- Stealth state: our own module range + a real procfs stat snapshot ----
static uintptr_t g_self_start = 0, g_self_end = 0;
static struct stat g_maps_stat;
static bool       g_maps_stat_ok = false;

// Called once from apply_all_libc_hooks(). Cheap, runs before any scan matters.
static void init_stealth_state() {
    // Snapshot real /proc/self/maps stat so hk_fstat can mirror it exactly.
    g_maps_stat_ok = (stat("/proc/self/maps", &g_maps_stat) == 0);

    // Bound our own injected .so so we can scrub it from the served map.
    Dl_info info;
    if (dladdr((void*)&init_stealth_state, &info) && info.dli_fname) {
        FILE* fp = fopen("/proc/self/maps", "r");
        if (fp) {
            char l[512];
            while (fgets(l, sizeof(l), fp)) {
                uintptr_t s, e;
                if (sscanf(l, "%" SCNxPTR "-%" SCNxPTR, &s, &e) == 2 &&
                    strstr(l, info.dli_fname)) {
                    if (g_self_start == 0 || s < g_self_start) g_self_start = s;
                    if (e > g_self_end) g_self_end = e;
                }
            }
            fclose(fp);
        }
...
// Drop emulator lines AND our own module's mappings from the served map.
static bool should_drop_maps_line(const char* line) {
    if (is_emu_maps_line(line)) return true;
    if (g_self_start && g_self_end) {
        uintptr_t s, e;
        if (sscanf(line, "%" SCNxPTR "-%" SCNxPTR, &s, &e) == 2 &&
            s >= g_self_start && e <= g_self_end)
            return true;
    }
    return false;
}
...
static int build_fake_maps_memfd() {
    std::lock_guard<std::mutex> lock(fake_maps_mutex);

    FILE* real = fopen("/proc/self/maps", "r");
    if (!real) return -1;

    int mfd = stealth_memfd_create("dalvik-jit-code-cache", MFD_CLOEXEC);
    if (mfd < 0) { fclose(real); return -1; }   // -> caller uses disk path

    char line[512];
    while (fgets(line, sizeof(line), real)) {
        if (!should_drop_maps_line(line)) {
            size_t n = strlen(line);
            if (write(mfd, line, n) != (ssize_t)n) { /* best-effort */ }
        }
    }
    fclose(real);
...
// fstat hook - make fake maps fd look like a genuine procfs handle
int (*o_fstat)(int fd, struct stat* statbuf);
int hk_fstat(int fd, struct stat* statbuf) {
    int ret = o_fstat(fd, statbuf);
    if (ret == 0 && is_fake_maps_fd(fd)) {
        if (g_maps_stat_ok) {
            // Mirror real /proc/self/maps: procfs st_dev, nonzero st_ino, size 0.
            statbuf->st_dev  = g_maps_stat.st_dev;
            statbuf->st_ino  = g_maps_stat.st_ino;
            statbuf->st_mode = g_maps_stat.st_mode;
        } else {
...
// readlink spoof - a fake maps fd must resolve to /proc/self/maps, not /memfd:* or /data/*
static ssize_t spoof_fd_readlink(const char* path, char* buf, size_t bufsiz) {
    int fd = -1;
    if (path && sscanf(path, "/proc/self/fd/%d", &fd) == 1 && is_fake_maps_fd(fd)) {
        static const char* target = "/proc/self/maps";
        size_t n = strlen(target);
        if (n > bufsiz) n = bufsiz;
        memcpy(buf, target, n);
        return (ssize_t)n;
    }
    return -2; // sentinel: not a fake fd, let caller fall through
}

ssize_t (*o_readlink)(const char* path, char* buf, size_t bufsiz);
ssize_t hk_readlink(const char* path, char* buf, size_t bufsiz) {
    ssize_t s = spoof_fd_readlink(path, buf, bufsiz);
    if (s != -2) return s;
    return o_readlink(path, buf, bufsiz);
}

ssize_t (*o_readlinkat)(int dirfd, const char* path, char* buf, size_t bufsiz);
ssize_t hk_readlinkat(int dirfd, const char* path, char* buf, size_t bufsiz) {
    ssize_t s = spoof_fd_readlink(path, buf, bufsiz);
    if (s != -2) return s;
    return o_readlinkat(dirfd, path, buf, bufsiz);
}

int (*o_open)(const char* pathname, int flags, ...);
int hk_open(const char* pathname, int flags, ...) {
    if (pathname) {
        if (is_emu_path(pathname)) { errno = ENOENT; return -1; }
        if (strstr(pathname, "/proc/self/maps") || strstr(pathname, "/proc/self/smaps")) {
            void* ret = __builtin_extract_return_addr(__builtin_return_address(0));
#if defined(__arm__)
            ret = (void*)((uintptr_t)ret & ~1);
#endif
            if (is_from_anogs_safe(ret)) {
                int mfd = build_fake_maps_memfd();
                if (mfd >= 0) return mfd;
                generate_fake_maps();
                if (fake_maps_path[0]) {
                    mode_t mode = 0;
                    if (flags & O_CREAT) { va_list args; va_start(args, flags); mode = va_arg(args, mode_t); va_end(args); }
                    int fd = o_open(fake_maps_path, flags, mode);
                    if (fd >= 0) track_fake_fd(fd);
                    return fd;
                }
            }
        }
...
#endif
        if (is_from_anogs_safe(ret) &&
            (strstr(pathname, "/proc/self/maps") || strstr(pathname, "/proc/self/smaps"))) {
            int mfd = build_fake_maps_memfd();
            if (mfd >= 0) return mfd;
            generate_fake_maps();
            if (fake_maps_path[0]) {
                mode_t mode = 0;
                if (flags & O_CREAT) { va_list args; va_start(args, flags); mode = va_arg(args, mode_t); va_end(args); }
                int fd = o_openat(dirfd, fake_maps_path, flags, mode);
                if (fd >= 0) track_fake_fd(fd);
                return fd;
            }
        }
...
struct dirent* hk_readdir(DIR* dirp) {
    ...
    while (entry != NULL && is_emu_dirent(entry->d_name)) {
...
int hk_open_2(const char* pathname, int flags) {
    if (pathname) {
        ...
        if (is_from_anogs_safe(ret) &&
            ...
            generate_fake_maps();
            if (fake_maps_path[0]) {
                int fd = o_open_2(fake_maps_path, flags);
                if (fd >= 0) track_fake_fd(fd);
                return fd;
            }
        }
...
FILE* hook_fopen(const char* filename, const char* mode) {
    if (filename) {
        if (is_emu_path(filename)) { errno = ENOENT; return NULL; }

        if (strstr(filename, "/proc/self/maps") || strstr(filename, "/proc/self/smaps")) {
            void* ret = __builtin_extract_return_addr(__builtin_return_address(0));
#if defined(__arm__)
            ret = (void*)((uintptr_t)ret & ~1);
#endif
            if (is_from_anogs_safe(ret)) {
                int mfd = build_fake_maps_memfd();
                if (mfd >= 0) {
                    FILE* fp = fdopen(mfd, mode ? mode : "r");
                    if (fp) return fp;          // fd stays tracked; freed on fclose->close hook
                    close(mfd);                 // fdopen failed: don't leak
                    untrack_fake_fd(mfd);
                }
                generate_fake_maps();
                if (fake_maps_path[0]) {
                    FILE* fp = o_fopen(fake_maps_path, mode);
                    if (fp) {
                        int fd = fileno(fp);
                        if (fd >= 0) track_fake_fd(fd);
                    }
...
// ioctl - return 0 (success no-op) instead of -1 (error)
...
// fcntl - return 0 (success) instead of -1 (error)
...
// write - return count (pretend success) instead of -1 (error) for emu devices
...
void apply_all_libc_hooks() {
   ...
   HOOKSYM_LIB("/system/lib/libc.so", "readlink", hk_readlink, o_readlink);
   HOOKSYM_LIB("/system/lib/libc.so", "readlinkat", hk_readlinkat, o_readlinkat);
   ...
   HOOKSYM_LIB("/system/lib/libc.so", "close", hk_close, o_close);
   HOOKSYM_LIB("/system/lib/libc.so", "fstat", hk_fstat, o_fstat);
   HOOKSYM_LIB("/system/lib/libc.so", "system", hk_system, o_system);
   HOOKSYM_LIB("/system/lib/libc.so", "__system_property_get", hk_system_property_get, o_system_property_get);
}
...
void *main_thread(void *)
{
    ...
    anogs_size_cache = getLibrarySize(HIDE_STR("libanogs.so"));

    Modules::Anogs.BaseAddr = libanogsheader;
    Modules::Anogs.Size = anogs_size_cache;
...
//    uintptr_t fake_damage = findPattern(Modules::UE4.BaseAddr, Modules::UE4.Size, SIG_FAKE_DAMAGE);
//     if (fake_damage) {
//      LOGI(HIDE_STR("fake_damage found at 0x%lx"), fake_damage - Modules::UE4.BaseAddr );
//       GlossHook((void*)fake_damage, (void*)hook_sub_468D0A8, (void**)&sub_468D0A8);
//     } else {
//          LOGE(HIDE_STR("fake_damage not found"));
//       GlossHook((void*)(libUE4header + 0x71C7E0C),(void*)hook_DisableFPS_Limits,(void**)&orig_DisableFPS_Limits);
    
//     }

 //  ShowMessageBox("Inverness", "You are using Inverness Free version\nVIP Features :\n1. 240 FPS with UHD Graphic\n2.Upto 4K resolution\n3.Inverness Internal & External\n4.Mod Skins");
