...
void patchOffset(const char *fileName, uint64_t offset, std::string hexBytes, bool isOn) {
    ...
    if (!patch.isValid()) {
        LOGE(HIDE_STR("Failing offset: 0x%llu, please re-check the hex"), offset);
        return;
    }
    if (isOn) {
        if (!patch.Modify()) {
            LOGE(HIDE_STR("Something went wrong while patching this offset: 0x%llu"), offset);
        }
    } else {
        if (!patch.Restore()) {
            LOGE(HIDE_STR("Something went wrong while restoring this offset: 0x%llu"), offset);
        }
    }
}
...
void patchOffsetSym(uintptr_t absolute_address, std::string hexBytes, bool isOn) {
    ...
    if (!patch.isValid()) {
        LOGE(HIDE_STR("Failing offset: 0x%llu, please re-check the hex"), absolute_address);
        return;
    }
    if (isOn) {
        if (!patch.Modify()) {
            LOGE(HIDE_STR("Something went wrong while patching this offset: 0x%llu"), absolute_address);
        }
    } else {
        if (!patch.Restore()) {
            LOGE(HIDE_STR("Something went wrong while restoring this offset: 0x%llu"), absolute_address);
        }
    }
}
