...
function LobbyPawn:OnEndActionHandle(actionID)
  ...
  EventSystem:postEvent(EVENTTYPE_LOBBY, EVENTID_ACTION_PALY_END, actionID)
end
...
function LobbyPawn:EnableFootLockAnim(bEnable)
  ...
  if not self.FootLockAnimPath or not self.FootLockAnimPath:IsValid() then
    print(bWriteLog and "LobbyPawn:EnableFootLockAnim FootLockAnimPath is invalid")
    return
  end
  self.AsyncLoadABPHandle = self:AsyncLoadAsset(self.FootLockAnimPath:ToString(), function(uFootLockAnim)
    self.AsyncLoadABPHandle = nil
    if not slua.isValid(uFootLockAnim) then
      print(bWriteLog and "LobbyPawn:EnableFootLockAnim failed to load FootLockAnim")
      return
    end
    if slua.isValid(self.Mesh) then
      print("LobbyPawn:EnableFootLockAnim FootLockAnim:" .. tostring(uFootLockAnim))
      self.Mesh:SetSubInstance("FootLock", uFootLockAnim.GeneratedClass)
    end
  end)
end
