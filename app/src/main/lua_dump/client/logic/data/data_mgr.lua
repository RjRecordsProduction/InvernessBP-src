...
function DataMgr.InitRoleData(roleDataTb)
  ...
  if LobbySystem.roleData.lgd_wpn_info and LobbySystem.roleData.lgd_wpn_info.selected ~= 0 and LobbySystem.roleData.lgd_wpn_info.scene_lobby == 1 then
    DataMgr.Weapon_ID = LobbySystem.roleData.lgd_wpn_info.selected
    DataMgr.Weapon_Skin_ResID = DataMgr.Weapon_ID
    DataMgr.Weapon_Skin_InsID = 0
    DataMgr.InitWeaponData(DataMgr.Weapon_ID, DataMgr.Weapon_Skin_ResID, DataMgr.Weapon_Skin_InsID)
  else
...
function DataMgr.OnRoleAttrChangeNotify(attriType, attriValue, attriTab, isClickReward)
  ...
  elseif attriType == struct_RoleAttri.attr_type_ugc_season_segment then
    log(bWriteLog and "[v_yibxu] DataMgr.OnRoleAttrChangeNotify attr_type_wow_season_segment Change")
    if attriValue then
      local logic_ugc_season = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.logic_ugc_season)
      logic_ugc_season:SetUGCSeasonData(attriValue, attriTab)
      EventSystem:postEvent(EVENTTYPE_DATA_MGR, EVNETID_DATAMGR_UGC_SEGMENT_CHANGE)
    end
  ...
  elseif attriType == struct_RoleAttri.attr_type_custom_presentation then
    if attriValue then
      ...
      logic_custom_presentation:SetData(attriValue.custom_presentation)
      LobbySystem.roleData.custom_presentation = attriValue.custom_presentation
...
function DataMgr.IsSelf(uid)
  return tostring(uid) == DataMgr.roleData.uid
end
return DataMgr