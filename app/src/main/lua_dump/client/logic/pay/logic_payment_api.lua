...
function logic_payment_api:set_Centauri_idc(idc)
  self:get_payment_sdk():SetCentauriIDC(idc)
end
...
function logic_payment_api:load_Centauri_product_info(product_list, is_subscrible)
  ...
  self:get_payment_sdk():GetProductInfo(product_map_to_load_price)
end
function logic_payment_api:load_Centauri_intro_price(product_list, is_subscrible)
  local product_map_to_load_price = {}
  local string_util = require("common.string_util")
  product_list = self:attach_special_product_for_query_price(product_list, is_subscrible)
  local product_list_arr = string_util.Split(product_list, ",")
  ...
  self:get_payment_sdk():GetIntroPrice(product_map_to_load_price)
end
function logic_payment_api:load_Centauri_mp(country, moneytary_uint)
  local app_extends = ""
  local drm_info = ""
  ...
  self:get_payment_sdk():GetMPInfo(country, moneytary_uint, app_extends, drm_info)
end
function logic_payment_api:Goods(product_id, pay_item, price, country, currency)
  ...
  local payChannel, payUrl = self:GetOptionPayParams("shop", product_id)
  self:get_payment_sdk():Goods(product_id, payItem, price, country, currency, channelExtras, appExtends, payChannel, payUrl)
end
function logic_payment_api:GoodsPresent(product_id, pay_item, price, country, currency, metaData)
  ...
  channelExtras = logic_cloud_game:ActtachPaymentChannelExtra(channelExtras)
  local payChannel, payUrl = self:GetOptionPayParams("shop", product_id)
  self:get_payment_sdk():GoodsPresent(product_id, payItem, price, country, currency, metaData, channelExtras, appExtends, payChannel, payUrl)
end
function logic_payment_api:Pay(product_id, pay_item, country, currency)
  local appExtends = self:get_common_pay_app_extends("recharge")
  local channelExtras = string.format("payItem=%d", pay_item)
  local logic_cloud_game = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.logic_cloud_game)
  channelExtras = logic_cloud_game:ActtachPaymentChannelExtra(channelExtras)
  local payChannel, payUrl = self:GetOptionPayParams("buy", product_id)
  self:get_payment_sdk():Pay(product_id, tostring(pay_item), country, currency, channelExtras, appExtends, payChannel, payUrl)
end
...
function logic_payment_api:H5Pay(country)
  ...
  self:get_payment_sdk():H5Pay(country, channelExtras, appExtends)
end
function logic_payment_api:GetOptionPayParams(pageid, product_id)
  ...
  local price_iap = self:get_product_price_by_product_id(product_id)
  local paymentUrl, parameterTb
  local directToPayPage = false
  if directToPayPage then
    local from = "sdk_buy.topup"
    paymentUrl = FuncUtil.GetDomainByID(3366245)
    if self:get_payment_sdk():GetPayEnvironment() == "test" then
      paymentUrl = FuncUtil.GetDomainByID(3366250)
    end
    parameterTb = {
      shopcode = "pubgminapp",
      region = region,
    ...
    paymentUrl = FuncUtil.GetDomainByID(3366244)
    if self:get_payment_sdk():GetPayEnvironment() == "test" then
      paymentUrl = FuncUtil.GetDomainByID(3366249)
    end
    parameterTb = {
      from = form,
      charac_name = roleNameEncoded,
...
function logic_payment_api:get_product_price_by_product_id(product_id)
