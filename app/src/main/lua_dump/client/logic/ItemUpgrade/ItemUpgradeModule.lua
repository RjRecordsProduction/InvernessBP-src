...
function ItemUpgradeModule:InitItemUpgradeConfig()
  ...
  for id, cfg in pairs(itemUpgradeCfgs) do
    if self.itemUpgradeConfigsKeyByGroupID[cfg.GroupID] == nil then
      self.itemUpgradeConfigsKeyByGroupID[cfg.GroupID] = {}
    end
    if #self.itemUpgradeConfigsKeyByGroupID[cfg.GroupID] < cfg.Level then
      table.insert(self.itemUpgradeConfigsKeyByGroupID[cfg.GroupID], cfg)
      self.itemUpgradeConfigsKeyByItemID[cfg.ItemID] = cfg
    end
  end
...
function ItemUpgradeModule:GetUpgradeGroupByID(groupID)
  ...
  if self.itemUpgradeConfigsKeyByGroupID[groupID] == nil then
    local GroupLevels = CDataTable.GetTableByFilter("ItemUpgradeConfig", "GroupID", groupID)
    if not GroupLevels then
      return nil
    end
    self.itemUpgradeConfigsKeyByGroupID[groupID] = {}
    for k, v in pairs(GroupLevels) do
      table.insert(self.itemUpgradeConfigsKeyByGroupID[groupID], v)
      self.itemUpgradeConfigsKeyByItemID[v.ItemID] = v
    end
    log(bWriteLog and "ItemUpgradeModule:GetUpgradeGroupByID groupID\239\188\154" .. tostring(groupID))
  end
  return self.itemUpgradeConfigsKeyByGroupID[groupID]
end
function ItemUpgradeModule:GetUpgradeCfg(itemID)
  ...
  local cfgData = CDataTable.GetTableDataByFilter("ItemUpgradeConfig", "ItemID", itemID)
  local GroupID = cfgData and cfgData.GroupID or 0
  if 0 < GroupID then
    self:GetUpgradeGroupByID(GroupID)
  else
    self.itemUpgradeConfigsKeyByItemID[itemID] = 0
  end
  log(bWriteLog and "ItemUpgradeModule:GetUpgradeCfg itemID:" .. tostring(itemID) .. " GroupID:" .. tostring(GroupID))
  return cfgData
end
...
function ItemUpgradeModule:GetUpgradeGroupByItemID(itemID)
  local cfg = self:GetUpgradeCfg(itemID)
  if cfg then
    return self:GetUpgradeGroupByID(cfg.GroupID)
  end
  return {}
end
...
function ItemUpgradeModule:GetAllUpgradeItemsExceptPadora()
  ...
  for groupID, cfgs in pairs(self.itemUpgradeConfigsKeyByGroupID) do
    if pandoraItemUpgradeGroupMap[groupID] == nil then
...
function ItemUpgradeModule:GetMaxLevelItem(itemID)
  ...
  if period then
    ...
    if baseInfo ~= nil then
      return LogicXSuit.GetBaseInfo(period).max_item_id
    end
  end
  return -1
end
...
function ItemUpgradeModule:upgrade_item_req(instid, group_id, item_level, item_cost)
  ...
  ItemUpGradeHandler.send_upgrade_item_req(instid, group_id, item_level, item_cost)
end
...
function ItemUpgradeModule:GetRefitGroupID(res_id)
  local cfg = self:GetUpgradeCfg(res_id) or {}
  return cfg.groupID or 0
end
...
function ItemUpgradeModule:GetNormalWeaponID(WeaponID)
  ...
  local BaseWeaponID = self:GetBaseItemID(WeaponID)
  local BaseUpgradeCfg = self:GetUpgradeCfg(BaseWeaponID)
  if BaseUpgradeCfg then
    local refitCfg = self:GetRefitCfgData(BaseUpgradeCfg.GroupID)
    if refitCfg and next(refitCfg) then
      local UpgradeCfg = CDataTable.GetTableDataByFilter("ItemUpgradeConfig", "GroupID", refitCfg.groupID, "Level", BaseUpgradeCfg.Level)
      if UpgradeCfg then
        BaseWeaponID = UpgradeCfg.ItemID
      end
    end
  end
...
function ItemUpgradeModule:IsRefershRefitMaterial(res_id)
  ...
  local isUnlock = self:IsRefitUnlockComplete(group_id, res_id)
