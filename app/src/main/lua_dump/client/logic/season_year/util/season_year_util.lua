...
function season_year_util.GetOldSeasonMaxId()
  ...
  if config then
    local startSeasonID = tonumber(config.Value)
    print(bWriteLog and string.format("season_year_util:GetOldSeasonMaxId - Calculated from startSeasonID: %d", startSeasonID - 1))
    return startSeasonID - 1
  end
  print(bWriteLog and string.format("season_year_util:GetOldSeasonMaxId - Using default value: %d", DEFAULT_OLD_SEASON_MAX_ID))
...
function season_year_util.GetOldSeasonYearMaxId()
  ...
  if currentSeasonCfg then
    ...
    print(bWriteLog and string.format("season_year_util:GetOldSeasonYearMaxId - Calculated from seasonYearID: %d", seasonYearID))
    return seasonYearID
  end
  print(bWriteLog and string.format("season_year_util:GetOldSeasonYearMaxId - Using default value: %d", DEFAULT_OLD_SEASON_YEAR_MAX_ID))
