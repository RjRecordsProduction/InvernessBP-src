...
function SeasonSystem.ShowSeasonSlap()
  ...
  local info = SeasonSystem.slapInfo
  if not info then
  ...
  info.show_season_on_close = true
  ...
  if bPromotion and not bIsImprintSlap and bIsPromotionLevel and aceImprintBaseId and not bIsBest then
    local AceImprintLogic = require("client.logic.season.AceImprintLogic")
    local AceImprintDetailS20 = AceImprintLogic.GetAceImprintDetailS20(tonumber(DataMgr.roleData.uid))
    log_format("SeasonSystem.ShowSeasonSlap AceImprintDetailS20=%s", AceImprintDetailS20)
...
function SeasonSystem.OnModePostSwitch(preState, nextState)
  ...
  if nextState == GameStatus.Lobby and SeasonSystem.isNeedGetState then
    local SeasonHandler = require("client.network.Protocol.SeasonHandler")
  ...
  if nextState == GameStatus.Login then
    ...
    logic_season_cycle_memory.ClearCfgTable()
    SeasonSystem.ChallengeExchangeTable = {}
    SeasonSystem.ConquerorMinScore = {}
  end
end
