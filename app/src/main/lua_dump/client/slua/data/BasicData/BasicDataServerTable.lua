...
function BasicDataServerTable:_GetIsCanSend()
  ...
  if nCount > _nTimeLimitCount then
    if Client and Client.IsDevelopment() then
      local sAllQueueTableName = ""
      for i = 1, #self._tQueueReqTableData do
        sAllQueueTableName = sAllQueueTableName .. self._tQueueReqTableData[i].sTableName .. ","
      end
      local utility = require("common.utility")
      local sMsg = "BasicDataServerTable:_GetIsCanSend >>>>> : " .. sAllQueueTableName
