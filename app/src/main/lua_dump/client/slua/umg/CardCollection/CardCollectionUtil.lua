...
function CardCollectionUtil.ShowFragmentTips(widget)
  ...
  UIManager.ShowUI(UIManager.UI_Config.ItemUp_Mat_Tips_UIBP, itemCfg, jumpSource, {
    widget = widget,
...
function CardCollectionUtil.OpenPanel(Paneltype, extraData)
  ...
  if Paneltype == EPaneltype.Share or Paneltype == EPaneltype.SwapShare then
  ...
  if Paneltype == EPaneltype.Main or Paneltype == EPaneltype.Set then
    ...
    local ctorData = {panelType = Paneltype, panelData = extraData}
...
function CardCollectionUtil.CardGetPanel(itemIdList, buttonList, extendData)
  ...
  for _, item in ipairs(itemIdList) do
    local cardData = logic_card_collection_season:GetCardShowDataByItemId(item.res_id) or {}
...
function CardCollectionUtil.OpenSwapShare(give_card_list, expect_res_id, order_id, create_time)
  ...
  orderData.show_data = logic_card_collection_season:GetCardShowDataByOrderInfo(orderData)
  Util.ShowShare(shareCfg, panel_config, {orderInfo = orderData})
end
function CardCollectionUtil.IsCardOnline(showData)
  if showData.IsTimeOnline == 0 then
    return true
  end
  ...
  local timestamp = time_util.TimeStringToUnixstamp(showData.OnlineTime)
