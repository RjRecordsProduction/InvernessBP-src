...
function Login_UIBP:CheckAndShowTimeStamp()
  ...
  local black_config = RequireBlackList("blacklist.slua.config.black_config")
  if black_config and not black_config.LoginLobby_Timestamp_BP.keyName then
    log(bWriteLog and "Login_UIBP:CheckBlackList. LoginLobby_Timestamp_BP keyName is nil")
  ...
  if self.loginChannel == login_module.CPhone_mail then
    if PublishRegionMacros.IsBLUEHOLE() then
      ...
      if self.bUnifiedAccountTryQuickLogin and localCacheLoginChannel == BP_ENUM_PLAYFORM_UnifiedAccountByiTOP then
        ...
        UIManager.ShowUI(UIManager.UI_Config.login_phone_mail)
      end
    end
  ...
  if not logic_cloud_game:IsCloudVersion() then
    if isBlueHole then
      para = {
        ...
        FirstLoginChannel = self.firstLoginChannel,
        SecondLoginChannel = self.secondLoginChannel,
        ThirdLoginChannel = self.thirdLoginChannel
      }
    else
      para = {
        LoginTypeList = self.loginChannels,
