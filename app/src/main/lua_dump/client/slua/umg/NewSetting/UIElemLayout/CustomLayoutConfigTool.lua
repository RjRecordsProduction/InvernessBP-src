...
local MainConfig = require("client.slua.umg.NewSetting.UIElemLayout.Setting_UIElemLayout_Config")
local MainIndexer
...
local _GetSourceLayoutConfig = function(ModName)
  local LuaPath = "GameLua.Mod." .. ModName .. ".Client.Config.SettingUIElemLayoutConfig"
...
function ConfigTool.GetIndexer(ModName)
  if ModName and ModName ~= "" then
    ...
    local IndexerClass = import("/Script/Client.DynamicCustomIndexer")
...
function ConfigTool.GetDefaultAnchorData(ModName, CustomType)
  local result
  if ModName then
    local Indexer = ConfigTool.GetIndexer(ModName)
    if slua.isValid(Indexer) then
  ...
  if not result then
    ...
    if slua.isValid(MainIndexer) then
      result = MainIndexer.DefaultLayoutData:Get(CustomType)
    end
  end
  return result
end
function ConfigTool.ApplyDefaultLayout(ModName, CustomPanel, Type)
  local AnchorData = ConfigTool.GetDefaultAnchorData(nil, Type)
  CustomPanel.DefaultAnchors = AnchorData.Anchors
  CustomPanel.DefaultPosition = FVector2D(AnchorData.Offsets.Left, AnchorData.Offsets.Top)
