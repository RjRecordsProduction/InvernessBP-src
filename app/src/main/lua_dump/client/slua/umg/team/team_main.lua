...
function UI_Team_Main:GetWidgetTargetPosition(index)
  ...
  log_tree("[DeanJYT] UI_Team_Main:GetWidgetTargetPosition avatarPos of " .. tostring(index) .. ":", avatarPos)
  ...
  local menuPos = UIUtil.ProjectWorldLocationToWidgetPosition(avatarPos[1] + offset[1], avatarPos[2] + offset[2], avatarPos[3] + offset[3])
  return menuPos
end
...
function UI_Team_Main:OnExitMember(_, _, uid)
  ...
  if uid then
    ...
    for i, v in ipairs(self.menuList) do
      if tonumber(v.nUID) == uidNum then
        v:Hide()
        v:Reset()
        break
      end
...
function UI_Team_Main:RefreshTeamMemberDetail(uid)
  ...
  team_member_detail:SetPosition(Pos.X, Pos.Y)
end
