...
function Lobby_Mid_Squad_Tips_UIBP:onGetRecommendFlashTeamRsp()
  ...
  elseif self.recommendType == RecommendType.WeeklyLogin then
    if not self.weeklyDominantMode then
    ...
    local squadId, squadName = logic_flash_match_team:getRecommendType4FromRecomSquads(self.weeklyDominantMode)
    if not squadId then
...
function Lobby_Mid_Squad_Tips_UIBP:onModeSelectChange()
  ...
  if not self.curSelectMode or self.curSelectMode == UEnums.GameMode.Rank_Competition or teamMainID ~= UEnums.FlashTeamGameMode.Team_Competition and teamMainID ~= UEnums.FlashTeamGameMode.WoW_Creative then
    return
  end
  if self.delayModeSwitchTip then
    self:RemoveTimer(self.delayModeSwitchTip)
...
function Lobby_Mid_Squad_Tips_UIBP:onAllMyTeamsChg()
  if self.isFirstOpen and self:CheckCanShow() then
    local isFound = self:GetRecommendType1()
    if isFound then
      self:ShowTip()
    end
...
function Lobby_Mid_Squad_Tips_UIBP:GMShow(_, _, type)
  ...
  elseif type == 9 then
    ...
    self.weeklyDominantMode = logic_teamquick_join:GetPreferModesOver60Percent()
    log_format(bWriteLog and "Lobby_Mid_Squad_Tips_UIBP:GMShow preferMode:%s", self.weeklyDominantMode)
    self:ReqSeverRecommend(10, self.weeklyDominantMode, RecommendType.WeeklyLogin)
  elseif type == 11 then
    self:ReqSeverRecommend(10, nil, RecommendType.DailyLogin)
  elseif type == 1 then
    local isFound = self:GetRecommendType1()
...
function Lobby_Mid_Squad_Tips_UIBP:CheckCanShow()
  ...
  if not hasShowLobbyGuide then
    log_warning(bWriteLog and "Lobby_Mid_Squad_Tips_UIBP:CheckCanShow hasShowLobbyGuide = false")
    return false
  end
...
function Lobby_Mid_Squad_Tips_UIBP:ShowTip()
...
function Lobby_Mid_Squad_Tips_UIBP:GetRecommendType4()
  ...
  if preferMode then
    self.weeklyDominantMode = preferMode
    self:ReqSeverRecommend(10, preferMode, RecommendType.WeeklyLogin)
    return true
  end
  return false
end
...
function Lobby_Mid_Squad_Tips_UIBP:ReqSeverRecommend(count, matchMode, rcmdType)
  ...
  local reqMode
  if matchMode then
    reqMode = {game_mode_id = matchMode}
  end
  FlashTeamHandler.send_get_flash_squad_recommend_req(count or 10, reqMode)
end
