...
function logic_flash_match_team:addToSaveFlashTeam(teams)
  for uid, team in pairs(teams) do
    self.flashTeamSummary[uid] = team
  end
end
...
function logic_flash_match_team:UpdateFlashTeamDetail(id, detail)
  log_tree(string.format("logic_flash_match_team:UpdateFlashTeamDetail id=%s, detail", id), detail)
  if id then
...
function logic_flash_match_team:addToSaveFlashMemberTeam(members_briefs)
  ...
  EventSystem:postEvent(EVENTTYPE_FLASH_TEAM, EVENTID_FLASH_TEAM_MEMBER_CHG)
end
...
function logic_flash_match_team:AddMyTeam(team)
  ...
  self.flashTeamSummary[team.squad_id] = team
  self.ownTeamInfo.squads[team.squad_id] = team
end
...
function logic_flash_match_team:SaveRapportClaimed(squad_id, claimInfo)
  log_tree(string.format("logic_flash_match_team:SaveRapportClaimed squad_id:%s, claimInfo", squad_id), claimInfo)
...
function logic_flash_match_team:JudgeMemberIsOnline(squadId, uid)
  ...
  return status and status.online == true
end
...
function logic_flash_match_team:getRecommendType4FromRecomSquads(matchMode)
  ...
  for _, squad in ipairs(recommendSquads) do
    if 0 < (squad.pre_team_count or 0) and squad.prefer_modes then
      for _, modeId in pairs(squad.prefer_modes) do
        if matchMode == modeId then
          return squad.squad_id, squad.name
        end
      end
    end
...
function logic_flash_match_team:CheckWeeklyRemindInviteTeam(onlyCheck)
  local TimeUtil = require("client.common.time_util")
  local remainDays = 7 - TimeUtil.GetServerWeekDay()
  local isRemind = PlayerPrefsSystem.CheckAndSaveCurrentDate_N(WeeklyRemindInviteTeam, onlyCheck, remainDays)
  return isRemind
end
function logic_flash_match_team:ClearWeeklyRemindInviteTeam()
...
function logic_flash_match_team:InitTeamTips(parentUILua, instantShow, GameMode)
  ...
  if not (parentUILua and parentUILua.UIRoot) or not parentUILua.UIRoot.TeamPlatform_RecommendedTeam_Tips then
    log_format(bWriteLog and "logic_flash_match_team:InitTeamTips parentUILua or tipsWidget is nil")
    return
  end
  ...
  if instantShow then
    ...
    self:SaveLastClosePopTime(curTime)
  end
  local tipsWidget = parentUILua.UIRoot.TeamPlatform_RecommendedTeam_Tips
  local tipsLua = self:showTipsWidget(parentUILua, tipsWidget, GameMode)
...
function logic_flash_match_team:showTipsWidget(parentUILua, tipsWidget, GameMode)
  local UIComponentModule = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.UIComponentModule)
  local tipsLua = UIComponentModule:InitWithParentComponent(parentUILua, UIComponentModule.Config.TeamPlatform_RecommendedTeam_Tips, tipsWidget, GameMode)
  return tipsLua
end
function logic_flash_match_team:StartPopTeamTimer(tipsWidget, tipsLua, parentUILua)
  ...
  self.loopPopTimer = self:AddTimerLoop(popInterval, function()
    ...
    if not slua.isValid(self.tipsWidget) or not slua.isValid(self.parentUILua) then
      self:StopPopTeamTimer()
      return
    end
    ...
    local TimeUtil = require("client.common.time_util")
    local curTime = TimeUtil.OSTime()
    local notOverInterval = curTime - self:GetLastClosePopTime() < popInterval
    if notOverInterval then
      return
    end
    self:SaveLastClosePopTime(curTime)
    ...
    self.closeTipsCb = self:AddTimerOnce(delayCloseTime, function()
      self:SaveLastClosePopTime(TimeUtil.OSTime())
  ...
  for _, profile in pairs(membersProfile) do
    ...
    if hasSocialCardLabel then
      ...
      for labelId, v in pairs(label) do
        ...
        local bShouldCnt = cfg and cfg.From == 2
        if bShouldCnt then
...
function logic_flash_match_team:OnGetFlashSquadApplyList(applications)
  ...
  log_tree(self.myFlashTeamApplyList)
...
function logic_flash_match_team:OnFlashSquadApplyNotify(application_info)
  log(bWriteLog and "logic_flash_match_team:OnFlashSquadApplyNotify")
...
function logic_flash_match_team:OnResultDataHandler(_, __, resultData)
  ...
  log_tree("logic_flash_match_team:OnResultDataHandler flash_squad_rapport_changes", resultData.flash_squad_rapport_changes)
  ...
  for i, change in ipairs(self.rapportChangeData.flash_squad_rapport_changes) do
    ...
    log_tree("logic_flash_match_team:OnResultDataHandler teamInfo", teamInfo)
    if teamInfo then
