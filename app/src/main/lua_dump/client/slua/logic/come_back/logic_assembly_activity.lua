...
function AssemblyActivitySystem.Enter()
  ...
  log_tree("reqDataKeys", reqDataKeys)
  log_tree("reqCfgKeys", reqCfgKeys)
  if next(reqDataKeys) == nil and next(reqCfgKeys) == nil then
...
function AssemblyActivitySystem.ReqAssemblyInfo()
  ...
  if AssemblyActivitySystem.GetActivityData() then
    ...
    if next(reqDataKeys) ~= nil then
      ...
      local reqCfgKeys = AssemblyActivitySystem.GetCfgKeys()
      AssemblyActivitySystem.assemb_task_query_req(reqDataKeys, reqCfgKeys)
    end
  end
end
...
function AssemblyActivitySystem.take_full_action_award_rsp(res, id, invoke_type)
  log(bWriteLog and "take_full_action_award_rsp:" .. tostring(res) .. ",id:" .. tostring(id))
...
function AssemblyActivitySystem.assemb_notify(is_all, assemb_data, cfg, is_back_user)
  ...
  if AssemblyActivitySystem.AssemblyCfg and not AssemblyActivitySystem.AssemblyCfg.team_score_plan_cfg and scoreCfg and scoreCfg.plan_id and scoreCfg.plan_id > 0 then
    AssemblyActivitySystem.assemb_task_query_req({}, {
      "team_score_plan_cfg"
    })
  end
...
function AssemblyActivitySystem.take_task_award_rsp(res, id, num, invoke_type)
  log(bWriteLog and "take_task_award_rsp:" .. tostring(res) .. ",id:" .. tostring(id))
  if res ~= NetErrorCode_NONE then
    return
  end
...
function AssemblyActivitySystem.assemb_invite_friend_rsp(res, id)
  log(bWriteLog and "assemb_invite_friend_rsp:" .. tostring(res) .. ",id:" .. tostring(id))
end
function AssemblyActivitySystem.take_assemb_award(short_code)
  log(bWriteLog and "take_assemb_award:" .. tostring(short_code))
  local AssemblyHandler = require("client.network.Protocol.AssemblyHandler")
  AssemblyHandler.send_take_assemb_award(short_code)
end
function AssemblyActivitySystem.take_assemb_award_rsp(res, awardList)
  ...
  log(bWriteLog and "take_assemb_award_rsp:" .. tostring(res))
  if res ~= NetErrorCode_NONE then
...
function AssemblyActivitySystem.on_assemb_get_back_user_list_res(ret, uid_list, callback)
  ...
  log_tree(bWriteLog and "AssemblyHandler.on_assemb_get_back_user_list_res callback", callback)
  if ret ~= 0 then
    ShowNotice(ret)
    return
  end
  ...
  if not Client.IsReleaseVersion(NetInterface) and not next(uid_list) then
    ShowNotice(8045)
  end
