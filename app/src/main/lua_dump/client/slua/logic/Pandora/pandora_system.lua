...
function PandoraSystem.RemoveOneRedPoint(actId)
  local subId = Pandora2ActivityId[actId]
  log(bWriteLog and "  :subId" .. tostring(subId))
  if subId then
    local ActivityCenterSystem = require("client.slua.logic.activity.logic_activity_center")
    local nType = ActivityCenterSystem.FindActSwitchTypeByID(subId)
    local ActivityRedDot = require("client.slua.logic.activity.RedPoint.ActivityRedDot")
    local SystemName = ActivityRedDot.GetActFirstRedDotSystemName(subId)
    ActivityRedDot.RemoveRedDotNode(SystemName, nType, subId)
  end
end
  ...
  if ActIsReady or ActPakIsReady then
    if ActIsReady and ActPakIsReady then
      ...
      if ActPakIsReady and not outTime then
        ...
        HostedProtoBridge:SetOpenFlag("Pandora", "0")
        local protocolLayer = require("client.slua.logic.Pandora.pandora_protocol_layer")
