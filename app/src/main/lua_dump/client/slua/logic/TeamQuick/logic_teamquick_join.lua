...
function logic_teamquick_join:GetPreferModesOver60Percent()
  ...
  if 0.6 < max_mode_count / total_count then
    return max_mode
  end
  return nil
end
...
function logic_teamquick_join:proc_get_prefer_modes_for_flash_squad_rsp(res, data)
  if res ~= 0 or not next(data) then
    return
  end
