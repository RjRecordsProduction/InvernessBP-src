...
function TeamUpNewSystem.on_team_invite_notify(inviter, teamId, team_type, inviteInfo, sub_mode_view_ids)
  ...
  if FriendHandler.friend_status_data and not GameStatus.IsInFightingNotSocialNotMainCityNotHome() and FriendHandler.friend_status_data.is_agree_invite and inviteApplyData.from == E_InviteFromType.FriendBar then
    ShowNotice(37411)
...
function TeamUpNewSystem.on_team_info_sync(teamid, teaminfo, isIncrement, isMemberQuit)
  ...
  if not isInGame or isMainCity then
    if IsWoWEditor and 0 < teaminfo.id and teaminfo.player_count > 1 then
      TeamUpNewSystem.team_quit_request(teaminfo.id)
      return
    end
...
function TeamUpNewSystem.on_team_change_wear(memberUid, isPushOn, mResId, mAvatarPos, wearInfo, gold_dress_set_info, gold_dress_set_info_all)
  ...
  if LogicXSuit.IsXSuit(mResId) then
    ...
    if period then
      ...
      if gold_dress_set_info and gold_dress_set_info[period] then
        newItemID = LogicXSuit.GetSwitchItemByItemAndSwitchLevel(newItemID, gold_dress_set_info[period])
      end
      if gold_dress_set_info_all and gold_dress_set_info_all[period] then
        local state = gold_dress_set_info_all[period].bicolor_state
        if state then
  ...
  if wearInfo and wearInfo[1] then
    if LogicXSuit.IsXSuit(wearInfo[1]) then
      ...
      if period then
        ...
        if gold_dress_set_info and gold_dress_set_info[period] and source ~= EWardrobeDataSource.InheritWardrobe then
          newItemID = LogicXSuit.GetSwitchItemByItemAndSwitchLevel(newItemID, gold_dress_set_info[period])
        end
        if gold_dress_set_info_all and gold_dress_set_info_all[period] then
          local state = gold_dress_set_info_all[period].bicolor_state
...
function TeamUpNewSystem.on_team_update_wear(op_uid, wear_res, wear_ext_res, knapsack_ext_info, gold_dress_set_info, gold_dress_set_info_all)
  ...
  if wear_ext_res and wear_ext_res[3] and wear_ext_res[3][1] then
    if LogicXSuit.IsXSuit(wear_ext_res[3][1]) then
      ...
      local period = LogicXSuit.GetPeriodByItemId(wear_ext_res[3][1])
      if period then
        local Source = wear_ext_res[3][ENUM_AVATAR_DATA_TYPE.Source] or 0
        if gold_dress_set_info and gold_dress_set_info[period] and Source ~= EWardrobeDataSource.InheritWardrobe then
          newItemID = LogicXSuit.GetSwitchItemByItemAndSwitchLevel(wear_ext_res[3][1], gold_dress_set_info[period])
        end
        if gold_dress_set_info_all and gold_dress_set_info_all[period] then
          local state = gold_dress_set_info_all[period].bicolor_state
          if state then
    ...
    for k, v in pairs(TeamUpNewSystem.teamInfo.members) do
      ...
      if tonumber(k) ~= selfUID then
        if v.weapon_wear_info then
          ...
          weapon_wear_info.lgdWpnResId = v.weapon_wear_info.lgd_wpn_resid
        else
    ...
    for k, v in pairs(newTeamInfo.members) do
      ...
      if k ~= selfUID then
        ...
        if wear_ext[3] and wear_ext[3][1] then
          ...
          local period = LogicXSuit.GetPeriodByItemId(newItemID)
          if period then
            if v.gold_dress_set_info and v.gold_dress_set_info[period] and source ~= EWardrobeDataSource.InheritWardrobe then
              newItemID = LogicXSuit.GetSwitchItemByItemAndSwitchLevel(newItemID, v.gold_dress_set_info[period])
            end
            if v.gold_dress_set_info_all and v.gold_dress_set_info_all[period] then
              local state = v.gold_dress_set_info_all[period].bicolor_state
              if state then
                newItemID = LogicXSuit.ChangeItemIDByState(newItemID, state)
              end
            end
  ...
  if memberInfo and memberInfo.weapon_wear_info then
    if not memberInfo.weapon_wear_info.weapon_id or tonumber(memberInfo.weapon_wear_info.weapon_id) == 0 then
      ...
      if memberInfo.weapon_wear_info then
        ...
        weapon_wear_info.lgdWpnResId = memberInfo.weapon_wear_info.lgd_wpn_resid
      else
        weapon_wear_info.weaponId = 0
        weapon_wear_info.skinId = 0
        weapon_wear_info.usingDiyRecommend = false
...
function TeamUpNewSystem.sync_player_action(player_uid, action_id, randSoundId, follow_id, extraParam)
  ...
  if action_id == logic_card_collection:GetActionItemID() then
