...
function long_txt_manager:GetPrivacyAgreementContent()
  ...
  elseif privacyAgreementConfig.bUseDiffLanguage then
    contentFile = privacyAgreementConfig.globalContent[language]
    if contentFile == nil then
      contentFile = privacyAgreementConfig.globalContent[privacyAgreementConfig.defaultLanguage]
    end
  else
    contentFile = privacyAgreementConfig.globalContent[privacyAgreementConfig.defaultLanguage]
  end
...
function long_txt_manager:GetUserAgreementContent()
  ...
  elseif userAgreementConfig.bUseDiffLanguage then
    contentFile = userAgreementConfig.globalContent[language]
    if contentFile == nil then
      contentFile = userAgreementConfig.globalContent[userAgreementConfig.defaultLanguage]
    end
  else
    contentFile = userAgreementConfig.globalContent[userAgreementConfig.defaultLanguage]
  end
...
function long_txt_manager:ShowPrivacyPolicy()
  local title = self:GetPrivacyAgreementTitle()
  local content = self:GetPrivacyAgreementContent()
  UIManager.ShowUI(UIManager.UI_Config.common_protocol_msg, 0, title, content)
end
function long_txt_manager:ShowUserAgreement()
  local title = self:GetUserAgreementTitle()
  local content = self:GetUserAgreementContent()
  UIManager.ShowUI(UIManager.UI_Config.common_protocol_msg, 0, title, content)
end
