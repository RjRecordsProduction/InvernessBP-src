...
function logic_chat_channel_friend.InitFriendList()
  ...
  if logic_chat_channel_friend.CurrentGid ~= "" then
    logic_chat_channel_friend.TopTargetUserChatInList(tonumber(logic_chat_channel_friend.CurrentGid))
  end
...
function logic_chat_channel_friend.update_friend_unread(chatMsg, message_count)
  ...
  logic_chat_channel_friend.TopTargetUserChatInList(uid)
end
  ...
  if logic_chat_channel_friend.CheckHistoryStructParam(friendChat) then
    for k, v in pairs(friendChat.msgList) do
      if LogicHistorySystem.CheckHistoryOneMsgParam(v) then
        ...
        if v.msgType == chat_macro.PandoraInviteMsg or v.msgType == chat_macro.temuInvite or v.msgType == chat_macro.temuTaskRemind or v.msgType == chat_macro.WarmUpGroupInvite or v.msgType == chat_macro.BF_Sub_InviteGroupMsgType or v.msgType == chat_macro.CardCollectionSwapMsgType or v.msgType == chat_macro.GroupBuyFriendsMsgType then
