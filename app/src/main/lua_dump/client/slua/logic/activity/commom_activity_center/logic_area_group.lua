    ...
    for _, v in ipairs(list) do
      if v.StartTime and nNowTime > v.StartTime and v.EndTime and nNowTime < v.EndTime then
        ...
        local data = {
          ...
          nHasDoneRate = AreaGroupSystem.GetCurActHasDoneRate(v.ID),
...
function AreaGroupSystem.MainActHasRedDot(mainID)
  ...
  log(bWriteLog and string.format("AreaGroupSystem.MainActHasRedDot. TabType=%s", tostring(activity.TabType)))
  if activity.TabType == ActivitySwitchType.Activity then
    local doneRate = AreaGroupSystem.GetCurActHasDoneRate(mainID)
    log(bWriteLog and string.format("AreaGroupSystem.MainActHasRedDot. ID=%s, doneRate=%s", tostring(mainID), tostring(doneRate)))
    if doneRate == 1 then
      return false, ActivityMacros.RedDotType.None
    end
  end
...
function AreaGroupSystem.HasAwardRedDot(ID)
  ...
  for _, subData in ipairs(actData.List) do
    if subData.Type == ActivityType.ITEM_EXCHANGE then
      Logic_Activity_Center.ResetExchangeParams(subData)
