...
local STATE_TIMEOUTS = {
  ...
  [Config_UGC_Copilot.Enum_UGC_LLM_State.NEW_CHAT_PENDING] = 10,
...
function StateMachineManager:HandleError(errorCode, reason)
  ...
  if self.CurrentState == Config_UGC_Copilot.Enum_UGC_LLM_State.INITIALIZING or self.CurrentState == Config_UGC_Copilot.Enum_UGC_LLM_State.INIT then
    if self.CurrentState ~= Config_UGC_Copilot.Enum_UGC_LLM_State.INIT then
      self:UpdateState(Config_UGC_Copilot.Enum_UGC_LLM_State.INIT, "init_failed:" .. reason)
    end
  else
