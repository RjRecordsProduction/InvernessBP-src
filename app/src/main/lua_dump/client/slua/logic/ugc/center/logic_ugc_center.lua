local LogicUGCCenter = {
  ...
  MaxLimitBuyCount = 99,
  CreativeWOWBundleState = nil,
  TimerCreativeWOW = nil,
...
function LogicUGCCenter:BackToChallenge(pre, next)
  if pre == GameStatus.Fighting and next == GameStatus.Lobby then
    if self.bIsShowUGCCenterMainUI then
      ...
      self:OpenUGCCenterMainUI(Config_UGC_Center_TabID.School, Config_UGC_Center_TabID.Challenge, {bBackFromFight = true, tabid = 2})
    end
...
function LogicUGCCenter:RemoveCreativeWOWTimer()
  if self.TimerCreativeWOW then
    self:RemoveTimer(self.TimerCreativeWOW)
    self.TimerCreativeWOW = nil
  end
end
function LogicUGCCenter:CreativeWOWTimer()
  local LogicUGCCreativeWow = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.logic_ugc_creativewow)
  self.CreativeWOWBundleState = LogicUGCCreativeWow:GetResState()
  self.TimerCreativeWOW = self:AddTimerLoop(1, function()
    if self.CreativeWOWBundleState ~= LogicUGCCreativeWow:GetResState() then
      self.CreativeWOWBundleState = LogicUGCCreativeWow:GetResState()
      EventSystem:postEvent(EVENTTYPE_UGC, EVENTID_UGC_CHALLENGE_ITEM_UPDATE)
    end
  end, TIMER_INFINITE, 0.5)
end
function LogicUGCCenter:DownLoadCreativeWOW()
  local content = LocUtil.LocalizeResFormat(511097)
  local clickOkCallback = function()
    local LogicUGCCreativeWow = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.logic_ugc_creativewow)
    LogicUGCCreativeWow:DownloadRes()
  end
    ...
    fExchangeCallback = function(Exchange, SelectCount)
      ...
      if TotalPrice > CurrentMoney then
        ...
        if not bIsCreativePoints then
          ...
          local ext = {
            openSource = config_ugc_commercialization.C_UGC_WOWCoin_Exchange_Popup_Source.Center,
            ExchangeType = config_ugc_commercialization.C_UGC_Exchange_Coin_Type.UC2WOWCoin
          }
          UIManager.ShowUI(UIManager.UI_Config.UGC_WOWCoin_Exchange_Popup_UIBP, ext, TotalPrice - CurrentMoney)
        end
        return
      end
...
function LogicUGCCenter:RefreshNoviceTeachingData(tabID, levelList)
  ...
  local templateID = AllData[first_level_id] and AllData[first_level_id].TemplateID or 0
  local mapKey = resManager:GetUGCMapKeyByTemplateId(templateID)
  local pakList = {}
  for _, value in pairs(self.NoviceTeachingList) do
    for _, v in pairs(value.ResourceList) do
      table.insert(pakList, v)
    end
  end
  table.insert(pakList, mapKey)
  for k, v in pairs(self.NoviceTeachingList) do
    v.ResourceList = pakList
    if first_level_id >= v.ID then
      v.bPreMissionCompleted = true
    else
...
function LogicUGCCenter:RspGetTeachingLevelAward(level_id, the_award_list, tutorial_award_info)
  ...
  local bGetPermission = function()
    ...
    return cur_level_id >= open_level_id
  end
