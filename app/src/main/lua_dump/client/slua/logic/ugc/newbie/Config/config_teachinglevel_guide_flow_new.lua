      ...
      Params = {
        Function = function()
          ...
          time_ticker.AddTimer(2, function()
            ...
            local name = ui.LocName .. "_" .. tonumber(uid)
            local desc = LocUtil.GetLocalizeResStr(8700790)
            ui.UIRoot.EditableText_Name:SetText(name)
            ui.UIRoot.MultiLineEditableText_Desc:SetText(desc)
            ui:OnChangeDesc(desc)
            ui:OnChangeName(name)
            ui:CheckCanPublish()
          end)
        end
      }
