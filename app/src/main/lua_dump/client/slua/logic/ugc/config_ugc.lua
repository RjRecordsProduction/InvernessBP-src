...
Config_UGC.Detail_Source_To_TLog_Config = {
  ...
  [Config_UGC.Source_Open_Detail.WOWSeason] = TLogEventDefine.UGC_Mod_Season_Main,
...
local _TabID = {
  ...
  Tournament = 114,
  HotRankTheme = 115,
  WoWLeaderboardSeason = 116,
  NewNewMap = 117,
  Mine = 118,
  FineMod = 119
}
Config_UGC.Config_UGClocal _RecommendTab = {
  ID = _TabID.Recommend,
  nNameID = 70038,
  sModule = "UGCRecommendPanel",
...
local _HotThemeTab = {
  ID = _TabID.HotTheme,
  nNameID = 70038,
  sModule = "UGC_Lobby_HotTheme_UIBP",
  sRoot = "CanvasPanel_Hot",
...
local _UGCLeaderboardSeasonTab = {
  ID = _TabID.WoWLeaderboardSeason,
  nNameID = 79487,
  sModule = "UGC_Main_Lobby_LeaderboardSeason_UIBP",
  sRoot = "CanvasPanel_Challenge",
  nSwitcherIndex = 11
}
local _NewMapTab = {
  ID = _TabID.NewNewMap,
...
Config_UGC.Clocal _Fixed_UGC_Tabs = {_PlayTab}
local _Fixed_UGC_Tabs_260 = {_RandomTab, _PlayTab}
local _Fixed_UGC_Tabs_Last_Added = {_PHomeTab}
...
function Config_UGC.GetRankTabs(bIsFineWork)
  ...
  if LogicUGCHall:CheckIsOpen() then
    local tb4 = {}
    local _TempTab = TableUtil.CopyTable(_FineModTab)
    _TempTab.Name = _TempTab.nNameID
    for k, Tab in pairs(tb1) do
      if Tab.ID ~= Config_UGC.Config_UGC_TabID.All and Tab.ID ~= Config_UGC.Config_UGC_TabID.NewNewMap then
        table.insert(tb4, Tab)
      end
    end
    table.insert(tb4, _TempTab)
    table.sort(tb4, function(a, b)
...
function Config_UGC.GetCurFixedTabShowConfig()
  local curRecommendCfg = _RecommendTab
  ...
  if Config_UGC.NewWOWHall == 2 then
    curRecommendCfg = _NewHotThemeTab
  end
  local TableUtil = require("common.table_util")
  ...
  if not Logic_UGC_Season:IsBeforeS4LegacyAPIEnabled() and not Logic_UGC_Season:IsAfterS5Season() then
    local _TmpTab = TableUtil.CopyTable(_UGCLeaderboardSeasonTab)
    _TmpTab.Name = _TmpTab.nNameID
    table.insert(tabList, _TmpTab)
  end
  local tb1, _TempTab
  ...
  if LobbySystem.CheckOpen(BP_UGC_RANDREC_SWITCH) then
  ...
  if LobbySystem.CheckOpen(BP_UGC_WONDERSEASON_SWITCH) then
    local tb2
    if Config_UGC.NewWOWHall ~= 2 then
      tb2 = TableUtil.CopyTable(_UGCSeasonTab)
    else
      local bUGCSeason = Logic_UGC_Season:IsBeforeS4LegacyAPIEnabled()
      local PublishRegionMacros = require("client.slua.config.ClientMacros.PublishRegionMacros")
      if PublishRegionMacros.IsJapanOrKorea() or PublishRegionMacros.IsBLUEHOLE() then
        if bUGCSeason then
          tb2 = TableUtil.CopyTable(_NewUGCSeasonTab)
...
local _PropShop = {
  ...
  Open = function(Modinfo)
    ...
    local PublishRegionMacros = require("client.slua.config.ClientMacros.PublishRegionMacros")
    if (PublishRegionMacros.IsJapanOrKorea() or PublishRegionMacros.IsTWVersion() or PublishRegionMacros.IsVNGVersion()) and not LobbySystem.CheckOpen(BP_ENUM_WOW_PROPSHOP_SWITCH) then
      return false
    end
    if Modinfo and Modinfo.setting.has_mod_market then
      return true
    end
    return false
  end
...
Config_UGC.Configlocal _UGC_DetailTabPrivate = {
  _DetailTab,
  _UpdateTab,
  _PropShop
}
...
local _ReviewUpdateTab = {
  nID = _DetailTabID.Update,
  sIcon = "/Game/UMG/Texture_200/Atlas/CreativeMode/Frames/UGC_tab01_png.UGC_tab01_png",
  dIcon = "/Game/UMG/Texture_200/Atlas/CreativeMode/Frames/UGC_tab02_png.UGC_tab02_png",
  sModule = "UGCDetailUpdateLogPanel",
  sRoot = "CanvasPanel_Update2"
}
local _UGC_MineReviewDetailTabs = {_MineReviewDetailTab, _ReviewUpdateTab}
...
Config_UGC.local _WOWDetailTabID = {Detail = 1, Achievements = 2}
Config_UGC.Config_UGClocal _WOWSeason = {
  nID = _WOWDetailTabID.Detail,
  sIcon = "/Game/UMG/Texture_200/Atlas/CreativeMode/Frames/UGC_tab03_png.UGC_tab03_png",
  dIcon = "/Game/UMG/Texture_200/Atlas/CreativeMode/Frames/UGC_tab04_png.UGC_tab04_png",
  sModule = "UGC_Lobby_RankMode_Sub_UIBP",
  sRoot = "CanvasPanel_SeasonRank"
}
local _WOWAchievements = {
  nID = _WOWDetailTabID.Achievements,
  sIcon = "/Game/UMG/Texture_200/Atlas/CreativeMode/Frames/UGC_Tab_Stats_Select_png.UGC_Tab_Stats_Select_png",
  dIcon = "/Game/UMG/Texture_200/Atlas/CreativeMode/Frames/UGC_Tab_Stats_png.UGC_Tab_Stats_png",
  sModule = "UGC_Season_Achievement_UIBP",
  sRoot = "CanvasPanel_SeasonRank"
}
local _UGC_WOWSeasonTabs = {_WOWSeason, _WOWAchievements}
Config_UGC.Configlocal _WOWAchievementReuseFallItemType = {
  Content = 1,
  Time = 2,
  Space = 3
}
...
function Config_UGC.SetNewWOWHall(num)
  Config_UGC.NewWOWHall = num
  log(bWriteLog and "Config_UGC.SetNewWOWHall = " .. tostring(Config_UGC.NewWOWHall))
end
...
Config_UGC.local UGC_FrozenID = {OnLine = 60008, WOWSeason = 60009}
Config_UGC.Config_local C_ModSearchRankType = {
  [1] = {
    Type = "type_default",
    Name = 72015
  },
...
Config_UGC.ForceDependAssetIDList = {}
