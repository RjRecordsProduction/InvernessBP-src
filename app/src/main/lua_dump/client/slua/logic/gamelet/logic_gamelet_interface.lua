...
function logic_gamelet_interface:SendMessageToApp(app_id, message)
  log(bWriteLog and string.format("logic_gamelet_interface:SendMessageToApp: %s", message))
  if self.gamelet == nil then
    return
  end
  self.gamelet:SendMessageToApp(app_id, message)
end
...
function logic_gamelet_interface:GetAppIdByShowEntrance(appType)
  if not appType or not self.AppType2AppId[appType] then
    return nil
  end
...
function logic_gamelet_interface:SetGameletReady(appId, appType, bReady)
  ...
  local appInfo = {}
  if self.gamelet_apps_info[key] then
    appInfo = self.gamelet_apps_info[key]
  end
  self.AppType2AppId[appType] = appId
  appInfo = {ready = bReady, appType = appType}
  self.gamelet_apps_info[key] = appInfo
end
...
function logic_gamelet_interface:SetupVideoLibrary()
  if not self:gamelet_enable() then
    return
  end
