...
function LobbySocialSystem.SharePhoto()
  ...
  EventSystem:postEvent(EVENTTYPE_LOBBY_SOCIAL, EVENTID_LOBBY_SOCIAL_PHOTO, true)
  ...
  timer = timer_ticker.AddTimerLoop(0, function()
    if ScreenshotMaker.HasCaptured(sSharePath) then
      EventSystem:postEvent(EVENTTYPE_LOBBY_SOCIAL, EVENTID_LOBBY_SOCIAL_PHOTO, false)
      local TimeUtil = require("client.common.time_util")
      local cfg = {
        capturePath = sSharePath,
        sceneType = ShareSceneType.SocialLobby,
        otherTLog = TLogEventDefine.SocialPhotoShare,
        campaign = "lobby_social",
        share_type = ShareBtnTLogShareTypeDefine.PersonalSpace,
