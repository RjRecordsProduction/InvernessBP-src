...
function PufferUGCPakManager:CollectUGCPakData(existPaks)
  ...
  for _, v in pairs(assetCfg) do
    local IsExpired = CreativeExpiredAssetConfig.IsCurVersionExpired(v.AssetID, true)
    if IsExpired then
      if bWriteLog then
        print("PufferUGCPakManager:CollectUGCPakData AssetID = " .. v.AssetID .. " is expired")
      end
    else
      local PackID = v.ResSeprateType
      local packData = self.paks[PackID]
      if packData then
        local paks = self:_GetPakNamesByAssetConfig(v)
        for pakName, _ in pairs(paks) do
          if pakName and pakName ~= LOCK_PAKNAME and not self:IsPakExistInPack(pakName) then
            ...
            if not packData.paks[pakName] then
              ...
              if bWriteLog then
                print("PufferUGCPakManager:CollectUGCPakData insert to " .. PackID .. ", pakName: " .. pakName .. "  state = " .. pakData.state .. " AssetId = " .. v.AssetID)
              end
            end
          end
        end
...
function PufferUGCPakManager:StepCollectUGCPakData(ExistPaks)
  ...
  self.StepCollectTimer = self:AddTimer(0, function()
    ...
    for _, v in pairs(Assets) do
      ...
      if IsExpired then
        if bWriteLog then
          print("PufferUGCPakManager:StepCollectUGCPakData AssetID = " .. v.AssetID .. " is expired")
        end
      else
        local PackID = v.ResSeprateType
        local packData = self.paks[PackID]
          ...
          for pakName, _ in pairs(paks) do
            if pakName and pakName ~= LOCK_PAKNAME and not self:IsPakExistInPack(pakName) then
              ...
              if not packData.paks[pakName] then
                ...
                if bWriteLog then
                  print("PufferUGCPakManager:StepCollectUGCPakData insert to " .. PackID .. ", pakName: " .. pakName .. "  state = " .. pakData.state .. " AssetId = " .. v.AssetID)
                end
              end
            end
          end
...
function PufferUGCPakManager:OnPufferJsonPostProcess()
  ...
  for PackID, v in pairs(self.PackFeatureList) do
    ...
    puffer_odpak_manager:InitVirtualODPack(PackID, PakNameMap, self.existPaks)
  end
end
...
function PufferUGCPakManager:GetStateByKeyList(downloadType, keyList, bSkipDepends, bSkipVidepDepends, bMinDepends)
  ...
  for i, v in pairs(keyList) do
    ...
    if downloadType == PufferConst.ENUM_DownloadType.UGCPAK then
      if type(v) == "number" then
        ...
        log(bWriteLog and "PufferUGCPakManager:GetStateByKeyList state = " .. state .. " v = " .. v)
    ...
    elseif downloadType == PufferConst.ENUM_DownloadType.UGCPACK then
      state = self:GetStateByPackID(v, bSkipDepends)
      log(bWriteLog and "PufferUGCPakManager:GetStateByKeyList state = " .. state .. " v = " .. v)
    end
...
function PufferUGCPakManager:GetStateByAssetID(assetID, bMinDepends)
  ...
  if bMinDepends then
    ...
    local puffer_odpak_manager = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.puffer_odpak_manager)
    local tempState = puffer_odpak_manager:GetStateByKeyList(PufferConst.ENUM_DownloadType.ODPAK, DependFeatureKeys)
    state = PufferManager.GetMixDownloadState(state, tempState)
    if state == PufferConst.ENUM_DownloadState.Download then
      self.assetToPaks[assetID].      return state
    end
    tempState = self:GetStateByKeyList(PufferConst.ENUM_DownloadType.UGCPAK, DependAssetIDs, nil, nil, {bMinDepends = true})
    state = PufferManager.GetMixDownloadState(state, tempState)
...
function PufferUGCPakManager:GetStateByPackID(packID, bSkipDepends)
...
function PufferUGCPakManager:GetSizeByKeyList(downloadType, keyList, bSkipDepends, ifGetPakSizeList_UGCPAK, bMinDepends)
  local curSize = 0
  local totalSize = 0
  ...
  for i, v in pairs(keyList) do
    ...
    elseif downloadType == PufferConst.ENUM_DownloadType.UGCPACK then
      local cSize, tSize = self:GetSizeByPackID(v, bSkipDepends)
      curSize = curSize + cSize
      totalSize = totalSize + tSize
    end
  end
...
function PufferUGCPakManager:GetSizeByAssetIDList(assetIDList, ifGetPakSizeList, bMinDepends)
  ...
  for i, v in pairs(assetIDList) do
    local paks = self:GetPakNamesByAssetID(v)
    for pakName, _ in pairs(paks) do
      allPaks[pakName] = true
    end
    local DependFeatureKeys, DependAssetIDs = self:GetDepends(v)
  ...
  for pakName, _ in pairs(allPaks) do
    ...
    for _, v in pairs(self.paks) do
      if v.paks and v.paks[pakName] then
        flag = true
        cSize = cSize + v.paks[pakName].cSize
        tSize = tSize + v.paks[pakName].tSize
        if pakSizeList and pakId then
          ...
          pakSizeList[pakId].cSize = tempCSize + v.paks[pakName].cSize
          pakSizeList[pakId].tSize = tempTSize + v.paks[pakName].tSize
        end
        break
      end
    end
    if not flag then
      local pakSize = PufferDownloader.GetFileSizeCompressed(GameFrontendHUD, pakName, true) / PufferConst.MB
      tSize = tSize + pakSize
      if pakSizeList and pakId then
        pakSizeList[pakId] = pakSizeList[pakId] or {}
        local tempCSize = pakSizeList[pakId].cSize or 0
  ...
  if bMinDepends then
    ...
    tempcSize, temptSize = self:GetSizeByKeyList(PufferConst.ENUM_DownloadType.UGCPAK, AssetIDList, nil, nil, {bMinDepends = true})
...
function PufferUGCPakManager:GetSizeByAssetID(assetID, skipDependBundle, bMinDepends)
  ...
  for pakName, _ in pairs(paks) do
    ...
    if not flag then
      tSize = tSize + PufferDownloader.GetFileSizeCompressed(GameFrontendHUD, pakName, true) / PufferConst.MB
    end
  end
  if bMinDepends then
  else
    local DependFeatureKeys, DependAssetIDs = self:GetDepends(assetID)
    local puffer_odpak_manager = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.puffer_odpak_manager)
    local tempcSize, temptSize = puffer_odpak_manager:GetSizeByKeyList(PufferConst.ENUM_DownloadType.ODPAK, DependFeatureKeys)
    cSize = cSize + tempcSize / PufferConst.MB
    tSize = tSize + temptSize / PufferConst.MB
    tempcSize, temptSize = self:GetSizeByKeyList(PufferConst.ENUM_DownloadType.UGCPAK, DependAssetIDs, nil, nil, {bMinDepends = true})
    cSize = cSize + tempcSize
    tSize = tSize + temptSize
  end
...
function PufferUGCPakManager:GetSizeByPakName(pakName)
  ...
  for _, v in pairs(self.paks) do
    if v.paks and v.paks[pakName] then
      cSize = v.paks[pakName].cSize
      tSize = v.paks[pakName].tSize
      break
    end
  end
  if tSize == 0 then
    tSize = PufferDownloader.GetFileSizeCompressed(GameFrontendHUD, pakName, true) / PufferConst.MB
  end
...
function PufferUGCPakManager:GetSizeByPackID(packID, bSkipDepends)
  local pack = self.paks[packID]
  if not pack then
    return 0, 0
  end
...
function PufferUGCPakManager:GetDependByPackID(packID)
  if not self.paks[packID] then
    self.paks[packID] = {}
  end
  if not self.paks[packID].DependFeatureKeys then
    local DependFeatureKeysMap = {}
    local DependAssetIDsMap = {}
    local assetCfg = CDataTable.GetTable("UGCAssetConfig")
    for _, v in pairs(assetCfg) do
      if v.ResSeprateType == packID then
        local FeatureKeys, AssetIDs = self:GetDepends(v.AssetID)
        for k, FeatureKey in pairs(FeatureKeys) do
          DependFeatureKeysMap[FeatureKey] = true
        end
        for k, AssetID in pairs(AssetIDs) do
    ...
    for FeatureKey, v in pairs(DependFeatureKeysMap) do
      table.insert(DependFeatureKeys, FeatureKey)
    end
    for AssetID, v in pairs(DependAssetIDsMap) do
      table.insert(DependAssetIDs, AssetID)
    end
    self.paks[packID].    self.paks[packID].  end
  return self.paks[packID].DependFeatureKeys, self.paks[packID].DependAssetIDs
end
...
function PufferUGCPakManager:InitUGCDependsODPak(existPaks)
  printf("PufferUGCPakManager:InitUGCDependsODPak.")
  self:WOWEditorAutoDownload()
  self.  local StringUtil = require("common.string_util")
...
function PufferUGCPakManager:DebugPrintUGCRes()
  ...
  for _, v in pairs(assetCfg) do
    ...
    print(string.format("[DebugPrintUGCRes] AssetID:%s,Size:%.2f", tostring(AssetID), tAssetSize))
  end
  ...
  for i, v in ipairs(AllInfo) do
    print(string.format("[DebugPrintUGCRes] FeatureKey :%s,tSize:%.2f", tostring(v.Key), v.Size / PufferConst.MB))
  end
  print(string.format("[DebugPrintUGCRes] FeatureKey TotalSize:%.2f", TotalSize / PufferConst.MB))
end
