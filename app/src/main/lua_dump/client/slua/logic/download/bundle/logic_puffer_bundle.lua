...
function LogicPufferBundle.InitBundle()
  ...
  local tableList = CDataTable.GetTableByFilter("DownloaderNewTable", "IsWoWEditorBundle", IsWoWEditor)
...
function LogicPufferBundle.GetDownloadingPercent()
  ...
  local totalSize = 0
  local percent = 1
  for i, v in pairs(CDataTable.GetTableByFilter("DownloaderNewTable", "IsWoWEditorBundle", IsWoWEditor)) do
...
function LogicPufferBundle.ChooseDownloadByType(key, bSkipDownloadDeleteHistory, keyList)
  ...
  if packID and not keyList then
    ...
    if not bSkip then
      local pufferType = PufferConst.ENUM_DownloadType.ODPACK
      local cfg = CDataTable.GetTableData("PakInfoTable", packID)
      if cfg and cfg.IsUGC == 1 then
        pufferType = PufferConst.ENUM_DownloadType.UGCPACK
      end
    ...
    if keyList then
      ...
      if StringUtil.Starts(key, PufferConst.PACK_SET_PREFIX) then
        ...
        elseif StringUtil.Starts(key, PufferConst.MAP_PACK_SET_PREFIX) then
          downloadType = PufferConst.ENUM_DownloadType.MAP
    ...
    if dType == PufferConst.ENUM_DownloadType.MAP then
      local bSkip = false
      ...
      if not bSkip then
        PufferManager.Download(PufferConst.ENUM_DownloadType.MAP, {key}, PufferTlog.Enum_TLog_From.LobbyDownloader)
      end
    elseif dType == PufferConst.ENUM_DownloadType.RES then
      PufferManager.Download(PufferConst.ENUM_DownloadType.RES, {key}, PufferTlog.Enum_TLog_From.LobbyDownloader)
    elseif dType == PufferConst.ENUM_DownloadType.PREFETCH then
      PufferManager.Download(PufferConst.ENUM_DownloadType.PREFETCH, {key}, PufferTlog.Enum_TLog_From.LobbyDownloader)
    ...
    elseif type(key) == "string" and StringUtil.Ends(key, ".mp4") then
      ...
      PufferManager.Download(PufferConst.ENUM_DownloadType.ODPAK, {videoDownloadPath}, PufferTlog.Enum_TLog_From.LobbyDownloader)
...
function LogicPufferBundle.ChooseStopDownloadByType(key, bNotStartDownload, keyList)
  ...
  if packID and not keyList then
    ...
    local dType = PufferManager.GetDownloadType(key)
    if dType == PufferConst.ENUM_DownloadType.MAP then
      PufferManager.Pause(PufferConst.ENUM_DownloadType.MAP, {key}, false, bNotStartDownload)
    elseif dType == PufferConst.ENUM_DownloadType.RES then
      PufferManager.Pause(PufferConst.ENUM_DownloadType.RES, {key})
    elseif dType == PufferConst.ENUM_DownloadType.PREFETCH then
      PufferManager.Pause(PufferConst.ENUM_DownloadType.PREFETCH, {key})
      PufferManager.Pause(PufferConst.ENUM_DownloadType.ODPACK, {
        PufferConst.EODPackID.PREFETCH_ODPACKID
      })
    elseif type(key) == "string" and StringUtil.Ends(key, ".mp4") then
      local videoDownloadPath = DataMgr.GetVideoDownloadPath(key)
      PufferManager.Pause(PufferConst.ENUM_DownloadType.ODPAK, {videoDownloadPath})
    elseif key == LogicPufferBundle.UGCFeatureKeys then
      local KeyList = LogicPufferBundle.GetUGCFeatureKeyList()
      PufferManager.Pause(PufferConst.ENUM_DownloadType.ODPAK, KeyList)
    end
  end
end
...
function LogicPufferBundle.GetPackListState(packList, bSkipDepends)
  ...
  for _, vv in pairs(packList) do
    ...
    elseif PufferManager.GetDownloadType(vv) == PufferConst.ENUM_DownloadType.MAP then
      state = PufferManager.GetState(PufferConst.ENUM_DownloadType.MAP, {vv}, bSkipDepends)
    elseif PufferManager.GetDownloadType(vv) == PufferConst.ENUM_DownloadType.RES then
      state = PufferManager.GetState(PufferConst.ENUM_DownloadType.RES, {vv})
    elseif PufferManager.GetDownloadType(vv) == PufferConst.ENUM_DownloadType.PREFETCH then
      state = PufferManager.GetState(PufferConst.ENUM_DownloadType.PREFETCH, {vv})
    elseif StringUtil.Ends(vv, ".mp4") then
      local videoDownloadPath = DataMgr.GetVideoDownloadPath(vv)
      state = PufferManager.GetState(PufferConst.ENUM_DownloadType.ODPAK, {videoDownloadPath})
    elseif vv == LogicPufferBundle.UGCFeatureKeys then
      local KeyList = LogicPufferBundle.GetUGCFeatureKeyList()
      state = PufferManager.GetState(PufferConst.ENUM_DownloadType.ODPAK, KeyList)
    elseif StringUtil.Starts(vv, PufferConst.PACK_SET_PREFIX) then
      if StringUtil.Starts(vv, PufferConst.MIX_PACK_SET_PREFIX) then
        ...
        for subType, subKeys in pairs(mixDownloadData) do
          local subState = PufferManager.GetState(subType, subKeys)
...
function LogicPufferBundle.GetPackListSize(packList, ifGetODPackSizeList)
  ...
  for _, vv in pairs(packList) do
    ...
    if packID then
      ...
      curSize = curSize + cSize * PufferConst.MB
      totalSize = totalSize + tSize * PufferConst.MB
    else
      local type = PufferManager.GetDownloadType(vv)
      if type == PufferConst.ENUM_DownloadType.MAP then
        local skip = false
        if not skip then
          cSize, tSize = PufferManager.GetSize(PufferConst.ENUM_DownloadType.MAP, {vv}, true)
        end
      elseif type == PufferConst.ENUM_DownloadType.RES then
        cSize, tSize = PufferManager.GetSize(PufferConst.ENUM_DownloadType.RES, {vv})
      elseif type == PufferConst.ENUM_DownloadType.PREFETCH then
        cSize, tSize = PufferManager.GetSize(PufferConst.ENUM_DownloadType.PREFETCH, {vv})
      elseif StringUtil.Ends(vv, ".mp4") then
        local videoDownloadPath = DataMgr.GetVideoDownloadPath(vv)
        cSize, tSize = PufferManager.GetSize(PufferConst.ENUM_DownloadType.ODPAK, {videoDownloadPath})
      elseif vv == LogicPufferBundle.UGCFeatureKeys then
        local KeyList = LogicPufferBundle.GetUGCFeatureKeyList()
        cSize, tSize = PufferManager.GetSize(PufferConst.ENUM_DownloadType.ODPAK, KeyList)
      elseif StringUtil.Starts(vv, PufferConst.PACK_SET_PREFIX) then
        if StringUtil.Starts(vv, PufferConst.MIX_PACK_SET_PREFIX) then
          local mixDownloadData = LogicPufferBundle.GetMixPackDownloadContent(vv)
          local subCSize = 0
...
function LogicPufferBundle.CheckRecommendReddot()
  ...
  if bundleID and LogicPufferBundle.GetBundleState(bundleID) == PufferConst.ENUM_DownloadState.Done then
