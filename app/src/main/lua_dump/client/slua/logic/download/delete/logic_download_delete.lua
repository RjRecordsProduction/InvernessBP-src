    ...
    for _, topItem in pairs(tList) do
      if topItem.subData then
        for _, subItem in pairs(topItem.subData) do
          ...
          local deleteOrder = 0
          if subItem.downloadType == PufferConst.ENUM_DownloadType.MAP then
            local mapCfg = CDataTable.GetTableData("MapPakTable", key)
            if mapCfg and mapCfg.deleteOrder and mapCfg.deleteOrder ~= 0 then
              deleteOrder = mapCfg.deleteOrder + 1000000
            end
          else
            local pakCfg = CDataTable.GetTableData("PakInfoTable", key)
            if pakCfg and pakCfg.deleteOrder and pakCfg.deleteOrder ~= 0 then
              deleteOrder = pakCfg.deleteOrder
            end
          end
          if 0 < deleteOrder then
