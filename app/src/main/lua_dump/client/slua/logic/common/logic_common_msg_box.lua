...
function CommonMsgBoxMgr.Show(styleType, title, msg, clickOkCallback, clickCancelCallback, btnOK, btnCancel, extraData)
  if msg then
    ...
    if msg == LocUtil.GetLocalizeResStr(32628) then
...
CommonMsgBoxMgr.Show msg == 32628 
 DataMgr.roleData.mil_info = %s]], TableUtil.TableToString(DataMgr.roleData.mil_info))
      ReportPlatformCrashKit:ForceSend(errMsg, ClientToolsReport.Enum_CrashKit_Type.Enum_Lua)
    end
  end
