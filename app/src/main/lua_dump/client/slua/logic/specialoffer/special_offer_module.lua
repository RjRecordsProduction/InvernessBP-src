...
function special_offer_module:Id2MarkType(id)
  ...
  if func then
    local utility = require("common.utility")
    local _, a, b = xpcall(func, utility.ErrorMessageHandler)
    return a, b
  end
end
...
function special_offer_module:JumpIn(_, _, vars)
  ...
  if not nId then
    log(bWriteLog and "special_offer_module JumpIn not vars")
    self:OpenOneAct()
    return
  end
...
function special_offer_module:RefreshOneRed(id)
  ...
  local newCount = 0
  local markType, day = self:Id2MarkType(id)
  if markType then
    local subID = 1
    ...
    log_warning(bWriteLog and "  : special_offer_module:RefreshOneRed :" .. tostring(id))
    local ReddotConfig = require("client.slua.logic.reddot.reddot_config")
    self.redData.pages[id].    self.redData.pages[id].weight = ReddotConfig:GetWeight(systemName, self.redData.pages[id])
  end
