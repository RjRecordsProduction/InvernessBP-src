...
function ActorVoiceSystem.GetFestivalWelcomeVoiceKey(originKey)
  ...
  local items = CDataTable.GetTableByFilter("WelcomeVoiceMapCfg", "OrigVoiceKey", originKey)
  if not items then
    return originKey
  end
  local cfgs = {}
...
function ActorVoiceSystem.InitSingleActorInfoFromTableInternal(actorID, voiceCfg, ignoreTime, publishID, regionID, curTime)
  ...
  if ActorID == 31 and not LobbySystem.CheckOpen(BP_ENUM_LOBBY_MENU_CHARACTER) then
    return
  end
  ...
  if openPublish ~= "" and string.find(openPublish, publishID) == nil then
    return
  end
  ...
  if ActorItemID ~= 0 then
    ...
    if uItemCfg and uItemCfg.ItemType ~= Voice_Pack then
      if Client and Client.IsDevelopment() and openPublish ~= "0" then
