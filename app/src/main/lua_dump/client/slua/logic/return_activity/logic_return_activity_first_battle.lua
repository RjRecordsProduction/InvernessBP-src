...
function logic_return_activity_first_battle:GetModeIntroductionList()
  ...
  for k, v in pairs(versionCfg) do
    local versionMatch = 0 <= version_util.CompareVersionMain(v.CurrentVersion, CurrentMainVersion) and lastLoginVersion >= v.VersionMin and lastLoginVersion < v.VersionMax
    if versionMatch then
      ...
      if isExactMatch then
        ...
        local dataList = {
          CDataTable.GetTableData("ReturnVersionInfoCardConfig1", v.InfoCard1ID),
          CDataTable.GetTableData("ReturnVersionInfoCardConfig2", v.InfoCard2ID),
          CDataTable.GetTableData("ReturnVersionInfoCardConfig3", v.InfoCard3ID)
        }
  ...
  if genericConfigID and genericConfigRow then
    ...
    local dataList = {
      CDataTable.GetTableData("ReturnVersionInfoCardConfig1", genericConfigRow.InfoCard1ID),
      CDataTable.GetTableData("ReturnVersionInfoCardConfig2", genericConfigRow.InfoCard2ID),
      CDataTable.GetTableData("ReturnVersionInfoCardConfig3", genericConfigRow.InfoCard3ID)
    }
    return dataList
  end
  return nil
...
function logic_return_activity_first_battle:ShouldShowReturnModeSelect()
  ...
  local PlayerPrefsSystem = require("client.logic.LogicPlayerPrefs.playerprefs")
  local startGameData = PlayerPrefsSystem.LoadFileToTable_N(PlayerPrefsSystem.ePlayerPrefsType.eReturnPlayerModeSelectStartGame) or {}
  if startGameData[userData.rejoin_start_time] == true then
    log(bWriteLog and "logic_return_activity_first_battle:ShouldShowReturnModeSelect - Already clicked start game")
    return false
  end
