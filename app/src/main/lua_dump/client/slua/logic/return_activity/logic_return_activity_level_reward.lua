...
function logic_return_activity_level_reward:GetMultChoose1Data(level)
  ...
  if not rewards then
    log_warning_format(bWriteLog and "logic_return_activity_level_reward:GetMultChoose1Data, rewards not found for level:%s", level)
    return nil
  end
...
function logic_return_activity_level_reward:GetNextSelectRewardLevel(currentLevel)
  ...
  for level = currentLevel + 1, #logic_longline_task.rewardList do
    if self:IsSelectReward(level) then
      return level
    end
  end
  return nil
