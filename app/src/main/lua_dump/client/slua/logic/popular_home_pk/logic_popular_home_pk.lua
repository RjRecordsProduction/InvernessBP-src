...
function logic_popular_home_pk:CanShowTipsEntry(uid)
  ...
  if not logic_popular_home_pk:IsSelfHasEnemy() then
...
function logic_popular_home_pk:UpdateMyHomePkData(ret_data)
  ...
  if not ret_data.right and self.myHomePkData.right then
    self.myHomePkData.right.cur_vote = 0
    self.myHomePkData.right.uid = 0
  end
  if not ret_data.left and self.myHomePkData.left then
    self.myHomePkData.left.cur_vote = 0
    self.myHomePkData.left.uid = 0
  end
  for k, v in pairs(ret_data) do
    self.myHomePkData[k] = v
  end
end
