...
function logic_roleInfo_background:UpdatePlayerEquipBGLevel(uid, loadedCallback)
  ...
  self:UpdateRoleInfoBGByBGID(curEquipID, loadedCallback)
end
...
function logic_roleInfo_background:UpdateRoleInfoBGByBGID(bgID, loadedCallback)
  ...
  if roleInfoBGCfg.HighSequencePath and roleInfoBGCfg.HighSequencePath ~= "" then
    ...
    if state ~= ENUM_DownloadState.Done then
      self:WaitDownloadThenUpdateBG(bgID, loadedCallback)
      return
    end
  end
  local Extra = {
...
function logic_roleInfo_background:WaitDownloadThenUpdateBG(bgID, loadedCallback)
  log(bWriteLog and string.format("logic_roleInfo_background:WaitDownloadThenUpdateBG bgID=%s", tostring(bgID)))
  ...
  local PufferManager = require("client.slua.logic.download.puffer.puffer_manager")
  local roleInfoBGCfg = CDataTable.GetTableDataByFilter("RoleInfoBackgroundCfg", "ID", bgID)
  local callback = function()
    log(bWriteLog and string.format("logic_roleInfo_background:WaitDownloadThenUpdateBG PufferManager.Download callback self.curPreviewBGID=%s, bgID=%s", tostring(self.curPreviewBGID), tostring(bgID)))
    if self.curPreviewBGID == self.defaultRoleInfoBGID or self.curPreviewBGID == self.curWaitToPreviewBGID then
    else
      return
    end
    self:UnloadCurrentRoleInfoBGLevel()
