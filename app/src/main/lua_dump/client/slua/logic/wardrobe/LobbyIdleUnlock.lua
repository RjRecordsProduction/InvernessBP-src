...
function LobbyIdleUnlock:IsAvatarWearingClothes(CurAvatar, ClothesID)
  ...
  local UpgradeInfo = XSuitUtil:GetUpgradeInfo(ClothesID)
  local ItemInfoList = Period and UpgradeInfo[Period]
  for _, EquipmentInfo in pairs(EquipmentInfoList) do
    if bIsXSuit and ItemInfoList then
...
function LobbyIdleUnlock:GetClothesBaseID(ItemID)
  ...
  local UpgradeInfo = XSuitUtil:GetUpgradeInfo(ItemID)
  local ItemInfoList = Period and UpgradeInfo[Period]
  if ItemInfoList and ItemInfoList[1] then
    baseClothesID = ItemInfoList[1].item_id
  end
