...
function UGCHandler.on_ugc_take_season_award_rsp(err_code, awards, invoke_type)
  ...
  LogicUGCSeasonAward:on_task_season_segment_prize_all_rsp(awards, invoke_type)
end
...
function UGCHandler.on_ugc_get_season_award_list_rsp(ok, season, cur_season_id, is_idle_time, pre_best_segment)
  local UGCSeason = require("client.slua.logic.ugc.logic_ugc_season")
  UGCSeason.get_season_award_list_rsp(ok, season, cur_season_id, is_idle_time, pre_best_segment)
end
...
function UGCHandler.on_ugc_get_mod_item_rsp(err, the_depot_data)
  ...
  local LogicUGCPropShop = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.logic_ugc_propshop)
  LogicUGCPropShop:RspGetModProp(the_depot_data)
end
