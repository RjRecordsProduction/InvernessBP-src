...
function FlashTeamHandler.send_create_flash_squad_req(name, active_slots, need_apply, allow_member_message, invite_uids)
  ...
  NetManager.SendPkg(1479256327, name, active_slots, need_apply, allow_member_message, invite_uids)
end
...
function FlashTeamHandler.send_join_flash_squad_req(squad_id, source)
...
function FlashTeamHandler.on_join_flash_squad_rsp(ret, squad_id, squad_summary, apply_id)
  ...
  EventSystem:postEvent(EVENTTYPE_FLASH_TEAM, EVENTID_FLASH_TEAM_PRETEAM_CHG)
end
...
function FlashTeamHandler.send_get_flash_squad_info_req(squad_id)
  log_format(bWriteLog and "FlashTeamHandler.send_get_flash_squad_info_req suqad_id:%d", squad_id)
...
function FlashTeamHandler.send_get_flash_squad_recommend_req(count, filter)
  log_format(bWriteLog and "FlashTeamHandler.send_get_flash_squad_recommend_req count:%s, filter:%s", count, filter)
...
function FlashTeamHandler.on_get_prefer_modes_for_flash_squad_rsp(prefer_modes, teamates_list)
  local logic_teamquick_join = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.logic_teamquick_join)
  logic_teamquick_join:proc_get_prefer_modes_for_flash_squad_rsp(prefer_modes, teamates_list)
end
