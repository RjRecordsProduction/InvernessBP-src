...
local GetParsedOneLightParam = function(OneLightParam)
  ...
  local Paramlist = StringUtil.Split(OneLightParam, "|")
  for i, v in ipairs(Paramlist) do
    Paramlist[i] = tonumber(v)
  end
  ParseLightParam.Intensity = Paramlist[1]
  if 4 <= #Paramlist then
    ParseLightParam.Location = FVector(Paramlist[2], Paramlist[3], Paramlist[4])
  end
  if 7 <= #Paramlist then
    ParseLightParam.Rotation = FRotator(Paramlist[5], Paramlist[6], Paramlist[7])
  end
