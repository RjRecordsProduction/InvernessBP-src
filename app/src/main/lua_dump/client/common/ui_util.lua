...
local UIUtil = {
  ...
  ClickFrequencyLimit = {
    ...
    golden_suit_upgrade = {DefalutFrequencyLimit, 0},
...
function UIUtil.SetAdaptation(widget)
  ...
  local left = tonumber(result[1]) or 0
  local right = tonumber(result[3]) or 0
  ...
  if Slot.SetAnchors then
    Slot:SetOffsets(FMargin(left, 0, right, 0))
  end
...
local TBgQualityPath = {
  ...
  [10] = "/Game/Mod/TPlan/Arts/UI/Atlas/Frames/Txmission_icon_shop_010_png.Txmission_icon_shop_010_png"
}
...
function UIUtil.GetAssetBigIcon(assetID)
  local OverrideIcon = UIUtil.GetOverrideIcon(assetID, true)
  ...
  if not assetCfg then
    ...
    if AssetManager and AssetManager:IsPreset(assetID) then
      local ProjectileIcon = UIUtil.CheckAndGetProjectileIcon(assetID, true, true)
      if ProjectileIcon then
        return ProjectileIcon
      else
        local SourceAssetID = AssetManager:GetPresetSourceAssetID(assetID, true)
        return UIUtil.GetAssetBigIcon(SourceAssetID)
      end
    end
    log(bWriteLog and "[Warning] Can not find asset cfg: " .. tostring(assetID))
    return UIUtil.GetDefaultIcon()
  end
  if assetCfg.HDIcon and assetCfg.HDIcon ~= "" then
    local pak_util = require("client.common.pak_util")
    if pak_util.IsPufferDownloaded(assetCfg.HDIcon) then
      return assetCfg.HDIcon
    end
  end
  return UIUtil.GetAssetSmallIcon(assetID)
end
function UIUtil.GetAssetSmallIcon(assetID)
  local OverrideIcon = UIUtil.GetOverrideIcon(assetID, false)
  if OverrideIcon then
    return OverrideIcon
  end
  local assetCfg = CDataTable.GetTableData("UGCAssetConfig", assetID)
  if not assetCfg then
    local AssetManager = GetAssetManager()
    if AssetManager and AssetManager:IsPreset(assetID) then
      local ProjectileIcon = UIUtil.CheckAndGetProjectileIcon(assetID, true, false)
      if ProjectileIcon then
        return ProjectileIcon
      else
        local SourceAssetID = AssetManager:GetPresetSourceAssetID(assetID, true)
        return UIUtil.GetAssetSmallIcon(SourceAssetID)
      end
    end
    log(bWriteLog and "[Warning] Can not find asset cfg: " .. tostring(assetID))
    return UIUtil.GetDefaultIcon()
  end
  if assetCfg.Icon and assetCfg.Icon ~= "" then
    local pak_util = require("client.common.pak_util")
    if pak_util.IsPufferDownloaded(assetCfg.Icon) then
      return assetCfg.Icon
    end
  end
  log(bWriteLog and "UIUtil.GetAssetSmallIcon Can't Get SmallIcon" .. assetID)
  return UIUtil.GetDefaultIcon()
end
function UIUtil.CheckAndGetProjectileIcon(AssetIdOrPresetId, bIsPreset, bIsHDIcon)
  local AssetManager = GetAssetManager ~= nil and GetAssetManager() or nil
  if AssetManager == nil then
    return
  end
  if bIsPreset then
    local CreativeModeProjectileDefine = require("GameLua.Mod.CreativeBase.Gameplay.Subsystem.Projectile.CreativeModeProjectileDefine")
    local SourceAssetID = AssetManager:GetPresetSourceAssetID(AssetIdOrPresetId, true)
    if CreativeModeProjectileDefine.AssetIdConfig[SourceAssetID] ~= nil then
      local PresetID = AssetIdOrPresetId
      local CustomPresetConfigData = AssetManager:GetCustomPresetConfigData()
      return CreativeModeProjectileDefine.GetMeshIconByPreset(CustomPresetConfigData[PresetID].Projectile.ProjectileMeshPreset, bIsHDIcon)
    end
  else
    local CreativeModeProjectileDefine = require("GameLua.Mod.CreativeBase.Gameplay.Subsystem.Projectile.CreativeModeProjectileDefine")
    if CreativeModeProjectileDefine.AssetIdConfig[AssetIdOrPresetId] ~= nil then
      local AssetConfig = AssetManager:GetAssetInfo(AssetIdOrPresetId)
      local SubstituteAssetID = AssetManager:GetSubstituteAssetID(AssetIdOrPresetId)
      if SubstituteAssetID ~= nil then
        local CustomPresetConfigData = AssetManager:GetCustomPresetConfigData()
        AssetConfig = CustomPresetConfigData[SubstituteAssetID]
      end
      if AssetConfig and AssetConfig.Projectile.ProjectileMeshPreset then
        return CreativeModeProjectileDefine.GetMeshIconByPreset(AssetConfig.Projectile.ProjectileMeshPreset, bIsHDIcon)
      end
    end
  end
end
function UIUtil.GetOverrideIcon(AssetID, bIsHDIcon)
  local AssetManager = GetAssetManager()
  local Real  if AssetManager and AssetManager:IsPreset(AssetID) then
    RealAssetID = AssetManager:GetPresetSourceAssetID(AssetID, true)
  end
  if RealAssetID and 0 < RealAssetID then
    local AssetConfig = AssetManager:GetAssetInfo(RealAssetID)
    if AssetConfig and AssetConfig.GetOverrideIcon then
      return AssetConfig.GetOverrideIcon(AssetID, RealAssetID ~= AssetID, bIsHDIcon)
    end
  end
end
...
function UIUtil.PreLoadBp(path, pool)
  if not path or path == "" then
    return
  end
