...
function ui_show_queue_manager.BlockSmallType(list)
  log_tree("i_show_queue_manager.BlockSmallType list = ", list)
...
function ui_show_queue_manager.ReleaseSmallTypeBlock(list)
  log_tree("i_show_queue_manager.ReleaseSmallTypeBlock list = ", list)
  for _, smallTypeID in pairs(list) do
...
function ui_show_queue_manager._ReShowUI()
  local time_ticker = require("common.time_ticker")
  if ui_show_queue_manager.reShowTimer then
    time_ticker.RemoveTimer(ui_show_queue_manager.reShowTimer)
    ui_show_queue_manager.reShowTimer = nil
  end
  local reshowFunc = function()
    ui_show_queue_manager._CheckListSignIsExist()
    ui_show_queue_manager._ShowUIFromSortList()
    ui_show_queue_manager._ShowUIFromDirectList()
  end
  if ui_show_queue_manager.onceBlockDelayTimer then
    log(bWriteLog and "ui_show_queue_manager._ReShowUI Now Show")
    ui_show_queue_manager.CancelOnceBlockDelayTimer()
    reshowFunc()
    return
  end
  ui_show_queue_manager.reShowTimer = time_ticker.AddTimerOnce(ui_show_queue_config.ReShowDelayTime, function()
...
function ui_show_queue_manager._AddOneUIToSortList(lqcUIConfig, uiConfig, ...)
  ...
  if ui_show_queue_manager.sortListCurUI == nil and len == 0 then
    local canShow = ui_show_queue_manager._CheckOneCanShow(lqcUIConfig)
    if canShow then
      local showUIInfo = UIManager.DirectShowUI(uiConfig, ...)
      local showSuccess = showUIInfo ~= nil
      if showSuccess then
        ui_show_queue_manager.sortListCurUI = uiConfig.keyName
      end
...
function ui_show_queue_manager._RemoveOneFromSortList(uiConfig, ParamTable)
  ...
  local hasParam = UIKey ~= nil or Param ~= nil
  local isRemove = false
  local len = #ui_show_queue_manager.sortList
  for i = len, 1, -1 do
    local element = ui_show_queue_manager.sortList[i]
    if element.args[1].keyName == keyName and hasParam and (element.lqcUIConfig.UIKey == UIKey or element.lqcUIConfig.Param == Param) then
      table.remove(ui_show_queue_manager.sortList, i)
      isRemove = true
      break
    end
...
function ui_show_queue_manager._ShowUIFromSortList()
  ...
  if element.args then
    for i = 2, element.args.n do
      local argVal = element.args[i]
      if argVal ~= nil and type(argVal) == "userdata" and not slua.isValid(argVal) then
        log_warning_format("ui_show_queue_manager._ShowUIFromSortList args[%d] had been freed, set to nil. UIKey = %s", i, element.lqcUIConfig and element.lqcUIConfig.UIKey)
        element.args[i] = nil
      end
    end
  end
...
function ui_show_queue_manager._ShowUIFromDirectList()
  ...
  if element.args then
    for i = 2, element.args.n do
      local argVal = element.args[i]
      if argVal ~= nil and type(argVal) == "userdata" and not slua.isValid(argVal) then
        log_warning_format("ui_show_queue_manager._ShowUIFromDirectList args[%d] had been freed, set to nil. UIKey = %s", i, element.lqcUIConfig and element.lqcUIConfig.UIKey)
        element.args[i] = nil
      end
    end
  end
