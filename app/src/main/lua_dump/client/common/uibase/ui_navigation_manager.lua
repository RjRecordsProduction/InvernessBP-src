...
function ui_navigation_manager:_OnAndroidBackWithClick()
  ...
  if uiInfo.styleType == self.EnumStyleType.SLuaUI then
    ...
    while uiInfo do
      ...
      count = count + 1
      if count >= #MainUIStack then
        log_error("ui_navigation_manager:_OnAndroidBackWithClick:stack count >= #MainUIStack")
        return
      end
      uiInfo = MainUIStack[#MainUIStack - count]
    end
...
function ui_navigation_manager:AndroidBackToUI(keyName, ignoreBan)
  ...
  while uiInfo do
    ...
    if skipCount >= #MainUIStack then
      log_error("ui_navigation_manager:AndroidBackToUI:stack skipCount >= #MainUIStack")
      return
    end
    lastUIInfo = uiInfo
    ...
    if maxCount < count then
      log_error("ui_navigation_manager:AndroidBackToUI:stack count >= maxCount")
      return
    end
  end
end
