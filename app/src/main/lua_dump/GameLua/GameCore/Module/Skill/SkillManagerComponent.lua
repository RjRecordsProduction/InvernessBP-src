...
local NamedFeatures = {
  CreativeSkillCompFeature = "GameLua.GameCore.Module.Skill.CreativeSkillCompFeature"
}
function SkillManagerComponent:BroadCastCallNamedFeatures(FunctionName)
  for FeatureName, _ in pairs(NamedFeatures) do
    if self[FeatureName] and self[FeatureName][FunctionName] and type(self[FeatureName][FunctionName]) == "function" then
      local Feature = self[FeatureName]
      self[FeatureName][FunctionName](Feature)
    end
  end
end
...
function SkillManagerComponent:_PostConstruct()
  ...
  self:BroadCastCallNamedFeatures("_PostConstruct")
end
...
function SkillManagerComponent:ReceiveBeginPlay()
  ...
  self:BroadCastCallNamedFeatures("ReceiveBeginPlay")
  if not Client then
...
function SkillManagerComponent:ReceiveEndPlay(EndPlayReason)
  ...
  self.SkillUIData = {}
  self:BroadCastCallNamedFeatures("ReceiveEndPlay")
...
function SkillManagerComponent:GetSkillCDBase(SkillID, Index)
  ...
  Index = Index or 0
  if Index < SkillCDs:Num() and 0 <= Index then
...
return require("combine_class").GenerateNamedFeatureClass(CSkillManagerComponent, NamedFeatures)