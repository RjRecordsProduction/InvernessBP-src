...
function GameModeBase:HandleOnPawnDie(_, __, Pawn, Killer, TypeID)
  ...
  if slua.isValid(Pawn) and Pawn.GetPlayerStateSafety then
    ...
    if uPlayerState and slua.isValid(uPlayerState) and (uPlayerState.GetRevivalCount and uPlayerState:GetRevivalCount() > 0 or uPlayerState.GetLeftBuyLifeCounts and 0 < uPlayerState:GetLeftBuyLifeCounts()) then
...
function GameModeBase:IsRevivalGameMode(uPlayerState)
  ...
  if bIsRevivalMode == false and uPlayerState.GetRevivalCount then
    bIsRevivalMode = uPlayerState:GetRevivalCount() > 0
  end
  if bIsRevivalMode == false and uPlayerState.GetLeftBuyLifeCounts then
    bIsRevivalMode = 0 < uPlayerState:GetLeftBuyLifeCounts()
  end
...
function GameModeBase:IsPlayerCanSelfRevival(uPlayerState)
  ...
  if bCanSelfRevival == false then
    local DSReviveSubsystem = SubsystemMgr:Get("DSReviveSubsystem")
    if DSReviveSubsystem and DSReviveSubsystem:GetExceptionalReviveConfig() and uPlayerState.GetLeftBuyLifeCounts and 0 < uPlayerState:GetLeftBuyLifeCounts() then
      bCanSelfRevival = true
    end
  end
...
function GameModeBase:ClearPlayerRevialAndSendBattleResult(uPlayerState, bSendBattleResult, bSyncSend, bSendTeamResult)
  if uPlayerState and slua.isValid(uPlayerState) then
    if uPlayerState.GetRevivalCount and uPlayerState:GetRevivalCount() > 0 then
