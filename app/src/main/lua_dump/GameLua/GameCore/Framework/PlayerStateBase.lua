          ...
          if IsAI then
            ...
            if ReviveInfoConfig and ReviveInfoConfig.AIProbability then
              ...
              if AIProbability >= Temp then
                self:SetLeftBuyLifeCounts(DefaultBuyLifeCount, InCharacter)
...
function PlayerStateBase:GetLeftBuyLifeCounts()
  return math.floor(self.nLeftBuyLifeCounts + 0.1)
end
function PlayerStateBase:SetLeftBuyLifeCounts(Count, Character)
  if Count > self.nLeftBuyLifeCounts then
    ...
    if Game:IsValid(Controller) and self:CanAddRevivalCount(Controller) == false then
      print(bWriteLog and "PlayerStateBase:SetLeftBuyLifeCounts, dont allow to add LifeCount, PlayerKey = " .. tostring(self.PlayerKey))
      return false
    end
  end
  if self.nLeftBuyLifeCounts == Count then
    print(bWriteLog and "PlayerStateBase:SetLeftBuyLifeCounts, Count = " .. tostring(Count) .. " = nLeftBuyLifeCounts")
    return false
  end
  print(bWriteLog and "PlayerStateBase:SetLeftBuyLifeCounts, Count = " .. tostring(Count) .. ", PlayerKey = " .. tostring(self.PlayerKey))
...
function PlayerStateBase:GetHelpRevivalCount()
  local RevivalCount = self:GetLeftBuyLifeCounts()
  local Temp = self:GetRevivalCount()
  if 0 < Temp then
    local DSReviveSubsystem = SubsystemMgr:Get("DSReviveSubsystem")
    if DSReviveSubsystem then
      local ReviveInfoConfig = DSReviveSubsystem:GetReviveInfoByModeType()
      if ReviveInfoConfig and (ReviveInfoConfig.ReviveTower or ReviveInfoConfig.Stronghold) then
        RevivalCount = RevivalCount + Temp
      end
    else
      print(bWriteLog and "PlayerStateBase:GetHelpRevivalCount, DSReviveSubsystem = nil")
    end
  end
  print(bWriteLog and "PlayerStateBase:GetHelpRevivalCount, RevivalCount = " .. tostring(RevivalCount) .. ", PlayerKey = " .. tostring(self.PlayerKey))
  return RevivalCount
end
function PlayerStateBase:LuaGetLeftBuyLifeCounts()
  return self:GetLeftBuyLifeCounts()
end
function PlayerStateBase:LuaSetLeftBuyLifeCounts(Count, Character)
  self:SetLeftBuyLifeCounts(Count, Character)
end
function PlayerStateBase:SetCharacterDestroyAndAnimationBlueprintVariable(Character)
  ...
  if uPlayerCharacter and slua.isValid(uPlayerCharacter) then
    local GeneralRevivalCount = self:GetRevivalCount()
    local StoreReviveCount = self:GetLeftBuyLifeCounts()
    if GeneralRevivalCount <= 0 and StoreReviveCount <= 0 then
...
function PlayerStateBase:SendBattleResultIfNeedAndDisappearOnDeath(Character)
  if self:HasAuthority() then
    local GeneralRevivalCount = self:GetRevivalCount()
    local StoreReviveCount = self:GetLeftBuyLifeCounts()
    if StoreReviveCount <= 0 and GeneralRevivalCount <= 0 and self:IsAlive() == false then
      ...
      if GameplayStatics and self.Object then
        ...
        if slua.isValid(uPlayerController) and uPlayerController.PlayerState == self.Object then
          ...
          if not uPlayerController:IsInSpectating() and not uPlayerController:IsInPetSpectator() and self.IsInWaittingRevivalState then
            ...
            if not self.bHasSendBattleResult and (0 < self:GetRevivalCount() or 0 < self:GetLeftBuyLifeCounts()) then
