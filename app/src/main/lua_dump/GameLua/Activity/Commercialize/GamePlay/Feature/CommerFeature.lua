...
function CommerFeature:OnTeamShowPreCreate()
  ...
  self:OnTeamShowPreCreateProcessXSuit()
  self:OnTeamShowPreCreateProcessPetData()
  self:OnTeaamShowPreCreateProcessMiniTvData()
end
...
CommerFeature.ClientRPC.RPCClient_TeamShowUnlockDataRsp = {
  Reliable = true,
  Params = {
    UEnums.EPropertyClass.Str
  }
}
function CommerFeature:OnTeamShowPreCreateProcessXSuit()
  print(bWriteLog and "CommerFeature:OnTeamShowPreCreateProcessXSuit")
  ...
  for _, uid in pairs(UIDList) do
    if not XSuitAvatarDataUtil.TeamShowLockData[uid] then
      local lockList = {}
      local UnLockFeatureCfg = XSuitUtil:GetUnlockFeatureCfg(UnLockFeatureType.TeamEnterAction2)
      if UnLockFeatureCfg and UnLockFeatureCfg.LockPeriod_a then
        local level, index = UnLockFeatureCfg.EffectLevel, UnLockFeatureCfg.UnlockIndex
        for _, period in pairs(UnLockFeatureCfg.LockPeriod_a) do
          local data = XSuitAvatarDataUtil.UnlockFeatureDataMap[uid]
          local lock = true
          if data and data[period] and data[period].unlock_info and data[period].unlock_info[level] and data[period].unlock_info[level][index] and data[period].unlock_info[level][index].state == 1 then
            lock = false
          end
          if lock then
            table.insert(lockList, period)
          end
        end
      end
      XSuitAvatarDataUtil.TeamShowLockData[uid] = table.concat(lockList, ",")
    end
    if XSuitAvatarDataUtil.TeamShowLockData[uid] ~= "" then
      table.insert(DataStrList, uid .. "_" .. XSuitAvatarDataUtil.TeamShowLockData[uid])
    end
  end
  if 0 < #DataStrList then
    print(bWriteLog and "CommerFeature:OnTeamShowPreCreateProcessXSuit RPC")
    self:RPCClient_TeamShowUnlockDataRsp(table.concat(DataStrList, "|"))
  end
end
...
function CommerFeature:RPCClient_TeamShowUnlockDataRsp(DataStr)
  print(bWriteLog and "CommerFeature:OnTeamShowPreCreate")
  if DataStr then
    print(bWriteLog and "CommerFeature:OnTeamShowPreCreate " .. tostring(DataStr))
    local StringUtil = require("common.string_util")
    local DataStrList = StringUtil.Split(DataStr, "|")
    for _, v in pairs(DataStrList) do
      local tmpDataList = StringUtil.Split(v, "_")
      local UID = tonumber(tmpDataList[1])
      local periodList = {}
      for _, period in pairs(StringUtil.Split(tmpDataList[2], ",")) do
        if tonumber(period) then
          periodList[tonumber(period)] = true
        end
      end
      self.TeamShowLockData[UID] = periodList
    end
  end
end
