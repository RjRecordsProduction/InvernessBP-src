...
function PHomeStoreUtils.GetIsCanBuy(phomeStoreCfg, storeId)
  local limitType, limitNum = PHomeStoreUtils.GetBuyLimitInfo(phomeStoreCfg)
  if limitType == 3 then
    local PHomeStoreProxy = require("client.slua.logic.homestore.PHomeStoreProxy")
    local depotCount = PHomeStoreProxy:GetDepotItemCount(phomeStoreCfg.PH_item_id)
    printf("PHomeStoreUtils.GetIsCanBuy depotCount:%s limitNum:%s canBuy:%s", depotCount, limitNum, limitNum > depotCount)
    if limitNum <= depotCount then
      return false
    end
    local hasBuyNum = PHomeStoreProxy:GetHasBuyStoreCount(storeId)
    printf("PHomeStoreUtils.GetIsCanBuy hasBuyNum:%s limitNum:%s canBuy:%s", hasBuyNum, limitNum, limitNum > hasBuyNum)
