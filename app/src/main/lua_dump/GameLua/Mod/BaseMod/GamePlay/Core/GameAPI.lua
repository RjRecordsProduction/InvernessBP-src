...
function Game:AddItemByResID(uPlayer, nResId, nNum, bWithTips, nInstanceId, nDurability, nPickupType, bAutoEquip)
  ...
  nDurability = nDurability or -1
  nDurability = math.floor(nDurability)
    ...
    if OwnerActor:HasAuthority() then
      ...
      for _, uComp in pairs(ChildrenComps) do
        if slua.isValid(uComp) then
          ...
          if slua.isValid(uActor) and (Game:IsClassOf(uActor, PickUpWrapperClass) or Game:IsClassOf(uActor, PlayerTombBoxClass) or Game:IsClassOf(uActor, IdeaDecalActorClass)) then
  ...
  if slua.isValid(CGameState) and uLoc then
    ...
    if PickUpActorList then
      for _, PickUpActor in pairs(PickUpActorList) do
        if slua.isValid(PickUpActor) and PickUpActor.UpdateLocation then
...
function Game:SetShowPawnAvatarFromPlayerKey(uShowPawn, nPlayerKey, AllMeshLoadedBack)
  ...
  if slua.isValid(uPawn) and slua.isValid(uShowPawn) then
    ...
    if slua.isValid(AvatarComp) and slua.isValid(CharacterAvartarCom) then
      ...
      CharacterAvartarCom.NetAvatarData = Game:CopyNetAvatarDataToLobbyPawn(AvatarComp, bRealHideHelmet)
...
function Game:CopyNetAvatarDataToLobbyPawn(uCharacterAvatarComponent, bHideHelmet)
  if bHideHelmet == nil then
    bHideHelmet = true
  end
  if slua.isValid(uCharacterAvatarComponent) then
    ...
    for Index, SlotData in pairs(TemData.SlotSyncData) do
      if SlotData and SlotData.ItemID ~= 0 then
        if SlotData.SlotID > 16 and SlotData.SlotID <= 21 or SlotData.SlotID == 15 or SlotData.SlotID == 11 or SlotData.SlotID == 8 or SlotData.SlotID == 10 or bHideHelmet and SlotData.SlotID == 9 then
