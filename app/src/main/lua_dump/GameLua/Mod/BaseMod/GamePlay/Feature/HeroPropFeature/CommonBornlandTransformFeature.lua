...
function CommonBornlandTransformFeature:EnsureGyroscope(bForbid)
  ...
  if not SettingSubsystem then
    print(bWriteLog and "CommonBornlandTransformFeature:EnsureGyroscope not SettingSubsystem")
    return
  end
  local uPlayerController = self.Owner:GetControllerSafety()
  ...
  if bForbid then
    if not self.PreGyroscope then
      SettingSubsystem:RegisterUserSettingsDelegate_Int("Gyroscope", function(Gyroscope)
        if CommonTransformConfig:CheckCommonBornlandTransform(self.Owner.HeroPropFeature:GetCurrentHeroID()) then
          print(bWriteLog and "CommonBornlandTransformFeature:EnsureGyroscope RegisterUserSettingsDelegate_Int Gyroscope=" .. Gyroscope)
