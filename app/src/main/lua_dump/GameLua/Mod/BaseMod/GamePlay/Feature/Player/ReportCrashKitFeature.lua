...
local CrashKitReportCharacterAttachedOnVehicleException = "CharacterAttachedOnVehicleException"
...
function ReportCrashKitFeature:ReportCharacterAttachedOnVehicleException(sAttachExceptionInfo)
  if not GameReportUtils.CheckCanBugglyPostException(CrashKitReportCharacterAttachedOnVehicleException) then
    return
  end
  if not self.Owner then
    return
  end
  local uReportCharacter = self.Owner:GetPlayerCharacterSafety()
  if slua.isValid(uReportCharacter) then
    GameReportUtils.BugglyPostExceptionFull(CrashKitReportCharacterAttachedOnVehicleException, sAttachExceptionInfo, Client.IsEditor() or Client.IsDevelopment())
  end
end
