...
function BackpackComponent:ReceiveBeginPlay()
  ...
  if Client then
    ...
    if SettingSubsystem then
      self.bDropUnusefulMeleeHandle = SettingSubsystem:RegisterUserSettingsDelegate_Bool("bDropUnusefulMelee", function(bDropUnusefulMelee)
        self:ServerSetDropMeelWeapon(bDropUnusefulMelee)
      end)
      local DropUnusefulMelee = SettingSubsystem:GetUserSettings_Bool("bDropUnusefulMelee")
      print(bWriteLog and "BackpackComponent:ReceiveBeginPlay Set DropUnusefulMelee", DropUnusefulMelee)
      self:ServerSetDropMeelWeapon(DropUnusefulMelee)
    end
  end
...
function BackpackComponent:ReceiveEndPlay(nEndPlayReason)
  ...
  if Client then
    local SettingSubsystem = SubsystemMgr:Get("SettingSubsystem")
    if self.bDropUnusefulMeleeHandle and SettingSubsystem then
      SettingSubsystem:UnregisterUserSettingDelegate(self.bDropUnusefulMeleeHandle)
...
function BackpackComponent:SelfBeginPlay()
  if Client or not Client and KismetSystemLibrary.IsStandalone(self) then
    ...
    local SettingSubsystem = SubsystemMgr:Get("SettingSubsystem")
    if SettingSubsystem then
      self:ModifyAimNotAutoUse(SettingSubsystem:GetUserSettings_Bool("AutoEquipAim"))
      self:ModifyAutoPickClipType(SettingSubsystem:GetUserSettings_Int("AutoPickClipType"))
      SettingSubsystem:RegisterUserSettingsDelegate_Bool("AutoEquipAim", function(Value)
        self:EventAutoEquipAim(Value)
      end)
      SettingSubsystem:RegisterUserSettingsDelegate_Int("AutoPickClipType", function(Value)
