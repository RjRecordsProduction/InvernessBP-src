    ...
    if v.wear_ext and v.wear_ext[3] and v.wear_ext[3][1] then
      ...
      if period then
        if result.gold_dress_level_set_info and source ~= 1 then
          ...
          if levelInfo and levelInfo[period] then
            local newItemID = XSuitUtil:GetSwitchItemByItemAndSwitchLevel(itemID, levelInfo[period])
        ...
        if result.gold_dress_state_info and result.gold_dress_state_info[v.UID] then
          local stateInfo = result.gold_dress_state_info[v.UID]
          if stateInfo and stateInfo[period] then
            local newItemID = XSuitUtil:ChangeItemIDByState(itemID, stateInfo[period].cur_state)
