...
function BlueCircleUI:ctor(selfType, ParentUI, MapUI)
  printf("BlueCircleUI:ctor")
  self.TickRat = 0.5
  self.  self.MapUIBase = MapUI.CurrentMapUI
...
function BlueCircleUI:CommonTick()
  if not self.bIsClosed then
    ...
    if uGameState:GetCurCircleWave() <= -1 or self.InBlueCircle:IsZero() then
      self:SetVisible(self.UIRoot.Border_FullRing, false)
      self:SetVisible(self.UIRoot.Border_RingFX, false)
    elseif self.InBlueCircle.Z < 1000.0 then
      self:SetVisible(self.UIRoot.Border_FullRing, true)
      self:SetVisible(self.UIRoot.Border_RingFX, false)
    else
      self:SetVisible(self.UIRoot.Border_FullRing, false)
      self:SetVisible(self.UIRoot.Border_RingFX, true)
      ...
      self.UIRoot.Border_RingFX:SetRenderTranslation(slua.IndexReference(self.RealTimeInfo, "BlueCircleCoord"))
