...
function KillInfo:SetSkillIcon(DamageType, AdditionalParam, PreviousHealthStatus)
  ...
  if SkillIconPath and SkillIconPath ~= "" then
    ...
    self.Image_WeaponIcon:SetBrushFromPathAsync(SkillIconPath, false)
...
function KillInfo:PlayKillBroadcastAudio(WeaponAvatarID, DamageRecordData)
  ...
  if DataMgr then
    ...
    if 0 < GroupID then
      local SwitchFlag = DataMgr.GetWeaponSkinSoundSwitchInfo(GroupID)
      if SwitchFlag then
        local EWeaponOverrideSoundSwitchType = import("/Script/ShadowTrackerExtra.EWeaponOverrideSoundSwitchType")
        if SwitchFlag & 1 << EWeaponOverrideSoundSwitchType.KillBroadcast == 0 then
          NeedPlayAudio = true
        end
      else
        NeedPlayAudio = true
...
function KillInfo:UpdateColorLua(RelationShip, WeaponAvatarID, IsUseColor, UseColor)
  ...
  if WeaponAvatarConfig and WeaponAvatarConfig.IsFixColor == 1 then
...
function KillInfo:SetZombieIconLua(CauserType, AdditionalParam)
  ...
  if AdditionalParam and FatalDamageMonsterID2WeaponIconMap[AdditionalParam] then
