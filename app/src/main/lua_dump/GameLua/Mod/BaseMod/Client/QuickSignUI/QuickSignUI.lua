...
function QuickSignUI:RegistEvents()
  ...
  self:AddSettingOptionEvent("UniversalSignSwitch", function(UniversalSignSwitch)
    if UniversalSignSwitch then
      self.UIRoot.CanvasPanel_SignBtn:SetWidgetVisibility(UEnums.ESlateVisibility.SelfHitTestInvisible)
    else
      self.UIRoot.CanvasPanel_SignBtn:SetWidgetVisibility(UEnums.ESlateVisibility.Hidden)
    end
  end, true)
...
function QuickSignUI:OnReplayUIInit()
  ...
  if not uCompletePlayBack or not uCompletePlayBack:IsInPlayState() then
    return
  end
  self:RegisterControllerEvent()
