...
function KillCounterUISubsystem:OnRegister()
  ...
  if SettingSubsystem then
    SettingSubsystem:RegisterUserSettingsDelegate_Bool("bShowKillCounter", function(bShowKillCounter)
      self:RefreshMainKCVisible()
    end)
  else
    self.RegistSettingFailed = true
  end
...
function KillCounterUISubsystem:UpdateCurUsingWeapon(TargetChangeSlot, ChangeType)
  ...
  local bIsShow = SettingSubsystem:GetUserSettings_Bool("bShowKillCounter")
  if not bIsShow then
    return
  end
...
function KillCounterUISubsystem:RefreshMainKCVisible(PlayerCharacter)
  ...
  if self.RegistSettingFailed then
    SettingSubsystem:RegisterUserSettingsDelegate_Bool("bShowKillCounter", function(bShowKillCounter)
      self:RefreshMainKCVisible()
    end)
    self.RegistSettingFailed = nil
  end
  local bIsShow = SettingSubsystem:GetUserSettings_Bool("bShowKillCounter")
  if not bIsShow then
    self:UpdateMainKillCounterUI(false)
