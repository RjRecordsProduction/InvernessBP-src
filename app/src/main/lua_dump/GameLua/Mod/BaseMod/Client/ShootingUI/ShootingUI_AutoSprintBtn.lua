...
function ShootingUI_AutoSprintBtn:RegistEvents()
  ...
  self:AddUIMessageEvent("UIMsg_BindOrUnBindJoyStickSprint", self.UIMsg_BindOrUnBindJoyStickSprint, self)
...
function ShootingUI_AutoSprintBtn:UIMsg_BindOrUnBindJoyStickSprint()
  print(bWriteLog and "ShootingUI_AutoSprintBtn:UIMsg_BindOrUnBindJoyStickSprint")
  local PlayerController = GameplayData.GetPlayerController()
  if not slua.isValid(PlayerController) then
    print(bWriteLog and "ShootingUI_AutoSprintBtn:UIMsg_BindOrUnBindJoyStickSprint not PlayerController")
    return
  end
  local PlayerCharacter = GameplayData.GetPlayerCharacter()
  if not slua.isValid(PlayerCharacter) then
    print(bWriteLog and "ShootingUI_AutoSprintBtn:UIMsg_BindOrUnBindJoyStickSprint not PlayerCharacter")
    return
  end
  self:RemoveControlEventByControl(PlayerCharacter, "OnJoyStickInteruptDelegate")
  if not PlayerController.IsBindJoystickSprint then
    self:AddControlEventByControl(PlayerCharacter, "OnJoyStickInteruptDelegate", self.SprintInterupted, self)
  end
end
