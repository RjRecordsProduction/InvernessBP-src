...
function MainControlBaseUI:ctor()
  ...
  self.TLog_SpeakerIndex = 0
  self.TLog_SpeakerTimeStamp = 0.0
  self.SetAllVoiceTLogTimer = nil
...
function MainControlBaseUI:RegistEvents()
  ...
  self:AddControlEvent(self.Button_showSpeaker, "OnClicked", self.OnShowSpeakerClick, self)
  ...
  self:AddControlEvent(self.Button_Setting, "OnClicked", self.OnClicked_Button_Setting, self)
  ...
  self.SetAllVoiceTLogTimer = self:AddGameTimer(30.0, true, function()
    self:SetAllVoiceTLog()
  end)
  ...
  local bIsInfectionGameMode = self:IsInfectionGameMode()
  if bIsInfectionGameMode then
    print(bWriteLog and "RefreshInfectionPlayerMode UI and join camp voice")
    local PlayerCharacter = GameplayData.GetPlayerCharacter()
    if slua.isValid(PlayerCharacter) then
      self:OnInfectionPlayerPawnInitType(PlayerCharacter:GetCharacterMainType())
      self.CanvasPanel_QuickSign:SetWidgetVisibility(ESlateVisibility.Collapsed)
    end
  end
  ...
  HandleStateCanvasUtils.RegisterCanvasVisibleEvent(self.CanvasPanel_QuickSign, self, "MainControlBaseUI_CanvasPanel_QuickSign")
  ...
  self:AddCommonEvent(EVENTTYPE_INGAME_BACKPACK, EVENTID_BACKPACK_SINGLE_ITEM_UPDATED_CONSUMABLE_6, function(_, _, uBackpackComponent, DefineID, bUpdateOrDelete)
    if DefineID.TypeSpecificID == StoreConfig.GoldID and UIManager.UI_Config_InGame.GoldUpdateUI then
      self:UpdateGold(uBackpackComponent)
    end
  end)
  ...
  GameplayData.AddSelfPlayerControllerEvent(self, "OnRepTeammateChange", self.OnRepTeammateChange, self)
  ...
  GameplayData.AddSelfPlayerControllerEvent(self, "OnPlayerEnterFighting", self.HideParachuteUI, self)
  ...
  if VoiceChatSubsytem then
    ...
    self:AddDataListener(VoiceChatSPData, "bIsPreTeamSpeaker", self.OnPreTeamSpeakerChange, self)
    self:AddDataListener(VoiceChatSPData, "bSpeakerSettingPanelOpen", self.OnSpeakerSettingPanelOpenChange, self)
  end
  ...
  EventSystem:postEvent(EVENTTYPE_INGAME_UI, EVENTID_ADD_TO_FORCE_HIDE_UI_ARRAY, self.CanvasPanel_Setting)
  EventSystem:postEvent(EVENTTYPE_INGAME_UI, EVENTID_ADD_TO_FORCE_HIDE_UI_ARRAY, self.SpeakerCanvas)
  self.CanvasPanel_MiniMapAndSetting.bIsVolatile = true
  self.CanvasPanel_MiniMapAndSetting.bIsVolatileThrottle = true
  if HDmpveRemote.HDmpveRemoteConfigGetBool("DisableInvalidationBoxes", true) == true then
    log_shipping_client("MainControlBaseUI:RegistEvents Disable Invalidation Boxes")
    self.InvalidationBox_9:SetCanCache(false)
    self.InvalidationBox_0:SetCanCache(false)
    self.InvalidationBox_3:SetCanCache(false)
    self.InvalidationBox_10:SetCanCache(false)
    self.STInvalidationBox_1:SetCanCache(false)
  end
end
function MainControlBaseUI:OnInitialize()
  MainControlBaseUI.__super.OnInitialize(self)
...
function MainControlBaseUI:OnPreTeamSpeakerChange(_, bIsPreTeamSpeaker)
  self.end
function MainControlBaseUI:OnSpeakerSettingPanelOpenChange(_, bSpeakerSettingPanelOpen)
...
function MainControlBaseUI:CheckShowWModeUI(bNeedShow)
  local GameState = GameplayData.GetGameState()
  if slua.isValid(GameState) and GameState.GetSuperData then
    local SPData = GameState:GetSuperData()
...
function MainControlBaseUI:OnDestroy()
  ...
  EventSystem:postEvent(EVENTTYPE_INGAME_UI, EVENTID_REMOVE_FROM_FORCE_HIDE_UI_ARRAY, self.CanvasPanel_Setting)
  EventSystem:postEvent(EVENTTYPE_INGAME_UI, EVENTID_REMOVE_FROM_FORCE_HIDE_UI_ARRAY, self.SpeakerCanvas)
  ...
  HandleStateCanvasUtils.UnRegisterCanvasVisibleEvent(self.CanvasPanel_QuickSign)
  if self.SetAllVoiceTLogTimer then
    self:RemoveGameTimer(self.SetAllVoiceTLogTimer)
    self.SetAllVoiceTLogTimer = nil
  end
...
function MainControlBaseUI:OnEntireMapTriggerOnPressDown(PointerIndex)
  ...
  local Setting = SettingSubsystem:GetUserSettings_Bool("bCanMapLongPress")
...
function MainControlBaseUI:UpdateGold(uBackpackComponent)
  local Count = uBackpackComponent:GetItemCountByItemSpecialID(StoreConfig.GoldID)
  if Count > self.GoldNum then
    local GoldUpdateUI = UIManager.ShowUI(UIManager.UI_Config_InGame.GoldUpdateUI)
    ...
    if BackpackClothingEntryUI then
      BackpackClothingEntryUI:AttachChildWindow("CanvasPanel_Gold", GoldUpdateUI)
    end
    GoldUpdateUI:ShowGoldUpdate(self.GoldNum, Count - self.GoldNum)
    if self.GoldUpdateTimer then
      self:RemoveGameTimer(self.GoldUpdateTimer)
    end
    self.GoldUpdateTimer = self:AddGameTimer(StoreConfig.GoldTipsShowTime, false, function()
      GoldUpdateUI:Collapsed()
      self.GoldUpdateTimer = nil
    end)
  end
  self.GoldNum = Count
end
...
function MainControlBaseUI:TickHitTips()
  local   local   local PlayerController = GameplayData.GetPlayerController()
  if not slua.isValid(PlayerController) or not self.DamageCauserLocation then
  ...
  if PlayerController.PlayerCameraManager then
    PlayerRotation = PlayerController.PlayerCameraManager:GetCameraRotation()
  else
    PlayerRotation = PlayerController:GetControlLookRotation()
  end
  local PlayerLocation = PlayerController:GetCurPawnLocation()
  local CauserLocation = self.DamageCauserLocation
  local Delta = CauserLocation - PlayerLocation
  local HitNormal2D = UKismetMathLibrary.Normal2D(FVector2D(Delta.X, Delta.Y))
  local HitNormal = FVector(HitNormal2D.X, HitNormal2D.Y, 0)
  PlayerRotation = UKismetMathLibrary.GetForwardVector(PlayerRotation)
  local PlayerRotation2D = UKismetMathLibrary.Normal2D(FVector2D(PlayerRotation.X, PlayerRotation.Y))
  PlayerRotation = FVector(PlayerRotation2D.X, PlayerRotation2D.Y, 0)
  local Angle = math.acos(FVector.DotProduct(PlayerRotation, HitNormal)) * 57.295827908797776
  local FAngle = FVector.DotProduct(FVector.CrossProduct(PlayerRotation, FVector(0, 0, 1)), HitNormal)
  if 0 < FAngle then
    Angle = 360.0 - Angle
  end
...
function MainControlBaseUI:OnShowSpeakerClick()
...
function MainControlBaseUI:ShowSpeakerPanel()
...
function MainControlBaseUI:CloseSpeakerPanel()
  self.SpeakerFlag = false
...
function MainControlBaseUI:UpdateSpeakerIconAndText(IsShowingPanel)
  if IsShowingPanel == nil then
    IsShowingPanel = self.SpeakerFlag
  end
  print(bWriteLog and "MainControlBaseUI:UpdateSpeakerIconAndText SpeakerFlag: " .. tostring(self.SpeakerFlag))
  if IsShowingPanel then
    self.Image_HideVoicePanel:SetWidgetVisibility(ESlateVisibility.SelfHitTestInvisible)
    self.TextBlock_SpeakerStatus:SetWidgetVisibility(ESlateVisibility.Collapsed)
    self.Image_Voice:SetWidgetVisibility(ESlateVisibility.Collapsed)
    self.Image_VoiceDisabled:SetWidgetVisibility(ESlateVisibility.Collapsed)
  else
    self.Image_HideVoicePanel:SetWidgetVisibility(ESlateVisibility.Collapsed)
    self.TextBlock_SpeakerStatus:SetWidgetVisibility(ESlateVisibility.SelfHitTestInvisible)
    local VoiceSDKInterface = self:GetVoiceSDKInterface()
    if not slua.isValid(VoiceSDKInterface) then
      print(bWriteLog and "MainControlBaseUI:UpdateSpeakerIconAndText get VoiceSDKInterface failed")
      return
    end
    if VoiceSDKInterface:LbsSpeakerEnable() then
      self.TextBlock_SpeakerStatus:SetText(LocUtil.GetLocalizeResStr(34960))
      self.Image_Voice:SetWidgetVisibility(ESlateVisibility.SelfHitTestInvisible)
      self.Image_VoiceDisabled:SetWidgetVisibility(ESlateVisibility.Collapsed)
      self:SetVoiceTLogSpeaker(2)
    elseif VoiceSDKInterface:TeamSpeakerEnable() and not VoiceSDKInterface:LbsSpeakerEnable() then
      ...
      self.TextBlock_SpeakerStatus:SetText(SpeakerStatusText)
      self.Image_Voice:SetWidgetVisibility(ESlateVisibility.SelfHitTestInvisible)
      self.Image_VoiceDisabled:SetWidgetVisibility(ESlateVisibility.Collapsed)
      self:SetVoiceTLogSpeaker(1)
    else
      self.TextBlock_SpeakerStatus:SetText("")
      self.Image_Voice:SetWidgetVisibility(ESlateVisibility.Collapsed)
      self.Image_VoiceDisabled:SetWidgetVisibility(ESlateVisibility.SelfHitTestInvisible)
      self:SetVoiceTLogSpeaker(0)
    end
  end
end
...
function MainControlBaseUI:RecordMicphoneTlogToServer()
  local PlayerController = GameplayData.GetPlayerController()
  if not slua.isValid(PlayerController) then
    return
  end
  ...
  if PlayerController.RPC_Server_SetMicphoneTLogToServer then
    PlayerController:RPC_Server_SetMicphoneTLogToServer(self.TLog_MicphoneIns)
  end
end
function MainControlBaseUI:RefreshVoiceAndSpeakPanel()
  self:UpdateSpeakerIconAndText()
end
...
function MainControlBaseUI:OnRepTeammateChange()
end
...
function MainControlBaseUI:HideVoicePanel()
  self:CloseSpeakerPanel()
  self:CheckShowWModeUI(true)
  self:UpdateSpeakerIconAndText()
end
...
function MainControlBaseUI:HandleEmoteSetting()
  ...
  if SettingSubsystem then
    SettingSubsystem:RegisterUserSettingsDelegate_Bool("ActorAnimationSwitch", function(ActorAnimationSwitch)
      self:SetEmoteControlVisibility(self.Emote_SettingControl, ActorAnimationSwitch)
    end)
    SettingSubsystem:RegisterUserSettingsDelegate_Bool("OBSBulletTrack", function(OBSBulletTrack)
      local PlayerController = GameplayData.GetPlayerController()
      if slua.isValid(PlayerController) then
...
function MainControlBaseUI:HideParachuteUI()
end
...
function MainControlBaseUI:OnInfectionPlayerPawnInitType(currentCampMode)
  print(bWriteLog and "join camp voice")
  local bIsInfectionGameMode = self:IsInfectionGameMode()
  if bIsInfectionGameMode then
  end
end
...
function MainControlBaseUI:IsInfectionGameMode()
  local GameState = GameplayData.GetGameState()
  if slua.isValid(GameState) then
...
function MainControlBaseUI:ResetJoystickWidgetRender()
  local PlayerController = GameplayData.GetPlayerController()
  if slua.isValid(PlayerController) then
...
function MainControlBaseUI:InitDelegate()
  GameplayData.AddSelfPlayerControllerEvent(self, "InfectionPlayerPawnInitType", self.OnInfectionPlayerPawnInitType, self)
  local GameInstance = UIUtil.GetGameInstance()
  if slua.isValid(GameInstance) then
    self:AddControlEvent(GameInstance, "OnCampRoomChanged", self.SwitchCampRoom, self)
  end
...
function MainControlBaseUI:EnterJumpingSetUI()
  self:ShowOrHideFreeCam(true)
  self:ResetJoystickWidgetRender()
end
function MainControlBaseUI:EnterFightingSetUI()
end
function MainControlBaseUI:RemindQuickChatBtn(bIsShowRedPoint)
  if not self.QuickMenu:IsShow() then
...
function MainControlBaseUI:InitFBTipsPanel()
end
...
function MainControlBaseUI:GetChangedModelName()
  ...
  if not slua.isValid(uGameInstance) then
    log_error(bWriteLog and "MainControlBaseUI:UIMSG_GameModeDisplayNameChanged uGameInstance is invalid")
    return sRet
  end
  local GameState = GameplayData.GetGameState()
  ...
  local TableUtil = require("common.table_util")
  if TableUtil.Find(MapRankModeConfig.MatchModeList, SubModeID) ~= -1 then
    sRet = LocUtil.LocalizeResFormat(73201, sRet)
  elseif TableUtil.Find(MapRankModeConfig.RankModeList, SubModeID) ~= -1 then
...
function MainControlBaseUI:SwitchCampRoom(CampRoomType)
end
...
function MainControlBaseUI:HideBattleVoiceUI()
  ...
  UIManager.CloseUI(UIManager.UI_Config_InGame.MicrophoneButton)
  self.CanvasPanel_Speaker:SetWidgetVisibility(ESlateVisibility.Collapsed)
end
function MainControlBaseUI:ShowBattleVoiceUI()
  ...
  self.CanvasPanel_Speaker:SetWidgetVisibility(ESlateVisibility.SelfHitTestInvisible)
end
function MainControlBaseUI:UIMsg_ChangeShowTipSwitch()
end
function MainControlBaseUI:SetVoiceTLogSpeaker(SetIndex)
  local ThisDeltaTime = 0
  local GameState = GameplayData.GetGameState()
  if slua.isValid(GameState) then
    local ServerWorldTimeSeconds = GameState:GetServerWorldTimeSeconds()
    ThisDeltaTime = ServerWorldTimeSeconds - self.TLog_SpeakerTimeStamp
    local PlayerState = GameplayData.GetPlayerState()
    if slua.isValid(PlayerState) and SetIndex ~= self.TLog_SpeakerIndex then
      if self.TLog_SpeakerIndex == 1 and PlayerState.TeammateSpeakerTime then
        PlayerState.TeammateSpeakerTime = ThisDeltaTime + PlayerState.TeammateSpeakerTime
        self:RecordMicphoneTlogToServer()
      elseif self.TLog_SpeakerIndex == 2 and PlayerState.EnemySpeakerTime then
        PlayerState.EnemySpeakerTime = ThisDeltaTime + PlayerState.EnemySpeakerTime
        self:RecordMicphoneTlogToServer()
      end
      self.TLog_SpeakerTimeStamp = ServerWorldTimeSeconds
      self.TLog_SpeakerIndex = SetIndex
    end
  end
end
...
function MainControlBaseUI:SetAllVoiceTLog()
  local SpeakerDeltaTime = 0
  local GameState = GameplayData.GetGameState()
  if slua.isValid(GameState) then
    local ServerWorldTimeSeconds = GameState:GetServerWorldTimeSeconds()
    SpeakerDeltaTime = ServerWorldTimeSeconds - self.TLog_SpeakerTimeStamp
    local PlayerState = GameplayData.GetPlayerState()
    if slua.isValid(PlayerState) then
      if self.TLog_SpeakerIndex == 1 and PlayerState.TeammateSpeakerTime then
        PlayerState.TeammateSpeakerTime = SpeakerDeltaTime + PlayerState.TeammateSpeakerTime
      elseif self.TLog_SpeakerIndex == 2 and PlayerState.EnemySpeakerTime then
        PlayerState.EnemySpeakerTime = SpeakerDeltaTime + PlayerState.EnemySpeakerTime
      end
      self:RecordMicphoneTlogToServer()
      self.TLog_SpeakerTimeStamp = ServerWorldTimeSeconds
    end
...
function MainControlBaseUI:InterruptThrow()
  local PlayerCharacter = GameplayData.GetPlayerCharacter()
  local PlayerController = GameplayData.GetPlayerController()
  if slua.isValid(PlayerCharacter) and slua.isValid(PlayerController) then
    ...
    if PlayerCharacter:HasState(EPawnState.HoldGrenade) then
      print(bWriteLog and "open backpack cancel all skill")
...
function MainControlBaseUI:EnterObserverStatus()
  ...
  UIManager.CloseUI(UIManager.UI_Config_InGame.MicrophoneButton)
  self.CanvasPanel_Speaker:SetWidgetVisibility(ESlateVisibility.Collapsed)
  self.Button_ReportBug:SetWidgetVisibility(ESlateVisibility.Collapsed)
  self.CanvasEnterSetting:SetWidgetVisibility(ESlateVisibility.Collapsed)
  self.CanvasPanel_ZTK:SetWidgetVisibility(ESlateVisibility.Collapsed)
end
