...
function BornIslandTeamShowSubSystem:PlayPlayerAnim()
  ...
  for PlayerIndex, PlayerTemp in pairs(self.PlayerPawnList) do
    ...
    elseif ShouldPlaySkinAnim then
      ...
      if slua.isValid(PlayerTemp.CharacterAvatarComp2_BP) then
        local EAvatarSlotType = import("EAvatarSlotType")
        PlayerTemp.CharacterAvatarComp2_BP:AddDisableClothRequest("TeamShow", EAvatarSlotType.EAvatarSlotType_ClothesEquipemtSlot)
      end
...
function BornIslandTeamShowSubSystem:PlayPrepareEmote()
  ...
  if slua.isValid(Pawn.CharacterAvatarComp2_BP) then
    local EAvatarSlotType = import("EAvatarSlotType")
    Pawn.CharacterAvatarComp2_BP:AddDisableClothRequest("TeamShow", EAvatarSlotType.EAvatarSlotType_ClothesEquipemtSlot)
  end
  ...
  if EmoteTableData then
    ...
    if Period then
      ...
      if slua.isValid(uPlayerController) and uPlayerController.CommerFeature then
        local TeamShowLockData = uPlayerController.CommerFeature.TeamShowLockData
        local UID = tonumber(PlayerPawn:GetPlayerUID())
        if UID and TeamShowLockData[UID] and TeamShowLockData[UID][Period] then
...
function BornIslandTeamShowSubSystem:HanleReceiveVehicleAccessory(playerUid, CarItemID, AccessoryList, ChassisLight)
