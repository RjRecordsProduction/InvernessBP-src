...
function BuffList:OnGameDataReady()
  ...
  local ShootingUIPanelUISubSystem = SubsystemMgr:Get("ShootingUIPanelUISubSystem")
  if not ShootingUIPanelUISubSystem then
    print(bWriteLog and "BuffList:OnGameDataReady - ShootingUIPanelUISubSystem is not valid")
    return
  end
  self:AddDataListener(ShootingUIPanelUISubSystem:GetSuperData(), "bBuffListUseTwoBox", function(bBuffListUseTwoBox)
    if self.bUseTwoBox ~= bBuffListUseTwoBox then
      print(bWriteLog and "BuffList:OnGameDataReady - bBuffListUseTwoBox changed to true, rebuild BuffItems")
      self:DestroyPreCreatedBuffItems()
      self:InitBoxAndPreCreateBuffItems(bBuffListUseTwoBox)
    end
  end)
end
