...
function MainControlPanelTochButton:RegistEvents()
  ...
  self:AddCommonEvent(EVENTTYPE_INGAME, EVENTID_INGAME_CONTROLLER_BEGINPLAY_FINISH, self.ApplyCustomUI, self)
  ...
  self:SetAdaptationDataToController(false)
  self:InitSpecialUI()
  local SettingSubsystem = SubsystemMgr:Get("SettingSubsystem")
  if SettingSubsystem then
    self.ProfiledScreenSwitchHandle = SettingSubsystem:RegisterUserSettingsDelegate_Int("ProfiledScreenSwitch", function()
      self:ProfiledScreenSwitchChanged()
    end)
  end
  ...
  if self.InvalidationBox_34 ~= nil and HDmpveRemote.HDmpveRemoteConfigGetBool("DisableInvalidationBoxes", true) == true then
    log_shipping_client("MainControlPanelTochButton:RegistEvents Disable InvalidationBox_34")
    self.InvalidationBox_34:SetCanCache(false)
  end
end
function MainControlPanelTochButton:ResetUIStateAfterRespawn()
end
function MainControlPanelTochButton:ProfiledScreenSwitchChanged()
  self:SetAdaptationDataToController(true)
end
function MainControlPanelTochButton:SetSprintTriggerLength()
  local PlayerController = GameplayData.GetPlayerController()
  if slua.isValid(PlayerController) then
    local RushTriggerLength
    if self._CachedSprintTriggerLayoutDetail then
      RushTriggerLength = self._CachedSprintTriggerLayoutDetail.RelativePos.Y
    else
...
function MainControlPanelTochButton:ApplyCustomUI()
  local bCheck = InGameUITools.CheckApplyCustomizeUI()
  if bCheck then
    print(bWriteLog and "MainControlPanelTochButton:ApplyCustomUI")
  else
    print(bWriteLog and "MainControlPanelTochButton:ApplyCustomUI not pass")
    return
  end
  EventSystem:postEvent(EVENTTYPE_INGAME_UI, EVENTID_INGAME_ON_SET_CUSTOMIZE_UIINFO)
  local SettingSubsystem = SubsystemMgr:Get("SettingSubsystem")
  if SettingSubsystem then
    self._CachedJoystickLayoutDetail = SettingSubsystem:GetLayoutDetailByType(2)
    self._CachedSprintTriggerLayoutDetail = SettingSubsystem:GetLayoutDetailByType(30)
  end
  local PlayerController = GameplayData.GetPlayerController()
...
function MainControlPanelTochButton:UpdateJoystickTransform()
  local PlayerController = GameplayData.GetPlayerController()
  if not slua.isValid(PlayerController) then
    return
  end
  print(bWriteLog and "MainControlPanelTochButton:UpdateJoystickTransform")
  local bCheck = InGameUITools.CheckApplyCustomizeUI()
  local MainControlBaseUI = InGameUITools.GetMainControlBaseUI()
  local UIUtil = require("client.common.ui_util")
  local ViewportSizebyScale = UIUtil.GetViewportSizebyScale()
  local ViewportCenter = FVector2D(self.CurDeviceAdaptationData.LeftOffset / ViewportSizebyScale.X, self.CurDeviceAdaptationData.TopOffset / ViewportSizebyScale.Y)
  local StickOpacity = 1
  local Scale = 1
  local Layout_Joystick = bCheck and self._CachedJoystickLayoutDetail
  local CalculatedRatio
  if Layout_Joystick then
    if math.abs(Layout_Joystick.RelativePos.X) < 1.01 and math.abs(Layout_Joystick.RelativePos.Y) < 1.01 then
      CalculatedRatio = Layout_Joystick.RelativePos
      print(bWriteLog and "MainControlPanelTochButton:UpdateJoystickTransform Data is Ratio")
    else
      CalculatedRatio = FVector2D(Layout_Joystick.RelativePos.X, Layout_Joystick.RelativePos.Y + ViewportSizebyScale.Y) / ViewportSizebyScale
      print(bWriteLog and "MainControlPanelTochButton:UpdateJoystickTransform Data is Pos")
    end
    StickOpacity = Layout_Joystick.Opacity
    Scale = Layout_Joystick.Scale
  else
    CalculatedRatio = FVector2D(225, -195 + ViewportSizebyScale.Y) / ViewportSizebyScale
  end
  PlayerController:SetJoyStickCenter(CalculatedRatio + ViewportCenter)
  PlayerController:SetJoyStickOpacity(StickOpacity)
  PlayerController:SetJoyStickScale(Scale)
  PlayerController:MakeFireModeEffect()
  if MainControlBaseUI then
    MainControlBaseUI:ResetJoystickWidgetRender()
  end
end
...
function MainControlPanelTochButton:IngameUIAdaptation()
  self.InvalidationBox_1.Slot:SetOffsets(FMargin(self.CurDeviceAdaptationData.LeftOffset, self.CurDeviceAdaptationData.TopOffset, self.CurDeviceAdaptationData.RightOffset, self.CurDeviceAdaptationData.BottomOffset))
end
...
function MainControlPanelTochButton:MainControlPanel_HideAllUI()
  print(bWriteLog and "[UIStateRefresh LLP] MainControlPanelTochButton:MainControlPanel_HideAllUI")
...
function MainControlPanelTochButton:SetAdaptationDataToController(bChangeProfiledScreenSwitch)
  local PlayerController = GameplayData.GetPlayerController()
  if not slua.isValid(PlayerController) then
    return
  end
  ...
  local UIRectOffset = ScriptHelperClient.GetUIRectOffset()
  local SubStringArray = kismet_string_library.ParseIntoArray(UIRectOffset, ",", true)
  if #SubStringArray < 4 then
    return
  end
  local Left = tonumber(SubStringArray[1])
  local Top = tonumber(SubStringArray[2])
  local Right = tonumber(SubStringArray[3])
  local Bottom = tonumber(SubStringArray[4])
  ...
  PlayerController.CurDeviceAdaptationData = PlayerController.CurDeviceAdaptationOffset
...
function MainControlPanelTochButton:OnDestroy()
  ...
  local SettingSubsystem = SubsystemMgr:Get("SettingSubsystem")
  if SettingSubsystem then
    if self.FireModeHandle then
      SettingSubsystem:UnregisterUserSettingDelegate(self.FireModeHandle)
      self.FireModeHandle = nil
    end
    if self.ProfiledScreenSwitchHandle then
      SettingSubsystem:UnregisterUserSettingDelegate(self.ProfiledScreenSwitchHandle)
      self.ProfiledScreenSwitchHandle = nil
    end
  end
end
