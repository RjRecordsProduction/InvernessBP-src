...
function InGameUITools.IsTDM()
  ...
  if ModType == "TDM" or ModType == "BRTDM" or ModType == "TPlanTDM" then
    return true
  else
    return false
  end
...
function InGameUITools.CheckApplyCustomizeUI()
  ...
  local GameMainConfig = require("GameLua.GameCore.Main.GameMainConfig")
  local ModType, _ = GameMainConfig.GetModType()
  local result = ModType ~= "PrimeGuide" and ModType ~= "MainCity"
...
function InGameUITools.SetJoystickSprintState(bShow)
  local UI = UIManager.GetUI(UIManager.UI_Config_InGame.AutoSprintPanel)
  if UI then
