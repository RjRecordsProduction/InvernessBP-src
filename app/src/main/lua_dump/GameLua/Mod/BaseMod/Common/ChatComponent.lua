  ...
  if SettingText then
    if SettingText.Key then
      if SettingText.Type == "Int" then
        local SettingValue = SettingSubsystem:GetUserSettings_Int(SettingText.Key)
      ...
      elseif SettingText.Type == "Bool" then
        local SettingValue = SettingSubsystem:GetUserSettings_Bool(SettingText.Key)
...
function ChatComponent:InitFromSetting()
  ...
  local SettingSubsystem = SubsystemMgr:Get("SettingSubsystem")
  if SettingSubsystem and SettingSubsystem.SettingSubsystem_CPP then
    local Delegate = slua.createDelegate(function()
      self:SetQuickChatList()
      print(bWriteLog and "ChatComponent:InitFromSetting [2]")
    end)
    SettingSubsystem.SettingSubsystem_CPP:RegisterUserSettingsDelegate(Delegate)
  end
  local UserSettings = slua_GameFrontendHUD:GetUserSettings()
  if UserSettings and slua.isValid(UserSettings) then
    self.SettingConfig = UserSettings
    print(bWriteLog and "ChatComponent:InitFromSetting [3]")
  end
