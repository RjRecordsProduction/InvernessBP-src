      ...
      if slua.isValid(PlayerController) then
        ...
        if WeaponOverrideSoundInfo then
          ...
          for GroupID, Flag in pairs(WeaponOverrideSoundInfo) do
            if GroupID == WeaponSkinGroupID then
              if Flag then
                print(bWriteLog and "WeaponAvatarComponent:OnWeaponAvatarPutOnSlotLua Set WeaponOverrideSoundTypeFlag: " .. Flag .. ", WeaponSkinID: " .. WeaponSkinDefineID.TypeSpecificID .. ", GroupID: " .. GroupID)
                Override              end
              break
            end
          end
        end
        print(bWriteLog and "WeaponAvatarComponent:OnWeaponAvatarPutOnSlotLua Set WeaponOverrideSoundTypeFlag: " .. OverrideFlag .. ", WeaponSkinID: " .. WeaponSkinDefineID.TypeSpecificID)
        OwnerWeapon.WeaponOverrideSoundTypeFlag = OverrideFlag
      end
    end
  end
end
...
function WeaponAvatarComponent:UpdateWeaponAvatarAudioVolume()
  ...
  for _, AudioParamCfg in pairs(AudioParamCfgs) do
    if AudioParamCfg.RTPCKey and AudioParamCfg.RTPCKey ~= "" then
      local RTPCKeyList = StringUtil.Split(AudioParamCfg.RTPCKey, ";")
      for _, rtpcKey in pairs(RTPCKeyList) do
        if rtpcKey and rtpcKey ~= "" then
          local AudioVolume = VolumeInfoMap and VolumeInfoMap[AudioParamCfg.AudioType] or 100
