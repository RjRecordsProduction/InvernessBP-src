...
function IngameSelfieSubsystem:EnterSelfieWithSmartCameraAndEmote(nTemplateID, EmoteList, SettingParams)
  ...
  self.bUseHighLightModeSetting = SettingParams and SettingParams.bUseHighLightModeSetting == true
...
function IngameSelfieSubsystem:OnEnterSelfie()
  ...
  UIManager.ShowUI(self:GetCurPhotoMainUIConfig(), self.bAutoStartSmartCamera, self.bUseHighLightModeSetting)
  self:AddControlEvent(character, "OnPostTakeDamage", self.OnSelfieModeInterrupt, self)
end
function IngameSelfieSubsystem:ExitSelfie()
  ...
  if self:HasControlEventByControl(character, "OnPostTakeDamage") then
    self:RemoveControlEvent(character, "OnPostTakeDamage")
  end
  local pc = GamePlayTools.GetPlayerControllerByIndex(0)
  local EPawnState = import("EPawnState")
  ...
  self.bUseHighLightModeSetting = nil
end
...
function IngameSelfieSubsystem:OnSelfieModeInterrupt()
  print(bWriteLog and "[DeanJYT] IngameSelfieSubsystem:OnSelfieModeInterrupt")
  self:ExitSelfie()
  self.bIsAsyncLoadingUI = false
  UIManager.CloseUI(self:GetCurPhotoMainUIConfig())
  UIManager.CloseUI(UIManager.UI_Config_InGame.Ingame_Photo_Popup_1_UIBP)
