...
function CustomAssetDownloader:_GetDownloadUrlByDownloadInfo(DownloadInfo)
  local DownloadDomain = CustomAssetDefine.GetCustomAssetCosDownloadUrl(CustomAssetDefine.CustomAssetCosTypeDefine.PlayerDefRes)
...
function CustomAssetDownloader:AddDownloadInfo(AssetKey, SuffixType)
  local NewDownloadInfo = Utility:DeepCopy(CustomAssetDownloadInfoDefine)
...
function CustomAssetDownloader:RequestDownloadAsset(AssetKey, SuffixType, DownloaderPriority, OutHandleFunc, ...)
...
function CustomAssetDownloader:StartDownloadAsset(DownloadInfo)
  ...
  if self:IsDedicatedServer() and not self:IsEditor() then
    ...
    local DownloadURL = self:_GetDownloadUrlByDownloadInfo(DownloadInfo)
    ...
    AWSHelper:DownloadBinary(DownloadURL, function(AWSRsp)
      ...
      printf(bWriteLog and "CustomAssetDownloader:DownloadBinary bOK = %s,Code = %s,URL = %s", tostring(bOK), tostring(rspCode), url)
      if bOK == true then
      ...
      printf(bWriteLog and "CustomAssetDownloader:DownloadBinary Progress = %s/%s", tostring(CurrentBytes), tostring(TotalBytes))
    end)
  end
