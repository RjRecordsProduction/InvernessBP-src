...
function ui_show_queue_element_builder.GetQueueElement(lqcUIConfig, uiConfig, ...)
  ...
  element.  local paramTable = args[args.n]
  local sortWeightOffset = 0
  if type(paramTable) == "table" and paramTable.IsConfig then
    sortWeightOffset = tonumber(paramTable.SortWeightOffset) or 0
  end
  element.sortWeight = ui_show_queue_element_builder._GetSortWeight(element.lqcUIConfig, sortWeightOffset)
  return element
end
function ui_show_queue_element_builder._GetSortWeight(lqcUIConfig, sortWeightOffset)
  ...
  local sortWeight = bigTypeWeight + smallTypeWeight + (tonumber(sortWeightOffset) or 0)
