...
function SettingSystem.sync_custom_setting(setting_info, slotType, setting_ver_info)
  ...
  elseif type(slotType) == "string" and string.find(slotType, "NCL_") and type(setting_info) == "string" then
    SettingSystem.SyncCustomConfig_New(setting_info, slotType)
  end
  SettingSystem.NotifySlotSynced(slotType)
end
function SettingSystem.SyncCustomConfig_New(setting_info, slotType)
  ...
  local SaveGame = CustomLayoutArchiver.LoadSaveGameFromTable(decodedData, slotType)
  if slua.isValid(SaveGame) then
    local OldSaveGame = CustomLayoutArchiver.LoadFile(SaveGame.SaveSlotName, slotType)
    if OldSaveGame then
      print(bWriteLog and string.format("SettingSystem.SyncCustomConfig_New old file name:%s", OldSaveGame.SaveSlotName))
    end
    print(bWriteLog and string.format("SettingSystem.SyncCustomConfig_New new file name:%s", SaveGame.SaveSlotName))
...
function SettingSystem.UpdateGameVersionIfNeeded()
  local SettingConfig = slua_GameFrontendHUD:GetUserSettings()
  if not SettingConfig or not SettingConfig.GameVersion then
    print(bWriteLog and "[LoginSync] PhaseA: no SettingConfig")
    return false
  end
  ...
  if SettingConfig.GameVersion == CurrentMainVersion then
    print(bWriteLog and "[LoginSync] PhaseA: ver same")
    return false
  end
  print(bWriteLog and string.format("[LoginSync] PhaseA: ver upgrade %s->%s (upload deferred)", tostring(SettingConfig.GameVersion), tostring(CurrentMainVersion)))
  return true
end
SettingSystem.LoginSync = SettingSystem.LoginSync or {
  BatchId = 0,
  Pending = nil,
  Done = false,
  bMainVerUpdated = false,
  SafetyTimer = nil
}
local ClearLoginSyncSafetyTimer = function(L)
  if L.SafetyTimer then
    Game:ClearTimer(L.SafetyTimer)
    L.SafetyTimer = nil
  end
end
function SettingSystem.BeginLoginSyncBatch(bMainVerUpdated)
  local L = SettingSystem.LoginSync
  ClearLoginSyncSafetyTimer(L)
  L.BatchId = L.BatchId + 1
  L.Pending = {}
  L.Done = false
  L.bMainVerUpdated = bMainVerUpdated and true or false
  print(bWriteLog and string.format("[LoginSync] Begin id=%d verUp=%s", L.BatchId, tostring(L.bMainVerUpdated)))
end
function SettingSystem.MarkPendingSlot(slotType)
  local L = SettingSystem.LoginSync
  if L.Pending then
    L.Pending[slotType] = true
  end
end
function SettingSystem.CommitLoginSyncBatch(timeoutSec)
  local L = SettingSystem.LoginSync
  if not L.Pending then
    return
  end
  local count = 0
  for _ in pairs(L.Pending) do
    count = count + 1
  end
  if count == 0 then
    SettingSystem.FinishLoginSyncBatch("empty")
    return
  end
  local timeout = timeoutSec or 8
  print(bWriteLog and string.format("[LoginSync] Commit id=%d pending=%d timeout=%ss", L.BatchId, count, tostring(timeout)))
  local id = L.BatchId
  L.SafetyTimer = Game:SetTimer(timeout, false, function()
    local L2 = SettingSystem.LoginSync
    if L2.BatchId == id and not L2.Done then
      SettingSystem.FinishLoginSyncBatch("timeout")
    end
  end)
end
function SettingSystem.NotifySlotSynced(slotType)
  local L = SettingSystem.LoginSync
  if not L.Pending then
    return
  end
  if L.Pending[slotType] == nil then
    return
  end
  L.Pending[slotType] = nil
  local left = 0
  for _ in pairs(L.Pending) do
    left = left + 1
  end
  print(bWriteLog and string.format("[LoginSync] Synced slot=%s left=%d", tostring(slotType), left))
  if left == 0 and not L.Done then
    SettingSystem.FinishLoginSyncBatch("complete")
  end
end
function SettingSystem.FinishLoginSyncBatch(reason)
  local L = SettingSystem.LoginSync
  if L.Done then
    return
  end
  ...
  if L.Pending then
    local nMissing = 0
    for slot in pairs(L.Pending) do
      missing = nMissing == 0 and tostring(slot) or missing .. "," .. tostring(slot)
      nMissing = nMissing + 1
    end
    if 0 < nMissing then
      missing = string.format(" missing=%d[%s]", nMissing, missing)
    end
  end
  L.Pending = nil
  ClearLoginSyncSafetyTimer(L)
  print(bWriteLog and string.format("[LoginSync] Finish id=%d reason=%s verUp=%s%s", id, tostring(reason), tostring(0), missing))
  local bMainVerUpdated = SettingSystem.UpdateGameVersionIfNeeded()
  if bMainVerUpdated then
    local SettingConfig = slua_GameFrontendHUD:GetUserSettings()
    if SettingConfig and SettingConfig.GameVersion then
      ...
      print(bWriteLog and "[LoginSync] PhaseA deferred upload done")
    end
    local Setting_UIElemLayout_Interface = require("client.slua.umg.NewSetting.UIElemLayout.Setting_UIElemLayout_Interface")
    Setting_UIElemLayout_Interface.RemoveBuggyFile()
    SettingSystem.SanitizeCustomLayouts()
  end
end
function SettingSystem.SanitizeCustomLayouts()
  local ScriptHelperClient = import("ScriptHelperClient")
  local LayoutUserSettingList = ScriptHelperClient.ListDirectoryFiles("SaveGames/CustomLayout")
  local Setting_UIElemLayout_Interface = require("client.slua.umg.NewSetting.UIElemLayout.Setting_UIElemLayout_Interface")
  local VH_C3_Save = Setting_UIElemLayout_Interface.GetSaveGame("NCL_VH_C3")
  local TransferTypes = {
    225,
    226,
    210,
  ...
  local bProcessedVH_C2 = false
  local PendingArchiveMap = {}
  if LayoutUserSettingList then
    for _, SlotName in pairs(LayoutUserSettingList) do
      local UserSetting = Setting_UIElemLayout_Interface.GetSaveGame(SlotName)
      local bModified = Setting_UIElemLayout_Interface.Sanitize(UserSetting)
      bModified = Setting_UIElemLayout_Interface.ReformJoystickAndSprintTrigger(UserSetting) or bModified
      if SlotName == "NCL_VH_C2" and slua.isValid(VH_C3_Save) and slua.isValid(UserSetting) then
        bProcessedVH_C2 = true
        for _, TypeId in ipairs(TransferTypes) do
          if VH_C3_Save:Contains(TypeId) then
            local Detail = VH_C3_Save:GetDataAsLayoutDetail(TypeId)
            if Detail then
              UserSetting:SetDataByLayoutDetail(TypeId, Detail)
              bModified = true
            end
          end
        end
      ...
      if bModified then
        local bSuccess, Archive = Setting_UIElemLayout_Interface.SaveUIElemLayoutSG(UserSetting)
        if bSuccess then
          PendingArchiveMap[SlotName] = Archive
        end
      end
    end
  end
  if not bProcessedVH_C2 and slua.isValid(VH_C3_Save) then
    local DstSave = Setting_UIElemLayout_Interface.GetSaveGame("NCL_VH_C2", true)
    if slua.isValid(DstSave) then
      local bDstModified = false
      for _, TypeId in ipairs(TransferTypes) do
        if VH_C3_Save:Contains(TypeId) then
          local Detail = VH_C3_Save:GetDataAsLayoutDetail(TypeId)
          if Detail then
            DstSave:SetDataByLayoutDetail(TypeId, Detail)
            bDstModified = true
          end
        end
      end
      if bDstModified then
        local bSuccess, Archive = Setting_UIElemLayout_Interface.SaveUIElemLayoutSG(DstSave)
        if bSuccess then
          PendingArchiveMap.NCL_VH_C2 = Archive
        end
      end
    end
  end
  if next(PendingArchiveMap) then
    Setting_UIElemLayout_Interface.UploadArchiveMap(PendingArchiveMap)
  end
end
...
function SettingSystem.SynAutoSaveSettingValue()
  ...
  if ServerSettingConfig then
    local UIUtil = require("client.common.ui_util")
    local LocalSettingConfig = UIUtil.GetGameFrontendHUD():GetUserSettings()
...
function SettingSystem.query_other_csetting_req(firemode, share_code)
  ...
  SettingSystem._LastQueryOtherCSettingFireMode = firemode
...
function SettingSystem.query_other_csetting_rsp(result, setting_info)
  ...
  if tostring(result) == "0" and setting_info ~= nil then
    ...
    if setting_info.share_info ~= nil and setting_info.Index and setting_info.Config then
      DataConfig = setting_info.Config
    elseif setting_info.UseIndex and setting_info.DataMap then
      local Data = setting_info.DataMap[setting_info.UseIndex]
      if Data and Data.Config then
        SettingSystem.SyncCustomConfig_Legacy({
    ...
    if DataConfig then
      local SavEncodeSystem = require("client.logic.setting.SavEncodeSystem")
      local bPassed, ActualEncodeType = SavEncodeSystem.VerifySavFile(DataConfig, DataConfig.EncodeType)
      if bPassed then
        SavEncodeSystem.SaveFile("UIElemLayout_SlotPreview", DataConfig, ActualEncodeType)
        bPassed = SavEncodeSystem.ValidateSaveFile("UIElemLayout_SlotPreview")
      end
      if not bPassed then
        print(bWriteLog and "SettingSystem.query_other_csetting_rsp: SlotPreview sav write failed")
        ShowNotice(11456)
        Client.DeleteFile(Client.ProjectSavedDir() .. "/SaveGames/UIElemLayout_SlotPreview.sav")
        return
      end
      local GameplayStatics = import("GameplayStatics")
      local Setting_UIElemLayout_Interface = require("client.slua.umg.NewSetting.UIElemLayout.Setting_UIElemLayout_Interface")
      local LegacySaveGame = GameplayStatics.LoadGameFromSlot("UIElemLayout_SlotPreview", 0)
      local DictIndex = SettingSystem._LastQueryOtherCSettingFireMode
      SettingSystem._LastQueryOtherCSettingFireMode = nil
      local NewSaveGame = Setting_UIElemLayout_Interface.ConvertOneDictToSaveGame(LegacySaveGame, DictIndex)
      Client.DeleteFile(Client.ProjectSavedDir() .. "/SaveGames/UIElemLayout_SlotPreview.sav")
      if not slua.isValid(NewSaveGame) then
        print(bWriteLog and string.format("SettingSystem.query_other_csetting_rsp: ConvertOneDictToSaveGame failed, DictIndex=%s", tostring(DictIndex)))
        ShowNotice(11456)
        return
      end
      UIManager.CloseUI(UIManager.UI_Config.setting_layout_selection_new)
      local UI = UIManager.GetUI(UIManager.UI_Config.setting_uielem_layout)
      if UI then
        UI:PreviewSharedLayout(NewSaveGame)
      end
    end
...
local _GetRoleUid = function()
  if DataMgr and DataMgr.roleData and DataMgr.roleData.uid then
    return tostring(DataMgr.roleData.uid)
  end
  return nil
end
function SettingSystem.GetPCDefaultInvisible()
  local uid = _GetRoleUid()
  if not uid then
    return false
  end
  local PlayerPrefsSystem = require("client.logic.LogicPlayerPrefs.playerprefs")
  local data = PlayerPrefsSystem.LoadFileToTable_DynamicPath(PlayerPrefsSystem.ePlayerPrefsType.ePCDefaultInvisible, uid)
  if data and data.bEnabled then
    return true
  end
  return false
end
function SettingSystem.SavePCDefaultInvisible(bEnabled)
  local uid = _GetRoleUid()
  if not uid then
    return
  end
  local PlayerPrefsSystem = require("client.logic.LogicPlayerPrefs.playerprefs")
  PlayerPrefsSystem.SaveTableToFile_DynamicPath({bEnabled = bEnabled}, PlayerPrefsSystem.ePlayerPrefsType.ePCDefaultInvisible, uid)
end
function SettingSystem.HasShownPCDefaultInvisiblePopup()
  local uid = _GetRoleUid()
  if not uid then
    return false
  end
  local PlayerPrefsSystem = require("client.logic.LogicPlayerPrefs.playerprefs")
  local data = PlayerPrefsSystem.LoadFileToTable_DynamicPath(PlayerPrefsSystem.ePlayerPrefsType.ePCDefaultInvisiblePopupShown, uid)
  if data and data.bShown then
    return true
  end
  return false
end
function SettingSystem.SetPCDefaultInvisiblePopupShown()
  local uid = _GetRoleUid()
  if not uid then
    return
  end
  local PlayerPrefsSystem = require("client.logic.LogicPlayerPrefs.playerprefs")
  PlayerPrefsSystem.SaveTableToFile_DynamicPath({bShown = true}, PlayerPrefsSystem.ePlayerPrefsType.ePCDefaultInvisiblePopupShown, uid)
end
function SettingSystem.IsCurrentlyInvisible()
  local FriendHandler = require("client.network.Protocol.FriendHandler")
  if FriendHandler.friend_status_data and FriendHandler.friend_status_data.sub_status_id == 9 then
    return true
  end
  return false
end
function SettingSystem.ApplyStealthIfNeeded(eventHost)
  if SettingSystem.IsCurrentlyInvisible() then
    return
  end
  local FriendHandler = require("client.network.Protocol.FriendHandler")
  if FriendHandler.friend_status_data and FriendHandler.friend_status_data.sub_status_id then
    FriendHandler.send_set_friend_status_new_req(9, "", 0)
  elseif eventHost and eventHost.AddCommonEvent then
    eventHost:AddCommonEvent(EVENTTYPE_FRIEND, EVENTID_GET_FRIEND_STATUS_DETAIL, function()
      eventHost:RemoveCommonEvent(EVENTTYPE_FRIEND, EVENTID_GET_FRIEND_STATUS_DETAIL)
      if not SettingSystem.IsCurrentlyInvisible() then
        FriendHandler.send_set_friend_status_new_req(9, "", 0)
      end
    end, eventHost)
  end
...
function SettingSystem.ShowBackLobbyNotice(bPeakGame)
  ...
  local GameState = GameplayData.GetGameState()
  local gameReplay = uGameInstance:GetObservingReplay()
  gameReplay:StopPlay()
