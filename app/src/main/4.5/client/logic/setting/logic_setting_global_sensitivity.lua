...
function LogicGlobalSensitivity.CompareGlobalSensitivityWithLocal(InData)
  if not InData then
    print(bWriteLog and "LogicGlobalSensitivity.CompareGlobalSensitivityWithLocal nil InData")
    return false
  end
  local SettingConfig = slua_GameFrontendHUD:GetUserSettings()
  local AllKeys = LogicGlobalSensitivity.GetAllSensitivityKeys()
  for _, SettingKey in ipairs(AllKeys) do
    if InData[SettingKey] ~= nil and SettingConfig[SettingKey] ~= nil and not _CompareFloatValue(InData[SettingKey], SettingConfig[SettingKey]) then
      print(bWriteLog and "LogicGlobalSensitivity.CompareGlobalSensitivityWithLocal mismatch key cloud local", SettingKey, InData[SettingKey], SettingConfig[SettingKey])
      return false
    elseif InData[SettingKey] == nil then
      print(bWriteLog and "LogicGlobalSensitivity.CompareGlobalSensitivityWithLocal missing key", SettingKey)
      return false
    end
  end
  print(bWriteLog and "LogicGlobalSensitivity.CompareGlobalSensitivityWithLocal matched")
  return true
end
