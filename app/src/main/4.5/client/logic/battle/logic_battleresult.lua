...
function BattleResult.on_game_result_impl(battle_result, result)
  ...
  if IngameEntry.UseCustomGameResult() then
    ...
    local get_promotion_unlocked_mode = function()
      if battle_result.promotion_layer == nil or not battle_result.promotion_result_info then
        return nil
      end
      local promo_data = battle_result.promotion_result_info.cur_promotion_data
      if not promo_data or not promo_data.locked_info then
        return nil
      end
      local tier = promo_data.locked_info[battle_result.promotion_layer]
      return tier and tier.unlocked_mode or nil
    end
    local slap_battle_type = get_promotion_unlocked_mode() or battle_result.battle_type
    if slap_battle_type then
      SeasonSystem.SetSlapBattleType(slap_battle_type)
    end
  end
