CentauriManager = CentauriManager or {
  ...
  country = nil,
  LocalUGCOrderInfo = {RechargeKey = "", Timestamp = 0},
  LocalUGCOrderCountDownTimerIdx = -1,
  Recharge_UGCCurrentRechargeKey = "",
  IsRechargedForThisRuntimeUGC = false,
  IsProcessUGCLocalOrder = false
}
...
function CentauriManager.Initialize()
  ...
  logic_payment_api:CentauriSDKInit()
...
function CentauriManager.OnLogin()
  ...
  CentauriManager.IsRechargedForThisRuntimeUGC = false
end
function CentauriManager.OnBackLogin()
  ...
  for _, cache in pairs(CentauriManager.MPCache) do
    cache.resultCode = ""
    cache.presentLevel = ""
    cache.timeStamp = 0
    cache.isRequesting = false
  end
end
function CentauriManager.GetMPCache(is_wow)
  return CentauriManager.MPCache[is_wow == true and "wow" or "uc"]
end
function CentauriManager.OnInitialize(_)
  ...
  CentauriHandler.send_wow_notify_client_charge()
end
...
function CentauriManager.OnPay(payCallBack)
  ...
  local isWOWRecharge = Logic_UGC_Recharge:CheckWOWRechargeByProductId(payCallBack.payProductId)
  log(bWriteLog and "CentauriManager.OnPay isWOWRecharge = " .. tostring(isWOWRecharge))
  local StringUtil = require("common.string_util")
  ...
  elseif payCallBack.resultCode == "0" then
    if paytype == "1" then
      if isWOWRecharge then
        CentauriManager.ugc_advanced_crystalNumBeforeUpdate = DataMgr.ugc_advanced_crystal
        CentauriManager.StartRequestNotifyClientRechargeTimerUGC()
        local CentauriHandler = require("client.network.Protocol.CentauriHandler")
        CentauriHandler.send_wow_notify_client_charge()
        if CentauriManager.IsRechargedForThisRuntimeUGC == false then
          CentauriManager.IsRechargedForThisRuntimeUGC = true
        end
      else
  ...
  elseif payCallBack.resultCode == "1" or Client.GetDevicePlatformName() == DevicePlatformNameMacros.Android and payCallBack.resultCode == "100" and payCallBack.resultInnerCode == "-2001" then
    if isWOWRecharge then
      CentauriManager.StartCancelRechargeGuildFlowUGC(paytype)
    else
  ...
  elseif Client.GetDevicePlatformName() == DevicePlatformNameMacros.IOS and payCallBack.resultCode == "100" and CentauriManager.IsErrorCodeInStoreErrorMsg(payCallBack.resultMsg, "-7003") then
    CentauriManager.StartCancelRechargeGuildFlow(paytype)
  elseif Client.GetDevicePlatformName() == DevicePlatformNameMacros.IOS and payCallBack.resultCode == "9" then
    log(bWriteLog and "logic_centauri:OnPayCallback.  payCallBack.resultCode == 9")
  else
    CentauriManager.ShowCommonPayErrorTip(nil)
  end
...
function CentauriManager.StartCancelRechargeGuildFlow(paytype)
  ...
  for i, v in pairs(CentauriManager.GetRechargeErrorMessages()) do
    while (string.lower(v.payType) ~= "recharge" or paytype == "1") and (string.lower(v.payType) ~= "goods" or paytype == "2") and (string.lower(v.payType) ~= "subscribe" or paytype == "3") do
      ...
      if v.rechargeKey ~= rechargeMessage.rechargeKey then
        if v.rechargeKey == tostring(CentauriManager.RechargeProductIdForLastTime) then
          rechargeMessage = v
          break
        end
        if rechargeMessage.rechargeKey ~= tostring(CentauriManager.RechargeProductIdForLastTime) then
          goto lbl_137
        end
        do break end
        ::lbl_137::
        if rechargeMessage.rechargeKey ~= tostring(CentauriManager.RechargeProductIdForLastTime) and v.rechargKey ~= tostring(CentauriManager.RechargeProductIdForLastTime) and v.impreciseMatchCounter > rechargeMessage.impreciseMatchCounter then
          rechargeMessage = v
        end
        break
      end
      if v.rechargeKey == rechargeMessage.rechargeKey and v.impreciseMatchCounter > rechargeMessage.impreciseMatchCounter then
        rechargeMessage = v
      end
      break
    end
...
function CentauriManager.AskPlayerToProcessLocalOrder(isCallFromTipClicked)
  ...
  if isCallFromTipClicked ~= nil and isCallFromTipClicked == true then
    ...
    log_tree("CentauriManager.AskPlayerToProcessLocalOrder CentauriManager.CachedProductInfoList = ", CentauriManager.CachedProductInfoList)
...
function CentauriManager.OnGetMP(getMPCallBack)
  ...
  local is_wow = CentauriManager.LastMPRequestIsWOW == true
  local mpCache = CentauriManager.GetMPCache(is_wow)
  mpCache.resultCode = "-2"
  mpCache.presentLevel = ""
  if getMPCallBack.mpInfoJson == nil then
    log(bWriteLog and "CentauriManager.OnGetMP failed 1")
    mpCache.isRequesting = false
    EventSystem:postEvent(EVENTTYPE_CENTAURI_NOTIFY, EVENTID_CENTAURI_GETMPINFO_NOTIFY, mpCache.resultCode, is_wow)
    return
  end
  ...
  if mpInfo.ret ~= nil and mpInfo.ret == 0 then
    ...
    if mpInfo.present_level ~= nil then
      ...
      local TimeUtil = require("client.common.time_util")
      mpCache.resultCode = tostring(mpInfo.ret)
      mpCache.presentLevel = mpInfo.present_level
      mpCache.timeStamp = TimeUtil.OSTime()
    end
  end
  if mpCache.resultCode == "0" then
...
function CentauriManager.OnGetProductInfo(getProductInfoCallBack)
  ...
  if CentauriManager.productInfoParsed ~= nil and CentauriManager.productInfoParsed.ret ~= nil and (CentauriManager.productInfoParsed.ret == "0" or CentauriManager.productInfoParsed.ret == 0) and CentauriManager.productInfoParsed.productInfo ~= nil and 0 < #CentauriManager.productInfoParsed.productInfo then
    local Logic_UGC_Recharge = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.logic_ugc_recharge)
    is_wow = Logic_UGC_Recharge:CheckWOWRecharge(CentauriManager.productInfoParsed.productInfo)
    ...
    log_tree("CentauriManager.OnGetProductInfo CentauriManager.CachedProductInfoList = ", CentauriManager.CachedProductInfoList)
    ...
    if CentauriManager.productInfoParsed.productInfo[1].productId ~= nil then
      local productType = CentauriManager.GetProductType(CentauriManager.productInfoParsed.productInfo[1].productId, is_wow)
      if productType == 1 then
        if is_wow then
          EventSystem:postEvent(EVENTTYPE_CENTAURI_NOTIFY, EVENTID_CENTAURI_GET_UGC_RECHARGE_PRODUCT_INFO_NOTIFY, CentauriManager.productInfoParsed.productInfo)
        else
      ...
      elseif productType == 2 then
        if is_wow then
          EventSystem:postEvent(EVENTTYPE_CENTAURI_NOTIFY, EVENTID_CENTAURI_GET_UGC_GOODS_PRODUCT_INFO_NOTIFY, CentauriManager.productInfoParsed.productInfo)
        else
...
function CentauriManager.GetProductType(productId, bUGC)
  local rechargeTable
  rechargeTable = FuncUtil.GetRechargeLevelTable(bUGC)
  if rechargeTable ~= nil then
...
function CentauriManager.IsMPExpire(is_wow)
  local mpCache = CentauriManager.GetMPCache(is_wow)
  if string.len(mpCache.resultCode) == 0 or mpCache.resultCode ~= "0" then
    return true
  end
  if string.len(mpCache.presentLevel) == 0 then
    return true
  end
  local TimeUtil = require("client.common.time_util")
  if TimeUtil.OSTime() < mpCache.timeStamp + CentauriManager.CachedMPExpireTime then
    return false
  end
  return true
end
...
function CentauriManager.SwitchPayChannel(channel)
  if CentauriManager.CurrentPayChannel ~= channel and channel ~= nil then
    ...
    PlayerPrefs.SaveTableToFile_N(personalRecords, PlayerPrefs.ePlayerPrefsType.ePersonalRecord)
    local logic_payment_api = require("client.logic.pay.logic_payment_api")
    logic_payment_api:SwitchPayChannel(channel - 1)
  end
end
...
function CentauriManager.ProcessSpecialDisplaySettingUGC(rechargeDataList)
  log(bWriteLog and "CentauriManager.ProcessSpecialDisplaySettingUGC called")
  local sCountry = DataMgr.RegionData and DataMgr.RegionData.region or "DEFAULT"
  log(bWriteLog and "CentauriManager.ProcessSpecialDisplaySettingUGC sCountry >>> " .. sCountry)
  local AOSSHOPMacros = require("client.slua.config.ClientMacros.AOSSHOPMacros")
  local sAOSSHOP = Client.GetAOSSHOP()
  local bIsCanCumulativeReplace = true
  if sAOSSHOP == AOSSHOPMacros.Samsung or sAOSSHOP == AOSSHOPMacros.Amazon or sAOSSHOP == AOSSHOPMacros.HMS or sAOSSHOP == AOSSHOPMacros.ThirdPartyPayment then
  ...
  log(bWriteLog and "CentauriManager.ProcessSpecialDisplaySettingUGC bIsCanCumulativeReplace >>>>" .. tostring(bIsCanCumulativeReplace) .. " sAOSSHOP >>> " .. sAOSSHOP)
  local returnRechargeData = CentauriManager.GetCountryRechargeList(rechargeDataList, sCountry, bIsCanCumulativeReplace)
  return returnRechargeData
end
function CentauriManager.OnUGCRechargeUIShow()
  CentauriManager.LoadUGCLocalOrder()
  CentauriManager.StopUGCLocalOrderCountDownTimer()
  CentauriManager.StartUGCLocalOrderCountDownTimer()
end
function CentauriManager.LoadUGCLocalOrder()
  log(bWriteLog and "[CentauriManager.LoadUGCLocalOrder] called")
  local PlayerPrefs = require("client.logic.LogicPlayerPrefs.playerprefs")
  local personalRecords = PlayerPrefs.LoadFileToTable_N(PlayerPrefs.ePlayerPrefsType.ePersonalUGCRecord)
  if personalRecords ~= nil and personalRecords[CentauriManager.LocalOrderKey_Timestamp] ~= nil and personalRecords[CentauriManager.LocalOrderKey_RechargeKey] ~= nil then
    CentauriManager.LocalUGCOrderInfo.Timestamp = personalRecords[CentauriManager.LocalOrderKey_Timestamp]
    CentauriManager.LocalUGCOrderInfo.RechargeKey = personalRecords[CentauriManager.LocalOrderKey_RechargeKey]
  end
  if CentauriManager.IsLocalUGCOrderExpire() then
    CentauriManager.CompleteLocalUGCOrder()
  end
end
function CentauriManager.IsLocalUGCOrderExpire()
  local TimeUtil = require("client.common.time_util")
  local deta = CentauriManager.LocalUGCOrderInfo.Timestamp + CentauriManager.LocalOrderExpireTime - TimeUtil.GetServerTimeInSec()
  return deta <= 0
end
function CentauriManager.CompleteLocalUGCOrder()
  log(bWriteLog and "[CentauriManager.CompleteLocalUGCOrder] called")
  CentauriManager.LocalUGCOrderInfo.Timestamp = 0
  CentauriManager.LocalUGCOrderInfo.RechargeKey = ""
  local PlayerPrefs = require("client.logic.LogicPlayerPrefs.playerprefs")
  local personalRecords = PlayerPrefs.LoadFileToTable_N(PlayerPrefs.ePlayerPrefsType.ePersonalUGCRecord)
  if personalRecords ~= nil and (personalRecords[CentauriManager.LocalOrderKey_Timestamp] ~= nil or personalRecords[CentauriManager.LocalOrderKey_RechargeKey] ~= nil) then
    personalRecords[CentauriManager.LocalOrderKey_Timestamp] = CentauriManager.LocalUGCOrderInfo.Timestamp
    personalRecords[CentauriManager.LocalOrderKey_RechargeKey] = CentauriManager.LocalUGCOrderInfo.RechargeKey
    PlayerPrefs.SaveTableToFile_N(personalRecords, PlayerPrefs.ePlayerPrefsType.ePersonalUGCRecord)
  end
end
function CentauriManager.StopUGCLocalOrderCountDownTimer()
  if CentauriManager.LocalUGCOrderCountDownTimerIdx then
    local time_ticker = require("common.time_ticker")
    time_ticker.RemoveTimer(CentauriManager.LocalUGCOrderCountDownTimerIdx)
    CentauriManager.LocalUGCOrderCountDownTimerIdx = nil
  end
end
function CentauriManager.StartUGCLocalOrderCountDownTimer()
  local time_ticker = require("common.time_ticker")
  CentauriManager.LocalUGCOrderCountDownTimerIdx = time_ticker.AddTimerLoop(0, CentauriManager.TickUpdateLocalUGCOrderRemainTime, TIMER_INFINITE, 1)
end
function CentauriManager.TickUpdateLocalUGCOrderRemainTime()
  local TimeUtil = require("client.common.time_util")
  local expireDetaTime = CentauriManager.LocalUGCOrderInfo.Timestamp + CentauriManager.LocalOrderExpireTime - TimeUtil.GetServerTimeInSec()
  local strCountDownTime = "00:00"
  if 0 < expireDetaTime then
    local _, hours, mins, seconds = TimeUtil.FormatCountDownTime_HMS(expireDetaTime)
    strCountDownTime = mins .. ":" .. seconds
  end
  EventSystem:postEvent(EVENTTYPE_SHOPLIMIT, EVENTID_UGC_SHOPLIMIT_CANCEL_RECHARGE_TIME_LIMIT_UPDATED, strCountDownTime)
  local RechargeSystem = require("client.logic.recharge.logic_recharge")
  local isShow = RechargeSystem.isRechargeUIShowing()
  if not isShow or expireDetaTime <= 0 then
    CentauriManager.StopUGCLocalOrderCountDownTimer()
...
function CentauriManager.GetUGCCurrentPayChannel()
  if CentauriManager.CurrentUGCPayChannel ~= nil then
    return CentauriManager.CurrentUGCPayChannel
  end
  local lastPayChannelKey = "LastPayChannel"
  local PlayerPrefs = require("client.logic.LogicPlayerPrefs.playerprefs")
  local personalRecords = PlayerPrefs.LoadFileToTable_N(PlayerPrefs.ePlayerPrefsType.ePersonalUGCRecord)
  if personalRecords ~= nil and personalRecords[lastPayChannelKey] ~= nil then
    CentauriManager.CurrentUGCPayChannel = personalRecords[lastPayChannelKey]
  end
  if CentauriManager.CurrentUGCPayChannel == nil then
    CentauriManager.CurrentUGCPayChannel = CentauriManager.E_PayChannel.H5
    if personalRecords == nil then
      personalRecords = {}
    end
    personalRecords[lastPayChannelKey] = CentauriManager.CurrentUGCPayChannel
    PlayerPrefs.SaveTableToFile_N(personalRecords, PlayerPrefs.ePlayerPrefsType.ePersonalUGCRecord)
  end
  return CentauriManager.CurrentUGCPayChannel
end
function CentauriManager.SwitchUGCPayChannel(channel)
  if CentauriManager.CurrentUGCPayChannel ~= channel and channel ~= nil then
    CentauriManager.CurrentUGCPayChannel = channel
    local lastPayChannelKey = "LastPayChannel"
    local PlayerPrefs = require("client.logic.LogicPlayerPrefs.playerprefs")
    local personalRecords = PlayerPrefs.LoadFileToTable_N(PlayerPrefs.ePlayerPrefsType.ePersonalUGCRecord)
    if personalRecords == nil then
...
function CentauriManager.UGCShouldDirectlyShowH5Pay()
  if CentauriManager.H5PayEnable() then
    local logic_cloud_game = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.logic_cloud_game)
    if logic_cloud_game:GetCloudGameType() == logic_cloud_game.CLOUD_GAME_TYPE.PIONEER_CLOUD then
      if logic_cloud_game:GetClientType() == logic_cloud_game.CLIENT_TYPE.MICRO_CLIENT then
    ...
    if CentauriManager.GetUGCCurrentPayChannel() ~= CentauriManager.E_PayChannel.GP then
      return true
    end
  end
  return false
end
function CentauriManager.UpdateUGCRechargeProductId(productId)
  log(bWriteLog and "[CentauriManager.UpdateUGCRechargeProductId] productId:" .. productId)
  CentauriManager.UGCRechargeProductIdForLastTime = productId
  CentauriManager.IsProcessUGCLocalOrder = false
  if string.len(productId) > 0 then
    CentauriManager.IsInUGCCentauriPaying = true
    CentauriManager.HitAppFrontGroundAfterPayUGC = false
    CentauriManager.HitIOSPsd2ErrorUGC = false
    CentauriManager.CachedUserTickectNumBeforeBuyUGC = DataMgr.ugc_advanced_crystal
  end
end
function CentauriManager.StartRequestNotifyClientRechargeTimerUGC()
  log(bWriteLog and "CentauriManager.StartRequestNotifyClientRechargeTimerUGC")
  local time_ticker = require("common.time_ticker")
  if CentauriManager.RequestNotifyClientChargeTimerUGC ~= nil then
    CentauriManager.ClearRequestNotifyClientRechargeTimerUGC()
  end
  CentauriManager.RequestNotifyClientChargeTimerUGC = time_ticker.AddTimerOnce(10, function()
    if GameStatus.IsInLobbyOrMainCity() and CentauriManager.ugc_advanced_crystalNumBeforeUpdate == DataMgr.ugc_advanced_crystal then
      log(bWriteLog and "CentauriManager.StartRequestNotifyClientRechargeTimerUGC SendPkg igame_notify_client_charge")
      local CentauriHandler = require("client.network.Protocol.CentauriHandler")
      CentauriHandler.send_wow_notify_client_charge()
    end
    CentauriManager.ClearRequestNotifyClientRechargeTimerUGC()
...
function CentauriManager.ClearRequestNotifyClientRechargeTimerUGC()
  log(bWriteLog and "CentauriManager.ClearRequestNotifyClientRechargeTimerUGC")
  local time_ticker = require("common.time_ticker")
  if CentauriManager.RequestNotifyClientChargeTimerUGC ~= nil then
    time_ticker.RemoveTimer(CentauriManager.RequestNotifyClientChargeTimerUGC)
    CentauriManager.RequestNotifyClientChargeTimerUGC = nil
  end
end
function CentauriManager.StartCancelRechargeGuildFlowUGC(paytype)
  local rechargeMessage
  local hasRecharged = DataMgr.Recharge == 0 or CentauriManager.IsRechargedForThisRuntimeUGC == true
  for i, v in pairs(CentauriManager.GetUGCRechargeErrorMessages()) do
    while (string.lower(v.payType) ~= "recharge" or paytype == "1") and (string.lower(v.payType) ~= "goods" or paytype == "2") and (string.lower(v.payType) ~= "subscribe" or paytype == "3") do
      if v.rechargeStatus ~= "skip" then
        if not ((v.rechargeStatus ~= "unpaid" or not hasRecharged) and (v.rechargeStatus ~= "paid" or hasRecharged) and (v.rechargeStatus ~= "unpaidinseason" or hasRecharged) and (v.rechargeStatus ~= "paidinseason" or hasRecharged)) then
          break
        end
        v.impreciseMatchCounter = v.impreciseMatchCounter + 1
      end
      if v.passStatus ~= "skip" then
        ...
        if not (v.passStatus ~= "paidpass" or logic_ugc_WOWPass:IsBuyElite()) or v.passStatus == "unpaidpass" and logic_ugc_WOWPass:IsBuyElite() then
          break
        end
        v.impreciseMatchCounter = v.impreciseMatchCounter + 1
      end
      if v.rechargeKey ~= "all" then
        if v.rechargeKey ~= tostring(CentauriManager.UGCRechargeProductIdForLastTime) then
          break
        end
        v.impreciseMatchCounter = v.impreciseMatchCounter + 1
      end
      ...
      if v.rechargKey ~= rechargeMessage.rechargKey then
        if v.rechargKey == tostring(CentauriManager.UGCRechargeProductIdForLastTime) then
          rechargeMessage = v
          break
        end
        if rechargeMessage.rechargKey ~= tostring(CentauriManager.UGCRechargeProductIdForLastTime) then
          goto lbl_132
        end
        do break end
        ::lbl_132::
        if rechargeMessage.rechargKey ~= tostring(CentauriManager.UGCRechargeProductIdForLastTime) and v.rechargKey ~= tostring(CentauriManager.UGCRechargeProductIdForLastTime) and v.impreciseMatchCounter > rechargeMessage.impreciseMatchCounter then
          rechargeMessage = v
        end
        break
      end
  ...
  local OnOkClickedCallback
  if paytype == "1" then
    OnOkClickedCallback = CentauriManager.ProgressUGCLocalOrder
  end
  if rechargeMessage ~= nil then
    log(bWriteLog and "[CentauriManager.StartCancelRechargeGuildFlowUGC] cat localization id:" .. rechargeMessage.translationKey)
    CentauriManager.ShowPayTip(LocUtil.GetLocalizeResStr(rechargeMessage.translationKey), OnOkClickedCallback)
  else
    CentauriManager.ShowPayTip(LocUtil.GetLocalizeResStr(7693), OnOkClickedCallback)
  end
end
function CentauriManager.ProgressUGCLocalOrder()
  log(bWriteLog and "[CentauriManager.ProgressUGCLocalOrder]")
  if not CentauriManager.IsProcessUGCLocalOrder then
    if CentauriManager.LocalUGCOrderInfo.RechargeKey ~= "" and not CentauriManager.IsLocalUGCOrderExpire() then
      CentauriManager.AskPlayerToProcessUGCLocalOrder()
    else
      CentauriManager.CreateUGCLocalOrder()
    end
  end
  CentauriManager.StopUGCLocalOrderCountDownTimer()
  CentauriManager.StartUGCLocalOrderCountDownTimer()
end
function CentauriManager.AskPlayerToProcessUGCLocalOrder(isCallFromTipClicked)
  local OnOkClicked = function()
    CentauriManager.IsProcessUGCLocalOrder = true
    CentauriManager.UGCRechargeProductIdForLastTime = CentauriManager.LocalUGCOrderInfo.RechargeKey
    EventSystem:postEvent(EVENTTYPE_SHOPLIMIT, EVENTID_SHOPLIMIT_REQUEST_DOPAY, CentauriManager.LocalUGCOrderInfo.RechargeKey)
    CentauriManager.CompleteUGCLocalOrder()
    CentauriManager.CreateUGCLocalOrder()
  end
  local OnCloseClicked = function()
    CentauriManager.CompleteUGCLocalOrder()
  ...
  if isCallFromTipClicked ~= nil and isCallFromTipClicked == true then
    ...
    local symbol = ""
    local Logic_UGC_Recharge = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.logic_ugc_recharge)
    local recharge_data = Logic_UGC_Recharge:GetRecharge_DataList_Array() or {}
    for i, v in ipairs(recharge_data) do
      if v.rechargeKey == CentauriManager.LocalUGCOrderInfo.RechargeKey then
        buyNum = v.buyNum
        money = v.money
        symbol = v.monetarySymbol
        break
    ...
    local symbol_money = symbol .. tostring(money)
    log_tree("CentauriManager.AskPlayerToProcessUGCLocalOrder CentauriManager.CachedProductInfoList = ", CentauriManager.CachedProductInfoList)
    for k, p in pairs(CentauriManager.CachedProductInfoList) do
      if p.productId == CentauriManager.LocalUGCOrderInfo.RechargeKey then
        symbol_money = p.price
        break
      end
    end
    ...
    local gen_ticket_index = -1
    local index = 0
    local recharge_PayNumList_Array = Logic_UGC_Recharge:GetRecharge_PayNumList_Array()
    if recharge_PayNumList_Array ~= nil then
      for k, v in pairs(recharge_PayNumList_Array) do
        index = index + 1
        if v == nBuyNum then
          gen_ticket_        end
      end
    ...
    local gen_ticket_number = 0
    index = 0
    local recharge_PayPresentList_Array = Logic_UGC_Recharge:GetRecharge_PayPresentList_Array()
    if 0 <= gen_ticket_index and recharge_PayPresentList_Array ~= nil then
      for k, v in pairs(recharge_PayPresentList_Array) do
        index = index + 1
        if index == gen_ticket_index then
          gen_ticket_number = v
        end
    ...
    local total_buy_number = tonumber(buyNum or 0) + gen_ticket_number
    msgContent = LocUtil.LocalizeResFormat("85505", symbol_money, tostring(total_buy_number))
  end
  local CommonMsgBoxMgr = require("client.slua.logic.common.logic_common_msg_box")
  CommonMsgBoxMgr.Show(2, msgTitle, msgContent, OnOkClicked, OnCloseClicked, nil, nil, {switchTextType = true})
end
function CentauriManager.CompleteUGCLocalOrder()
  log(bWriteLog and "[CentauriManager.CompleteUGCLocalOrder] called")
  CentauriManager.LocalUGCOrderInfo.Timestamp = 0
  CentauriManager.LocalUGCOrderInfo.RechargeKey = ""
  local PlayerPrefs = require("client.logic.LogicPlayerPrefs.playerprefs")
  local personalRecords = PlayerPrefs.LoadFileToTable_N(PlayerPrefs.ePlayerPrefsType.ePersonalUGCRecord)
...
function CentauriManager.CreateUGCLocalOrder()
  log(bWriteLog and "[CentauriManager.CreateUGCLocalOrder] called")
  if CentauriManager.Recharge_UGCCurrentRechargeKey ~= nil and CentauriManager.Recharge_UGCCurrentRechargeKey ~= "" then
    local TimeUtil = require("client.common.time_util")
    CentauriManager.LocalUGCOrderInfo.Timestamp = TimeUtil.GetServerTimeInSec()
    CentauriManager.LocalUGCOrderInfo.RechargeKey = CentauriManager.Recharge_UGCCurrentRechargeKey
    CentauriManager.Recharge_UGCCurrentRechargeKey = ""
    local PlayerPrefs = require("client.logic.LogicPlayerPrefs.playerprefs")
    local personalRecords = PlayerPrefs.LoadFileToTable_N(PlayerPrefs.ePlayerPrefsType.ePersonalUGCRecord)
    if personalRecords == nil then
      personalRecords = {}
...
function CentauriManager.PreLoadUGCRechargeErrorMessages()
  local rechargeMessages = CDataTable.GetTable("UGCRechargeErrorMessageConfig")
  CentauriManager.UGCRechargeErrorMessages = {}
  local PublishRegionMacros = require("client.slua.config.ClientMacros.PublishRegionMacros")
  if PublishRegionMacros.IsJapanOrKorea() then
    return
  end
  for i, v in pairs(rechargeMessages) do
    if string.lower(v.osplatform) == string.lower(Client.GetDevicePlatformName()) then
      ...
      table.insert(CentauriManager.UGCRechargeErrorMessages, vTemp)
    end
  end
end
function CentauriManager.GetUGCRechargeErrorMessages()
  if CentauriManager.UGCRechargeErrorMessages == nil then
    CentauriManager.PreLoadUGCRechargeErrorMessages()
  end
  return CentauriManager.UGCRechargeErrorMessages
end
