...
function level_unlock_manager:OnClickFeature(featureDef)
  ...
  self:_CancelPendingUIWaiter(featureDef)
end
...
function level_unlock_manager:StartUnlockGuideImpl(guideConfig, featureDef, isMain)
  if guideConfig.TargetUIConfig then
    self:_TryStartUnlockTipWithWait(guideConfig, featureDef, isMain)
  end
  self.ShowFoldButtonNewSign = guideConfig.ShowFoldButtonNewSign
  level_unlock_ui_util:ShowNewMark(guideConfig, isMain)
end
function level_unlock_manager:_TryStartUnlockTipWithWait(guideConfig, featureDef, isMain)
  log_format("level_unlock_manager:_TryStartUnlockTipWithWait feature = [%s], target = [%s], isMain = [%s]", featureDef, guideConfig.TargetUIConfig, isMain)
  self:_CancelPendingUIWaiter(featureDef)
  local tryShow = function()
    local childWidget = level_unlock_ui_util:GetTargetWidget(guideConfig, isMain)
    if childWidget then
      log_format("level_unlock_manager:_TryStartUnlockTipWithWait tryShow hit. feature = [%s], target = [%s]", featureDef, guideConfig.TargetUIConfig)
  ...
  self:AddTimerOnce(0.1, function()
    ...
    log_warning_format("level_unlock_manager:_TryStartUnlockTipWithWait first try miss, register waiter. feature = [%s], target = [%s]", featureDef, guideConfig.TargetUIConfig)
    self:_RegisterPendingUIWaiter(guideConfig, featureDef, isMain, tryShow)
  end)
end
function level_unlock_manager:_RegisterPendingUIWaiter(guideConfig, featureDef, isMain, tryShow)
  local targetKey = guideConfig.TargetUIConfig
  local listenKeyName
  if isMain then
    ...
    listenKeyName = mainUIConfig and mainUIConfig.keyName
  else
    local uiConfig = UIManager.UI_Config[targetKey]
    listenKeyName = uiConfig and uiConfig.keyName
  end
  if not listenKeyName then
    log_warning_format("level_unlock_manager:_RegisterPendingUIWaiter no listenKeyName. feature = [%s], target = [%s], isMain = [%s]", featureDef, targetKey, isMain)
    return
  end
  log_format("level_unlock_manager:_RegisterPendingUIWaiter regist. feature = [%s], target = [%s], listenKeyName = [%s], isMain = [%s]", featureDef, targetKey, listenKeyName, isMain)
  local waiter = {}
  local onUIShow = function(_, _, config)
    if not config or config.keyName ~= listenKeyName then
      return
    end
    log_format("level_unlock_manager:_RegisterPendingUIWaiter onUIShow hit. feature = [%s], listenKeyName = [%s]", featureDef, listenKeyName)
    if tryShow() then
      self:_CancelPendingUIWaiter(featureDef)
    else
      log_warning_format("level_unlock_manager:_RegisterPendingUIWaiter onUIShow hit but widget still nil. feature = [%s], target = [%s]", featureDef, targetKey)
    end
  end
  waiter.timerHandle = self:AddTimerOnce(30, function()
    if tryShow() then
      log_warning_format("level_unlock_manager:_RegisterPendingUIWaiter timeout but final tryShow hit. feature = [%s], target = [%s]", featureDef, targetKey)
    else
      log_warning_format("level_unlock_manager:_RegisterPendingUIWaiter timeout, give up. feature = [%s], target = [%s], listenKeyName = [%s]", featureDef, targetKey, listenKeyName)
    end
    self:_CancelPendingUIWaiter(featureDef)
  end)
  waiter.  EventSystem:registEvent(EVENTID_UI, BP_ENUM_UI_SHOW, onUIShow)
  self.pendingUIWaiters[featureDef] = waiter
end
function level_unlock_manager:_CancelPendingUIWaiter(featureDef)
  local waiter = self.pendingUIWaiters and self.pendingUIWaiters[featureDef]
  if not waiter then
    return
  end
  log_format("level_unlock_manager:_CancelPendingUIWaiter cancel. feature = [%s]", featureDef)
  if waiter.onUIShow then
    EventSystem:unregistEvent(EVENTID_UI, BP_ENUM_UI_SHOW, waiter.onUIShow)
  end
  if waiter.timerHandle then
    self:RemoveTimer(waiter.timerHandle)
  end
  self.pendingUIWaiters[featureDef] = nil
end
function level_unlock_manager:_CancelAllPendingUIWaiters()
  if not self.pendingUIWaiters then
    return
  end
  for featureDef, _ in pairs(self.pendingUIWaiters) do
    self:_CancelPendingUIWaiter(featureDef)
  end
end
function level_unlock_manager:OnDestroy()
  log(bWriteLog and "level_unlock_manager:OnDestroy")
  self:_CancelAllPendingUIWaiters()
  level_unlock_manager.__super.OnDestroy(self)
end
