...
function LobbyAvatar:_GetReplaceItemID(itemID)
  ...
  local tItemCfg = CDataTable.GetTableData("Item", ActualUsedItemID)
  if tItemCfg and tItemCfg.ItemSubType == 108 then
    ...
    local nUid = slua.isValid(operateAvatar) and operateAvatar:GetPlayerUID() or 0
    if not nUid or nUid == "" or tonumber(nUid) == 0 then
      return ActualUsedItemID
    end
    local nLgdResId = logic_legend_weapon:GetActiveLegendWeaponIdForUid(nUid)
    printf("[LGD-DBG] _GetReplace itemID=%s uid=%s lgdResId=%s", tostring(ActualUsedItemID), tostring(nUid), tostring(nLgdResId))
    ...
    local bFashionBagEdit = WardrobeLogicManager:GetWardrobeEditMode() == wardrobe_macro.EWardrobeEditMode.FashionBag
    if not bFashionBagEdit then
      local bIsSelf = nUidNum == (DataMgr.roleData and tonumber(DataMgr.roleData.uid) or 0)
      if bIsSelf then
        if 0 < nLgdResId and nLgdResId ~= ActualUsedItemID and logic_legend_weapon:IsSceneLobbyOn() then
          ActualUsedItemID = nLgdResId
        elseif nLgdResId == 0 and logic_legend_weapon:IsLegendWeaponItem(ActualUsedItemID) then
          ActualUsedItemID = 0
        end
      elseif 0 < nLgdResId and nLgdResId ~= ActualUsedItemID then
        ActualUsedItemID = nLgdResId
      end
    end
  end
...
function LobbyAvatar:SetClothSchemeIDList(idList)
  local Pawn = self:GetModel()
  if slua.isValid(Pawn) then
    Pawn:SetClothSchemeIDList(idList)
  end
end
function LobbyAvatar:ClearClothSchemeIDList()
  local Pawn = self:GetModel()
  if slua.isValid(Pawn) then
    Pawn:ClearClothSchemeIDList()
  end
end
function LobbyAvatar:AddClothSchemeID(schemeID)
  local Pawn = self:GetModel()
  if slua.isValid(Pawn) then
    Pawn:AddClothSchemeID(schemeID)
  end
end
function LobbyAvatar:RemoveClothSchemeID(schemeID)
  local Pawn = self:GetModel()
  if slua.isValid(Pawn) then
    Pawn:RemoveClothSchemeID(schemeID)
  end
end
...
function LobbyAvatar:SetDisableClothSimulation(bDisable)
  log(bWriteLog and "LobbyAvatar:SetDisableClothSimulation")
  local AvatarComp = self:GetModelAvatarComp()
  if not slua.isValid(AvatarComp) then
    log(bWriteLog and "[WARN] LobbyAvatar:SetDisableClothSimulation AvatarComp is invalid")
    return
  end
  AvatarComp:SetDisableClothSimulation(bDisable)
end
...
function LobbyAvatar:CheckAndExecuteChangeOperate(operateAvatar, itemID, tAvatarCustom)
  if self:_CheckItemPairChangeAction(operateAvatar, itemID, tAvatarCustom) then
    return true
  end
  if self:_CheckXSuitBranchChangeAction(operateAvatar, itemID, tAvatarCustom) then
    return true
  end
  return false
end
function LobbyAvatar:_CheckItemPairChangeAction(operateAvatar, itemID, tAvatarCustom)
  ...
  for _, tCurActionCfg in pairs(tAllActionCfg) do
    ...
    if nBeforeCloth == 1407632 or nBeforeCloth == 1407668 then
      ...
      log(bWriteLog and "LobbyAvatar:_CheckItemPairChangeAction Change action from: " .. nActionID .. " to: " .. NewActionID)
      nActionID = NewActionID
    end
    if operateAvatar:IsItemHasEquipped(nBeforeCloth) then
      self:_ExecuteChangeAction(operateAvatar, itemID, tAvatarCustom, nActionID, tCurActionCfg.IsPlayAnimAfterTransform)
      return true
    elseif self:IsSwitchingState(nBeforeCloth) then
      log(bWriteLog and "LobbyAvatar:_CheckItemPairChangeAction SwitchingState PlayEmoteBeforePutOnEquipment " .. tostring(itemID) .. " EmoteID " .. tostring(nActionID))
...
function LobbyAvatar:_CheckXSuitBranchChangeAction(operateAvatar, itemID, tAvatarCustom)
  local LogicXSuit = require("client.slua.logic.XSuit.logic_xsuit")
  if not LogicXSuit.IsXSuit(itemID) then
    return false
  end
  local toPeriod = LogicXSuit.GetPeriodByItemId(itemID)
  local toBranch = LogicXSuit.GetBranchByItemId(itemID)
  if not toPeriod or not toBranch then
    return false
  end
  local fromPeriod, fromBranch = LogicXSuit.GetCurrentEquippedXSuitPeriodAndBranch(self)
  if not fromPeriod or not fromBranch then
    return false
  end
  if fromPeriod ~= toPeriod then
    return false
  end
  if fromBranch == toBranch then
    return false
  end
  local actionID, isPlayAnimAfter = LogicXSuit.GetXSuitBranchChangeAction(toPeriod, fromBranch, toBranch)
  if not actionID or actionID == 0 then
    return false
  end
  log(bWriteLog and string.format("LobbyAvatar:_CheckXSuitBranchChangeAction hit period=%s fromBranch=%s toBranch=%s actionID=%s", tostring(toPeriod), tostring(fromBranch), tostring(toBranch), tostring(actionID)))
  self:_ExecuteChangeAction(operateAvatar, itemID, tAvatarCustom, actionID, isPlayAnimAfter)
  return true
end
function LobbyAvatar:_ExecuteChangeAction(operateAvatar, itemID, tAvatarCustom, actionID, isPlayAnimAfter)
  if isPlayAnimAfter == 1 then
    log(bWriteLog and "LobbyAvatar:_ExecuteChangeAction PutOnEquipmentAfterPlayEmote " .. tostring(itemID) .. " nActionID " .. tostring(actionID))
    operateAvatar:PutOnEquipmentByResID(itemID, tAvatarCustom)
    operateAvatar.LobbyPlayEmoteComponent_BP.bSyncEmote = true
    self._SyncEmote = actionID
    local adaptActionID = _FindAdaptEmotionID(operateAvatar, actionID)
    self.PlayingChangeActionMap[adaptActionID] = itemID
    self:PlayAction(actionID, "Default")
  else
    log(bWriteLog and "LobbyAvatar:_ExecuteChangeAction PlayEmoteBeforePutOnEquipment " .. tostring(itemID) .. " EmoteID " .. tostring(actionID))
    self:PlayEmoteBeforePutOnEquipment(actionID, itemID, tAvatarCustom)
  end
end
