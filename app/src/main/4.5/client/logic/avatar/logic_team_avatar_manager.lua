...
function TeamAvatarManager.GetAvatarLevel(isTeammate)
  ...
  if not lowDevice and gc_util.IsNeedDropAvatarFeature(1) then
    lowDevice = true
  end
...
function TeamAvatarManager.StopAllAvatarAction()
...
function TeamAvatarManager.CreatePet(AvatarUID, PetData)
  ...
  if uidStr ~= DataMgr.roleData.uid then
    local gc_util = require("common.gc_util")
    if gc_util.IsNeedDropAvatarFeature(4) then
      return
    end
  end
...
function TeamAvatarManager.OnBeginLobbySkinChange()
  TeamAvatarManager.StopAllAvatarAction()
