...
function LobbySystem.SetIsNewPlayer(value, reason)
  local prev = LobbySystem.isNewPlayer
  LobbySystem.isNewPlayer = value
  log(bWriteLog and "LobbySystem.SetIsNewPlayer: " .. tostring(prev) .. " -> " .. tostring(value) .. " | reason: " .. tostring(reason))
end
function LobbySystem.OnLogin(bReLogin)
  ...
  if roleData.ugc_market_white_list_info ~= nil then
    LobbySystem.LobbyMarketWhiteList = roleData.ugc_market_white_list_info
  end
  if not bReLogin then
    ...
    if FixCrash and FixCrash.OpenID and FixCrash.OpenID == roleData.openid then
      ...
      SettingSystem.BeginLoginSyncBatch(bMainVerUpdated)
      for SlotType, SlotTypeVersion in pairs(setting_ver_info) do
        local bNeedQuery = false
        if not bIsSameOpenID then
          bNeedQuery = true
        elseif SettingSystem.IsNeedQueryCustomSetting(SlotType, SlotTypeVersion) then
          bNeedQuery = true
        elseif 1 < SlotTypeVersion and type(SlotType) == "string" then
        ...
        if bNeedQuery then
          SettingSystem.MarkPendingSlot(SlotType)
          SettingSystem.query_custom_setting(SlotType)
        end
      end
      SettingSystem.CommitLoginSyncBatch(8)
      local SettingHandler = require("client.network.Protocol.SettingHandler")
...
function LobbySystem.DoConsoleCmdFromRemoteConfig()
  ...
  local DisableMeshLODDrop = HDmpveRemote.HDmpveRemoteConfigGetBool("DisableMeshLODDrop", false)
  if DisableMeshLODDrop then
    FuncUtil.UE4ExecuteConsoleCommand("r.SkeletalMesh.EnableDropSpecificLOD 0")
  end
...
function LobbySystem.on_please_create_role(defaultWearInfo, inNewGuideSwitch)
  ...
  LobbySystem.SetIsNewPlayer(true, "on_please_create_role")
...
function LobbySystem.on_sync_base_info(roleData)
  ...
  log(bWriteLog and "  :roleData.promo_guide_abtest_hit = " .. tostring(roleData.promo_guide_abtest_hit))
  log(bWriteLog and "  :roleData.promo_guide_abtest_id = " .. tostring(roleData.promo_guide_abtest_id))
  local login_module = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.login_module)
  ...
  if roleData.qrcode_login_info then
    log(bWriteLog and "LobbySystem.on_sync_base_info, qrcode_login_info sync")
    local QRcodeRestrictManager = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.QRcodeRestrictManager)
    QRcodeRestrictManager:on_get_qrcode_login_data_rsp(roleData.qrcode_login_info.can_scan_qrcode, roleData.qrcode_login_info.qrcode_login_info, roleData.qrcode_login_info.button_gray)
  else
    log(bWriteLog and "LobbySystem.on_sync_base_info, qrcode_login_info not found")
  end
  ...
  LobbySystem.  if roleData.is_first_login == LobbySystem.NewbieRoleState.NormalRole then
    LobbySystem.SetIsNewPlayer(false, "on_sync_base_info reset")
  end
  log(bWriteLog and "LobbySystem.on_sync_base_info roleData.wow_hall_is_open = " .. tostring(roleData.wow_hall_is_open))
  ...
  local roleDataTb = {
    ...
    is_user_recharged = roleData.is_user_recharged,
    ...
    last_wow_recharge_amount = roleData.last_wow_recharge_amount,
    last_wow_recharge_time = roleData.last_wow_recharge_time,
    ...
    open_llm_pilot = roleData.open_llm_pilot,
    ugc_advanced_crystal = roleData.ugc_advanced_crystal,
    wow_uc_gen_balance = roleData.wow_uc_gen_balance,
  ...
  if roleData.google_ad_weekly_disable then
    DataMgr.roleData.google_ad_weekly_disable = roleData.google_ad_weekly_disable
    log(bWriteLog and "LobbySystem.on_sync_base_info. roleData.google_ad_weekly_disable: " .. tostring(roleData.google_ad_weekly_disable))
  else
    log(bWriteLog and "LobbySystem.on_sync_base_info.  no google_ad_weekly_disable")
  end
  ...
  if roleData.bgmi_wiki and roleData.common_red_point_cfg then
    ...
    ActivityCenterModule:SetWebRedPointByCfg(roleData.common_red_point_cfg)
...
function LobbySystem.OnBattleBeginPlay()
  ...
  if IsWoWEditor then
    local GameFrontendHUD = UIUtil.GetGameFrontendHUD()
    if GameFrontendHUD ~= nil then
      local UKismetSystemLibrary = import("KismetSystemLibrary")
      UKismetSystemLibrary.ExecuteConsoleCommand(GameFrontendHUD:GetWorld(), "r.ReflectionCapture")
    end
  end
  local timer_ticker = require("common.time_ticker")
...
function LobbySystem.on_match_cancel_rsp(msg, userid, krjp_rematch)
  ...
  if msg == NetErrorCode_NONE or msg == "failed" then
    ...
    local bWasTimedOut = MatchSystem.IsSameLanguageMatchTimeOut()
    LobbySystem.ResetMatchInfo()
    MatchSystem.ResetData()
    local TeamUpNewSystem = require("client.slua.logic.teamup.logic_team_up")
    ...
    if bWasTimedOut then
      EventSystem:postEvent(EVENTTYPE_MATCH, EVENTID_ON_SAME_LANGUAGE_MATCH_TIMEOUT_RECOVER)
    end
  end
...
function LobbySystem.on_match_success(msg, team_info, antsvoice_url, deathmatch_teams, game_team_size, gameid, team_id, camp_id, sub_mode, feedback_flag, ugc_mod, promotion_layer)
  ...
  log(bWriteLog and "LobbySystem.on_match_success promotion_layer = " .. tostring(promotion_layer))
  local logic_multi_select_match = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.logic_multi_select_match)
  ...
  if msg == NetErrorCode_NONE then
    ...
    logic_promotion:SetLoadingPromotionLayer(promotion_layer)
...
local FIT_HIDE_MENU_IDS = {
  [BP_ENUM_LOBBY_MINI_TV] = true,
  [BP_ENUM_MAIN_CITY_ENTRY_SWITCH] = true,
  [BP_ENUM_MODULE_WARZONE_RANK] = true,
  [BP_ENUM_LOBBY_MENU_NEW_SUPPLY] = true
}
function LobbySystem.CheckOpen(menuId)
  if FIT_HIDE_MENU_IDS[menuId] then
    ...
    if not LogicPufferBundle.IsFitLobbyResDownloaded() then
      log_format("LobbySystem.CheckOpen. menuId=%s, not downloaded", tostring(menuId))
      return false
    end
  end
  local isOpen
...
function LobbySystem.CheckLobbyMenuOpen(menuId, isShowTip)
  ...
  if FIT_HIDE_MENU_IDS[menuId] then
    local LogicPufferBundle = require("client.slua.logic.download.bundle.logic_puffer_bundle")
    if not LogicPufferBundle.IsFitLobbyResDownloaded() then
      log_format("LobbySystem.CheckLobbyMenuOpen. menuId=%s, not downloaded", tostring(menuId))
      if isShowTip then
        ShowNotice(180110)
      end
      return false
...
function LobbySystem.IsRpEntry(URL)
  if not URL then
    return false
  end
  local StringUtil = require("common.string_util")
  ...
  local moduleid = tonumber(params.module)
  return moduleid == BP_ENUM_MODULE_UNKNOW_PASS
end
function LobbySystem.on_get_activity_display_res(activity_display_table)
  ...
  for k, v in pairs(activity_display_table) do
    local data = {
      ...
      isRPEntry = LobbySystem.IsRpEntry(v.jump),
      ActId = v.act_id,
      CreatedUtc = 0,
      CreatedProcess = 0,
...
function LobbySystem.RefreshBannerDisplayList()
  ...
  for i = #LobbySystem.activityDisplayDataList, 1, -1 do
    ...
    data.isRPEntry = LobbySystem.IsRpEntry(data.JumpUrl)
    if LocalIfActivityShow(data.ActivityType, data) then
...
function LobbySystem.CheckJumpWhiteList(url)
  ...
  elseif module == BP_ENUM_MODULE_CARD_COLLECTION_MAIN then
    ModuleManager.GetModule(ModuleManager.JumpModuleConfig.CardCollectionMainJumpModule, function(m)
      if m then
        m:JumpCheck(urlParam)
      end
    end)
    return true
  end
...
function LobbySystem.OnRequestPermissionsResult(requestCode, permissions, grantResult)
  ...
  if requestCode == 100 and permissions == logic_chat_voice_const.Enum_AndroidPermissionName.RecordAudio and (grantResult == logic_chat_voice_const.Enum_AndroidPermissionResult.Denied or grantResult == logic_chat_voice_const.Enum_AndroidPermissionResult.Granted) then
...
function LobbySystem.IsInNarutoVersionTime()
  local LinkedElementDisplayDuration = CDataTable.GetTableData("LinkedElementDisplayDuration", 1)
  local ServerTimeInSec = math.floor(TimeUtil.GetServerTimeInSec())
  log(bWriteLog and "LobbySystem.IsInNarutoVersionTime ServerTimeInSec: " .. tostring(ServerTimeInSec))
  if ServerTimeInSec < tonumber(TimeUtil.TimeStringToUnixstamp(LinkedElementDisplayDuration.StartTime)) or ServerTimeInSec > tonumber(TimeUtil.TimeStringToUnixstamp(LinkedElementDisplayDuration.EndTime)) then
    return false
  end
  local version_util = require("client.common.version_util")
  local clientVersion = version_util.GetClientFormat(Client.GetAppVersion())
  log(bWriteLog and "LobbySystem.IsInNarutoVersionTime clientVersion: " .. tostring(clientVersion))
  if version_util.LowerVersion(clientVersion, LinkedElementDisplayDuration.MinVersion) or version_util.HigherVersion(clientVersion, LinkedElementDisplayDuration.MaxVersion) then
    return false
  end
  log(bWriteLog and "LobbySystem.IsInNarutoVersionTime true")
  return true
end
function LobbySystem.CheckMarketWhiteListModID(ModID)
  if not ModID then
    print(bWriteLog and "LobbySystem.CheckMarketWhiteListModID ModID is nil")
    return false
  end
  if not LobbySystem.LobbyMarketWhiteList then
    print(bWriteLog and "LobbySystem.CheckMarketWhiteListModID LobbySystem.LobbyMarketWhiteList is nil")
    return false
  end
  for _, whiteListModID in ipairs(LobbySystem.LobbyMarketWhiteList) do
    if whiteListModID == ModID then
      return true
    end
  end
  return false
