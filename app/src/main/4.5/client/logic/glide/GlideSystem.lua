...
function GlideSystem:ExitGlideScene(TarSceneType)
  ...
  if slua.isValid(self.CameraActor) then
    ...
    local bSkipSwitchCamera = LoadingCameraID and LoadingCameraID ~= Lobby_camera_manager_module.Enum_CameraID.GlidePreview
    if bSkipSwitchCamera then
      log(bWriteLog and string.format("GlideSystem:ExitGlideScene skip switch camera while loading, LoadingCameraID = %s", tostring(LoadingCameraID)))
    elseif Lobby_camera_manager_module.currentCameraID == Lobby_camera_manager_module.Enum_CameraID.GlidePreview then
