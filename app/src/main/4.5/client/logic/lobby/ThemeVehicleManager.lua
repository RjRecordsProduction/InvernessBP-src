...
function ThemeVehicleManager:RegistEvents()
  ...
  self:AddCommonEvent(EVENTTYPE_LOBBY, EVENTID_ACTION_PLAY_START, self.OnAvatarPlayAction, self)
  self:AddCommonEvent(EVENTTYPE_LOBBY, EVENTID_ACTION_PALY_END, self.OnAvatarStopAction, self)
end
...
function ThemeVehicleManager:_ShowSelfVehicle()
  ...
  for Position, Info in pairs(VehicleInfos) do
    if Info then
      ...
      local MultiSlotParts = LogicVehicleExtendedFeature:GetEquipedMultiSlotParts(ItemID, source)
      self:_TryCreateVehicleModel(ItemID, StyleList, EnableHighTire, Position, accessoryList, ChassisLight, DataMgr.roleData.uid, MultiSlotParts)
    end
  end
...
function ThemeVehicleManager:_ShowTeamVehicle(UID)
  ...
  for Position, ItemID in pairs(VehicleIDs) do
    ...
    local MultiSlotParts = LogicVehicleExtendedFeature:GetVehicleMultiSlotParts(UID, ItemID, Position, EWardrobeDataSource.Wardrobe)
    self:_TryCreateVehicleModel(ItemID, StyleList, EnableHighTire, Position, accessoryList, ChassisLightData, UID, MultiSlotParts)
  end
...
local CombineTag = function(ItemId, StyleList, EnableHighTire, vehicleAccssory, ChassisLightId, UID, MultiSlotParts)
  ...
  if MultiSlotParts and next(MultiSlotParts) then
    local slotList = {}
    for slotType, _ in pairs(MultiSlotParts) do
      table.insert(slotList, slotType)
    end
    table.sort(slotList)
    for _, slotType in ipairs(slotList) do
      Tag = Tag .. tostring(slotType) .. "-" .. tostring(MultiSlotParts[slotType]) .. "_"
    end
  end
  Tag = Tag .. tostring(EnableHighTire) .. tostring(UID)
  return Tag
end
function ThemeVehicleManager:_TryCreateVehicleModel(ItemId, StyleList, EnableHighTire, Position, vehicleAccssory, ChassisLight, UID, MultiSlotParts)
  ...
  local Tag = CombineTag(ItemId, StyleList, EnableHighTire, vehicleAccssory, ChassisLight, UID, MultiSlotParts)
  ...
  self:_CreateVehicleModel(ItemId, StyleList, EnableHighTire, Position, TransForm, vehicleAccssory, ChassisLight, UID, MultiSlotParts)
end
function ThemeVehicleManager:_ReinitShowModelActor(VehicleActor, ItemId, StyleList, EnableHighTire, Position, TransForm, vehicleAccssory, ChassisLight, UID, MultiSlotParts)
...
function ThemeVehicleManager:_CreateVehicleModel(ItemId, StyleList, EnableHighTire, Position, TransForm, vehicleAccssory, ChassisLight, UID, MultiSlotParts)
  ...
  self:_ReinitShowModelActor(VehicleActor, ItemId, StyleList, EnableHighTire, Position, TransForm, vehicleAccssory, ChassisLight, UID, MultiSlotParts)
end
...
function ThemeVehicleManager:SetGarageCenterVehicleTranslucent(bOpen)
  local uSTExtraBlueprintFunctionLibrary = import("STExtraBlueprintFunctionLibrary")
  local bAA = uSTExtraBlueprintFunctionLibrary.GetConsoleVariableIntValue("r.MobileMSAA")
  if 1 < bAA and bOpen == true then
    log(bWriteLog and "ThemeVehicleManager:SetGarageCenterVehicleTranslucent bAA: " .. tostring(bAA))
    return
  end
  local TargetMeshes = {}
  if GetGarageThemeSystem():IsInGarageTheme() then
    local Vehicle = self.Vehicles[1]
    if slua.isValid(Vehicle) then
      local ModelActor = Vehicle._SubOperator._Entity.ModelActor
      if ModelActor then
        local MeshComponentClass = import("/Script/Engine.MeshComponent")
        local uComponentArray = ModelActor:GetComponentsByClass(MeshComponentClass)
        for _, comp in pairs(uComponentArray) do
          if comp and slua.isValid(comp) then
            table.insert(TargetMeshes, comp)
          end
        end
      end
    end
  end
  self.VehicleTranslucentCount = self.VehicleTranslucentCount or 0
  if bOpen then
    if self.VehicleTranslucentCount == 0 then
      FuncUtil.UE4ExecuteConsoleCommand("r.EnableCustomRenderPass 1")
      FuncUtil.UE4ExecuteConsoleCommand("r.CustomRenderPass.BlendAlpha 0.1")
      if next(TargetMeshes) then
        for _, mesh in ipairs(TargetMeshes) do
          mesh:SetRenderInCustomPass(true)
        end
        log(bWriteLog and "ThemeVehicleManager:SetGarageCenterVehicleTranslucent SetRenderInCustomPass true count:%s", #TargetMeshes)
      else
        log(bWriteLog and "ThemeVehicleManager:SetGarageCenterVehicleTranslucent TargetMeshes is empty")
      end
    end
    self.VehicleTranslucentCount = self.VehicleTranslucentCount + 1
  else
    self.VehicleTranslucentCount = self.VehicleTranslucentCount - 1
    if self.VehicleTranslucentCount <= 0 then
      self.VehicleTranslucentCount = 0
      if next(TargetMeshes) then
        for _, mesh in ipairs(TargetMeshes) do
          mesh:SetRenderInCustomPass(false)
        end
        log(bWriteLog and "ThemeVehicleManager:SetGarageCenterVehicleTranslucent SetRenderInCustomPass false count:%s", #TargetMeshes)
      else
        log(bWriteLog and "ThemeVehicleManager:SetGarageCenterVehicleTranslucent TargetMeshes is empty")
      end
      FuncUtil.UE4ExecuteConsoleCommand("r.CustomRenderPass.BlendAlpha 1")
      FuncUtil.UE4ExecuteConsoleCommand("r.EnableCustomRenderPass 0")
    end
  end
end
function ThemeVehicleManager:OnAvatarPlayAction(_, _, CharacterAvatarComp, ActionID)
  log(bWriteLog and "ThemeVehicleManager:OnAvatarPlayAction ActionID = " .. ActionID)
  local EAvatarSlotType = import("EAvatarSlotType")
  local ClothID = CharacterAvatarComp:GetAvatarIDBySlotID(EAvatarSlotType.EAvatarSlotType_ClothesEquipemtSlot)
  local Cfg = CDataTable.GetTableData("RealGoldenSuitFeature", ClothID)
  if Cfg and Cfg.TemmupEmote == ActionID then
    self:SetGarageCenterVehicleTranslucent(true)
  end
  Cfg = CDataTable.GetTableDataByFilter("GoldenSuitMapCfg", "TeamupActionID", ActionID)
  if Cfg then
    self:SetGarageCenterVehicleTranslucent(true)
  end
end
function ThemeVehicleManager:OnAvatarStopAction(_, _, ActionID, CharacterAvatarComp)
  local EAvatarSlotType = import("EAvatarSlotType")
  local ClothID = CharacterAvatarComp:GetAvatarIDBySlotID(EAvatarSlotType.EAvatarSlotType_ClothesEquipemtSlot)
  local Cfg = CDataTable.GetTableData("RealGoldenSuitFeature", ClothID)
  if Cfg and Cfg.TemmupEmote == ActionID then
    self:SetGarageCenterVehicleTranslucent(false)
  end
  Cfg = CDataTable.GetTableDataByFilter("GoldenSuitMapCfg", "TeamupActionID", ActionID)
  if Cfg then
    self:SetGarageCenterVehicleTranslucent(false)
