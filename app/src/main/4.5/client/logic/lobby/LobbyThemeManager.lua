...
function LobbyThemeManager:ShowTheme(bTips)
  ...
  if isCEVersion then
    log(bWriteLog and "LobbyThemeManager:ShowTheme itemID set default for CE, OriginalID: " .. tostring(itemID))
    itemID = self:GetDefaultThemeItemID()
  end
  ...
  if loadingSkinCfg then
    ...
    if LevelPath == nil then
      log(bWriteLog and "LobbyThemeManager:ShowTheme LevelPath is nil")
      itemID = self:GetDefaultThemeItemID()
      bDownloaded = true
    else
