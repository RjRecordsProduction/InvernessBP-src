...
function logic_season_year_badge:OnPostSwitchGameStatus(preState, nextState)
  if preState == GameStatus.Login and nextState == GameStatus.Lobby then
    if self.deletaTimer then
      self:RemoveTimer(self.deletaTimer)
      self.deletaTimer = nil
    end
    self.deletaTimer = self:AddTimerOnce(30, function()
      log(bWriteLog and "[logic_season_year_badge:OnPostSwitchGameStatus GetBadgeShowType")
      self:GetBadgeShowType()
    end)
  end
end
...
function logic_season_year_badge:on_set_season_year_badge_show_scene_rsp(scene_key, show_value, year_id)
  self.show_scene_info[scene_key] = {show = show_value, year_id = year_id}
  EventSystem:postEvent(EVENTTYPE_SEASON_YEAR, EVENTID_SEASON_YEAR_SELECT_YEAY_ID_UPDATE)
end
function logic_season_year_badge:on_get_season_year_badge_show_info_rsp(show_info)
  for k, v in pairs(show_info) do
    self.show_scene_info[k] = {
      show = v.show,
      year_id = v.year_id
    }
  end
...
function logic_season_year_badge:GetBadgeInfoBySeasonYearID(season_year_id)
  return self.seasonYearBadgeInfo and self.seasonYearBadgeInfo[season_year_id] or {}
end
...
function logic_season_year_badge:GetBadgeShowType()
  local need_req = true
  if self.show_scene_info then
    local count = 0
    for k, v in pairs(self.show_scene_info) do
      count = count + 1
    end
    need_req = count == 0
  end
  if need_req then
    local SeasonYearHandler = require("client.network.Protocol.SeasonYearHandler")
    SeasonYearHandler.send_get_season_year_badge_show_info_req()
  end
  return self.show_scene_info
end
function logic_season_year_badge:GetCurSeasonYearLoginDays()
  return self.loginDays
end
function logic_season_year_badge:GetAllSeasonYearBadgeInfo(target_uid)
  target_uid = tonumber(target_uid) or 0
  if target_uid == 0 or target_uid == tonumber(DataMgr.roleData.uid) then
    return self.seasonYearBadgeInfo
  else
    return self.otherBadgeData[target_uid]
  end
end
