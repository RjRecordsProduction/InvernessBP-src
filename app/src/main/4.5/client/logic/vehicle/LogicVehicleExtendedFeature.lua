...
function LogicVehicleExtendedFeature:DefineAndResetData()
  ...
  self.car_brake_caliper = nil
  self.equip_brake_caliper = nil
  self.car_wheel_hub = nil
  self.equip_wheel_hub = nil
  self.car_sunroof = nil
  self.equip_sunroof = nil
  ...
  self.vehicleSupportParts = nil
  self.partFitVehicleIDs = nil
end
function LogicVehicleExtendedFeature:OnInitialize()
  self:ParseSportCarSupportParts()
end
function LogicVehicleExtendedFeature:ParseSportCarSupportParts()
  self.vehicleSupportParts = {}
  self.partFitVehicleIDs = {}
  local SportCarSupportParts = CDataTable.GetTable("SportCarSupportParts")
  if not SportCarSupportParts then
    log(bWriteLog and "LogicVehicleExtendedFeature:ParseSportCarSupportParts SportCarSupportParts is nil")
    return
  end
  local StringUtil = require("common.string_util")
  for _, cfg in pairs(SportCarSupportParts) do
    local fitVehicleIDs = cfg.FitVehicleIDs
    local fitPartIDs = cfg.FitPartIDs
    local slotType = cfg.SlotType
    if fitVehicleIDs and fitPartIDs and slotType and 0 < slotType then
      local vehicleIDList = StringUtil.Split(fitVehicleIDs, "|")
      local partIDList = StringUtil.Split(fitPartIDs, "|")
      for _, vehicleIDStr in ipairs(vehicleIDList) do
        local vehicleID = tonumber(vehicleIDStr)
        if vehicleID and 0 < vehicleID then
          if not self.vehicleSupportParts[vehicleID] then
            self.vehicleSupportParts[vehicleID] = {}
          end
          if not self.vehicleSupportParts[vehicleID][slotType] then
            self.vehicleSupportParts[vehicleID][slotType] = {}
          end
          for _, partIDStr in ipairs(partIDList) do
            local partID = tonumber(partIDStr)
            if partID and 0 < partID then
              table.insert(self.vehicleSupportParts[vehicleID][slotType], partID)
              if not self.partFitVehicleIDs[partID] then
                self.partFitVehicleIDs[partID] = {}
              end
              table.insert(self.partFitVehicleIDs[partID], vehicleID)
            end
          end
        end
      end
...
function LogicVehicleExtendedFeature:GetWheelHubList(vehicleID)
  local WheelHubList = {}
  if not vehicleID or vehicleID <= 0 then
    log(bWriteLog and "LogicVehicleExtendedFeature:GetWheelHubList vehicleID is invalid")
    return WheelHubList
  end
  local vehicleData = self.vehicleSupportParts and self.vehicleSupportParts[vehicleID]
  local partIDs = vehicleData and vehicleData[EVehiclePartSlot.WheelHub]
  if not partIDs then
    return WheelHubList
  end
  for _, partID in ipairs(partIDs) do
    table.insert(WheelHubList, {ItemID = partID})
  end
  return WheelHubList
end
function LogicVehicleExtendedFeature:GetBrakeCaliperList(vehicleID)
  local BrakeCaliperList = {}
  if not vehicleID or vehicleID <= 0 then
    log(bWriteLog and "LogicVehicleExtendedFeature:GetBrakeCaliperList vehicleID is invalid")
    return BrakeCaliperList
  end
  local vehicleData = self.vehicleSupportParts and self.vehicleSupportParts[vehicleID]
  local partIDs = vehicleData and vehicleData[EVehiclePartSlot.BrakeCaliper]
  if not partIDs then
    return BrakeCaliperList
  end
  for _, partID in ipairs(partIDs) do
    table.insert(BrakeCaliperList, {ItemID = partID})
  end
  return BrakeCaliperList
end
function LogicVehicleExtendedFeature:GetCanopyList(vehicleID)
  local CanopyList = {}
  if not vehicleID or vehicleID <= 0 then
    log(bWriteLog and "LogicVehicleExtendedFeature:GetCanopyList vehicleID is invalid")
    return CanopyList
  end
  local vehicleData = self.vehicleSupportParts and self.vehicleSupportParts[vehicleID]
  local partIDs = vehicleData and vehicleData[EVehiclePartSlot.Canopy]
  if not partIDs then
    return CanopyList
  end
  for _, partID in ipairs(partIDs) do
    table.insert(CanopyList, {ItemID = partID})
  end
  return CanopyList
end
function LogicVehicleExtendedFeature:CheckVehicleSupportMultiSlot(vehicleID)
  if not vehicleID or vehicleID <= 0 then
    return false
  end
  local vehicleData = self.vehicleSupportParts and self.vehicleSupportParts[vehicleID]
  ...
  for slotType, partIDs in pairs(vehicleData) do
    if partIDs and 0 < #partIDs then
      return true
    end
  end
  return false
...
function LogicVehicleExtendedFeature:GetFeatureItemStateFlag(featureId)
  ...
  HasFlag = HasFlag or self.car_wheel_hub and self.car_wheel_hub[featureId]
  HasFlag = HasFlag or self.car_brake_caliper and self.car_brake_caliper[featureId]
  HasFlag = HasFlag or self.car_sunroof and self.car_sunroof[featureId]
...
function LogicVehicleExtendedFeature:CheckHasEquippedItem(featureId, vehicleId)
  ...
  elseif itemSubType == ENUM_ITEM_SUBTYPE.VehicleWheelHub then
    return vehicleId and self.equip_wheel_hub and self.equip_wheel_hub[vehicleId] and self.equip_wheel_hub[vehicleId] == featureId
  elseif itemSubType == ENUM_ITEM_SUBTYPE.VehicleBrakeCaliper then
    return vehicleId and self.equip_brake_caliper and self.equip_brake_caliper[vehicleId] and self.equip_brake_caliper[vehicleId] == featureId
  elseif itemSubType == ENUM_ITEM_SUBTYPE.VehicleCanopy then
    return vehicleId and self.equip_sunroof and self.equip_sunroof[vehicleId] and self.equip_sunroof[vehicleId] == featureId
  end
  return false
end
...
function LogicVehicleExtendedFeature:GetEquipedBrakeCaliperData(vehicleId, source)
  if not vehicleId then
    return nil
  end
  if source == EWardrobeDataSource.InheritWardrobe then
    return self.inherit_feature_data and self.inherit_feature_data.equip_brake_caliper and self.inherit_feature_data.equip_brake_caliper[vehicleId]
  end
  return self.equip_brake_caliper and self.equip_brake_caliper[vehicleId] or nil
end
function LogicVehicleExtendedFeature:GetEquipedWheelHubData(vehicleId, source)
  if not vehicleId then
    return nil
  end
  if source == EWardrobeDataSource.InheritWardrobe then
    return self.inherit_feature_data and self.inherit_feature_data.equip_wheel_hub and self.inherit_feature_data.equip_wheel_hub[vehicleId]
  end
  return self.equip_wheel_hub and self.equip_wheel_hub[vehicleId] or nil
end
function LogicVehicleExtendedFeature:GetEquipedSunroofData(vehicleId, source)
  if not vehicleId then
    return nil
  end
  if source == EWardrobeDataSource.InheritWardrobe then
    return self.inherit_feature_data and self.inherit_feature_data.equip_sunroof and self.inherit_feature_data.equip_sunroof[vehicleId]
  end
  return self.equip_sunroof and self.equip_sunroof[vehicleId] or nil
end
function LogicVehicleExtendedFeature:GetEquipedMultiSlotParts(vehicleId, source, bWithDefault)
  local parts = {}
  if not vehicleId then
    return parts
  end
  local brakeCaliper = self:GetEquipedBrakeCaliperData(vehicleId, source)
  if brakeCaliper then
    parts[EVehiclePartSlot.BrakeCaliper] = brakeCaliper
  end
  local wheelHub = self:GetEquipedWheelHubData(vehicleId, source)
  if wheelHub then
    parts[EVehiclePartSlot.WheelHub] = wheelHub
  end
  local canopy = self:GetEquipedSunroofData(vehicleId, source)
  if canopy then
    parts[EVehiclePartSlot.Canopy] = canopy
  end
  if bWithDefault then
...
function LogicVehicleExtendedFeature:GetVehicleDefaultPartId(vehicleId, slot)
  if not vehicleId or not slot then
    return 0
  end
  local cfg = CDataTable.GetTableData("SportCarDefaultSet", vehicleId)
  if not (cfg and cfg.SetIDList) or cfg.SetIDList == "" then
    return 0
  end
  local StringUtil = require("common.string_util")
  local defaultItems = StringUtil.Split(cfg.SetIDList, "|")
  local itemId = tonumber(defaultItems[slot])
  if itemId and 0 < itemId then
    return itemId
  end
  return 0
end
function LogicVehicleExtendedFeature:GetVehicleMultiSlotParts(uid, vehicleId, position, source)
  if not uid or not vehicleId then
    return {}
  end
  if tonumber(uid) == tonumber(DataMgr.roleData.uid) or tonumber(uid) == 0 then
    return self:GetEquipedMultiSlotParts(vehicleId, source)
  else
    position = position or 1
    local GarageThemeSystem = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.GarageThemeSystem)
    local TeamUpNewSystem = require("client.slua.logic.teamup.logic_team_up")
    local partsData
    if GarageThemeSystem:IsInGarageTheme() then
      partsData = TeamUpNewSystem.GetGarageVehicleMultiSlotData(uid, vehicleId, position)
    else
      partsData = TeamUpNewSystem.GetTeamShowVehicleMultiSlotData(uid, vehicleId)
    end
    return partsData
  end
  return {}
...
function LogicVehicleExtendedFeature:CheckHasNewItemReddot(extendedFeatureType, vehicleID)
  log(bWriteLog and "LogicVehicleExtendedFeature:CheckHasNewItemReddot extendedFeatureType:" .. tostring(extendedFeatureType) .. " vehicleID:" .. tostring(vehicleID))
  ...
  elseif extendedFeatureType == ENUM_Vehicle_ExtendedFeatureType.VEHICLE_BRAKE_CALIPER then
    tableData = self.car_brake_caliper
  elseif extendedFeatureType == ENUM_Vehicle_ExtendedFeatureType.VEHICLE_WHEEL_HUB then
    tableData = self.car_wheel_hub
  elseif extendedFeatureType == ENUM_Vehicle_ExtendedFeatureType.VEHICLE_CANOPY then
    tableData = self.car_sunroof
  end
  if not tableData then
    log(bWriteLog and "LogicVehicleExtendedFeature:CheckHasNewItemReddot tableData is nil")
    return false
  end
  if vehicleID and (extendedFeatureType == ENUM_Vehicle_ExtendedFeatureType.VEHICLE_WHEEL_HUB or extendedFeatureType == ENUM_Vehicle_ExtendedFeatureType.VEHICLE_BRAKE_CALIPER or extendedFeatureType == ENUM_Vehicle_ExtendedFeatureType.VEHICLE_CANOPY) then
    local slotType
    if extendedFeatureType == ENUM_Vehicle_ExtendedFeatureType.VEHICLE_WHEEL_HUB then
      slotType = EVehiclePartSlot.WheelHub
    elseif extendedFeatureType == ENUM_Vehicle_ExtendedFeatureType.VEHICLE_BRAKE_CALIPER then
      slotType = EVehiclePartSlot.BrakeCaliper
    else
      slotType = EVehiclePartSlot.Canopy
    end
    local vehicleData = self.vehicleSupportParts and self.vehicleSupportParts[vehicleID]
    local partIDs = vehicleData and vehicleData[slotType]
    if not partIDs then
      log(bWriteLog and "LogicVehicleExtendedFeature:CheckHasNewItemReddot vehicleID:" .. tostring(vehicleID) .. " slotType:" .. tostring(slotType) .. " no partIDs")
      return false
    end
    for _, partID in ipairs(partIDs) do
      if tableData[partID] == 1 then
        log(bWriteLog and "LogicVehicleExtendedFeature:CheckHasNewItemReddot vehicle-filtered has new data, partID:" .. tostring(partID))
        return true
      end
    end
    log(bWriteLog and "LogicVehicleExtendedFeature:CheckHasNewItemReddot vehicle-filtered false, vehicleID:" .. tostring(vehicleID))
    return false
  end
  if extendedFeatureType == ENUM_Vehicle_ExtendedFeatureType.VEHICLE_SWITCH_EFFECT then
    local VehicleSwitchEffectCfg = CDataTable.GetTable("VehicleSwitchEffectCfg")
    if not VehicleSwitchEffectCfg then
      log(bWriteLog and "LogicVehicleExtendedFeature:CheckHasNewItemReddot VehicleSwitchEffectCfg is nil")
      return false
    end
    for _, cfg in pairs(VehicleSwitchEffectCfg) do
      if cfg.ItemID and tableData[cfg.ItemID] == 1 then
        log(bWriteLog and "LogicVehicleExtendedFeature:CheckHasNewItemReddot switch-effect cfg-filtered has new data, itemID:" .. tostring(cfg.ItemID))
        return true
      end
    end
    log(bWriteLog and "LogicVehicleExtendedFeature:CheckHasNewItemReddot switch-effect cfg-filtered false")
    return false
  end
...
function LogicVehicleExtendedFeature:CheckHasNewItemReddot_AllType()
  ...
  bHasReddot = bHasReddot or self:CheckHasNewItemReddot(ENUM_Vehicle_ExtendedFeatureType.VEHICLE_BRAKE_CALIPER)
  bHasReddot = bHasReddot or self:CheckHasNewItemReddot(ENUM_Vehicle_ExtendedFeatureType.VEHICLE_WHEEL_HUB)
  bHasReddot = bHasReddot or self:CheckHasNewItemReddot(ENUM_Vehicle_ExtendedFeatureType.VEHICLE_CANOPY)
  return bHasReddot
end
...
function LogicVehicleExtendedFeature:ClearItemReddotData(resid)
  ...
  elseif itemSubType == ENUM_ITEM_SUBTYPE.VehicleBrakeCaliper then
    dataTable = self.car_brake_caliper
  elseif itemSubType == ENUM_ITEM_SUBTYPE.VehicleWheelHub then
    dataTable = self.car_wheel_hub
  elseif itemSubType == ENUM_ITEM_SUBTYPE.VehicleCanopy then
    dataTable = self.car_sunroof
  end
...
function LogicVehicleExtendedFeature:InitServerData(feature_data, inherit_feature_data)
  ...
  self.car_brake_caliper = feature_data.car_brake_caliper
  self.equip_brake_caliper = feature_data.equip_brake_caliper
  self.car_wheel_hub = feature_data.car_wheel_hub
  self.equip_wheel_hub = feature_data.equip_wheel_hub
  self.car_sunroof = feature_data.car_sunroof
  self.equip_sunroof = feature_data.equip_sunroof
  self.end
...
function LogicVehicleExtendedFeature:GetSupportVehicleIDListForPart(partId)
  if not partId then
    return nil
  end
  return self.partFitVehicleIDs and self.partFitVehicleIDs[partId]
end
