...
function logic_promotion_homepage:GetCurPromotionMinSegmentLevel()
  local cur_promotion_config = self:GetCurPromotionBaseConfig()
  local SeasonSystem = require("client.logic.season.logic_season")
  if cur_promotion_config then
    log_tree(bWriteLog and "logic_promotion_homepage:GetCurPromotionMinSegmentLevel cur_promotion_config: ", cur_promotion_config)
    return SeasonSystem.CurLevel(cur_promotion_config.cross_rating_lower, DataMgr.season_id), SeasonSystem.CurLevel(cur_promotion_config.cross_rating_upper, DataMgr.season_id)
  end
  return nil
end
...
function logic_promotion_homepage:ShowHelpGuide()
  ...
  local bShowPromoEntry = false
  if cur_promotion_data and not cur_promotion_data.all_tier_unlocked and promotion_match_util.IsPromotionLookbackSeasonValid() then
    local cur_lock_index = cur_promotion_data.cur_lock_index
    local locked_info = cur_promotion_data.locked_info and cur_promotion_data.locked_info[cur_lock_index]
    if locked_info and locked_info.status >= 2 and locked_info.status <= 4 then
      bShowPromoEntry = true
    end
  end
  UIManager.ShowUI(UIManager.UI_Config.Lobby_Season_Guide_UIBP, promotionGuideItems, bShowPromoEntry)
end
