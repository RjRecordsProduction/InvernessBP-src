...
function logic_season_segment_target:GetTargetScoreDif()
  local targetSegmentId, targetScore = self:GetCurTargetSegmentIdAndRating()
  local _, curRating = self:GetCurMaxSegmentAndRating()
  local ratingDiff = 0
  local hasTarget = false
  if curRating and targetScore then
    local ratingDiff = targetScore - curRating
    if ratingDiff < 0 then
      ratingDiff = 0
    end
    hasTarget = true
  end
  return ratingDiff, hasTarget
end
