...
function UI_Loading:OnPostInitialize()
  ...
  local promotionLayer = logic_promotion:GetLoadingPromotionLayer()
  log(bWriteLog and "UI_Loading:OnPostInitialize promotionLayer = " .. tostring(promotionLayer))
  local ValidPromotionLayers = {
    [1] = true,
    [2] = true,
    [3] = true
  }
  if promotionLayer and ValidPromotionLayers[promotionLayer] and self.UIRoot.WidgetSwitcher_1 then
  ...
  NPercent = 0
  self.isShow450 = LoadingSystem.GetShow450Naruto()
  ...
  self:RefreshPromotionInfo(promotionLayer)
  self:RefreshSeasonYearBadgeInfo()
end
function UI_Loading:RefreshPromotionInfo(promotionLayer)
  log(bWriteLog and "UI_Loading:RefreshPromotionInfo promotionLayer = " .. tostring(promotionLayer))
  local ValidPromotionLayers = {
    [1] = true,
    [2] = true,
    [3] = true
  }
  if promotionLayer and ValidPromotionLayers[promotionLayer] then
    self:CreateChildWindow("CanvasPanel_PromotionMatch", UIManager.UI_Config.Login_PromotionMatch_UIBP, promotionLayer)
  end
end
function UI_Loading:RefreshSeasonYearBadgeInfo()
  local logic_season_year_badge = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.logic_season_year_badge)
  if not logic_season_year_badge then
    log(bWriteLog and "UI_Loading:RefreshSeasonYearBadgeInfo logic_season_year_badge is nil")
    self:SetWidgetVisible(self.UIRoot.CanvasPanel_Season, false)
    self:SetWidgetVisible(self.UIRoot.CanvasPanel_Helmet, true)
    return
  end
  local show_scene_info = logic_season_year_badge.show_scene_info
  local season_year_config = require("client.logic.season_year.config.season_year_config")
  if show_scene_info and show_scene_info[season_year_config.EShowSceneType.battle_load] and show_scene_info[season_year_config.EShowSceneType.battle_load].show == season_year_config.EBadgeShowType.Show then
    self:SetWidgetVisible(self.UIRoot.CanvasPanel_Season, true)
    self:SetWidgetVisible(self.UIRoot.CanvasPanel_Helmet, false)
    if self.CompBadge01 == nil then
      local season_year_util = require("client.logic.season_year.util.season_year_util")
      local logic_season_year_badge = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.logic_season_year_badge)
      local show_year_id = show_scene_info[season_year_config.EShowSceneType.battle_load].year_id
      local badge_data = logic_season_year_badge:GetBadgeInfoBySeasonYearID(show_year_id)
      local season_year_badge_util = require("client.logic.season_year.util.season_year_badge_util")
      local actionLevel = season_year_badge_util.GetBadgeShowLevel(badge_data)
      self.CompBadge01 = season_year_util.CreateSeasonBadgeToWidget(self, self.UIRoot.SizeBox_Badge, show_year_id, badge_data, {loadingAniLevel = actionLevel})
    end
    if self.CompBadge01 == nil then
      self:SetWidgetVisible(self.UIRoot.CanvasPanel_Season, false)
      self:SetWidgetVisible(self.UIRoot.CanvasPanel_Helmet, true)
...
function UI_Loading:RefreshLoadingGuide(main_mode, sub_mode)
  ...
  for _, config in ipairs(guideConfig) do
    if config.showFunc(main_mode, sub_mode) and not self.guideUIBP then
      ...
      if config.guideType == loading_attach_ui_config.EAttachUIType.UGC_Edit or config.guideType == loading_attach_ui_config.EAttachUIType.UGC_Multi then
...
function UI_Loading:UpdatePercent(percent)
  ...
  if percent > NPercent then
    ...
    if self.UIRoot.ProgressBar_ThemeLoading then
      self.UIRoot.ProgressBar_ThemeLoading:SetPercent(NPercent)
    end
    self:RefreshPercentText()
    self:SetWidgetVisible(self.UIRoot.Image_Man, self.isShow450)
    local WidgetLayoutLibrary = import("WidgetLayoutLibrary")
    local uObj_slot0 = WidgetLayoutLibrary.SlotAsHorizontalBoxSlot(self.UIRoot.Spacer_0)
    local HorizontalSize0 = slua.IndexReference(uObj_slot0, "Size")
    HorizontalSize0.Value = percent
    uObj_slot0:SetSize(HorizontalSize0)
    local rightPercent = 1 - percent
    local uObj_slot1 = WidgetLayoutLibrary.SlotAsHorizontalBoxSlot(self.UIRoot.Spacer_1)
    local HorizontalSize1 = slua.IndexReference(uObj_slot1, "Size")
    HorizontalSize1.Value = rightPercent
    uObj_slot1:SetSize(HorizontalSize1)
  ...
  if 1 <= NPercent then
    ...
    self:RefreshPercentText()
    self:SetWidgetVisible(self.UIRoot.Image_Man, false)
