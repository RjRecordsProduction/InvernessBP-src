...
function UpgradeEffectUtil.GetOwnerActor()
...
function UpgradeEffectUtil.EnsureOwnerActor(weaponID)
  if not weaponID or weaponID == 0 then
    return nil
  end
  local ModelDisplayer = require("client.logic.avatar.ModelDisplayer")
  ModelDisplayer.Display(weaponID, true, {weaponPendantType = 3})
  ModelDisplayer.SwitchToModel()
  return UpgradeEffectUtil.GetOwnerActor()
end
function UpgradeEffectUtil.ApplyDualFormEquipment(info, lastPutOn)
  local actor = UpgradeEffectUtil.GetOwnerActor()
  if not actor then
    return
  end
  if lastPutOn then
    actor:PutoffEquipmentByResid(lastPutOn)
  end
  for _, attachMap in pairs(info) do
    for _, resId in pairs(attachMap) do
      actor:PutonEquipmentByResid(resId)
    end
  end
end
function UpgradeEffectUtil.TriggerDualFormShowModel(actor, weaponID_A, bpid_A, weaponID_B, bpid_B, extraFields)
  local LobbyShowActorConfig = require("client.slua.logic.show_actor.common.LobbyShowActorConfig")
  actor.ExtraTable = actor.ExtraTable or {}
  actor.ExtraTable.SlaveItemID = weaponID_B
  actor.ExtraTable.SlaveBPID = bpid_B
  if extraFields then
    for k, v in pairs(extraFields) do
      actor.ExtraTable[k] = v
    end
  end
  actor.ShowType = LobbyShowActorConfig.Type.DualForm
  actor:TryShowModel(weaponID_A, bpid_A)
end
function UpgradeEffectUtil.RestoreActorFromDualForm(extraKeys)
  local ModelDisplayer = require("client.logic.avatar.ModelDisplayer")
  ModelDisplayer.Hide()
  local actor = UpgradeEffectUtil.GetOwnerActor()
  if not actor then
  ...
  if actor.ExtraTable and extraKeys then
    for _, key in ipairs(extraKeys) do
      actor.ExtraTable[key] = nil
    end
  end
  local LobbyShowActorConfig = require("client.slua.logic.show_actor.common.LobbyShowActorConfig")
  if actor.ShowType == LobbyShowActorConfig.Type.DualForm then
    actor.ShowType = LobbyShowActorConfig.Type.Weapon
  end
end
