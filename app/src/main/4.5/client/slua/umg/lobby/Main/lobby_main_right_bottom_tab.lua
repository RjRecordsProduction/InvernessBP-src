...
function lobby_main_right_bottom_tab:ctor()
  ...
  self.newbieGuideSeasonUI = nil
  self.newbieGuideMissionUI = nil
  self.bubbleWorkshopUI = nil
end
...
function lobby_main_right_bottom_tab:RegistEvents()
  ...
  self:AddCommonEvent(EVENTTYPE_LEVEL_UNLOCK, EVENTID_ON_LEVEL_UNLOCK_START, self.OnLevelUnLockGuideStart, self)
  self:AddCommonEvent(EVENTTYPE_NINJA_TRAINING, EVENTID_NINJA_TRAINING_LEVEL_UP_PENDING, self.RefreshNarutoLevelUpIcon, self)
  self:AddCommonEvent(EVENTTYPE_NINJA_TRAINING, EVENTID_NINJA_TRAINING_LEVEL_DATA_RSP, self.RefreshThemeEntryVersionSwitch, self)
  self:AddCommonEvent(EVENTTYPE_NINJA_TRAINING, EVENTID_NINJA_TRAINING_CREDIT_UPDATE, self.RefreshThemeEntryVersionSwitch, self)
end
...
function lobby_main_right_bottom_tab:CheckForHideFeature()
  ...
  if not LogicPufferBundle.IsFitLobbyResDownloaded() then
    self.UIRoot.SizeBox_Workshop:SetWidgetVisibility(UEnums.ESlateVisibility.Collapsed)
...
function lobby_main_right_bottom_tab:GetRedPointType()
  ...
  if not tempRedPoint then
    ...
    for k, v in pairs(moduleList) do
      local tempRedPoint2 = self:SetSystemAdditionalRedPoint(v)
      if tempRedPoint2 and logic_lobby_system_extension:CheckInSystemInfoByModuleID(v) then
        log(bWriteLog and "lobby_main_right_bottom_tab:GetRedPointType moduleList " .. ":" .. tostring(v) .. ":" .. tostring(k))
  ...
  if not tempRedPoint then
    ...
    if bInBottem == false then
      ...
      if tempRedPoint then
        log(bWriteLog and "lobby_main_right_bottom_tab:GetRedPointType setting  is red")
        redpointType = 2
      end
    end
  end
  ...
  log(bWriteLog and "lobby_main_right_bottom_tab:GetRedPointType" .. ":" .. tostring(tempRedPoint) .. ":" .. tostring(redpointType))
...
function lobby_main_right_bottom_tab:UpdateWeakGuide()
  log(bWriteLog and "lobby_main_right_bottom_tab:UpdateWeakGuide")
  local growthprojectMgrB = require("client.slua.logic.growth_project.logic_growth_project_b")
  local seasonTextResId
  self.bShowPromotionGuide = false
  self:SetWidgetVisible(self.UIRoot.NewbieGuide_Season02, false, false)
  if growthprojectMgrB.IsWeakGuideSeasonStep1() then
    seasonTextResId = 12763
  else
    local PlayerPrefsSystem = require("client.logic.LogicPlayerPrefs.playerprefs")
    local tLocalCache = PlayerPrefsSystem.LoadFileToTable_N(PlayerPrefsSystem.ePlayerPrefsType.eGuidePromotion) or {}
    log_tree(bWriteLog and "lobby_main_right_bottom_tab:UpdateWeakGuide tLocalCache = ", tLocalCache)
    local TimeUtil = require("client.common.time_util")
    local time = TimeUtil.GetServerTimeInSec()
    local nCurSeasonId = DataMgr.season_id or 0
    log(bWriteLog and "lobby_main_right_bottom_tab:UpdateWeakGuide nCurSeasonId = " .. tostring(nCurSeasonId) .. " tLocalCache.nSeasonId = " .. tostring(tLocalCache.nSeasonId))
    local promotion_match_util = require("client.logic.season.promotion_match.promotion_match_util")
    if promotion_match_util.IsPromotionLookbackSeasonValid() and tLocalCache and not tLocalCache.bClick and tLocalCache.nUnlockTime and tLocalCache.nSeasonId == nCurSeasonId and time >= tLocalCache.nUnlockTime and time - tLocalCache.nUnlockTime < 86400 then
      local sType = tLocalCache.sType
      if sType == "SoftLimit" then
        seasonTextResId = 85488
      elseif sType == "ResultPopup" then
        seasonTextResId = 85489
      else
        seasonTextResId = 85487
      end
      self.bShowPromotionGuide = true
      self:SetWidgetVisible(self.UIRoot.NewbieGuide_Season02, true, false)
      self.UIRoot.TextBlock_1:SetText(LocUtil.GetLocalizeResStr(seasonTextResId))
    end
  end
  if seasonTextResId and not self.bShowPromotionGuide then
    self:_OpenNewbieGuideSeasonUI(seasonTextResId)
  else
    self:_CloseNewbieGuideSeasonUI()
  end
  if growthprojectMgrB.IsWeakGuideTaskStep1() then
    self:_OpenNewbieGuideMissionUI(12757)
  else
    self:_CloseNewbieGuideMissionUI()
  end
  if growthprojectMgrB.IsWeakGuideDeleteResidEntentry() then
    self:CreateChildWindow(self.UIRoot.CanvasPanel_0, UIManager.UI_Config.Lobby_Mid_Newbie_Tab_Close_UIBP)
...
function lobby_main_right_bottom_tab:_OpenNewbieGuideCommonUI(existingUI, parentWidget, textResId)
  log(bWriteLog and "lobby_main_right_bottom_tab:_OpenNewbieGuideCommonUI textResId = " .. tostring(textResId))
  if existingUI then
    if existingUI.OnRefreshTextResId then
      existingUI:OnRefreshTextResId(textResId)
    end
    return existingUI
  end
  if not parentWidget then
    log_warning(bWriteLog and "lobby_main_right_bottom_tab:_OpenNewbieGuideCommonUI no parent widget")
    return nil
  end
  return self:CreateChildWindow(parentWidget, UIManager.UI_Config.Lobby_Tab_Tip_NewbieGuide_Common, textResId)
end
function lobby_main_right_bottom_tab:_OpenNewbieGuideSeasonUI(textResId)
  self.newbieGuideSeasonUI = self:_OpenNewbieGuideCommonUI(self.newbieGuideSeasonUI, self.UIRoot.NewbieGuide_Season, textResId)
end
function lobby_main_right_bottom_tab:_CloseNewbieGuideSeasonUI()
  if self.newbieGuideSeasonUI then
    self.newbieGuideSeasonUI:Close()
    self.newbieGuideSeasonUI = nil
  end
end
function lobby_main_right_bottom_tab:_OpenNewbieGuideMissionUI(textResId)
  self.newbieGuideMissionUI = self:_OpenNewbieGuideCommonUI(self.newbieGuideMissionUI, self.UIRoot.SizeBox_NewbieGuide, textResId)
end
function lobby_main_right_bottom_tab:_CloseNewbieGuideMissionUI()
  if self.newbieGuideMissionUI then
    self.newbieGuideMissionUI:Close()
    self.newbieGuideMissionUI = nil
  end
end
function lobby_main_right_bottom_tab:OnLevelUnLockGuideStart()
  ...
  self:SetWidgetVisible(self.UIRoot.SizeBox_Missions, true)
  self:CheckForHideFeature()
end
...
function lobby_main_right_bottom_tab:OnButton_SeasonClick()
  log(bWriteLog and "lobby_main_right_bottom_tab:OnButton_SeasonClick bShowPromotionGuide = " .. tostring(self.bShowPromotionGuide))
  if self.bShowPromotionGuide then
    self.bShowPromotionGuide = false
    local PlayerPrefsSystem = require("client.logic.LogicPlayerPrefs.playerprefs")
    local tLocalCache = PlayerPrefsSystem.LoadFileToTable_N(PlayerPrefsSystem.ePlayerPrefsType.eGuidePromotion) or {}
    log_tree(bWriteLog and "lobby_main_right_bottom_tab:OnButton_SeasonClick tLocalCache1 = ", tLocalCache)
    tLocalCache.bClick = true
    tLocalCache.nSeasonId = DataMgr.season_id or 0
    log_tree(bWriteLog and "lobby_main_right_bottom_tab:OnButton_SeasonClick tLocalCache2 = ", tLocalCache)
    PlayerPrefsSystem.SaveTableToFile_N(tLocalCache, PlayerPrefsSystem.ePlayerPrefsType.eGuidePromotion)
    self:SetWidgetVisible(self.UIRoot.NewbieGuide_Season02, false, false)
    local logic_promotion = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.logic_promotion)
    logic_promotion:SetbAutoShowPromotionGuide(true)
  ...
  if SeasonVerCfg and 0 > version_util.CompareVersionStandard(_clientVersion3, SeasonVerCfg.MinVersion) then
    ShowNotice(9409)
    return
  end
...
function lobby_main_right_bottom_tab:UpdateLobbyLabEntranceRedPoint()
  ...
  if bannerData and not labEntrance then
    ...
    if videoCfg then
      self:_OpenBubbleWorkshopUI(videoCfg.icon)
    else
      print(bWriteLog and "lobby_main_right_bottom_tab:UpdateLobbyLabEntranceRedPoint videoCfg is nil")
      self:_CloseBubbleWorkshopUI()
    end
  else
    self:_CloseBubbleWorkshopUI()
  end
...
function lobby_main_right_bottom_tab:_OpenBubbleWorkshopUI(iconPath)
  if self.bubbleWorkshopUI then
    self.bubbleWorkshopUI:SetIconPath(iconPath)
    return
  end
  if not self.UIRoot.SizeBox_Bubble_Workshop then
    log_warning(bWriteLog and "lobby_main_right_bottom_tab:_OpenBubbleWorkshopUI no parent SizeBox_Bubble_Workshop")
    return
  end
  self.bubbleWorkshopUI = self:CreateChildWindow(self.UIRoot.SizeBox_Bubble_Workshop, UIManager.UI_Config.Lobby_Main_Bubble_Workshop, iconPath)
end
function lobby_main_right_bottom_tab:_CloseBubbleWorkshopUI()
  if self.bubbleWorkshopUI then
    self.bubbleWorkshopUI:Close()
    self.bubbleWorkshopUI = nil
  end
end
...
function lobby_main_right_bottom_tab:RefreshThemeEntrance()
  ...
  if Client.IsCEVersion() then
    self:SetWidgetVisible(self.UIRoot.SizeBox_Theme, false)
    return
  end
  if entryConfig then
    ...
    self:RefreshThemeEntryVersionSwitch()
    self:_ApplyThemeEntryConfig(entryConfig)
...
function lobby_main_right_bottom_tab:RefreshThemeEntryVersionSwitch()
  local logic_ninja_training = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.logic_ninja_training)
  local isNarutoOpen = logic_ninja_training:IsNarutoActivityOpen()
  self:SetWidgetVisible(self.UIRoot.CanvasPanel_Version1, not isNarutoOpen)
  self:SetWidgetVisible(self.UIRoot.CanvasPanel_Version2, isNarutoOpen)
  self.UIRoot.TextBlock_2:SetText(LocUtil.GetLocalizeResStr(77491))
  local levelIcon = logic_ninja_training:GetCurrentLevelIcon()
  if levelIcon ~= "" then
    self:SetWidgetVisible(self.UIRoot.Theme_EntranceIcon_Version2, true)
    self:SetTexture(self.UIRoot.Theme_EntranceIcon_Version2, levelIcon)
  else
    self:SetWidgetVisible(self.UIRoot.Theme_EntranceIcon_Version2, false)
  end
end
function lobby_main_right_bottom_tab:_ApplyThemeEntryConfig(entryConfig)
  if not entryConfig then
    log(bWriteLog and "lobby_main_right_bottom_tab:_ApplyThemeEntryConfig entryConfig is nil")
    return
  end
  ...
  if not urlPath or urlPath == "" then
    log(bWriteLog and "lobby_main_right_bottom_tab:_ApplyThemeEntryConfig urlPath is empty")
    return
  end
...
function lobby_main_right_bottom_tab:OnButton_ThemeClick()
  self:PlayAudio(sound_config.click_v1)
  log(bWriteLog and "lobby_main_right_bottom_tab:OnButton_ThemeClick")
...
function lobby_main_right_bottom_tab:RefreshThemeExchangeReddotUI()
  ...
  local theme_system_reddot = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.theme_system_reddot)
  local ThemeConfig = require("client.slua.logic.theme_system.theme_config")
  self:_ApplyThemeEntryConfig(ThemeConfig.GetEntryBannerConfig())
  ...
  elseif theme_system_reddot:HasNextVersionPreheatRedDot() then
    ...
    self:_ApplyThemeEntryConfig(ThemeConfig.GetNextVersionEntryBannerConfig())
  end
end
...
function lobby_main_right_bottom_tab:RefreshNarutoLevelUpIcon()
  if not self.UIRoot.Image_LevelUp then
    return
  end
  local logic_ninja_training = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.logic_ninja_training)
  self:SetWidgetVisible(self.UIRoot.Image_LevelUp, logic_ninja_training:HasPendingLevelUp())
end
function lobby_main_right_bottom_tab:SetCardCollectionButton()
  ...
  if self.UIRoot.CanvasPanel_Download then
    common_download_handler.CreateDownloadUI(PufferConst.ENUM_DownloadType.ODPAK, {
      CardCollectionUIConfig.ModAssetPath,
      80724
    }, self.UIRoot.CanvasPanel_Download, {hideMask = true})
  end
end
function lobby_main_right_bottom_tab:BindCardCollectionRedDot()
  ModuleManager.GetSplitModuleDownload(ModuleManager.LobbyModuleConfig.card_collection_reddot_data, function(card_collection_reddot_data)
    if not card_collection_reddot_data then
      return
    end
