...
function lobby_bottom_right_uibp:ShowCardCollectionAction()
  ...
  if not playerDisplayModule then
    return
  end
  self:AddTimerOnce(1, function()
    if logic_card_collection and logic_card_collection:CheckToLobbyFlag() then
      self:ShowPetAndPlayerExpressionPanel(playerDisplayModule:GetActionItemID())
    end
  end)
end
