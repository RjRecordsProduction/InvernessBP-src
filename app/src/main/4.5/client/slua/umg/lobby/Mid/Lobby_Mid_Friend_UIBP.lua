...
function Lobby_Mid_Friend_UIBP:ctor()
  self.teamquick_guide_timer = nil
  self.teamquick_loop_List = nil
end
function Lobby_Mid_Friend_UIBP:OnInitialize()
  ...
  if UIComponentModule then
    self.Avatar01 = UIComponentModule:InitWithParentComponent(self, UIComponentModule.Config.Common_Avatar_Round_UIBP, self.UIRoot.Avatar01)
    self.Avatar02 = UIComponentModule:InitWithParentComponent(self, UIComponentModule.Config.Common_Avatar_Round_UIBP, self.UIRoot.Avatar02)
    self.Avatar03 = UIComponentModule:InitWithParentComponent(self, UIComponentModule.Config.Common_Avatar_Round_UIBP, self.UIRoot.Avatar03)
  end
end
function Lobby_Mid_Friend_UIBP:RegistEvents()
  ...
  self:AddCommonEvent(EVENTTYPE_FLASH_TEAM, EVENTID_FLASH_TEAM_DATA_CHG, self.RefreshFlashTeamShow, self)
...
function Lobby_Mid_Friend_UIBP:OnClose()
  if self.ScrollBox then
    self.ScrollBox:SetItemCount(0)
    self.ScrollBox = nil
  end
  if self.teamquick_guide_timer then
    self:RemoveTimer(self.teamquick_guide_timer)
    self.teamquick_guide_timer = nil
  end
  Lobby_Mid_Friend_UIBP.__super.OnClose(self)
end
...
function Lobby_Mid_Friend_UIBP:_implOnFriendChange()
  ...
  local IsShowHint = SettingModule:GetOptionValue("DoubleIntimacyHint")
  if IsShowHint then
...
function Lobby_Mid_Friend_UIBP:RefreshFlashTeamShow()
  ...
  if myPreTeamInfo then
    self.UIRoot.WidgetSwitcher_0:SetActiveWidgetIndex(2)
    self.UIRoot.Text_Value2:SetText(("%d/%d"):format(myPreTeamInfo.member_count, myPreTeamInfo.max_member))
  else
    self.UIRoot.WidgetSwitcher_0:SetActiveWidgetIndex(0)
...
function Lobby_Mid_Friend_UIBP:InitEntryTips()
  ...
  local UIComponentModule = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.UIComponentModule)
  if not self.Lobby_Mid_Squad_Tips_UIBP then
  ...
  self:OpenTeamQuickEntryAnimTimer()
end
...
function Lobby_Mid_Friend_UIBP:PlayTeamQuickEntryAnim()
  log(bWriteLog and "Lobby_Mid_Friend_UIBP:PlayTeamQuickEntryAnim")
  self:RemoveOnAnimationFinishedEvent("Anim_Loop02")
  if not self.UIRoot or not slua.isValid(self.UIRoot) then
    return
  end
  if not self.Avatar01 then
    log(bWriteLog and "Lobby_Mid_Friend_UIBP:PlayTeamQuickEntryAnim Avatar01 is nil")
    return
  end
  if not self.teamquick_loop_List or #self.teamquick_loop_List == 0 then
    log(bWriteLog and "Lobby_Mid_Friend_UIBP:PlayTeamQuickEntryAnim teamquick_loop_List is empty")
    return
  end
  if self.Lobby_Mid_Squad_Tips_UIBP and self.Lobby_Mid_Squad_Tips_UIBP.recommendIsShowing then
    log(bWriteLog and "Lobby_Mid_Friend_UIBP:PlayTeamQuickEntryAnim Lobby_Mid_Squad_Tips_UIBP is showing")
    return
  end
  if UIManager.IsUIShow(UIManager.UI_Config.level_unlock_bubble) then
    log(bWriteLog and "Lobby_Mid_Friend_UIBP:PlayTeamQuickEntryAnim level_unlock_bubble is showing")
    return
  end
  local index = math.random(1, #self.teamquick_loop_List)
  local mod = #self.teamquick_loop_List
  local play = function()
    if not self.UIRoot or not slua.isValid(self.UIRoot) then
      return
    end
    index = (index + 3) % mod
    for i = 1, 3 do
      if self["Avatar0" .. i] then
        if i <= #self.teamquick_loop_List then
          local value = self.teamquick_loop_List[(index + i) % mod + 1]
          local uid = value.uid
          local url = value.url
          local avatar_box = value.avatar_box
          self["Avatar0" .. i]:SetData(uid, url, avatar_box)
          self:SetWidgetVisible(self.UIRoot["Avatar0" .. i], true)
        else
          self:SetWidgetVisible(self.UIRoot["Avatar0" .. i], false)
        end
      end
    end
    self:PlayUserWidgetAnimation(self.UIRoot.Anim_Loop02, 0, 1, 0, 1)
  end
  local count = 0
  ...
  self:AddOnAnimationFinishedEvent("Anim_Loop02", finished)
  ...
  local check = function()
    local bBan = false
    if self.Lobby_Mid_Squad_Tips_UIBP and self.Lobby_Mid_Squad_Tips_UIBP.recommendIsShowing then
      log(bWriteLog and "Lobby_Mid_Friend_UIBP:PlayTeamQuickEntryAnim check Lobby_Mid_Squad_Tips_UIBP showing")
      bBan = true
    end
    if UIManager.IsUIShow(UIManager.UI_Config.level_unlock_bubble) then
      log(bWriteLog and "Lobby_Mid_Friend_UIBP:PlayTeamQuickEntryAnim check level_unlock_bubble is showing")
      bBan = true
    end
    if bBan then
      self:RemoveOnAnimationFinishedEvent("Anim_Loop02")
      self:RefreshFlashTeamShow()
      if check_timer then
        self:RemoveTimer(check_timer)
        check_timer = nil
      end
    end
  end
  check_timer = self:AddTimerLoop(0, check, 16, 0.5)
end
function Lobby_Mid_Friend_UIBP:OpenTeamQuickEntryAnimTimer()
  log(bWriteLog and "Lobby_Mid_Friend_UIBP:OpenTeamQuickEntryAnimTimer")
  self:SetWidgetVisible(self.UIRoot.CanvasPanel_TeamQucikTips, false)
  self:AddTimerOnce(15, function()
    if not self.UIRoot or not slua.isValid(self.UIRoot) then
      return
    end
    if self.teamquick_guide_timer then
    ...
    if not logic_teamquick_guide:CheckNeedShowLoopGuide() then
      log(bWriteLog and "Lobby_Mid_Friend_UIBP:OpenTeamQuickEntryAnimTimer has guide")
      return
    end
    logic_teamquick_guide.hasShownLoginLoopGuide = true
    local loop_anim = function()
      log(bWriteLog and "Lobby_Mid_Friend_UIBP:OpenTeamQuickEntryAnimTimer loop_anim")
      local logic_flash_match_team = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.logic_flash_match_team)
      local TeamInfo = logic_flash_match_team:getOwnFlashTeamInfo()
      if not (TeamInfo and TeamInfo.squad_count) or TeamInfo.squad_count <= 0 then
        self.teamquick_loop_List = logic_teamquick_guide:GetGuideFriendIconList()
        if not self.teamquick_loop_List or #self.teamquick_loop_List < 4 then
          self.teamquick_loop_List = logic_teamquick_guide:GetGuideCustomIconList()
        end
      else
        self.teamquick_loop_List = logic_teamquick_guide:GetTeamMemberGuideIconList()
      end
      self:PlayTeamQuickEntryAnim()
    end
    self.teamquick_guide_timer = self:AddTimerLoop(0, loop_anim, 100, 60)
  end)
end
