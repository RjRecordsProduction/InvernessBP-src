...
function Lobby_Mid_Message_UIBP:RegistEvents()
  ...
  self:AddCommonEvent(EVENTTYPE_TEAMUP, EVENTID_TEAM_PLATFORM_RECRUIT_UPDATE, self.CheckTeamPlatformTips, self)
  ...
  self:AddCommonEvent(EVENTTYPE_LOBBY, EVENTID_LOBBY_MID_TEAM_PLATFORM_ENTRY_CLICKED, self.OnTeamPlatformEntryClicked, self)
...
function Lobby_Mid_Message_UIBP:CheckCrazyWeekendAct()
  ...
  log(bWriteLog and "Lobby_Mid_Message_UIBP:CheckCrazyWeekendAct actIsOpen=" .. tostring(actIsOpen))
  if actIsOpen then
    if not self.crazyWeekendEntryUI or not slua.isValid(self.crazyWeekendEntryUI.UIRoot) then
      self.crazyWeekendEntryUI = self:CreateChildWindow(self.UIRoot.SizeBox_CrazyWeekend, UIManager.UI_Config.Lobby_Mid_CrazyWeekend_Entry)
    end
  elseif self.crazyWeekendEntryUI then
    self.crazyWeekendEntryUI:Close()
    self.crazyWeekendEntryUI = nil
  end
  self:SetWidgetVisible(self.UIRoot.SizeBox_CrazyWeekend, actIsOpen)
end
...
function Lobby_Mid_Message_UIBP:OnClose()
  ...
  self.crazyWeekendEntryUI = nil
  self.teamPlatformEntryUI = nil
  self.Lobby_Mid_NewbieGuide_TeamPlatformEntry = nil
end
...
function Lobby_Mid_Message_UIBP:OnTeamPlatformEntryClicked()
  log(bWriteLog and "Lobby_Mid_Message_UIBP:OnTeamPlatformEntryClicked")
...
function Lobby_Mid_Message_UIBP:CheckTeamPlatformTips()
  log(bWriteLog and "Lobby_Mid_Message_UIBP:CheckTeamPlatformTips()")
  local TeamPlatformSystem = require("client.slua.logic.teamup.logic_team_platform")
  local TeamUpNewSystem = require("client.slua.logic.teamup.logic_team_up")
  if not TeamPlatformSystem.IsOpen() then
    log(bWriteLog and "Lobby_Mid_Message_UIBP:CheckTeamPlatformTips() TeamPlatformSystem.IsOpen() false")
  ...
  if not logic_access_restriction.CheckAccess(logic_access_restriction.EAccessType.TeamPlatform) then
    log(bWriteLog and "Lobby_Mid_Message_UIBP:CheckTeamPlatformTips() logic_access_restriction.CheckAccess(logic_access_restriction.EAccessType.TeamPlatform) false")
    return
  end
...
function Lobby_Mid_Message_UIBP:ShowWeakGuidePlatFormTips()
  log(bWriteLog and " Lobby_Mid_Message_UIBP:ShowWeakGuidePlatFormTips")
  local item = self.Lobby_Mid_NewbieGuide_TeamPlatformEntry
  if not (item and item.UIRoot) or not slua.isValid(item.UIRoot) then
    if self.closeWeakGuideTimer then
      self:RemoveTimer(self.closeWeakGuideTimer)
      self.closeWeakGuideTimer = nil
    end
    local path = "/Game/UMG/UI_BP/Lobby/Mid/Item/Lobby_Mid_NewbieGuide_TeamPlatformEntry.Lobby_Mid_NewbieGuide_TeamPlatformEntry"
    self:SetWidgetVisible(self.UIRoot.NewbieGuide_TeamPlatformEntry, true)
    self.Lobby_Mid_NewbieGuide_TeamPlatformEntry = self:CreateChildWindowWithBpPath(self.UIRoot.NewbieGuide_TeamPlatformEntry, nil, path)
    self.Lobby_Mid_NewbieGuide_TeamPlatformEntry.UIRoot.TextBlock_1:SetText(LocUtil.GetLocalizeResStr(12760))
    self.closeWeakGuideTimer = self:AddTimerOnce(self.teamPlatFormTipTime, function()
      log(bWriteLog and " Lobby_Mid_Message_UIBP:ShowWeakGuidePlatFormTips closeWeakGuideTimer")
      if self.Lobby_Mid_NewbieGuide_TeamPlatformEntry then
        self.Lobby_Mid_NewbieGuide_TeamPlatformEntry:Close()
        self.Lobby_Mid_NewbieGuide_TeamPlatformEntry = nil
...
function Lobby_Mid_Message_UIBP:CheckOpenTeamPlatform()
  log(bWriteLog and "Lobby_Mid_Message_UIBP:CheckOpenTeamPlatform")
  local TeamPlatformSystem = require("client.slua.logic.teamup.logic_team_platform")
  local bPlatformOpen = TeamPlatformSystem.IsOpen()
  local bShow = bPlatformOpen
  log(bWriteLog and "Lobby_Mid_Message_UIBP:CheckOpenTeamPlatform bPlatformOpen=" .. tostring(bPlatformOpen))
  if bShow then
    if not self.teamPlatformEntryUI or not slua.isValid(self.teamPlatformEntryUI.UIRoot) then
      self.teamPlatformEntryUI = self:CreateChildWindow(self.UIRoot.SizeBox_TeamPlatformEntry, UIManager.UI_Config.Lobby_TeamPlatform_Entry)
    end
  elseif self.teamPlatformEntryUI then
    self.teamPlatformEntryUI:Close()
    self.teamPlatformEntryUI = nil
  end
  self:SetWidgetVisible(self.UIRoot.SizeBox_TeamPlatformEntry, bShow)
end
...
function Lobby_Mid_Message_UIBP:ShowHomeEntry()
  ...
  if not logic_home_switch:CheckHomeSwitchOpen(nil, true) then
...
function Lobby_Mid_Message_UIBP:OnClickCollectEntrance()
  ...
  if not LogicPufferBundle.IsFitLobbyResDownloaded() then
    log(bWriteLog and "Lobby_Mid_Message_UIBP:OnClickCollectEntrance FitLobbyRes not downloaded, show fit popup")
