...
function Lobby_Segment_Protect_UIBP:RegistEvents()
  ...
  self:AddCommonEvent(EVENTTYPE_COME_BACK, EVENTID_PLAYER_RETURN_CLOSE, self.OnReturnActExpire, self)
end
...
function Lobby_Segment_Protect_UIBP:CheckGiftEntryShow(isManualClick, onlySetVisible)
  ...
  if UIManager.GetUI(UIManager.UI_Config.ReturnActivity_Tips_Child_UIBP) then
    log(bWriteLog and "Lobby_Segment_Protect_UIBP:CheckGiftEntryShow close existing Tips")
    UIManager.CloseUI(UIManager.UI_Config.ReturnActivity_Tips_Child_UIBP)
  end
  ...
  if isManualClick then
    ...
    self:AddTimerOnce(0.1, function()
      ...
      self:CreateChildWindow(parentUI.UIRoot.Canvas_FirstBattle_Newbie, UIManager.UI_Config.ReturnActivity_Tips_Child_UIBP, {
        anchorWidget = self.UIRoot.Button_Gift,
        isManualClick = false,
        anchorOffsetX = 5
      })
...
function Lobby_Segment_Protect_UIBP:OnReturnActExpire()
  log(bWriteLog and "Lobby_Segment_Protect_UIBP:OnReturnActExpire")
  self:SetWidgetVisible(self.UIRoot.Button_Gift, false, true)
end
...
function Lobby_Segment_Protect_UIBP:SetImageUpOrDown(b_up)
  if b_up then
    self.UIRoot.Image_DoubleIcon:SetRenderAngle(0)
  else
    self.UIRoot.Image_DoubleIcon:SetRenderAngle(180)
  end
end
