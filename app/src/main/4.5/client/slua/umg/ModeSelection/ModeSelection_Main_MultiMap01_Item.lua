...
function ModeSelection_Main_MultiMap01_Item:RefreshArenaLimitState()
  if not self.data then
    return
  end
  if self.data.id ~= 10068 and self.data.id ~= 10113 then
    return
  end
  local filterData = self.data
  for index, value in ipairs(self.selectViewIDs or {}) do
    if value == 10068 then
      local logic_mode_selection = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.logic_mode_selection)
      filterData = logic_mode_selection:GetSubviewInfoBySubviewID(value)
    end
  end
  self:SetLimitStateFilter(filterData, self.filterInfo)
end
