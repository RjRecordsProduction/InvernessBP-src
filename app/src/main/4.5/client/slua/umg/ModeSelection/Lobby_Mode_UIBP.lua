...
function Lobby_Mode_UIBP:ctor()
  ...
  self.specialPlayPandoraActID = 9001026
  self.alpharTimer = nil
end
function Lobby_Mode_UIBP:OnInitialize()
  ...
  self:SetWidgetVisible(self.UIRoot.Canvas_Panel_HandGuide, false)
end
function Lobby_Mode_UIBP:RegistEvents()
  ...
  self:AddCommonEvent(EVENTTYPE_TEAMUP, EVENTID_TEAMUP_NEW_TEAM_MATCH_MODE, self.UpdateInfoBar, self)
  ...
  self:AddCommonEvent(EVENTTYPE_MATCH, EVENTID_MATCH_UPDATE_LANGUAGE, self.UpdateMatchPreInfoBar, self)
  self:AddCommonEvent(EVENTTYPE_MATCH, EVENTID_ON_SAME_LANGUAGE_MATCH_TIMEOUT, self.OnSameLanguageMatchTimeOut, self)
  self:AddCommonEvent(EVENTTYPE_MATCH, EVENTID_ON_SAME_LANGUAGE_MATCH_TIMEOUT_RECOVER, self.OnSameLanguageMatchRecover, self)
  self:AddCommonEvent(EVENTTYPE_MATCH, EVENTID_MATCH_MODE_FILTER_CHANGE, self.UpdateUI, self)
  if self.UIRoot.CheckBox_SameLanguage then
    self:AddOnCheckStateChangedEventByControl(self.UIRoot.CheckBox_SameLanguage, self.OnClickedCheckBox, self)
  end
  if self.UIRoot.Button_Match then
    self:AddOnClickedEventByControl(self.UIRoot.Button_Match, self.OnButton_MatchClick, self)
  end
  if self.UIRoot.Button_MatchingTips then
    self:AddOnClickedEventByControl(self.UIRoot.Button_MatchingTips, self.OnClickedMatchingTips, self)
  end
  ...
  self:AddCommonEvent(EVENTTYPE_UGC, EVENTID_UGC_MATCH_ENTER_MOD, self.UpdateUI, self)
  ...
  self:AddCommonEvent(EVENTTYPE_UGC, EVENTID_UGC_CACHE_CLEAR, self.CheckAbnormalStatusShow, self)
  ...
  self:AddCommonEvent(EVENTTYPE_MATCH, EVENTID_MATCH_UPDATE_LANGUAGE_TEAM, self.UpdateMatchPreInfoBar, self)
  self:AddCommonEvent(EVENTTYPE_PANDORA, EVENTID_PANDORA_ALL_READY_BATCH_NOTIFY, self.ShowSpecialPlayPanel, self)
  self:AddCommonEvent(EVENTTYPE_DATA_MGR, EVENTID_PANDORA_UPDATE_RED_POINT, self.UpdateSpecialPlayPanelRedDot, self)
  ...
  self:AddOnClickedEventByControl(self.UIRoot.Button_1, self.OnButton_BackToCommunity, self)
  self:AddControlEventByControl(self.UIRoot.Anim_Refresh_02, "OnAnimationFinished", self.OnGetMatchAnimationEnd, self)
  self:AddCommonEvent(EVENTTYPE_PROMOTION, EVENTID_PROMOTION_SHOW, self.ShowSpecialPlayPanel, self)
end
...
function Lobby_Mode_UIBP:OnPostInitialize()
  ...
  match_pre_info_bar_helper.SetMatchPreInfoBarStaticText(self)
  local LanguageSelectSystem = require("client.logic.language_select.logic__language_select")
  LanguageSelectSystem.send_query_match_langs_req()
  self:UpdateMatchPreInfoBar()
...
function Lobby_Mode_UIBP:UpdateUIForUGC()
  ...
  if logic_community.bBackToCommunityTip then
    self.UIRoot.UTRichTextBlock_VideoStream:SetText(LocUtil.GetLocalizeResStr(468890126))
    self:SetWidgetVisible(self.UIRoot.CanvasPanel_VideoStream, true, false)
...
function Lobby_Mode_UIBP:UpdateUgcMulti()
  ...
  self:SetWidgetVisible(self.UIRoot.Image_13, true)
...
function Lobby_Mode_UIBP:UpdateUGCMultiResState()
  ...
  if state == PufferConst.ENUM_DownloadState.Done then
    ...
    elseif self.UIRoot.CanvasPanel_TeamStatus then
      self:_SetMatchTeamStatusIndex(true)
...
function Lobby_Mode_UIBP:OnButton_UGCModClick()
  ...
  if LobbySystem.isInMatch then
    ...
    if UGCPlayHallRoom and not UGCPlayHallRoom:GetRoomMatchInfo() then
      log(bWriteLog and "[edward] Lobby_Mode_UIBP:OnButton_UGCModClick, is matching!!!")
      ShowNotice(110014)
      return
    end
  end
  ...
  if bIsUGCOpen and not self.isMTModel then
    ...
    if LogicUGCMatch:HasUGCMatchInfo() then
      ...
      local logic_return_tlog_report = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.logic_return_tlog_report)
      logic_return_tlog_report:ReportReturnTLogEvent(TLogEventDefine.NewModeSelection_LobbyEntrance)
      self:HideLevelUnlockGuide()
      return
    end
    local LogicUGCMulti = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.LogicUGCMulti)
    ...
    LogicUGC:BatchGetModInfo({
      ...
      if LobbySystem.isInMatch then
        UIManager.ShowUI(UIManager.UI_Config.UGCDetailMainPanel, ugc_detail_config, Mod.pub_mod_meta, {
          buttontype = TLogEventDefine.UGC_MagnifyingGlass_Jump_TO_WorkDetail
        })
      else
...
function Lobby_Mode_UIBP:OnButton_EnterClick()
  ...
  if self.act then
    ...
    if logic_return_activity_first_battle:ShouldShowReturnModeSelect() then
      ...
      UIManager.ShowUI(UIManager.UI_Config.Return_ModeSelect_UIBP, promise, return_activity_macro.ReturnFrontTLog.OpenSource.ModeSelectPopup, ParamTable)
...
function Lobby_Mode_UIBP:OnUpdateMatchStatus()
  ...
  if status == ENUM_MatchStatus.Matching then
    if not self.bInMatchingState then
      self.bInMatchingState = true
    end
  else
    self.bInMatchingState = false
  end
  if status == ENUM_MatchStatus.Matching then
...
function Lobby_Mode_UIBP:UpdateMatchPreInfoBar()
  local helper = require("client.slua.umg.lobby.Main.Helper.match_pre_info_bar_helper")
  if not helper then
    log(bWriteLog and "[ERROR] Lobby_Mode_UIBP:UpdateMatchPreInfoBar - match_pre_info_bar_helper require failed")
    return
  end
  helper.UpdateCheckBox(self)
  helper.UpdateMatchPreInfoBar(self)
end
function Lobby_Mode_UIBP:OnSameLanguageMatchTimeOut()
  log(bWriteLog and "Lobby_Mode_UIBP:OnSameLanguageMatchTimeOut")
  local helper = require("client.slua.umg.lobby.Main.Helper.match_pre_info_bar_helper")
  if not helper then
    log(bWriteLog and "[ERROR] Lobby_Mode_UIBP:OnSameLanguageMatchTimeOut - match_pre_info_bar_helper require failed")
    return
  end
  helper.PlaySameLanguageTimeoutAnim(self)
  helper.UpdateMatchPreInfoBar(self)
end
function Lobby_Mode_UIBP:OnSameLanguageMatchRecover()
  log(bWriteLog and "Lobby_Mode_UIBP:OnSameLanguageMatchRecover")
  local helper = require("client.slua.umg.lobby.Main.Helper.match_pre_info_bar_helper")
  if not helper then
    log(bWriteLog and "[ERROR] Lobby_Mode_UIBP:OnSameLanguageMatchRecover - match_pre_info_bar_helper require failed")
    return
  end
  helper.PlaySameLanguageRecoverAnim(self)
  helper.UpdateMatchPreInfoBar(self)
end
function Lobby_Mode_UIBP:ApplyMatchToggleVisual(textWidget, checkBox, bOn)
  local helper = require("client.slua.umg.lobby.Main.Helper.match_pre_info_bar_helper")
  if not helper then
    log(bWriteLog and "[ERROR] Lobby_Mode_UIBP:ApplyMatchToggleVisual - match_pre_info_bar_helper require failed")
    return
  end
  helper.ApplyMatchToggleVisual(self, textWidget, checkBox, bOn)
end
function Lobby_Mode_UIBP:IsSameLanguageMatchOn()
  local helper = require("client.slua.umg.lobby.Main.Helper.match_pre_info_bar_helper")
  if not helper then
    log(bWriteLog and "[ERROR] Lobby_Mode_UIBP:IsSameLanguageMatchOn - IsSameLanguageMatchOn require failed")
    return
  end
  return helper.IsSameLanguageMatchOn()
end
function Lobby_Mode_UIBP:IsTeammateMatchOn()
  local helper = require("client.slua.umg.lobby.Main.Helper.match_pre_info_bar_helper")
  if not helper then
    log(bWriteLog and "[ERROR] Lobby_Mode_UIBP:IsTeammateMatchOn - IsTeammateMatchOn require failed")
    return
  end
  return helper.IsTeammateMatchOn()
end
function Lobby_Mode_UIBP:CloseSameLanguageMatch()
  local matchLang = DataMgr and DataMgr.MatchLanguage
  if not matchLang or not next(matchLang) then
    log(bWriteLog and "Lobby_Mode_UIBP:CloseSameLanguageMatch - MatchLanguage empty, skip")
    return
  end
  if not matchLang.only_match then
    log(bWriteLog and "Lobby_Mode_UIBP:CloseSameLanguageMatch - already off, skip")
    return
  end
  local LanguageSelectSystem = require("client.logic.language_select.logic__language_select")
  LanguageSelectSystem.MatchLanguageSelectReq(matchLang[1], matchLang[2], false, matchLang.lang_timeout)
end
function Lobby_Mode_UIBP:OnButton_MatchClick()
  self:PlayAudio(sound_config.click)
  log(bWriteLog and "Lobby_Mode_UIBP:OnButton_MatchClick - open Lobby_MatchSettingAndStatus_UIBP PageIndex=1")
  UIManager.ShowUI(UIManager.UI_Config.Lobby_MatchSettingAndStatus_UIBP, {PageIndex = 1})
end
function Lobby_Mode_UIBP:OnClickedCheckBox(isCheck)
  self:PlayAudio(sound_config.click)
  local helper = require("client.slua.umg.lobby.Main.Helper.match_pre_info_bar_helper")
  if not helper then
    log(bWriteLog and "Lobby_Mode_UIBP:OnClickedCheckBox helper not found")
    return
  end
  helper.OnClickedCheckBox(isCheck)
end
function Lobby_Mode_UIBP:OnClickedMatchingTips(isCheck)
  self:PlayAudio(sound_config.click)
  ShowNotice(525104)
end
function Lobby_Mode_UIBP:OnClose()
  ...
  if self.alpharTimer then
    self:RemoveTimer(self.alpharTimer)
    self.alpharTimer = nil
  end
end
...
function Lobby_Mode_UIBP:CheckSoloMapDownloadStatus()
  ...
  if bNotDownloaded then
    ...
    self:_SetMatchTeamStatusIndex(false)
    log(bWriteLog and "Lobby_Mode_UIBP:CheckSoloMapDownloadStatus all downloaded, hide")
  end
end
function Lobby_Mode_UIBP:_SetMatchTeamStatusIndex(bAbnormal)
  self.bAbnormalStatus = bAbnormal or false
  local helper = require("client.slua.umg.lobby.Main.Helper.match_pre_info_bar_helper")
  if not helper then
    log(bWriteLog and "Lobby_Mode_UIBP:_SetMatchTeamStatusIndex helper not found")
    return
  end
  helper.SetMatchTeamStatusIndex(self, bAbnormal)
end
...
function Lobby_Mode_UIBP:CheckAbnormalStatusShow()
  ...
  if not bSelfMapNotDownloaded then
    bTeammateMapNotDownloaded = logic_team_match_state:HasAnyTeammateMapIssue()
...
function Lobby_Mode_UIBP:OnButton_TeamStatusClick()
  ...
  local TeamUpNewSystem = require("client.slua.logic.teamup.logic_team_up")
  if TeamUpNewSystem.IsInTeam() then
    UIManager.ShowUI(UIManager.UI_Config.Lobby_MatchSettingAndStatus_UIBP, {PageIndex = 2})
  else
...
function Lobby_Mode_UIBP:SpecialPlayUIUpdate(table_name, data)
  ...
  local actIndex = LobbySystem.roleData and LobbySystem.roleData.region_info and tonumber(LobbySystem.roleData.region_info.act_index) or 0
  log_format(bWriteLog and "Lobby_Mode_UIBP:SpecialPlayUIUpdate gameId:%s actIndex:%s", tostring(gameId), tostring(actIndex))
      ...
      if 0 <= version_util.CompareVersionStandard(curFormatVersion, reqFormatVersion) then
        ...
        for _, appID in pairs(appIDList) do
          if appID == gameId then
            local region_group_id = tonumber(v.region_group_id)
            if region_group_id and region_group_id ~= 0 then
              if region_group_id == actIndex then
                bIsAppIDValid = true
              end
              break
            end
...
function Lobby_Mode_UIBP:Button_SpecialPlayClick()
  self:PlayAudio(sound_config.click)
  local playerPrefsSystem = require("client.logic.LogicPlayerPrefs.playerprefs")
  local showCfg = playerPrefsSystem.LoadFileToTable_N(playerPrefsSystem.ePlayerPrefsType.eCommerceSpecialPlayPanel)
  showCfg = showCfg or {}
  if not showCfg.hasClicked then
    showCfg.hasClicked = true
  end
  if self._hasShowSpecialPlayHandGuide then
    showCfg.hasShowBubble = true
    self._hasShowSpecialPlayHandGuide = false
    self:SetWidgetVisible(self.UIRoot.Canvas_Panel_HandGuide, false)
  end
  playerPrefsSystem.SaveTableToFile_N(showCfg, playerPrefsSystem.ePlayerPrefsType.eCommerceSpecialPlayPanel)
  ...
  if jump_url and jump_url ~= "" then
    ...
    local BasicDataTLogReport = ModuleManager.GetModule(ModuleManager.DataModuleConfig.BasicDataTLogReport)
    BasicDataTLogReport:ReportDelay(TLogEventDefine.NewModeSelection_ClickSpecialPlayPanel, 0, jump_url)
  elseif jump_id and jump_id ~= "" then
    UIManager.ShowUI(UIManager.UI_Config.Lobby_BT_Guide_UIBP, jump_id)
  end
end
function Lobby_Mode_UIBP:ShowSpecialPlayPanel(EventType, EventID, params)
  log(bWriteLog and "Lobby_Mode_UIBP:ShowSpecialPlayPanel")
  local jumpNoData = false
  if EventType == EVENTTYPE_PANDORA and EventID == EVENTID_PANDORA_ALL_READY_BATCH_NOTIFY and (not (params and next(params)) or not params[self.specialPlayPandoraActID]) then
    jumpNoData = true
    log(bWriteLog and "Lobby_Mode_UIBP:ShowSpecialPlayPanel no pandora data")
  end
  if jumpNoData then
    return
  end
  ...
  self.DelayUpdateSpecialPlayPanelTimer = self:AddTimerOnce(0, function()
    ...
    if DataMgr.roleData.level < Config_UGC.UGCActivationLevel then
      log(bWriteLog and "Lobby_Mode_UIBP:ShowSpecialPlayPanel level too low")
      return
    end
    if tonumber(self.specialPlayActivityCfg.pandora_activity) == 1 then
      local pandora_logic = require("client.slua.logic.Pandora.pandora_logic")
      local isReady = pandora_logic.ActIsAllReady(self.specialPlayPandoraActID)
      if not isReady then
        log(bWriteLog and "Lobby_Mode_UIBP:ShowSpecialPlayPanel  no pandora ready")
        return
      end
    else
      log(bWriteLog and "Lobby_Mode_UIBP:ShowSpecialPlayPanel pandora_activity 0")
    end
    ...
    if UGCPlayHallRoom then
      if UGCPlayHallRoom:GetRoomInfo() then
        log(bWriteLog and "Lobby_Mode_UIBP:ShowSpecialPlayPanel UGCPlayHallRoomShow")
        return
      end
      if UGCPlayHallRoom:CheckPendingMatchState() then
        log(bWriteLog and "Lobby_Mode_UIBP:ShowSpecialPlayPanel UGCPendingMatchState")
        return
      end
    end
    ...
    if self.bAbnormalStatus then
      log(bWriteLog and "Lobby_Mode_UIBP:ShowSpecialPlayPanel bAbnormal.")
      return
    end
    local logic_promotion_mode = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.logic_promotion_mode)
    if logic_promotion_mode:IsCanSelect() then
      log(bWriteLog and "Lobby_Mode_UIBP:ShowSpecialPlayPanel promotion mode.")
      return
    end
    self:UpdateSpecialPlayPanelRedDot()
    self:SetWidgetVisible(self.UIRoot.CanvasPanel_SpecialPlay, true)
  end)
end
function Lobby_Mode_UIBP:UpdateSpecialPlayPanelRedDot()
  local pandoraLogic = require("client.slua.logic.Pandora.pandora_logic")
  local hasRedDot = pandoraLogic.ActHasRedPoint(self.specialPlayPandoraActID)
  local playerPrefsSystem = require("client.logic.LogicPlayerPrefs.playerprefs")
  local showCfg = playerPrefsSystem.LoadFileToTable_N(playerPrefsSystem.ePlayerPrefsType.eCommerceSpecialPlayPanel)
  showCfg = showCfg or {}
  log_format(bWriteLog and "Lobby_Mode_UIBP:UpdateSpecialPlayPanelRedDot hasRedDot:%s showNew:%s hasShowBubble:%s", tostring(hasRedDot), tostring(showCfg.hasClicked), tostring(showCfg.hasShowBubble))
  local showNew = false
  if not showCfg.hasClicked then
    showNew = true
  end
  if showCfg.hasClicked then
    self.hasShowSpecialPlayPanelUnlockBubble = true
  end
  if not self.hasShowSpecialPlayPanelUnlockBubble and not showCfg.hasShowBubble then
    log(bWriteLog and "Lobby_Mode_UIBP:UpdateSpecialPlayPanelRedDot hasShowSpecialPlayPanelUnlockBubble")
    self.hasShowSpecialPlayPanelUnlockBubble = true
    self:AddTimerOnce(0.1, function()
      if not slua.isValid(self.UIRoot) then
        return
      end
      self._hasShowSpecialPlayHandGuide = true
      self:SetWidgetVisible(self.UIRoot.Canvas_Panel_HandGuide, true)
      self:PlayUserWidgetAnimation(self.UIRoot.Animation_Hand, 0, 0, 0, 1)
    end)
  end
  local ModReddotSystem = require("GameLua.Mod.Library.Client.ModReddotSystem")
  local redPointType = showNew and ModReddotSystem.ERedDotStyle.NEW or ModReddotSystem.ERedDotStyle.GIFT
  self.UIRoot.Reddot_Anchor:ShowRedPointByPath(redPointType)
  self:SetWidgetVisible(self.UIRoot.Reddot_Anchor, hasRedDot or showNew)
end
...
function Lobby_Mode_UIBP:OnButton_BackToCommunity()
  self:PlayAudio(sound_config.click)
  local logic_community = require("client.slua.logic.community.logic_community")
  local res = logic_community.ReturnToCommunity(logic_community.bBackToCommunityTip, "Lobby_Mode_UIBP")
  if res then
    logic_community.bBackToCommunityTip = nil
    self:SetWidgetVisible(self.UIRoot.CanvasPanel_VideoStream, false, false)
    self:SetWidgetVisible(self.UIRoot.Button_1, false, true)
    return
  end
  log(bWriteLog and "Lobby_Mode_UIBP:OnButton_BackToCommunity back to community failed")
end
function Lobby_Mode_UIBP:UpdateInfoBar()
  log(bWriteLog and "Lobby_Mode_UIBP:OnModeChange")
  self:UpdateMatchPreInfoBar()
  self:CheckAbnormalStatusShow()
end
function Lobby_Mode_UIBP:OnGetMatchAnimationEnd()
  local bNotDownloaded, leftSize = self:GetSelfResDownloadState()
  log(bWriteLog and "Lobby_Mode_UIBP:OnGetMatchAnimationEnd bNotDownloaded: " .. tostring(bNotDownloaded) .. " leftSize: " .. tostring(leftSize))
  if bNotDownloaded and leftSize then
    self.UIRoot.UTRichTextBlock_Text:SetText(LocUtil.LocalizeResFormat(48498, string.format("%.1f", leftSize)))
  else
    self.UIRoot.UTRichTextBlock_Text:SetText(LocUtil.GetLocalizeResStr(817439))
  end
end
function Lobby_Mode_UIBP:RecoverAlpha(WidgetAnimationName, bIsUnNormal)
  log(bWriteLog and "Lobby_Mode_UIBP:RecoverAlpha WidgetAnimation:" .. tostring(WidgetAnimationName) .. " bIsUnNormal:" .. tostring(bIsUnNormal))
  if self.alpharTimer then
    self:RemoveTimer(self.alpharTimer)
    self.alpharTimer = nil
  end
  local duration = self:GetAnimationDuration(WidgetAnimationName)
  if duration then
    self.alpharTimer = self:AddTimerOnce(duration, function()
      if not self or not slua.isValid(self.UIRoot) then
        log(bWriteLog and "Lobby_Mode_UIBP:RecoverAlpha self or self.UIRoot is invalid")
        return
      end
      if bIsUnNormal then
        if self.UIRoot.Border_UnNormal then
          local opacity = self.UIRoot.Border_UnNormal.ContentColorAndOpacity.A
          if opacity == 1 then
            log(bWriteLog and "Lobby_Mode_UIBP:RecoverAlpha opacity1 is " .. tostring(opacity))
          else
            self.UIRoot.Border_UnNormal:SetContentColorAndOpacity(FLinearColor(1, 1, 1, 1))
          end
        else
          log(bWriteLog and "Lobby_Mode_UIBP:RecoverAlpha self.UIRoot.Border_UnNormal is nil")
        end
      elseif self.UIRoot.Border_Normal then
        local opacity = self.UIRoot.Border_Normal.ContentColorAndOpacity.A
        if opacity == 1 then
          log(bWriteLog and "Lobby_Mode_UIBP:RecoverAlpha opacity2 is " .. tostring(opacity))
        else
          self.UIRoot.Border_Normal:SetContentColorAndOpacity(FLinearColor(1, 1, 1, 1))
        end
      else
        log(bWriteLog and "Lobby_Mode_UIBP:RecoverAlpha self.UIRoot.Border_Normal is nil")
      end
    end)
  else
    log(bWriteLog and "Lobby_Mode_UIBP:RecoverAlpha duration is nil.")
  end
end
