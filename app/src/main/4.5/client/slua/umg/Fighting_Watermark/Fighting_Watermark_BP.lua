...
function Fighting_Watermark_BP:UpdateUI()
  ...
  for k, v in pairs(self.TextBlock) do
    v:SetText(strOutput)
    if Client.IsUEPUBGM() and Client.IsShowingWatermark() == false then
      self:SetWidgetVisible(v, false)
    end
  end
  if Client.IsUEPUBGM() and Client.IsShowingWatermark() then
    self:SetWidgetVisible(self.TextBlock[2], false)
    self:SetWidgetVisible(self.TextBlock[4], false)
    self:SetWidgetVisible(self.TextBlock[6], false)
  end
end
