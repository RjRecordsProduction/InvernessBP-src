...
function Common_Avatar_All_UIBP:_CreateCommonItemChildUI(sChildName, ...)
  ...
  if not self[sChildName] then
    log_error(string.format("Common_Avatar_All_UIBP:_CreateCommonItemChildUI create child failed, sChildName=%s, baseType=%s, sParentName=%s, sBpPath=%s", tostring(sChildName), tostring(tChildCfg.baseType), tostring(tChildCfg.sParentName), tostring(tChildCfg.sBpPath)))
    return nil
  end
