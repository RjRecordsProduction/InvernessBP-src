...
local DELAY_PROGRESS_REVEAL_THRESHOLD = 0.25
...
function common_download_ui:ResetData()
  ...
  self.delayProgressUntilThreshold = false
  self._bRevealProgressUnlocked = false
  self.data = nil
...
function common_download_ui:SetData(downloadType, data, params)
  ...
  if params then
    ...
    self.delayProgressUntilThreshold = params.delayProgressUntilThreshold == true
    self.enableAutoDownloadMultipleItems = params.enableAutoDownloadMultipleItems
...
function common_download_ui:AutoDownload()
  ...
  if self.downloadType == PufferConst.ENUM_DownloadType.ODPAK and (#self.data == 1 and type(self.data[1]) == "number" or self.enableAutoDownloadMultipleItems) then
...
function common_download_ui:ShowNotDownloadState()
  if PufferSwitch.CanAutoDownload() then
    return false
  end
  if self._bRevealProgressUnlocked then
    return false
  end
  local curPercent = 0
  if self.totalSize and 0 < self.totalSize then
    curPercent = self.curSize / self.totalSize
  end
  return curPercent < DELAY_PROGRESS_REVEAL_THRESHOLD
end
...
function common_download_ui:RefreshDownloadProgress()
  ...
  local bIsShowNotDownloadState = self:ShowNotDownloadState()
  if not (isPause or state == ENUM_DownloadState.Download or self.bShowSize) or self.bShowProgress then
    ...
    if self.bShowSize then
      ...
      self:_UpdateProgress2Visible(not bIsShowNotDownloadState, true)
    end
    local percent = 1
    ...
    if self.totalSize > 0 then
      ...
      if bIsShowNotDownloadState then
        imageShow = true
      end
    end
...
function common_download_ui:OnClickDownload()
  ...
  self._bRevealProgressUnlocked = true
  local PufferTlog = require("client.slua.logic.download.report.puffer_tlog")
