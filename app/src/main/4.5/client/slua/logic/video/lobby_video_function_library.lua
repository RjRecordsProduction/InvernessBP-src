...
function VideoLibrary.PlayVideo(videoPath, extra, bMask, ParamTable, assetOverrides)
  ...
  if VideoLibrary.IsCanVideoFileAndReady(videoPath) then
    ...
    UIManager.ShowUI(UIManager.UI_Config.video_player_system, videoPath, extra, assetOverrides, ParamTable)
    result = true
  end
...
function VideoLibrary.CreateVideoSource(videoPath, mediaSource)
  if not VideoLibrary.IsInGameVideoFileReady(videoPath) then
    return nil
  end
  local source = mediaSource
  if not source then
  ...
  if not VideoLibrary.IsStreamPath(videoPath) then
...
function VideoLibrary.IsCanVideoFileAndReady(videoPath)
  local isCanPlayVideo = VideoLibrary.IsCanPlayVideo()
  local isVideoFileReady = VideoLibrary.IsVideoFileReady(videoPath)
  log_format("VideoLibrary.IsCanVideoFileAndReady, isCanPlayVideo = %s, isVideoFileReady = %s", isCanPlayVideo, isVideoFileReady)
  return isCanPlayVideo and isVideoFileReady
end
...
function VideoLibrary.PlayVideoPure(videoPath, extra)
  ...
  UIManager.ShowUI(UIManager.UI_Config.video_player_system_pure, realVideoPath, extra)
  return true
end
function VideoLibrary.StopVideoPure()
  log(bWriteLog and "VideoLibrary.StopVideoPure")
  UIManager.CloseUI(UIManager.UI_Config.video_player_system_pure)
end
function VideoLibrary.StopVideo()
