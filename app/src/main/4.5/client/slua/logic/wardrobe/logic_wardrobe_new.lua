      ...
      elseif itemCfg.WardrobeMainTab == wardrobePageTypeId.ENUM_WardrobePageType_Avatar then
        if logic_wardrobe_avatar:IsTabString_Bag_Helmet_Armor(itemCfg.WardrobeTab) then
          ...
          if LogicXSuit.IsXSuit(displayResID) then
            ...
            if olditem and olditem.res_id and LogicXSuit.GetPeriodByItemId(item.res_id) == LogicXSuit.GetPeriodByItemId(olditem.res_id) then
            else
              local level = LogicXSuit.GetDefaultSwitchLevelByItemID(item.res_id)
              local branchId = LogicXSuit.GetBranchByItemId(item.res_id)
              LogicXSuit.SetSwitchLevelByPeriod(LogicXSuit.GetPeriodByItemId(item.res_id), level, branchId)
            end
...
function WardrobeLogic:ShowVehicleModel(res_id, source, bNotDownloadShowDefaultModel, callback)
  ...
  local MultiSlotParts = LogicVehicleExtendedFeature:GetEquipedMultiSlotParts(res_id, source)
  ...
  if bNotDownloadShowDefaultModel then
    local PufferManager = require("client.slua.logic.download.puffer.puffer_manager")
    local PufferConst = require("client.slua.logic.download.puffer_const")
    if not PufferManager.GetListIsDownloaded(PufferConst.ENUM_DownloadType.ODPAK, {res_id}) then
      local SkinCfg = CDataTable.GetTableData("VehiclePlaneSkinMapping", res_id)
      if SkinCfg then
        ModelDisplayer.Display(SkinCfg.OrginalID, true)
      end
    else
  ...
  EventSystem:postEvent(EVENTTYPE_WARDROBE, EVENTID_WARDROBE_CreateDownloader, res_id, nil, callback)
end
