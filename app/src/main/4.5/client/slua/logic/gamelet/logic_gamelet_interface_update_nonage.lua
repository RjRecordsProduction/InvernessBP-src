...
function logic_gamelet_interface_update_nonage:OnEnterLobby(needUpdateRole)
  ...
  self.  self.timer = self:AddTimerLoop(30, self:LoopCheckAndSetNonage(), 10, 30)
end
function logic_gamelet_interface_update_nonage:LoopCheckAndSetNonage()
  ...
  local func = function()
    ...
    log(bWriteLog and string.format("logic_gamelet_interface_update_nonage:LoopCheckAndSetNonage self.hasSet = %s, count = %s", tostring(self.hasSet), tostring(count)))
    if self.hasSet then
      if self.timer then
        self:RemoveTimer(self.timer)
        self.timer = nil
    ...
    local antiaddction_adult = AntiaddctionSystem and AntiaddctionSystem.is_nonage
    ...
    if antiaddction_adult ~= nil then
