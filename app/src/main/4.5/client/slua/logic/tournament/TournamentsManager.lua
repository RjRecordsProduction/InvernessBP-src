...
function TournamentsManager.GetEntryInfo()
  ...
  local info = {
    ...
    logo_close = "/Game/MultiRegion/Content/IN/UMG/Texture/Lobby_NoAtlas/ESport/Esport_Game_logo_hui_06.Esport_Game_logo_hui_06",
    logo_open = "/Game/MultiRegion/Content/IN/UMG/Texture/Lobby_NoAtlas/ESport/Esport_Game_logo_06.Esport_Game_logo_06",
...
function TournamentsManager.get_tournaments_rsp(tournaments, appendInfo)
  ...
  if appendInfo then
    ...
    TournamentsManager.h5MarketUrl = appendInfo.market_h5_url or "game://?module=1001300&id=645008014"
...
function TournamentsManager.CheckMoneyIsEnough(itemId, needNum, id)
  ...
  if itemId == 1109 then
    if needNum > DataMgr.eternal_diamond then
      ShowNotice(450198)
      return false
    end
  elseif itemId == 1702018 then
    ...
    if needNum > hasIndiaTicket then
      ...
      local moneyCost = TournamentsManager.indiaTicketConfig and TournamentsManager.indiaTicketConfig.moneyCount or 100
      local needTicket = needBuyIndiaTicket * moneyCost
      local tip = LocUtil.LocalizeResFormat(7109, tostring(needTicket), tostring(needBuyIndiaTicket))
      local CommonMsgBoxMgr = require("client.slua.logic.common.logic_common_msg_box")
      CommonMsgBoxMgr.Show(2, title, tip, function()
        if TournamentsManager.CheckMoneyIsEnough(1109, needTicket) then
          local item_list = {
