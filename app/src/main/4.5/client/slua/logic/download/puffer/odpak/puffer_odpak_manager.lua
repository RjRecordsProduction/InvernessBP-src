...
local UnMatchPackID = PufferConst.EODPackID.UnMatchPackID
...
function PufferODPakManager:InitODPaks(existPaks, callback, async)
  ...
  PufferUGCPakManager:InitUGCDependsODPak()
...
function PufferODPakManager:InitUGCExpiredDepends()
  ...
  local CurVersionExpiredAssetSet, Cfg
  local PublishRegionMacros = require("client.slua.config.ClientMacros.PublishRegionMacros")
  if PublishRegionMacros.IsBLUEHOLE() then
    CurVersionExpiredAssetSet = CreativeExpiredAssetConfig.GetBlueholeCurVersionExpiredAssetSet()
    Cfg = CDataTable.GetTable("UGCDelayPublishConfigBluehole")
  else
    CurVersionExpiredAssetSet = CreativeExpiredAssetConfig.GetCurVersionExpiredAssetSet()
...
function PufferODPakManager:InitVirtualODPack(id, paks, existPaks)
  ...
  for pakName, featureKey in pairs(paks) do
    ...
    if pakData then
      if not pakData.virtualPackIDs then
        pakData.virtualPackIDs = {}
      end
      pakData.virtualPackIDs[id] = true
      if existPaks and existPaks[pakName] then
...
function PufferODPakManager:GetStateByPakName(pakName, skipCache)
  ...
  if not skipCache and pakNameData then
  ...
  if puffer_queue:GetDownloadingTask(pakName) then
    return ENUM_DownloadState.Download
  end
  if puffer_queue:GetWaitTask(pakName) then
    return ENUM_DownloadState.Wait
  end
  if state ~= ENUM_DownloadState.Done then
    return state
  end
  if skipCache then
    local isExist = PufferDownloader.GetPakExist(pakName, skipCache)
    pakNameData.state = isExist and ENUM_DownloadState.Done or ENUM_DownloadState.Not
  end
  return pakNameData.state
end
function PufferODPakManager:AddUnMatchPak(pakName)
  local isPakExist = PufferDownloader.GetPakExist(pakName, true)
  local tSize = GCPufferDownloader.GetFileSizeCompressed(Puffer, pakName) / PufferConst.MB
  local MinSize = 0.1
  if tSize == 0 then
    tSize = MinSize
  end
  local state = isPakExist and PufferConst.ENUM_DownloadState.Done or PufferConst.ENUM_DownloadState.Not
  local cSize = isPakExist and tSize or 0
  local pakData = {
    cSize = cSize,
    tSize = tSize,
      }
  log_format("PufferODPakManager:AddUnMatchPak. pakName=%s, state=%s, cSize=%.2f, tSize=%.2f", pakName, state, cSize, tSize)
  self.PakDatas[pakName] = {packID = UnMatchPackID, data = pakData}
  local packData = self.ODPaks[UnMatchPackID]
  if not packData then
    packData = {
      paks = {},
      curCnt = 0,
    ...
    self.ODPaks[UnMatchPackID] = packData
  end
  packData.paks[pakName] = pakData
  packData.totalCnt = packData.totalCnt + 1
  packData.totalSize = packData.totalSize + tSize
  if isPakExist then
    packData.curCnt = packData.curCnt + 1
    packData.curSize = packData.curSize + cSize
  end
  self:_UpdatePackDataState(packData)
  return pakData
end
function PufferODPakManager:GetStateByPath(path)
  if not Client or Client.bEditorSkipDownload then
...
function PufferODPakManager:ResetStateByItemID(itemID)
  log(bWriteLog and string.format("PufferODPakManager:ResetStateByItemID. itemID=%s", tostring(itemID)))
  if not itemID then
    return
  end
  local state = PufferConst.ENUM_DownloadState.Done
  ...
  log(bWriteLog and "PufferODPakManager:ResetStateByItemID. fix state = " .. tostring(state))
...
function PufferODPakManager:GetPakNameByTableKey(Key)
  ...
  if AllPakNames then
    self.TableKeyToPaks = self.TableKeyToPaks or {}
...
function PufferODPakManager:GetBPPathsByItemID(itemID, itemCfg)
  ...
  local addListFunc = function(addList, addPath)
    if addPath and addPath ~= "" then
      table.insert(addList, addPath)
    end
  end
  for _, v in pairs(BPMapping) do
    ...
    if ItemType == ENUM_ITEM_TYPE.Vehicle or ItemType == ENUM_ITEM_TYPE.Extra then
      ...
      if vehicleAnimData and vehicleAnimData.ID and vehicleAnimData.ID ~= "" and vehicleAnimData.DataAssetPath ~= "" then
        addListFunc(list, vehicleAnimData.DataAssetPath)
      end
    end
    if ItemType == ENUM_ITEM_TYPE.Weapon then
      local fxData = CDataTable.GetTableData("AvatarWeaponHitFXData", id)
      if fxData then
        addListFunc(list, fxData.EffectPath)
      end
  ...
  if itemCfg and itemCfg.ItemType == ENUM_ITEM_TYPE.Partner_Stance and itemCfg.ItemSubType == ENUM_ITEM_SUBTYPE.PartnerStance then
    ...
    if intimacyPoseMapping and intimacyPoseMapping.PoseType then
      ...
      if IntimacyPoseConfig then
        addListFunc(list, IntimacyPoseConfig.MeshPath)
        addListFunc(list, IntimacyPoseConfig.MeshAnimPath)
        addListFunc(list, IntimacyPoseConfig.LeftPose)
        addListFunc(list, IntimacyPoseConfig.RightPose)
        addListFunc(list, IntimacyPoseConfig.LeftAnimation)
        addListFunc(list, IntimacyPoseConfig.RightAnimation)
      end
    end
  end
  if itemCfg then
    ...
    elseif itemCfg.ItemType == ENUM_ITEM_TYPE.CYCLE_Memory_Item and itemCfg.ItemSubType == ENUM_ITEM_SUBTYPE.Aid_Gift then
      ...
      if giftData then
        addListFunc(list, giftData.GiftAni)
      end
    elseif itemCfg.ItemType == ENUM_ITEM_TYPE.Vehicle then
      ...
      addListFunc(list, parachutePropertyBPPath)
    elseif itemCfg.ItemType == ENUM_ITEM_TYPE.Buddy then
      local cfg = CDataTable.GetTableData("PetLevelTable", itemID * 10000 + 1)
      if cfg then
        addListFunc(list, cfg.LobbyPetBP)
        addListFunc(list, cfg.PetCharacter)
        addListFunc(list, cfg.PetFPPCharacter)
      end
    elseif itemCfg.ItemType == ENUM_ITEM_TYPE.MVPEmotion and itemCfg.ItemSubType == ENUM_ITEM_SUBTYPE.MVPIcon then
      ...
      if cfg and cfg.PreviewPath then
        addListFunc(list, cfg.PreviewPath)
        addListFunc(list, cfg.Path)
      end
    elseif itemCfg.ItemType == ENUM_ITEM_TYPE.MVPEmotion and itemCfg.ItemSubType == ENUM_ITEM_SUBTYPE.MVPAction then
      ...
      if cfg and cfg.LevelPath ~= "" then
        addListFunc(list, cfg.LevelPath)
      end
    elseif itemCfg.ItemType == ENUM_ITEM_TYPE.ClickEffect and itemCfg.ItemSubType == ENUM_ITEM_SUBTYPE.ClickEffect then
      ...
      if cfg and cfg.EffectPath then
        addListFunc(list, cfg.EffectPath)
      end
    end
  end
  return list
...
function PufferODPakManager:GetSizeByKeyList(downloadType, keyList, bSkipDepends)
  ...
  if downloadType == PufferConst.ENUM_DownloadType.ODPAK then
    ...
    for i, v in pairs(keyList) do
      ...
      elseif valType == "string" then
        if StringUtil.Starts(v, PufferConst.ODPAKS_RELATIVE_DIR) then
          allPakNames[v] = true
        elseif StringUtil.Starts(v, PufferConst.PUFFERPATCH) then
          pufferPatchKeys[v] = true
        elseif self:GetPakNamesByFeatureID(v) then
          local paks = self:GetPakNamesByFeatureID(v)
          for pakName, _ in pairs(paks) do
            allPakNames[pakName] = true
          end
        else
    ...
    for pakName, _ in pairs(allPakNames) do
      local cS, tS = self:GetSizeByPakName(pakName)
      curSize = curSize + cS
      totalSize = totalSize + tS
    end
    local PufferResManager = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.puffer_res_manager)
    for patchKey, _ in pairs(pufferPatchKeys) do
      local cS, tS = PufferResManager:GetSize(patchKey)
      curSize = curSize + cS
      totalSize = totalSize + tS
    end
  elseif downloadType == PufferConst.ENUM_DownloadType.ODPACK then
...
function PufferODPakManager:GetSizeByPakName(pakName)
  ...
  local pakNameData = self:GetPakDataByPakName(pakName)
  return pakNameData.cSize * PufferConst.MB, pakNameData.tSize * PufferConst.MB
end
...
function PufferODPakManager:ResetPakData(pakName)
  ...
  if self.needRecoverBattleData then
    ...
    if packData then
      ...
      if pakData then
        wasDone = pakData.state == ENUM_DownloadState.Done
        pakTSize = pakData.tSize
        if wasDone then
    ...
    if wasDone then
      local pakDataInfo = self.PakDatas[pakName]
      if pakDataInfo and pakDataInfo.virtualPackIDs then
        for virtualID, _ in pairs(pakDataInfo.virtualPackIDs) do
          local vPackData = self.ODPaks[virtualID]
          if vPackData and vPackData.paks[pakName] then
            if 0 < vPackData.curCnt then
              vPackData.curCnt = vPackData.curCnt - 1
              vPackData.curSize = vPackData.curSize - pakTSize
            end
            if vPackData.curCnt == 0 then
              vPackData.curSize = 0
              vPackData.state = ENUM_DownloadState.Not
            elseif vPackData.curCnt < vPackData.totalCnt then
              vPackData.state = ENUM_DownloadState.Pause
            end
          end
        end
      end
...
function PufferODPakManager:GetItemBPPathByItemID(itemID, itemCfg, bpID)
  ...
  elseif ModelDisplayTypeHelper.IsVehiclePartDisplay(itemCfg.ItemType, itemCfg.ItemSubType) then
    bpCfg = CDataTable.GetTableData("VehicleBPTable", bpID)
    if bpCfg ~= nil then
      self.itemToPaths[itemID] = bpCfg.Path
      return bpCfg.Path
...
function PufferODPakManager:_GetRelatedItems(itemID, itemCfg, bNotMapping)
  ...
  if ItemType == ENUM_ITEM_TYPE.Extra then
    ...
    if period then
      local goldList = LogicXSuit.GetItemIDListByPeriod(period, LogicXSuit.GetBranchByItemId(itemID))
  ...
  if ModelDisplayTypeHelper.IsGlide(itemCfg.ItemSubType) then
    ...
    local allSpecialGlideIDs = LogicXSuit.GetAllSpecialGlideIDs(LogicXSuit.GetNormalGlideID(itemID))
    for _, SpecialGlideID in ipairs(allSpecialGlideIDs) do
  ...
  if ItemType == ENUM_ITEM_TYPE.VehicleDecoration then
    local partsDisplayCfg = CDataTable.GetTableData("PartsDisplayTable", itemID)
    if partsDisplayCfg and partsDisplayCfg.DisplayVehicleID and partsDisplayCfg.DisplayVehicleID ~= 0 then
      depend[partsDisplayCfg.DisplayVehicleID] = true
    end
  end
  depend[itemID] = true
  local list = {}
...
function PufferODPakManager:GetPakDataByPakName(pakName)
  ...
  return self:AddUnMatchPak(pakName)
end
...
function PufferODPakManager:OnDownloadFinish(task, isSuccess, errorCode, IsUGC)
    ...
    if pakData then
      ...
      if not (not isSuccess and (errorCode ~= 1 or Client.IsDevelopment())) or IsWoWEditor then
        if pakData.state ~= ENUM_DownloadState.Done then
          ...
          packData.curSize = packData.curSize + pakData.tSize
          packData.curCnt = packData.curCnt + 1
          pakData.state = ENUM_DownloadState.Done
        end
        ...
        if packData.curCnt >= packData.totalCnt then
          ...
          packData.state = ENUM_DownloadState.Done
          if not IsUGC then
        ...
        pakData.state = ENUM_DownloadState.Error
      end
    end
    if not IsUGC then
  ...
  if pakDataInfo and pakDataInfo.virtualPackIDs then
    for virtualID, _ in pairs(pakDataInfo.virtualPackIDs) do
      if virtualID ~= ODPackID then
        local vPackData = self.ODPaks[virtualID]
        if vPackData and vPackData.paks[pakName] then
          local vPakData = vPackData.paks[pakName]
          if not (not isSuccess and (errorCode ~= 1 or Client.IsDevelopment())) or IsWoWEditor then
            vPakData.cSize = vPakData.tSize
            vPackData.curSize = vPackData.curSize + vPakData.tSize
            vPackData.curCnt = vPackData.curCnt + 1
            vPakData.state = ENUM_DownloadState.Done
            if vPackData.curCnt >= vPackData.totalCnt then
              vPackData.curCnt = vPackData.totalCnt
              vPackData.curSize = vPackData.totalSize
              vPackData.state = ENUM_DownloadState.Done
              if not IsUGC then
                PufferDownloader.SetDownloadKeyRecord(virtualID, true)
                local PufferTlog = require("client.slua.logic.download.report.puffer_tlog")
                PufferTlog.SendTLog(task.from, PufferTlog.Enum_TLog_Optype.Finish, virtualID, nil, virtualID == PufferConst.EODPackID.PREFETCH_ODPACKID)
                self:JumpODPack(virtualID)
                log(bWriteLog and string.format("PufferODPakManager:OnDownloadFinish virtual ODPack done:%s", virtualID))
              end
            end
          end
        end
