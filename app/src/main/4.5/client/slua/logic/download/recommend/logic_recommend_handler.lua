...
function RecommendHandler.OnGameStateChange(eventType, eventID, vars)
  ...
  if vars.current == GameStatus.Lobby and vars.pre == GameStatus.Fighting then
    ...
    time_ticker.AddTimerOnce(2, function()
      ...
      RecommendHandler.DownloadNewbieVersionPV()
    end)
  end
  if vars.current == GameStatus.Lobby and vars.pre ~= GameStatus.Fighting then
...
function RecommendHandler.DownloadNewbieVersionPV()
  ...
  if not LogicNewbie.NeedShowNewbieGuide(ELobbyGuideID.LOBBY_NEWBIE_THEME_SLAP_GUIDE_ID) then
    log(bWriteLog and "RecommendHandler.DownloadNewbieVersionPV no need show theme slap guide, skip")
    return
  end
  local KeyPlayVideoSystem = require("client.slua.logic.lobby_activity.logic_keyplayvideo")
  local downloadPath = KeyPlayVideoSystem.FetchCurrentVideoDownloadPath()
  if downloadPath == "" then
    log_shipping_client("RecommendHandler.DownloadNewbieVersionPV videoId not ready yet, skip")
    return
  end
  downloadPath = downloadPath .. ".mp4"
  printf("RecommendHandler.DownloadNewbieVersionPV download path = %s", downloadPath)
  local pak_util = require("client.common.pak_util")
  pak_util.CheckAndDownloadWithCallback(downloadPath)
end
