...
function DownloadSystem.GetMapImageByKey(sMapKey)
  ...
  local showImage = "/Game/Mod/Lobby/Base/MatchSelectMap/450/ModeSelection_New/MapEntrance/Small/Lobby_match_MapEntrance_S004.Lobby_match_MapEntrance_S004"
...
local fitVersionDeleteOrdersCache = {}
local ParseFitVersionOrders = function(rawStr)
  local StringUtil = require("common.string_util")
  local parts = StringUtil.Split(rawStr, ";")
  local result = {}
...
function DownloadSystem._InitSmartDownloadCfg(srcTable, cfgOrderKey, itemData, selectFunc, fitVersionKey)
  local orderMap = {}
  local keyList = {}
  local mergeKeyOrderMap = {}
  local mergeKeySizeInfo = {}
  local PufferConst = require("client.slua.logic.download.puffer_const")
  local bUseFitVersion = false
  if fitVersionKey then
    local PublishRegionMacros = require("client.slua.config.ClientMacros.PublishRegionMacros")
    bUseFitVersion = PublishRegionMacros.IsFITVersion()
  end
  local fitPlayerType
  if bUseFitVersion then
    local DataMgr = require("client.logic.data.data_mgr")
    fitPlayerType = DataMgr.GetPlayerType()
  end
  local GetCfgOrder = function(v)
    if bUseFitVersion then
      local fitStr = v[fitVersionKey]
      if fitStr and fitStr ~= "" then
        local fitOrders = ParseFitVersionOrders(fitStr)
        local order = fitOrders[fitPlayerType]
        if order then
          return order
        end
      end
    end
    return v[cfgOrderKey]
  end
...
local RecommendDownloadTableCfg
function DownloadSystem._InitSmartDownloadDownloadSelect(srcTable, keyList, itemData, mergeKeyMap)
  ...
  local cfgList
  if RecommendDownloadTableCfg == nil then
    local PublishRegionMacros = require("client.slua.config.ClientMacros.PublishRegionMacros")
    local isFitVersion = PublishRegionMacros.IsFITVersion()
    cfgList = CDataTable.GetTableByFilter("RecommendDownloadTable", "IsFitVersion", isFitVersion)
    RecommendDownloadTableCfg = cfgList
  else
    cfgList = RecommendDownloadTableCfg
  end
  local matchCfg
  ...
  for k, v in pairs(cfgList) do
    lastCfg = v
    if freeSpaceGB <= v.FreeSpace then
      matchCfg = v
      break
    end
  end
  matchCfg = matchCfg or lastCfg
...
function DownloadSystem.InitSmartDownloadInfo(itemData, mergeBundleDataMap)
  ...
  if not next(PufferSwitch.DownloadDefaultCfg) then
    DownloadSystem._InitSmartDownloadCfg(PufferSwitch.DownloadDefaultCfg, "downloadOrder", itemData, DownloadSystem._InitSmartDownloadDownloadSelect, "fitVersionDownloadOrders")
  end
  ...
  if not next(PufferSwitch.DeleteDefaultCfg) then
    DownloadSystem._InitSmartDownloadCfg(PufferSwitch.DeleteDefaultCfg, "deleteOrder", itemData, DownloadSystem._InitSmartDownloadDeleteSelect, "fitVersionDeleteOrders")
  end
...
function DownloadSystem.GetItemDeleteOrder(key, downloadType)
  local isMap = downloadType == PufferConst.ENUM_DownloadType.MAP
  local cfg = isMap and CDataTable.GetTableData("MapPakTable", key) or CDataTable.GetTableData("PakInfoTable", key)
  if not cfg then
    return 0
  end
  local deleteOrder = 0
  local PublishRegionMacros = require("client.slua.config.ClientMacros.PublishRegionMacros")
  if PublishRegionMacros.IsFITVersion() and cfg.fitVersionDeleteOrders and cfg.fitVersionDeleteOrders ~= "" then
    if not fitVersionDeleteOrdersCache[key] then
      fitVersionDeleteOrdersCache[key] = ParseFitVersionOrders(cfg.fitVersionDeleteOrders)
    end
    local DataMgr = require("client.logic.data.data_mgr")
    local playerType = DataMgr.GetPlayerType()
    deleteOrder = fitVersionDeleteOrdersCache[key][playerType] or cfg.deleteOrder or 0
  else
    deleteOrder = cfg.deleteOrder or 0
  end
  if deleteOrder ~= 0 and isMap then
    deleteOrder = deleteOrder + MAP_ADD_ORDER_NUM
  end
  return deleteOrder
end
