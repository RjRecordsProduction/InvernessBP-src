...
local _GetDownloadType = function(keyName)
  ...
  elseif StringUtil.Starts(keyName, PufferConst.PUFFERPATCH) then
    downloadType = PufferConst.ENUM_DownloadType.RES
  end
  return downloadType
end
  ...
  for _, v in pairs(mapCfg) do
    ...
    for _, vv in pairs(dependPak) do
      ...
      if vv ~= "" then
        ...
        elseif StringUtil.Starts(vv, "/Game/") then
          local pathPakName = PufferManager.GetPakName(vv)
          if pathPakName ~= "" then
            data.depends[pathPakName] = vv
            dependKey = pathPakName
          end
...
function PufferMapManager:GetState(mapKey, bSkipDepends, bSkipVideoDepends, excludeMapKey)
  excludeMapKey = excludeMapKey or {}
      ...
      for k, v in pairs(depends) do
        ...
        if self.MapPaks[k] then
          if not excludeMapKey or not excludeMapKey[k] then
            tempState = self:GetState(k, nil, bSkipVideoDepends, excludeMapKey)
          end
        elseif bSkipVideoDepends and type(v) == "string" and StringUtil.Ends(v, ".mp4") then
...
function PufferMapManager:GetAllDependPakByMapKey(mapKey, dependPakMap, visited)
  dependPakMap = dependPakMap or {}
  visited = visited or {}
  if visited[mapKey] then
    return dependPakMap
  end
  visited[mapKey] = true
  local map = self.MapPaks[mapKey]
  ...
  for k, v in pairs(map.depends) do
    if k == v then
      self:GetAllDependPakByMapKey(k, dependPakMap, visited)
    else
      dependPakMap[k] = true
    end
  end
...
function PufferMapManager:UploadClientMapState(dependKey, force)
  ...
  if not allUpdate and dependKey then
    ...
    local queue = {}
    for mapKey, _ in pairs(changeMapKeys) do
      queue[#queue + 1] = mapKey
    end
    while 0 < #queue do
      local key = queue[#queue]
      queue[#queue] = nil
      if self.antiDepends[key] then
        for _, v in ipairs(self.antiDepends[key]) do
          if not changeMapKeys[v] then
            changeMapKeys[v] = true
            queue[#queue + 1] = v
          end
        end
...
function PufferMapManager:MountPakBySubMode(subMode, map_id, excludeMapKey)
  ...
  if mapFiles and next(mapFiles) then
    ...
    if next(pakList) then
      ...
      Client.AddAttachFileString("dependsNum", false, tostring(dependsNum))
    end
  end
...
function PufferMapManager:IsDepend(mapKey, pakName, needMapCheck, visited)
  local map = self.MapPaks[mapKey]
  if not map then
    return false
  end
  ...
  if needMapCheck then
    ...
    visited[mapKey] = true
    for k, v in pairs(map.depends) do
      if k == v and self:IsDepend(k, pakName, true, visited) then
        return true
      end
    end
  end
