...
function PufferResManager:DefineAndResetData()
  self.ResPaks = {}
  self.bResPakInitCalled = false
  self.VulkanShaderData = nil
  self.bHaveInitLobbyRes = false
  self.LobbyResKeyList = {}
  self.LobbyResMap = {}
  self.bFromLogin = false
  self.fitShaderName = nil
  self:InitFitShaderName()
end
function PufferResManager:InitFitShaderName()
  ...
  if isFitVersion then
    ...
    self.    log(bWriteLog and "PufferResManager:InitFitShaderName. fitShaderName = " .. tostring(fitShaderName))
  end
end
...
function PufferResManager:InitResPaks()
  ...
  local isNameMatch = function(pakDefaultName)
    if string.find(pakDefaultName, PufferConst.RES_FILE_PREFIX) then
      return true
    elseif isFitVersion and string.find(pakDefaultName, self.fitShaderName) then
      return true
    end
    return false
  end
  ...
  for pakDefaultName, _ in pairs(pufferList.version_mapping) do
    if isNameMatch(pakDefaultName) then
      ...
      elseif isFitVersion and resKey == self.fitShaderName then
...
function PufferResManager:InitResData(totalSize, pakName)
  local data = {}
  data.  data.percent = 0
  data.  if Client.IsFileExistsWithOutPakCheck(Client.ProjectSavedDir() .. PufferConst.PAKS_RELATIVE_DIR .. pakName) or totalSize <= 0 then
...
function PufferResManager:GetState(resKey)
  if not PufferDownloader.PufferJsonDownloadReturn then
    if resKey == PufferConst.FIT_SHADER_KEY and self.fitShaderName ~= nil then
      resKey = self.fitShaderName
    end
    local pakName = ""
    if StringUtil.Ends(resKey, ".pak") then
      pakName = resKey
    else
      pakName = resKey .. "_" .. Client.GetApplicationVersion() .. ".pak"
      pakName = PufferDownloader.GetRealFilename(pakName)
    end
