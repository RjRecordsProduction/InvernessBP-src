...
local FIT_HIDE_SYSTEM_IDS = {
  [lobby_system_entrance_marco.SystemIDDefine.LBS_WARZONE] = true,
  [lobby_system_entrance_marco.MaincitySystemIDDefine.LBS_WARZONE] = true
}
...
function logic_lobby_system_extension:_CheckUISwithcer(info)
  ...
  if FIT_HIDE_SYSTEM_IDS[SystemId] then
    ...
    if not LogicPufferBundle.IsFitLobbyResDownloaded() then
      log_format("logic_lobby_system_extension:_CheckUISwithcer SystemId:%d", SystemId)
      return false
    end
  end
  return bMenuOpen
