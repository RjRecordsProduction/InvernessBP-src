...
function logic_lobby_music.SwitchLobbyBGM()
  ...
  if not musicManager:PlayLobbyMusic() and not musicManager:GetOpenMusicBox() then
    ...
    if isJaguar then
      ...
      local state = PufferManager.GetState(PufferConst.ENUM_DownloadType.ODPAK, {bgmPath})
      if state ~= PufferConst.ENUM_DownloadState.Done then
        local sound_config = require("client.slua.config.sound_config")
        bgmPath = sound_config.Jaguar_defaultBGM
        bgmTime = sound_config.Jaguar_defaultBGM_duration
        log_format("logic_lobby_music.SwitchLobbyBGM() jaguar fallback bgmPath=%s bgmTime=%s", bgmPath, bgmTime)
      end
    end
