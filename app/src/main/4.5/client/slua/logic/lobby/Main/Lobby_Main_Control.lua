...
function Lobby_Main_Control.MoveToPage(toPage)
  ...
  if not LogicPufferBundle.IsFitLobbyResDownloaded() and toPage == ENUM_LobbyPageType.Left then
    log_format("Lobby_Main_Control.MoveToPage. left page not downloaded")
    return false
  end
...
function Lobby_Main_Control.HandleToRightScreenLogic()
  ...
  if not ModeSelectionUtil.IsModeSelectionModDownloaded() then
    printf("Lobby_Main_Control:HandleToRightScreenLogic ModeSelection mod not downloaded")
...
function Lobby_Main_Control.EnterXmission(from)
  ...
  if (not IsEditor or slua_GameFrontendHUD.bEnableEditorPufferDownload) and not PufferMapManager.bHaveInitMapPaks then
    canEnter = false
  end
  if canEnter then
    local logic_xmission_entrance = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.logic_xmission_entrance)
...
function Lobby_Main_Control.OnScrollEnd(offsetX, offsetY, Geometry, touchStartPos)
  ...
  local UIUtil = require("client.common.ui_util")
  local DPI = UIUtil.GetViewportScale()
  local CM2_IN_PIXELS = 135
  local physicalOffsetX = math.abs(offsetX) / DPI
  local physicalOffsetY = math.abs(offsetY) / DPI
  if physicalOffsetX > physicalOffsetY and CM2_IN_PIXELS < physicalOffsetX then
    if 0 < offsetX then
      if 0 < curPage then
        toPage = curPage - 1
      end
