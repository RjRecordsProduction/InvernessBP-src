...
function RankSystem.EventEnterRank(type)
  ...
  local rankAssetPath = UIManager.UI_Config.ui_rank.path
  if not PufferManager.CheckDownloadOnJump(rankAssetPath, true) then
    printf("RankSystem:EventEnterRank rank mod not downloaded, asset = %s", rankAssetPath)
    return
  end
