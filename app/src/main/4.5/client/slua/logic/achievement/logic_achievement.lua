...
function logic_achievement.GetOutDateSec(idList)
  ...
  local id = idList[1]
  local cfg = CDataTable.GetTableData("AchievementCfg", id)
  if cfg == nil or cfg.AchType ~= 1 then
    return nil
  end
  local outdateSec = AchieveHandler.GetExtinctTimeStampWithCfg(id, cfg)
  if not outdateSec then
    return nil
  end
...
function logic_achievement.FromChatShareAchievement(id, groupId)
  ...
  if cfg then
    ...
    if cfg.AchType == 1 then
      local bOut = AchieveHandler.IsExtinctByID(id, cfg)
      if bOut and not bFinish then
        bSkip = true
      end
    else
      if cfg.AchType == 2 then
        local bGet = AchieveHandler.IsGetAchRewardByID(id)
        if bGet or bFinish then
        else
          bSkip = true
        end
      else
