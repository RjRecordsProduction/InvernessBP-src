...
function logic_ping_delay_report:OnInitialize()
  ...
  if LobbyPingCheckNum and tonumber(LobbyPingCheckNum.ParamValue) then
    self.MaxDelayPing = tonumber(LobbyPingCheckNum.ParamValue)
  else
  ...
  if LobbyPingCheckTime and tonumber(LobbyPingCheckTime.ParamValue) then
    self.MaxDelayCount = tonumber(LobbyPingCheckTime.ParamValue)
  else
...
function logic_ping_delay_report:OnPostSwitchGameStatus(preState, nextState)
  ...
  self.ReportPingDelayTimer = time_ticker.AddTimerLoop(TIMER_INFINITE, function()
    ...
    local delay = tonumber(logic_zone_delay.GetChoosenZoneDelay(360, 10000))
