...
function LogicTxMissionMatch.IsCrossoverOpen()
  return LobbySystem.CheckOpen(BP_ENUM_XMISSION_CROSSOVER_SWITCH) == true
end
function LogicTxMissionMatch.IsMapCrossover(modeID)
  if not LogicTxMissionMatch.IsCrossoverOpen() then
    return false
  end
  if not modeID then
    return false
  ...
  local modeInfo = CDataTable.GetTableData("TxMissionMapMode", modeID)
  return modeInfo and modeInfo.IsCrossover and modeInfo.IsCrossover == 1 or false
end
function LogicTxMissionMatch.ShouldShowCrossoverChatBroadcast(topicId)
  if not LogicTxMissionMatch.IsCrossoverOpen() then
    return false
  end
  local chat_macro = require("client.slua.logic.lobby_chat.chat_macro")
  local bIsMetroChannel = topicId == chat_macro.MetroRoyale_EN or topicId == chat_macro.MetroRoyale_SC
  if not bIsMetroChannel then
    return false
  end
  ...
  if cfg.sCrossoverChatVersion and cfg.sCrossoverChatVersion == curVersion then
    return false
  end
  return true
end
