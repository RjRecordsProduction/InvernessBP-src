...
function xMission_Wardrobe_Data.IsDepotItemOwned(itemId)
  return DepotItemExistCache[itemId] == true
end
function xMission_Wardrobe_Data.QueryDepotItemsExist(itemIds, callback)
  local TxMissionHandler = require("client.network.Protocol.TxMissionHandler")
  TxMissionHandler.send_metro_query_depot_items_req(itemIds):Then(function(err_code, exist_item_ids)
    if err_code ~= 0 then
      if callback then
        callback({})
      end
    ...
    for _, id in ipairs(itemIds) do
      DepotItemExistCache[id] = false
    end
    if exist_item_ids then
      for _, id in ipairs(exist_item_ids) do
        DepotItemExistCache[id] = true
      end
    end
    if callback then
      callback(exist_item_ids or {})
    end
  end)
end
