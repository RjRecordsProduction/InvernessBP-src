...
function NewFaceSlapSystem:DefineAndResetData()
  ...
  self._onWidgetHideLock = false
  self._onWidgetHideLockTimer = nil
end
...
function NewFaceSlapSystem:OnLogOut()
  if self._onWidgetHideLockTimer then
    self:RemoveTimer(self._onWidgetHideLockTimer)
    self._onWidgetHideLockTimer = nil
  end
...
function NewFaceSlapSystem:OnWidgetHide(_, _, keyName)
  ...
  if self._onWidgetHideLock then
    log(bWriteLog and "NewFaceSlapSystem:OnWidgetHide return of _onWidgetHideLock, keyName = " .. tostring(keyName))
    return
  end
  self:_ShowNext()
end
...
function NewFaceSlapSystem:StartFaceSlap2DLobby()
  ...
  if LobbySystem.roleData.is_in_metro then
    log(bWriteLog and "NewFaceSlapSystem:StartFaceSlap2DLobby return of is_in_metro = true")
    return
  end
...
function NewFaceSlapSystem:StartFaceSlapTxMission()
  ...
  if not LobbySystem.roleData.is_in_metro then
    log(bWriteLog and "NewFaceSlapSystem:StartFaceSlapTxMission return of is_in_metro = false")
    return
  end
...
function NewFaceSlapSystem:_Show(index)
  ...
  self:_AcquireOnWidgetHideLock()
  self.tShownMap[info.ModuleID] = true
end
function NewFaceSlapSystem:_AcquireOnWidgetHideLock()
  self._onWidgetHideLock = true
  if self._onWidgetHideLockTimer then
    self:RemoveTimer(self._onWidgetHideLockTimer)
    self._onWidgetHideLockTimer = nil
  end
  self._onWidgetHideLockTimer = self:AddTimerOnce(0, function()
    self._onWidgetHideLock = false
    self._onWidgetHideLockTimer = nil
  end)
end
...
function NewFaceSlapSystem:IsCanShow(ignoreAndroidStack)
  ...
  if RoomSystem then
    if RoomSystem.CurrentRoomInfo and not RoomSystem.CheckIsEmptyOrDisband(RoomSystem.CurrentRoomInfo) then
      log(bWriteLog and "NewFaceSlapSystem:IsCanShow. already in room")
      return false
    end
    if RoomSystem.bPendingRoomSync then
      log(bWriteLog and "NewFaceSlapSystem:IsCanShow. bPendingRoomSync = true")
      return false
    end
  end
...
function NewFaceSlapSystem:IsInSlap()
  return self.bStart and not self.bEnd
end
