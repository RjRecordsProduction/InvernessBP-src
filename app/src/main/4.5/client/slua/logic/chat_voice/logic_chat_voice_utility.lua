...
function logic_chat_voice_utility.CreateSkillVoiceRoomName(languageId, battleId)
  local openid = DataMgr.roleData.openID or ""
  local BusinessHelper = import("BusinessHelper")
  local gvoiceAppId = BusinessHelper.GetVoiceSdkGameId() or ""
  local roomName = string.format("ats_%s_%s_%s_%s", languageId or 0, openid, battleId, gvoiceAppId)
  log(bWriteLog and string.format("[skill_trigger] _CreateSkillVoiceRoomName: %s", roomName))
  return roomName
end
