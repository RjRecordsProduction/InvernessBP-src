...
function logic_chat_voice_voice_room:OnLogOut()
  self:sdkQuitVoiceRoom("OnLogOut", self.logic_antsvoice_interface:GetTeamRoomName())
end
...
function logic_chat_voice_voice_room:OnJoinRoomNotify(code, room, member)
  ...
  if code == Enum_OperationCompleteCode.JoinRoomSucc then
    ...
    if RoomType == Enum_AntsVoiceRoomType.BattleTeam or RoomType == Enum_AntsVoiceRoomType.BattleLBS then
...
function logic_chat_voice_voice_room:JoinLobbyVoiceRoom()
  ...
  if logic_chat_voice_data_manager:GetCurRoomType() == Enum_AntsVoiceRoomType.LobbyTeam and logic_chat_voice_data_manager:GetRoomIDByRoomType(Enum_AntsVoiceRoomType.LobbyTeam) == roomId then
    printf("logic_chat_voice_voice_room:JoinLobbyVoiceRoom skip rejoin by dedup, softRoomID:%s, sdkCurRoom:%s", tostring(roomId), tostring(self.logic_antsvoice_interface:GetTeamRoomName()))
...
function logic_chat_voice_voice_room:QuitVoiceRoomByConflict(newRoomType, newRoomID)
  ...
  self:sdkQuitVoiceRoom("QuitVoiceRoomByConflict", curRoomID)
end
...
function logic_chat_voice_voice_room:ReenterVoiceRoom(roomIdOrRoomType, bForce)
  ...
  if need_to_quit_room_before_join then
    self:sdkQuitVoiceRoom("ReenterVoiceRoom", self.logic_antsvoice_interface:GetTeamRoomName())
  end
...
function logic_chat_voice_voice_room:QuitAntsVoiceRoom(roomType)
  ...
  self:sdkQuitVoiceRoom("QuitAntsVoiceRoom", roomID)
end
function logic_chat_voice_voice_room:sdkQuitVoiceRoom(quitReason, expectRoomID)
  local sdkCurRoom = self.logic_antsvoice_interface:GetTeamRoomName()
  printf("logic_chat_voice_voice_room:sdkQuitVoiceRoom enqueue quit, reason:%s, expectRoomID:%s, sdkCurRoom:%s", tostring(quitReason), tostring(expectRoomID), tostring(sdkCurRoom))
  logic_chat_voice_room_operation_manager:AddRoomOperation(Enum_AntsVoiceRoomOpera.Quit, function(data)
    self:DoQuitRoomTaskAction(data)
  end, {room_id = expectRoomID, quit_reason = quitReason})
end
...
function logic_chat_voice_voice_room:DoQuitRoomTaskAction(data)
  local sdkCurRoom = self.logic_antsvoice_interface:GetTeamRoomName()
  printf("logic_chat_voice_voice_room:DoQuitRoomTaskAction execute quit, reason:%s, expectRoomID:%s, sdkCurRoom:%s", tostring(data and data.quit_reason), tostring(data and data.room_id), tostring(sdkCurRoom))
  local logic_chat_voice = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.logic_chat_voice)
  logic_chat_voice:DisableVoiceChanger()
  local expectRoomID = data and data.room_id
  if expectRoomID ~= nil and expectRoomID ~= "" and sdkCurRoom ~= nil and sdkCurRoom ~= logic_chat_voice_const.NO_TEAM_ROOM_ROOM_NAME and tostring(expectRoomID) ~= tostring(sdkCurRoom) then
    printf("logic_chat_voice_voice_room:DoQuitRoomTaskAction skip quit by room mismatch, reason:%s, expectRoomID:%s, sdkCurRoom:%s", tostring(data and data.quit_reason), tostring(expectRoomID), tostring(sdkCurRoom))
    logic_chat_voice_room_operation_manager:OnRoomOperationCompleteCallback(Enum_AntsVoiceRoomOpera.Quit, Enum_OperationCompleteCode.QuitRoomSucc, sdkCurRoom)
    return
  end
