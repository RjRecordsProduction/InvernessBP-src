...
local Enum_AndroidPermissionName = logic_chat_voice_const.Enum_AndroidPermissionName
local Enum_AndroidPermissionResult = logic_chat_voice_const.Enum_AndroidPermissionResult
...
function logic_chat_voice_hardware:OnInitialize()
  ...
  self._pendingVoiceRoomOpenMic = nil
  self.AntsVoiceInterface = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.logic_antsvoice_interface)
end
function logic_chat_voice_hardware:MarkPendingVoiceRoomOpenMic(opType)
  log(bWriteLog and string.format("[muidarzhang] logic_chat_voice_hardware:MarkPendingVoiceRoomOpenMic, opType:%s", tostring(opType)))
  if opType == nil then
    self._pendingVoiceRoomOpenMic = nil
  else
    self._pendingVoiceRoomOpenMic = {opType = opType}
  end
end
...
function logic_chat_voice_hardware:GetMicPermissionStatus()
  local ok, SystemPermissionHelper = pcall(import, "SystemPermissionHelper")
  if not ok or not SystemPermissionHelper then
    return nil
  end
  ...
  return instance:GetPermissionStatus(2)
end
function logic_chat_voice_hardware:OnTestMicFail(error)
  ...
  self.bIsOpenMicHitPermissionErr = false
  if error == HDmpveVoiceErrno.HDMPVE_VOICE_PERMISSION_MIC_ERR or error == Enum_OperationErrorCode.AndroidTesterPermissionError or error == HDmpveVoiceErrno.HDMPVE_VOICE_INTERNAL_TVE_ERR or error == Enum_OperationErrorCode.IosTesterPermissionError then
    self.bIsOpenMicHitPermissionErr = true
    if self.sDevicePlatformName == DevicePlatformNameMacros.IOS then
      local permStatus = self:GetMicPermissionStatus()
      local bShouldGotoSetting = permStatus == nil or permStatus == 2 or permStatus == 3
      log(bWriteLog and string.format("[muidarzhang] OnTestMicFail, mic permission status:%s, bShouldGotoSetting:%s", tostring(permStatus), tostring(bShouldGotoSetting)))
      if bShouldGotoSetting then
...
function logic_chat_voice_hardware:CheckMicPermission()
  ...
  if self.sDevicePlatformName == DevicePlatformNameMacros.Windows then
    self.bIsOpenMicHitPermissionErr = false
  end
...
function logic_chat_voice_hardware:OnRequestPermissionsResult(code, permission, result)
  ...
  if self.sDevicePlatformName == DevicePlatformNameMacros.Android then
    if code == 100 and permission == Enum_AndroidPermissionName.RecordAudio and result == Enum_AndroidPermissionResult.Denied and self.bIsOpenMicHitPermissionErr == true then
    ...
    elseif code == 100 and permission == Enum_AndroidPermissionName.RecordAudio and result == Enum_AndroidPermissionResult.Granted and self.bIsOpenMicHitPermissionErr == true and self._pendingVoiceRoomOpenMic ~= nil then
      local pending = self._pendingVoiceRoomOpenMic
      self._pendingVoiceRoomOpenMic = nil
      self.bIsOpenMicHitPermissionErr = false
      log(bWriteLog and string.format("[muidarzhang] logic_chat_voice_hardware:OnRequestPermissionsResult, retry voice room open mic, opType:%s", tostring(pending.opType)))
      if pending.opType == "OpenAll" then
        self:RefreshAntsVoiceMicrophone(true, true)
      elseif pending.opType == "OpenTeamOnly" then
        self:RefreshAntsVoiceMicrophone(true, false)
      end
    elseif code == 100 and permission == Enum_AndroidPermissionName.RecordAudio and result == Enum_AndroidPermissionResult.Granted then
      EventSystem:postEvent(EVENTTYPE_ANTSVOICE, EVENTID_ANTSVOICE_MIC_PERMISSION_GRANTED)
    end
  end
end
