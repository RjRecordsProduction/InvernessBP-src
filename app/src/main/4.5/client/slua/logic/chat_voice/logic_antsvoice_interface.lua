...
function logic_antsvoice_interface:OpenTeamMicphoneOnly()
  self:_MarkPendingVoiceRoomOpenMic("OpenTeamOnly")
...
function logic_antsvoice_interface:OpenAllMicphone()
  self:_MarkPendingVoiceRoomOpenMic("OpenAll")
  return self.AntsVoiceInterface:OpenAllMicphone(false)
end
function logic_antsvoice_interface:_MarkPendingVoiceRoomOpenMic(opType)
  local logic_chat_voice = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.logic_chat_voice)
  if logic_chat_voice and logic_chat_voice.logic_chat_voice_hardware_ins then
    logic_chat_voice.logic_chat_voice_hardware_ins:MarkPendingVoiceRoomOpenMic(opType)
  end
end
function logic_antsvoice_interface:CloseAllMicphone()
  self:_MarkPendingVoiceRoomOpenMic(nil)
...
function logic_antsvoice_interface:EnableMagicVoice(magicType, enable)
  if enable then
    self.LastMagicType = magicType
  elseif magicType == nil or magicType == "" then
    magicType = self.LastMagicType
  end
...
function logic_antsvoice_interface:SetAntsVoiceServerInfo(antsVoiceUrl)
  ...
  if self.GMVoiceServerUrl and self.GMVoiceServerUrl ~= "" then
    antsVoiceUrl = self.GMVoiceServerUrl
    log(bWriteLog and "logic_antsvoice_interface:SetAntsVoiceServerInfo use GM URL: " .. antsVoiceUrl)
  end
...
function logic_antsvoice_interface:SpeechStreamTranslateStart(fileID, srcLang, targetLang, timeoutMS)
  local outFilePath = ""
  local volume = 10
  local voiceType = "default"
  if timeoutMS == nil then
    timeoutMS = 10000
  end
  return self.AntsVoiceInterface:SpeechStreamTranslateStart(fileID, outFilePath, srcLang, targetLang, voiceType, volume, timeoutMS)
end
