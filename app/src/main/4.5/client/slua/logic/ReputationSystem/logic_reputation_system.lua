...
function logic_reputation_system:ShouldShowReputationNotice()
  ...
  if PublishRegionMacros.IsFITVersion() then
    ...
    if not pak_util.IsFileExist(UIManager.UI_Config.ReputationSystem_Popup_UIBP.path) then
      log(bWriteLog and "logic_reputation_system:ShouldShowReputationNotice ui not exist")
      needSlap = false
      return false
    end
  end
