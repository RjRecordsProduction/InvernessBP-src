...
function logic_return_activity_guide:HasValidGuideUI()
  ...
  if guideType == return_activity_macro.Enum_Guide_Type.TryNeW then
    return true
  end
  if guideType == return_activity_macro.Enum_Guide_Type.Social then
    local logic_player_return = require("client.slua.logic.player_return.logic_player_return")
    local friendList = logic_player_return.GetNotifyFriendList()
    local idx = friendList and 1 < #friendList and 1 or 2
    return guideUIs[idx] ~= nil and guideUIs[idx].uiConfig ~= nil
  end
  return guideUIs[1] ~= nil and guideUIs[1].uiConfig ~= nil
end
function logic_return_activity_guide:ShowNewGuide(ignoreQueue)
  if not DataMgr.roleData.back_user_data then
    return
  end
  local ParamTable
  ...
  if DataMgr.roleData.back_user_data.version_update_guide_cfg then
    UIManager.ShowUI(UIManager.UI_Config.ReturnActivity_Version_Update_UIBP, ParamTable)
    return
  end
  local guideType = self:GetGuideType()
  if not guideType then
  ...
  elseif guideType == return_activity_macro.Enum_Guide_Type.Content then
    UIManager.ShowUI(guideUIs[1].uiConfig, ParamTable)
  elseif guideType == return_activity_macro.Enum_Guide_Type.TryNeW then
    self:ShowFBGuideUI(ignoreQueue)
  else
    UIManager.ShowUI(guideUIs[1].uiConfig, ParamTable)
  end
...
function logic_return_activity_guide:ShowFBGuideUI(ignoreQueue)
  if not DataMgr.roleData.back_user_data then
    return
  end
  ...
  if version_util.CompareVersionStandard(returnClientVersion, ClientVersion) >= 0 then
    UIManager.ShowUI(UIManager.UI_Config.ReturnActivity_TastingType_UIBP, ParamTable)
  else
    UIManager.ShowUI(UIManager.UI_Config.ReturnActivity_FristBattle_Popup_UIBP, ParamTable)
  end
end
