...
local _origOpenJumpModule
local _bJumpCancelHookInstalled = false
...
function logic_return_activity:_OnRecvChatMsg(_, _, msg)
  ...
  if msg and msg.content and msg.content and msg.content.msgType == chat_macro.friendComebackMsgType then
    ...
    if not uid then
      printf("logic_return_activity:_OnRecvChatMsg invalid sender_uid:%s", tostring(msg.sender_uid))
      return
    end
    if not saveData[uid] then
      saveData[uid] = {}
    end
    local sendTime = msg.content.sendTime
    saveData[uid][sendTime] = msg.content.other
    PlayerPrefsSystem.SaveTableToFile_N(saveData, PlayerPrefsSystem.ePlayerPrefsType.eReturnPlayerChatCache)
    printf("logic_return_activity:_OnRecvChatMsg cache saved, uid:%s, sendTime:%s, hasOther:%s", tostring(uid), tostring(sendTime), tostring(msg.content.other ~= nil))
  end
end
function logic_return_activity:GetChatMsgCache(msg)
  ...
  local saveData = PlayerPrefsSystem.LoadFileToTable_N(PlayerPrefsSystem.ePlayerPrefsType.eReturnPlayerChatCache) or {}
  local uid = tonumber(msg.sender_uid)
  if not uid or not saveData[uid] then
    printf("logic_return_activity:GetChatMsgCache miss key1, uid:%s", tostring(uid))
    return nil
  end
  local sendTime = msg.content and msg.content.sendTime or msg.send_time
  if not sendTime or not saveData[uid][sendTime] then
    printf("logic_return_activity:GetChatMsgCache miss key2, uid:%s, sendTime:%s", tostring(uid), tostring(sendTime))
    return nil
  end
  printf("logic_return_activity:GetChatMsgCache hit, uid:%s, sendTime:%s", tostring(uid), tostring(sendTime))
  return saveData[uid][sendTime]
end
function logic_return_activity:OnLogin()
  self:_InstallJumpCancelHook()
  self:ReqGetTaskList()
end
function logic_return_activity:OnLogOut()
  self:_CancelGuideMatch()
  self._watchingGuideMatch = false
  self:_UninstallJumpCancelHook()
  _InitData()
  if self.TeamUpGuideTimer then
    self:RemoveTimer(self.TeamUpGuideTimer)
    self.TeamUpGuideTimer = nil
  end
  self:StopActExpireTimer()
end
...
function logic_return_activity:TeamUpGuideShow()
  ...
  self.TeamUpGuideTimer = self:AddTimerLoop(0, function()
...
function logic_return_activity:StartReturnGuideMatchDelay()
  self:AddTimerOnce(1, function()
    EventSystem:postEvent(EVENTTYPE_LOBBY, EVENTID_LOBBY_COME_BACK_FIRSTBATTLE_START_MATCH)
  end)
end
function logic_return_activity:StartReturnGuideMatchAfterFaceSlap(modeName, bWaitFaceSlap)
  if self._pendingFaceSlapMatch then
    return
  end
  self._pendingFaceSlapMatch = true
  self._pendingFaceSlapCounting = false
  self._pendingMatchModeName = modeName or ""
  EventSystem:registEvent(EVENTTYPE_STATE, EVENTID_ON_MODE_POST_SWITCH, self._OnModeSwitchCancelGuideMatch, self)
  if bWaitFaceSlap == false then
    self:_StartGuideMatchCountdown()
    return
  end
  local NewFaceSlapSystem = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.NewFaceSlapSystem)
  if NewFaceSlapSystem and NewFaceSlapSystem:IsSlapEnd() then
    self:_StartGuideMatchCountdown()
  else
    EventSystem:registEvent(EVENTTYPE_LOBBY, EVENTID_ON_FACESLAP_END, self._OnFaceSlapEnd, self)
  end
end
function logic_return_activity:_OnModeSwitchCancelGuideMatch()
  if self._pendingFaceSlapMatch and not self._pendingFaceSlapCounting then
    self:_CancelGuideMatch()
  end
end
function logic_return_activity:_CancelGuideMatch()
  self._pendingFaceSlapMatch = false
  self._pendingFaceSlapCounting = false
  EventSystem:unregistEvent(EVENTTYPE_LOBBY, EVENTID_ON_FACESLAP_END, self._OnFaceSlapEnd, self)
  EventSystem:unregistEvent(EVENTTYPE_STATE, EVENTID_ON_MODE_POST_SWITCH, self._OnModeSwitchCancelGuideMatch, self)
end
function logic_return_activity:_OnFaceSlapEnd()
  EventSystem:unregistEvent(EVENTTYPE_LOBBY, EVENTID_ON_FACESLAP_END, self._OnFaceSlapEnd, self)
  self:_StartGuideMatchCountdown()
end
function logic_return_activity:_StartGuideMatchCountdown()
  self._pendingFaceSlapCounting = true
  EventSystem:unregistEvent(EVENTTYPE_LOBBY, EVENTID_ON_FACESLAP_END, self._OnFaceSlapEnd, self)
  EventSystem:unregistEvent(EVENTTYPE_STATE, EVENTID_ON_MODE_POST_SWITCH, self._OnModeSwitchCancelGuideMatch, self)
  local countdown = 1
  local modeName = self._pendingMatchModeName or ""
  local function Tick()
    if not self._pendingFaceSlapMatch then
      return
    end
    if countdown <= 0 then
      self._pendingFaceSlapMatch = false
      self._pendingFaceSlapCounting = false
      EventSystem:postEvent(EVENTTYPE_LOBBY, EVENTID_LOBBY_COME_BACK_FIRSTBATTLE_START_MATCH)
      self:_WatchMatchStarted()
      return
    end
    ShowNotice(LocUtil.LocalizeResFormat(9551, countdown, modeName), true, 1)
    countdown = countdown - 1
    self:AddTimerOnce(1, Tick)
  end
  Tick()
end
function logic_return_activity:_WatchMatchStarted()
  if self._watchingGuideMatch then
    return
  end
  self._watchingGuideMatch = true
  EventSystem:registEvent(EVENTTYPE_MATCH, EVENTID_MATCH_UPDATE_STATUS, self._OnMatchStatusForGuide, self)
end
function logic_return_activity:_OnMatchStatusForGuide()
  local MatchSystem = require("client.slua.logic.match.logic_match")
  if MatchSystem.nMatchStatus == ENUM_MatchStatus.Matching then
    EventSystem:unregistEvent(EVENTTYPE_MATCH, EVENTID_MATCH_UPDATE_STATUS, self._OnMatchStatusForGuide, self)
    self._watchingGuideMatch = false
    ShowNotice(LocUtil.GetLocalizeResStr(9858), true)
  end
end
function logic_return_activity:_InstallJumpCancelHook()
  if _bJumpCancelHookInstalled then
    return
  end
  _bJumpCancelHookInstalled = true
  local ui_jump_manager = require("client.common.uibase.ui_jump_manager")
  _origOpenJumpModule = ui_jump_manager.OpenJumpModule
  local selfRef = self
  function ui_jump_manager.OpenJumpModule(jumpModuleID, ctorData)
    selfRef:_OnJumpCancelGuideMatch(jumpModuleID)
    return _origOpenJumpModule(jumpModuleID, ctorData)
  end
end
function logic_return_activity:_UninstallJumpCancelHook()
  if _bJumpCancelHookInstalled and _origOpenJumpModule then
    local ui_jump_manager = require("client.common.uibase.ui_jump_manager")
    ui_jump_manager.OpenJumpModule = _origOpenJumpModule
  end
  _bJumpCancelHookInstalled = false
end
function logic_return_activity:_OnJumpCancelGuideMatch(jumpModuleID)
  if not self._pendingFaceSlapMatch or self._pendingFaceSlapCounting then
    return
  end
  if self:_IsSlapModule(jumpModuleID) then
    return
  end
  log_format("logic_return_activity: jump to module[%s] during face-slap wait, cancel pending guide match", tostring(jumpModuleID))
  self:_CancelGuideMatch()
end
function logic_return_activity:_IsSlapModule(jumpModuleID)
  local NewFaceSlapSystem = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.NewFaceSlapSystem)
  local tFaceSlapCategory = NewFaceSlapSystem.tFaceSlapCategory
  if not tFaceSlapCategory then
    return false
  end
  for _, info in ipairs(tFaceSlapCategory) do
    if info and info.ModuleID == jumpModuleID then
      return true
    end
  end
  return false
...
function logic_return_activity:GetMenuList()
  ...
  if AssemblyActivitySystem.HasActivity() and isActivityOpen then
    table.insert(tabList, return_activity_macro.Enum_MenuID.AssemblyTeamUp)
  end
...
function logic_return_activity:StartActExpireTimer()
  if self._actExpireTimer then
    return
  end
  local logic_return_activity_utils = require("client.slua.logic.return_activity.logic_return_activity_utils")
  self._actExpireTimer = self:AddTimerLoop(0, function()
    if not logic_return_activity_utils.IsActInProgress() then
      self:StopActExpireTimer()
...
function logic_return_activity:StopActExpireTimer()
  if self._actExpireTimer then
    self:RemoveTimer(self._actExpireTimer)
    self._actExpireTimer = nil
  end
end
function logic_return_activity:EnterMainUI(menuId, bFromClick, ignoreQueue, openSource)
  local logic_return_activity_utils = require("client.slua.logic.return_activity.logic_return_activity_utils")
  if not logic_return_activity_utils.IsActInProgress() then
  ...
  local ParamTable
  if ignoreQueue or bFromClick then
    local ui_show_queue_config = require("client.common.uibase.ui_show_queue_config")
    ParamTable = ui_show_queue_config.GetParamTable(nil, "Click")
  end
  UIManager.ShowUI(UIManager.UI_Config.ReturnActivity_Main_UIBP, menuId, openSource, ParamTable)
  local tlog_report_utils = require("client.slua.config.tlog.tlog_report_utils")
  local TimeUtil = require("client.common.time_util")
...
function logic_return_activity:ShowFBUI(ignoreQueue)
  ...
  if not self.isShowFBSlap then
    ...
    UIManager.ShowUI(UIManager.UI_Config.ReturnActivity_First_UIBP, ParamTable)
    self.isShowFBSlap = true
  end
end
function logic_return_activity:ShowSignRewardUI(ignoreQueue)
  local ParamTable
  if ignoreQueue then
    local ui_show_queue_config = require("client.common.uibase.ui_show_queue_config")
    ParamTable = ui_show_queue_config.GetParamTable(nil, "IsSlapUI")
  end
  UIManager.ShowUI(UIManager.UI_Config.ReturnActivity_7days02_UIBP, ParamTable)
end
function logic_return_activity:ShowRebateUI(ignoreQueue)
  local ParamTable
  if ignoreQueue then
    local ui_show_queue_config = require("client.common.uibase.ui_show_queue_config")
    ParamTable = ui_show_queue_config.GetParamTable(nil, "IsSlapUI")
  end
  UIManager.ShowUI(UIManager.UI_Config.Return_Packs_Popup_UIBP, ParamTable)
end
function logic_return_activity:ShowModeSelectUI(ignoreQueue)
  local logic_return_activity_first_battle = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.logic_return_activity_first_battle)
  if not logic_return_activity_first_battle:GetModeIntroductionList() then
    log(bWriteLog and "logic_return_activity:ShowModeSelectUI - GetModeIntroductionList is nil, skip")
    return
  end
  local ParamTable
  if ignoreQueue then
  ...
  UIManager.ShowUI(UIManager.UI_Config.Return_ModeSelect_UIBP, nil, return_activity_macro.ReturnFrontTLog.OpenSource.LoginPopup, ParamTable)
end
function logic_return_activity:ShowFBGuideUI(ignoreQueue)
  local ParamTable
  if ignoreQueue then
    local ui_show_queue_config = require("client.common.uibase.ui_show_queue_config")
    ParamTable = ui_show_queue_config.GetParamTable(nil, "IsSlapUI")
  end
  UIManager.ShowUI(UIManager.UI_Config.ReturnActivity_Openning_Page_Slap_UIBP, ParamTable)
end
