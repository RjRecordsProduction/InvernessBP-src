...
local channelConfig = {
  ...
  [macro.Channel.channelGlobalPlanCH] = "client.slua.logic.lobby_chat.hall.Logic_ChatChannelGlobalHall",
  [macro.Channel.channelCurPlanCH] = "client.slua.logic.lobby_chat.hall.Logic_ChatChannelCurrentHall",
...
function logic_chat_main.OpenChatMainByHouseKeeperId(uid)
  ...
  local logic_chat_channel_friend = require("client.slua.logic.lobby_chat.logic_chat_channel_friend")
  logic_chat_channel_friend.CurrentGid = tostring(uid)
  log(bWriteLog and "logic_chat_main.OpenChatMainByHouseKeeperId currentChannel" .. tostring(logic_chat_main.currentChannel))
  logic_chat_main.OpenChatMainUI()
end
...
function logic_chat_main.OpenChatMainUI()
  local time_ticker = require("common.time_ticker")
  log(bWriteLog and "logic_chat_main.OpenChatMainUI() currentChannel 1", logic_chat_main.currentChannel)
  time_ticker.AddTimerOnce(0.1, function()
    log(bWriteLog and "logic_chat_main.OpenChatMainUI() currentChannel 2", logic_chat_main.currentChannel)
  ...
  if res == NetErrorCode_NONE then
    if tabContent then
      ...
      elseif tabContent.msgType == macro.GroupBuyChannelsMsgType then
        ShowNotice(102031)
      elseif tabContent.msgType == macro.BargainFriendsMsgType then
        ShowNotice(102031)
      elseif tabContent.msgType == macro.BargainCorpsMsgType then
        ShowNotice(102031)
      elseif tabContent.msgType == macro.BargainChannelsMsgType then
        ShowNotice(102031)
      elseif tabContent.msgType == macro.FlashMatchTeamInvite then
        ShowNotice(102031)
      elseif tabContent.msgType == macro.PromotionLookbackShare then
        ShowNotice(102031)
        if tabContent.share_report_data then
          local ShareMgr = require("client.logic.share.share_logic")
          ShareMgr.ShareSuccReq(tabContent.share_report_data.sceneType, 0, tabContent.share_report_data.nItemId)
        end
      end
  ...
  elseif nextState == GameStatus.Fighting then
    if not GameStatus.IsInMainCity() then
      ...
      if modeSystem.IsSocialIslandMode() then
        ...
        if PlanPH_GamePlay_Tools.IsPHomeMode() or GameStatus.IsCollectionHallMode() then
          log(bWriteLog and "logic_chat_main.OnModePostSwitch PHomeMode or CollectionHallMode")
          return
        end
...
function logic_chat_main.GetHeadDataByProfile(profile)
  ...
  if uid == DataMgr.roleData.uid then
    ...
    if NicknameColorManager then
      headdata.NicknameColor = NicknameColorManager:GetUserData(uid)
    end
...
function logic_chat_main.InitMyHeadData(headdata)
  ...
  local NicknameColorManager = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.NicknameColorManager)
  if NicknameColorManager then
    headdata.NicknameColor = NicknameColorManager:GetUserData(DataMgr.roleData.uid)
  end
end
...
function logic_chat_main.SendPromotionLookbackMsg(promotion_share_data, share_report_data)
  log(bWriteLog and "logic_chat_main.SendPromotionLookbackMsg enter")
  log_tree(bWriteLog and "logic_chat_main.SendPromotionLookbackMsg promotion_share_data", promotion_share_data)
  local logic_chat_channel_world = require("client.slua.logic.lobby_chat.logic_chat_channel_world")
  local satisfy = logic_chat_channel_world.CheckHasRightToChat()
  if not satisfy then
    log(bWriteLog and "logic_chat_main.SendPromotionLookbackMsg has no right to chat")
    return
  end
  if logic_chat_channel_world.current_topicId == macro.TopicLbsPredefinedChannel then
  ...
  local msg = {}
  msg.msgType = macro.PromotionLookbackShare
  msg.text = LocUtil.GetLocalizeResStr(85474)
  msg.topic = macro.TopicWorldChannel
  msg.  msg.  log(bWriteLog and "logic_chat_main.SendPromotionLookbackMsg msgType=" .. tostring(msg.msgType) .. ", topic=" .. tostring(msg.topic))
  local msgId = logic_chat_main.CacheMsg(msg)
  log(bWriteLog and "logic_chat_main.SendPromotionLookbackMsg CacheMsg done, msgId=" .. tostring(msgId))
...
function logic_chat_main.GetPlanCHVisitChatEntranceLogic()
  return ModuleManager.GetModule(ModuleManager.CommonModuleConfig.Logic_PlanCHVisitChatEntranceModule)
end
...
function logic_chat_main.IsSelectToShield(chat_content)
  if not chat_content or not chat_content.msgType then
    return false
  end
  local logic_shield = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.logic_shield)
  if not logic_shield then
    return false
  end
  local shielded = logic_shield:IsMsgTypeShielded(chat_content.msgType)
  log(bWriteLog and "logic_chat_main.IsSelectToShield msgType:" .. tostring(chat_content.msgType) .. " shielded:" .. tostring(shielded))
  return shielded
end
...
function logic_chat_main.SetChatEntranceMsg(chatMsg)
  ...
  local entranceGetters = {
    "GetLogicChatEntrance",
    "GetIslandChatEntranceLogic",
    "GetPlanPHEditChatEntranceLogic",
    "GetPlanPHVisitChatEntranceLogic",
    "GetPlanCHVisitChatEntranceLogic"
  }
  for _, sFunName in ipairs(entranceGetters) do
    local cObj_chatEntranceModule = logic_chat_main[sFunName]()
    if cObj_chatEntranceModule then
      cObj_chatEntranceModule:ReceiveNewMsg(chatMsg)
    end
  end
end
