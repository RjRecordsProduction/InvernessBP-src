      ...
      for k = 1, #friend.messageList do
        if k > index then
          ...
          if msgTable.msgType ~= chat_macro.ChatSecurityRemind and msgTable.msgType ~= chat_macro.LudoInviteMsgType then
            ...
            if msgTable.msgType == chat_macro.friendComebackMsgType and msgTable.content.other then
              msgTable.other = msgTable.content.other
              printf("LogicHistorySystem:SaveFriendChat friendComebackMsg packed, friendUid:%s, sendTime:%s, hasOther:%s", tostring(friendUid), tostring(msgTable.send_time), tostring(msgTable.other ~= nil))
            end
            ...
            if (msgTable.msgType == chat_macro.BargainFriendsMsgType or msgTable.msgType == chat_macro.BargainCorpsMsgType or msgTable.msgType == chat_macro.BargainChannelsMsgType) and msgTable.content.other then
              msgTable.other = msgTable.content.other
              if not msgTable.msg or msgTable.msg == "" then
                msgTable.msg = tostring(msgTable.msgType)
              end
            end
