...
function logic_chat_channel_manager:UpdateChannelList()
  ...
  log_tree(bWriteLog and "logic_chat_channel_manager:UpdateChannelList, self.chatChannelList is:", self.chatChannelList)
end
...
function logic_chat_channel_manager:UpdateOneChannelData(channelId)
  ...
  if channelConfig == nil then
    log(bWriteLog and "logic_chat_channel_manager:UpdateOneChannelData error, channelId is:" .. tostring(channelId))
    return
  end
...
function logic_chat_channel_manager:OnTxmissionStateChange()
  log(bWriteLog and "logic_chat_channel_manager:OnTxmissionStateChange")
...
function logic_chat_channel_manager:OnTeamChange()
  log(bWriteLog and "logic_chat_channel_manager:OnTeamChange")
  local chat_macro = require("client.slua.logic.lobby_chat.chat_macro")
...
function logic_chat_channel_manager:OnUpdateSwitch()
  if not GameStatus.IsInLobbyOrMainCity() then
    log(bWriteLog and "logic_chat_channel_manager:OnUpdateSwitch return of not GameStatus.IsInLobbyOrMainCity()")
    return
  end
  log(bWriteLog and "logic_chat_channel_manager:OnUpdateSwitch")
  self:AddTimerOnce(1, function()
    self:UpdateChannelList()
  end)
end
function logic_chat_channel_manager:OnWaitingRoomStateChange()
  log(bWriteLog and "logic_chat_channel_manager:OnWaitingRoomStateChange")
  self:UpdateChannelList()
end
...
function logic_chat_channel_manager:OnPostSwitchGameStatus(preState, nextState)
  log(bWriteLog and "logic_chat_channel_manager:OnPostSwitchGameStatus, preState is:" .. tostring(preState) .. ";  next is:" .. tostring(nextState))
  if nextState == GameStatus.Lobby then
    log(bWriteLog and "logic_chat_channel_manager:OnPostSwitchGameStatus, back to lobby")
    self:UpdateChannelList()
  elseif nextState == GameStatus.Fighting then
    ...
    if modeSystem.IsSocialIslandMode() then
      log(bWriteLog and "logic_chat_channel_manager:OnPostSwitchGameStatus, enter socialLand")
      self:UpdateChannelList()
    ...
    elseif GameStatus.IsCollectionHallMode() then
      log(bWriteLog and "logic_chat_channel_manager:OnPostSwitchGameStatus, enter collection hall")
      self:UpdateChannelList()
    end
  end
end
...
function logic_chat_channel_manager:UpdateChannelRedPoint(channelId, isShow)
  log(bWriteLog and "logic_chat_channel_manager:UpdateChannelRedPoint, channelId is:" .. tostring(channelId) .. "; isShow:" .. tostring(isShow))
