...
function AdjustSystem:RegistEvents()
  self:AddCommonEvent(EVENTTYPE_GAMELET, EVENTID_GAMELET_ACT_CENTER_READY_UPDATED, self.AdjustReadyJump, self)
end
...
function AdjustSystem:CheckAdjustJumpTo(bFromHome)
  ...
  local isReadyState = logic_gamelet_interface:IsInterfaceReady(989)
  log(bWriteLog and "AdjustSystem:CheckAdjustJumpTo isReadyState = " .. tostring(isReadyState) .. " validDeepLink = " .. tostring(validDeepLink) .. " bFind = " .. tostring(string.find(validDeepLink, "module=100031")))
  if validDeepLink ~= "" and not isReadyState and string.find(validDeepLink, "module=100031") then
    log(bWriteLog and "AdjustSystem:CheckAdjustJumpTo return pxiui 989 is not ready of validDeepLink = " .. tostring(validDeepLink))
    self.bNotEnter = true
    return
  end
...
function AdjustSystem:_AdjustJumpTo(validDeepLink)
  ...
  log(bWriteLog and "[BargainShare] _RestoreBargainParamsIfNeeded")
  url = self:_RestoreBargainParamsIfNeeded(url)
...
local ParseBargainInviteCode = function(invitecode)
  if not invitecode or invitecode == "" then
    return nil
  end
  invitecode = invitecode:gsub("%%3A", ":"):gsub("%%3a", ":")
  local parts = {}
  for part in invitecode:gmatch("[^:]+") do
  ...
  return {
    cfg_id = tonumber(parts[1]) or 0,
    owner_uid = tonumber(parts[2]) or 0,
    bargain_id = parts[3],
    tabId = parts[4],
    from_share = parts[5]
  }
end
function AdjustSystem:_RestoreBargainParamsIfNeeded(url)
  if not url or not url:find("module=9000230") then
    return url
  end
  local has_bargain_id = url:match("bargain_id=")
  if has_bargain_id then
    return url
  end
  local invitecode = url:match("invitecode=([^&]+)")
  if not invitecode then
    log(bWriteLog and "[BargainShare] _RestoreBargainParamsIfNeeded: invitecode not found")
    return url
  end
  local bargain_info = ParseBargainInviteCode(invitecode)
  if not bargain_info then
    log(bWriteLog and "[BargainShare] _RestoreBargainParamsIfNeeded: failed to parse invitecode=" .. tostring(invitecode))
    return url
  end
  local restored_params = string.format("&tabId=%s&from_share=%s&bargain_id=%s&owner_uid=%s&cfg_id=%s", bargain_info.tabId, bargain_info.from_share, bargain_info.bargain_id, bargain_info.owner_uid, bargain_info.cfg_id)
  url = url .. restored_params
  log(bWriteLog and "[BargainShare] _RestoreBargainParamsIfNeeded: url=" .. url)
  return url
end
...
function AdjustSystem:AdjustReadyJump()
  if not self.bNotEnter then
    return
  end
  local validDeepLink = self:GetDeepLinkUrl()
  if not validDeepLink or validDeepLink == "" then
  ...
  local logic_gamelet_interface = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.logic_gamelet_interface)
  local isReadyState = logic_gamelet_interface:IsInterfaceReady(989)
  if isReadyState and string.find(validDeepLink, "module=100031") then
    self:_AdjustJumpTo(validDeepLink)
    self.bNotEnter = false
  end
end
