...
local LastTime = {}
local recordTime = 0
local ErrorMessageHandler = function(jsonStr, appType)
  local info = string.format("HostedProtoBridge frequency: appType:%s, jsonStr:%s", appType, jsonStr)
...
function HostedProtoBridge:OnReceiveMessage(appType, appId, jsonStr, data)
  ...
  if not GameStatus.IsInLobbyOrSpecialFighting() and not BattleAppIdAllowMap[appId] then
    log(bWriteLog and "HostedProtoBridge:OnReceiveMessage not response because not in lobby and appId = " .. tostring(appId))
    return
  end
  if not data then
...
function HostedProtoBridge:SendMessage(appType, data, appId)
  ...
  local ProxyConfig = HostedProxyConfig[appType] or HostedProxyConfig[HostedConst.HostedType.Gamelet]
  if not ProxyConfig then
    log_error(bWriteLog and string.format("HostedProtoBridge:OnSendMessage. appType=%s, ProxyConfig isn't valid", tostring(appType)))
    return
  end
  local SendFuncName = ProxyConfig.sendFunc
  if not SendFuncName or SendFuncName == "" then
  ...
  if ProxyConfig.logicName and ProxyConfig.logicName ~= "" then
    ...
    if not Func then
      log_error(bWriteLog and string.format("HostedProtoBridge:OnSendMessage. func don't exists:%s", tostring(SendFuncName)))
      return
    end
...
function HostedProtoBridge:IsFrequencyLimit(jsonStr, data, appType, appId, protoCfg)
  ...
  if protoCfg.Static then
    ...
    if StaticDataLoaded[appType][name] then
      ErrorMessageHandler(jsonStr, appType)
...
function HostedProtoBridge:SendFrequently(appType, appId, data)
  if not appType or not appId then
    return
  end
...
function HostedProtoBridge:SendToAllHosted(data)
  local HostedConst = require("client.slua.logic.HostedProtoBridge.HostedConst")
  self:SendMessage(HostedConst.HostedType.Pandora, data)
  local logic_gamelet_interface = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.logic_gamelet_interface)
...
function HostedProtoBridge:ClearStaticState(appType)
  log(bWriteLog and "HostedProtoBridge:ClearStaticState.appType" .. appType)
  StaticDataLoaded[appType] = {}
end
