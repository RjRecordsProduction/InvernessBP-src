...
function logic_group_buying:OnGetNewGroupInfo(group_list, gifts_id_list, gifts_config, gifts_buy_config, gifts_buy_info, groups_info, other_config)
  ...
  for id, v in pairs(gifts_id_list) do
    if v then
      ...
      if gifts_config[id] and nowTime > gifts_config[id].begin_time and nowTime < gifts_config[id].end_time then
...
function logic_group_buying:SortGiftIdListByPriority(gift_id_list)
  ...
  local list = {}
  for _, id in pairs(gift_id_list) do
    if self.gifts_config[id] then
      local TimeUtil = require("client.common.time_util")
      local nowTime = TimeUtil.GetServerTimeInSec()
      if self.gifts_config[id] and nowTime > self.gifts_config[id].begin_time and nowTime < self.gifts_config[id].end_time then
