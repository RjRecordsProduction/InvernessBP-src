...
function logic_mode_selection:ShowMainUI(params, isFromLobby)
  ...
  local ModeSelectionUtil = require("GameLua.Mod.Lobby.Base.ModeSelection.ModeSelectionUtil")
  if not ModeSelectionUtil.IsModeSelectionModDownloaded() then
    printf("logic_mode_selection:ShowMainUI ModeSelection mod not downloaded")
    return
  end
...
function logic_mode_selection:SetSelectInfo()
  ...
  self.old_teamNum = self.teamNum
  if not modeCfg then
    log_error(string.format("logic_mode_selection:SetSelectInfo get nil config by id = %s", self.matchMode))
    return
  end
  self.perspective = modeCfg.PersonPerspective
  self.teamNum = modeCfg.MaxTeamPlayerNum
end
function logic_mode_selection:ConvertMenuTable(menu_object)
  local menuTable = CDataTable.GetSplitTable("Lobby", "ModeSelection", "view_menu_table")
  if not menuTable then
    log(bWriteLog and "logic_mode_selection:ConvertMenuTable no menuTable")
    return
  end
  local function processMenu(menu, level)
    level = level or 0
    if menu and menu.sub_menus and next(menu.sub_menus) then
      log(bWriteLog and "logic_mode_selection:processMenu - Processing level " .. level .. " menu with " .. #menu.sub_menus .. " submenus")
      for k, v in pairs(menu.sub_menus) do
        local id = tonumber(v.id)
        if id then
          local menuCfg = menuTable[id]
          if menuCfg then
            log(bWriteLog and "logic_mode_selection:processMenu - Setting images for menu " .. id)
            v.menu_bg = menuCfg.menu_bg or ""
            v.icon = menuCfg.icon or ""
          else
            log(bWriteLog and "logic_mode_selection:processMenu - Warning: Menu config not found for ID " .. id)
          end
        else
          log(bWriteLog and "logic_mode_selection:processMenu - Skipping invalid menu ID: " .. tostring(v.id))
        end
        if v.sub_menus and next(v.sub_menus) then
          log(bWriteLog and "logic_mode_selection:processMenu - Recursing to level " .. level + 1)
          processMenu(v, level + 1)
        end
      end
    end
  end
  processMenu(menu_object)
end
function logic_mode_selection:ConvertViewMultiTable(selection_details)
  if not selection_details or not next(selection_details) then
    return
  end
  local menuTable = CDataTable.GetSplitTable("Lobby", "ModeSelection", "view_multi_select_table")
  if not menuTable then
    log(bWriteLog and "logic_mode_selection:ConvertViewMultiTable no client cfg")
    return
  end
  for k, v in pairs(selection_details) do
    local menuCfg = menuTable[k]
    if menuCfg then
      v.big_bg = menuCfg.big_bg
      v.small_bg = menuCfg.small_bg
      v.lobby_bg = menuCfg.lobby_bg
    end
  end
end
function logic_mode_selection:ConvertViewTable(default_view, viewId)
  if not default_view then
    return
  end
  local viewId = tonumber(viewId)
  if not viewId then
    log(bWriteLog and "logic_mode_selection:ConvertViewTable viewId:" .. tostring(viewId))
    return
  end
  local viewTable = CDataTable.GetSplitTableData("Lobby", "ModeSelection", "view_contain_table", viewId)
  if viewTable then
    default_view.big_bg = viewTable.big_bg or ""
    default_view.small_bg = viewTable.small_bg or ""
    default_view.lobby_bg = viewTable.lobby_bg or ""
    default_view.icon = viewTable.icon or ""
    default_view.group_icon = viewTable.group_icon or ""
    default_view.group_icon_background = viewTable.group_icon_background or ""
  else
    log(bWriteLog and "logic_mode_selection:ConvertViewTable no client cfg viewId:" .. tostring(viewId))
  end
end
...
function logic_mode_selection:CheckIsTargetThemeView(targetViewID, targetMenuID)
  ...
  if not viewInfo then
    log_warning_format("logic_mode_selection:CheckIsTargetThemeView, not view. targetViewID = [%s]", targetViewID)
    return false
  end
...
function logic_mode_selection:SetSelectView(view_id, filter_info, button_close, bDontneedCloseMain)
  if not default then
    return
  end
  ...
  if #view_ids == 0 then
    if type(view_id) == "number" then
      log_error("logic_mode_selection:SetSelectView get nil info by id= " .. tostring(view_id))
    elseif type(view_id) == "table" then
      log_error("logic_mode_selection:SetSelectView get nil info by id= " .. tostring(view_id[1]))
    end
  ...
  if not teamType[filter_info.perspective] then
    log_error("logic_mode_selection:SetSelectView invalid perspective id=" .. tostring(view_ids[1]) .. " perspective=" .. tostring(filter_info.perspective))
    filter_info.perspective = options.default_person
  end
  if not teamType[filter_info.perspective][filter_info.teamNum] then
    log_error("logic_mode_selection:SetSelectView invalid team num id=" .. tostring(view_ids[1]) .. " perspective=" .. tostring(filter_info.perspective) .. " team number=" .. tostring(filter_info.teamNum))
  ...
  if bDontneedCloseMain then
    log(bWriteLog and "logic_mode_selection:SetSelectView bDontneedCloseMain is true, not close main ui.")
  else
    self:DelayCloseMainUI()
  end
...
function logic_mode_selection:OpenModeSelectionDetails(viewID, popIndex)
  ...
  local ModeSelectionUtil = require("GameLua.Mod.Lobby.Base.ModeSelection.ModeSelectionUtil")
  if not ModeSelectionUtil.IsModeSelectionModDownloaded() then
    printf("logic_mode_selection:OpenModeSelectionDetails ModeSelection mod not downloaded")
    return
  end
...
function logic_mode_selection:FindAllArenaGameView()
  ...
  table.sort(AllArenaGameViewData, function(a, b)
    return a.id < b.id
  end)
...
function logic_mode_selection:OnGetModeShield(menu_object, default_view_id, _, sub_mode_to_view, default_viewid, sub_mode_to_mutiviews, selection_details)
  ...
  self:ConvertMenuTable(menu_object)
  self:ConvertViewMultiTable(selection_details)
  menu = menu_object
...
function logic_mode_selection:PlayAnimOnMatchSuccess()
  ...
  log(bWriteLog and "PlayAnimOnMatchSuccess " .. tostring(MatchModeMgrSystem.nInGameModeID) .. " view=" .. tostring(actViewId))
  if not actViewId then
    return false
  end
  if not self.viewId then
    log(bWriteLog and "PlayAnimOnMatchSuccess no valid viewId")
    return false
  end
  if not DataMgr.roleData.match_success_animation or DataMgr.roleData.match_success_animation ~= 1 then
    log(bWriteLog and "PlayAnimOnMatchSuccess no valid switch")
    return false
  end
  ...
  if equipedOpeningID ~= nil then
    ...
    if openingCfg and logic_roleInfo_opening:CheckAndDownloadOpeningBP(equipedOpeningID) then
      ...
      log(bWriteLog and "logic_mode_selection:PlayAnimOnMatchSuccess not openingCfg or not download")
    end
  end
  ...
  for _, v in pairs(uObj_animCfg) do
    if version_util.HigherVersion(current_version, v.MinVersion) and version_util.LowerVersion(current_version, v.MaxVersion) then
      ...
      if now >= nStartTime and now < nEndTime then
        local strAllViewID = string.gsub(tostring(v.AllViewID), "[{}%s]", "")
        local tAllViewId = StringUtil.Split(strAllViewID, ";")
  ...
  if not animInfo then
    log(bWriteLog and "PlayAnimOnMatchSuccess no valid animInfo")
    return false
  end
  local ModeSelectionUtil = require("GameLua.Mod.Lobby.Base.ModeSelection.ModeSelectionUtil")
  if not ModeSelectionUtil.IsModeSelectionModDownloaded() then
    printf("logic_mode_selection:PlayAnimOnMatchSuccess ModeSelection mod not downloaded, skip match anim")
    return false
  end
