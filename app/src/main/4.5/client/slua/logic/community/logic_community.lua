...
function logic_community.Init()
  ...
  EventSystem:registEvent(EVENTTYPE_UGC, EVENTID_UGC_MATCH_ENTER_MOD, logic_community.OnUGCMatchEnterMod)
...
function logic_community.InitModuleCfg()
  ...
  logic_community.ClubToGameCfg = {
    ...
    [BP_ENUM_MODULE_UGC_FAVORITE_PANEL] = {
      func = logic_community.OnJumpUGCFavoritePanel
    },
    [BP_ENUM_MODULE_UGC_AND_START_MATCH] = {
      func = logic_community.OnJumpUGCAndStartMatch
    }
  }
...
function logic_community.OnClubToGame(eventType, eventID, vars)
  ...
  if module_id == BP_ENUM_MODULE_QUIT_COMMUNITYH5 then
    if logic_community.CheckClubMatchSwitch() and vars and vars.subscribe_match == "1" then
      logic_community.ReqClubMatchSubscription(true)
    end
    if vars and vars.publish_success then
      logic_community.NotifyClubShareResult(vars.publish_success)
    end
  end
...
function logic_community._SendToCommunity(data)
  if type(data) == "table" then
    data = json.encode(data)
  end
  log(bWriteLog and string.format("logic_community._SendToCommunity data:%s", data))
  local bp_pluginBPLibrary = import("bp_pluginBPLibrary")
  if bp_pluginBPLibrary and bp_pluginBPLibrary.bp_pluginSendEvent then
    bp_pluginBPLibrary.bp_pluginSendEvent(data)
  end
end
function logic_community._OnGetWoWMapStatus(body)
  local modIds = body and body.modIds
  logic_community.GetWoWMapStatusInfo(modIds, function(statusInfo)
    statusInfo.action = "GetWoWMapStatusRtn"
    logic_community._SendToCommunity(statusInfo)
  end)
end
function logic_community._OnDownloadWoWMap(body)
  local modId = tonumber(body.modId)
  logic_community.DownloadWoWMap(modId, function(downloadResult)
    downloadResult.action = "DownloadWoWMapRtn"
    logic_community._SendToCommunity(downloadResult)
  end)
end
function logic_community._OnPauseWoWMap(body)
  local modId = tonumber(body.modId)
  local pauseResult = logic_community.PauseWoWMapDownload(modId)
  pauseResult.action = "PauseWoWMapRtn"
  logic_community._SendToCommunity(pauseResult)
end
logic_community.SnsNotifyGameHandlers = {
  GetWoWMapStatus = logic_community._OnGetWoWMapStatus,
  DownloadWoWMap = logic_community._OnDownloadWoWMap,
  PauseWoWMap = logic_community._OnPauseWoWMap
}
function logic_community.OnBpPluginNotify(methodId, retJson, extraJson)
  ...
  elseif jsonObj.message == "snsNotifyGame" then
    local body = jsonObj.body
    if not body or not body.action then
      log(bWriteLog and "logic_community.OnBpPluginNotify snsNotifyGame: body or action is nil")
      return
    end
    local action = body.action
    log(bWriteLog and string.format("logic_community.OnBpPluginNotify snsNotifyGame action:%s", tostring(action)))
    local handler = logic_community.SnsNotifyGameHandlers[action]
    if handler then
      handler(body)
    else
      log(bWriteLog and string.format("logic_community.OnBpPluginNotify snsNotifyGame: unknown action:%s", tostring(action)))
    end
  end
end
