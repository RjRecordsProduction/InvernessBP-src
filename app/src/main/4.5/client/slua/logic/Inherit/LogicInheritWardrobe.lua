...
function LogicInheritWardrobe:CacheXSuitStateInfo(StateInfo)
  self.XSuitStateInfo = {}
  for period, v in pairs(StateInfo or {}) do
    if not v[0] then
      self.XSuitStateInfo[period] = {}
      self.XSuitStateInfo[period][0] = v
    else
      self.XSuitStateInfo[period] = v
    end
  end
end
...
function LogicInheritWardrobe:ChangeXSuitStateInfo(period, branchId, data)
  self.XSuitStateInfo = self.XSuitStateInfo or {}
  self.XSuitStateInfo[period] = self.XSuitStateInfo[period] or {}
  self.XSuitStateInfo[period][branchId] = self.XSuitStateInfo[period][branchId] or {}
  self.XSuitStateInfo[period][branchId].cur_state = data.cur_state
end
