...
function PlayerStatusUtil.CheckCanJoinFriendGame(status, callback)
  ...
  if PlayerStatusUtil.IsInHome(status) or PlayerStatusUtil.IsMainCity(status) or PlayerStatusUtil.IsInCollectionHall(status) or PlayerStatusUtil.IsIsland(status) then
