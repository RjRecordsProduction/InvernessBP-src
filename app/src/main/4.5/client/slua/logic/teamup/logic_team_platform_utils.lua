...
function logic_team_platform_utils.CheckSameMainCityThenJoin(uid, isSameMainCity, callback)
  local TeamUpNewSystem = require("client.slua.logic.teamup.logic_team_up")
  if uid and tonumber(uid) == TeamUpNewSystem.GetSelfUID() then
    printf("logic_team_platform_utils:CheckSameMainCityThenJoin skip, cannot join own recruit, uid=%s", uid)
    ShowNotice(110045)
    return
  end
  if not (isSameMainCity and ENUM_RECRUIT_OPENMIC) or isSameMainCity ~= ENUM_RECRUIT_OPENMIC.HAVETO then
    callback()
    return
  end
  local reqSwitchMainCity = function()
    ...
    logic_main_city_enter_report.SetReportData(nil, nil, "TeamRecruit")
    main_city_switch_util.ReqEnterMainCityByUID(uid, MainCityMatchConfig.Enum_Match_From.Normal)
  end
  ...
  local tb = LogicPlayerPrefs.LoadFileToData_N(PlayerPrefsConfig.eRecruitSameMainCityIgnore)
  if tb and type(tb.ignoreTime) == "number" then
    local TimeUtil = require("client.common.time_util")
    if TimeUtil.GetServerTimeInSec() - tb.ignoreTime < 86400 then
      printf("logic_team_platform_utils:CheckSameMainCityThenJoin ignore hit, silent switch main city and join")
      reqSwitchMainCity()
      callback()
      return
    end
  end
  printf("logic_team_platform_utils:CheckSameMainCityThenJoin show same main city dialog")
  local CommonMsgBoxMgr = require("client.slua.logic.common.logic_common_msg_box")
  local title = LocUtil.GetLocalizeResStr(101001)
  local msg = LocUtil.GetLocalizeResStr(77917)
  local btnOK = LocUtil.GetLocalizeResStr(5078)
  local extraData = {
    isShowCheckBox = true,
    checkBoxText = LocUtil.GetLocalizeResStr(12096)
  }
  CommonMsgBoxMgr.Show(2, title, msg, function(isCheck)
    if isCheck then
      local TimeUtil = require("client.common.time_util")
      LogicPlayerPrefs.SaveDataToFile_N({
        ignoreTime = TimeUtil.GetServerTimeInSec()
      }, PlayerPrefsConfig.eRecruitSameMainCityIgnore)
      printf("logic_team_platform_utils:CheckSameMainCityThenJoin saved ignore time")
    end
    printf("logic_team_platform_utils:CheckSameMainCityThenJoin confirmed, req main city switch and execute join callback")
    reqSwitchMainCity()
    callback()
  end, nil, btnOK, nil, extraData)
end
