...
function LogicPHomeStore:_GetSetCfgsByLabelId(labelId)
  if not self._setCfgsByLabel then
    local BasicDataServerTable = ModuleManager.GetModule(ModuleManager.DataModuleConfig.BasicDataServerTable)
    ...
    for storeId, setCfg in pairs(setCfgs) do
      local lid = setCfg.label_ID
      local byId = indexed[lid]
      if not byId then
        byId = {}
        indexed[lid] = byId
      end
    ...
    self._setCfgsByLabel = indexed
  end
  return self._setCfgsByLabel[labelId] or {}
end
function LogicPHomeStore:_GetShopCfgsByLabel(labelA, labelB)
  local byB = self._shopCfgsByLabel and self._shopCfgsByLabel[labelA]
  if not byB then
    return {}
  end
  if labelB == nil then
    local result = {}
    for _, cfgsForB in pairs(byB) do
      for storeId, storeCfg in pairs(cfgsForB) do
        result[storeId] = storeCfg
      end
    end
    return result
  end
  return byB[labelB] or {}
end
function LogicPHomeStore:fillStoreObj(storeObj)
  ...
  elseif FirstTab == PHomeStoreConst.TabType.Recommend then
    ...
    local recommendCfg = recommendCfgs[SecondTab]
    if recommendCfg then
      if PHomeStoreUtils.CheckVersionPassed_LoopOptimize(recommendCfg.open_ver, recommendCfg.close_ver, recommendCfg.sale_begin_time, recommendCfg.sale_end_time, clientVersion, now, FnCmpVersion, FnCmpTime, timeZone, refTimeTable) then
        local setItems = StringUtil.Split(recommendCfg.set_items_list, "|")
        local group_list = {}
        ...
        log(bWriteLog and string.format("LogicPHomeStore.fillStoreObj recommendCfg not CheckVersionPassed id:%s", SecondTab))
      end
    end
  elseif FirstTab == PHomeStoreConst.TabType.Set then
    if SecondTab == PHomeStoreConst.SubTabType.Mould then
      local matchedShopCfgs = self:_GetShopCfgsByLabel(PHomeStoreConst.TabType.Mould, nil)
      for storeId, storeCfg in pairs(matchedShopCfgs) do
        if false == PHomeStoreUtils.CheckVersionPassed_LoopOptimize(storeCfg.open_ver, storeCfg.close_ver, storeCfg.sale_begin_time, storeCfg.sale_end_time, clientVersion, now, FnCmpVersion, FnCmpTime, timeZone, refTimeTable) then
          log(bWriteLog and string.format("LogicPHomeStore.fillStoreObj CheckVersionPassed failed. storeId:%s", storeId))
        elseif false == PHomeStoreUtils.CheckAppIdList(storeCfg.app_list, itop_app_id) then
          log(bWriteLog and string.format("LogicPHomeStore.fillStoreObj CheckAppIdList failed. storeId:%s", storeId))
        else
          items[#items + 1] = storeId
        end
      end
    else
      local matchedSetCfgs = self:_GetSetCfgsByLabelId(SecondTab)
      for storeId, setCfg in pairs(matchedSetCfgs) do
        local storeCfg = self:GetHomeStoreCfg(storeId)
        ...
        elseif false == PHomeStoreUtils.CheckAppIdList(storeCfg.app_list, itop_app_id) then
          ...
          local set_items_num_arr = StringUtil.Split(setCfg.set_items_num, "|")
          local set_items_list_arr = StringUtil.Split(setCfg.set_items_list, "|")
          local child_items = {}
          for i, setItem in ipairs(set_items_list_arr) do
            if setItem ~= "" and set_items_num_arr[i] ~= "" then
              child_items[#child_items + 1] = {
                phomeStoreId = tonumber(setItem),
                count = tonumber(set_items_num_arr[i])
              }
            end
          end
          items[#items + 1] = {
  ...
  elseif FirstTab == PHomeStoreConst.TabType.Prop or FirstTab == PHomeStoreConst.TabType.Mould then
    local matchedShopCfgs = self:_GetShopCfgsByLabel(FirstTab, nil)
    for storeId, storeCfg in pairs(matchedShopCfgs) do
      if false == PHomeStoreUtils.CheckVersionPassed_LoopOptimize(storeCfg.open_ver, storeCfg.close_ver, storeCfg.sale_begin_time, storeCfg.sale_end_time, clientVersion, now, FnCmpVersion, FnCmpTime, timeZone, refTimeTable) then
        log(bWriteLog and string.format("LogicPHomeStore.fillStoreObj CheckVersionPassed failed. storeId:%s", storeId))
      elseif false == PHomeStoreUtils.CheckAppIdList(storeCfg.app_list, itop_app_id) then
  ...
  elseif FirstTab == PHomeStoreConst.TabType.Chest then
    ...
    local matchedShopCfgs = self:_GetShopCfgsByLabel(FirstTab, SecondTab)
    for storeId, storeCfg in pairs(matchedShopCfgs) do
      if false == PHomeStoreUtils.CheckVersionPassed_LoopOptimize(storeCfg.open_ver, storeCfg.close_ver, storeCfg.sale_begin_time, storeCfg.sale_end_time, clientVersion, now, FnCmpVersion, FnCmpTime, timeZone, refTimeTable) then
        log(bWriteLog and string.format("LogicPHomeStore.fillStoreObj CheckVersionPassed failed. storeId:%s", storeId))
      elseif false == PHomeStoreUtils.CheckAppIdList(storeCfg.app_list, itop_app_id) then
...
function LogicPHomeStore:InitGuraanteAwardProgress(ret_code, exchange_coin, guarantee_cnts_bin, take_record_bin)
  ...
  if guraanteAward ~= nil then
    self.guraanteAwardProgress = guraanteAward
  end
  if take_record ~= nil then
...
function LogicPHomeStore:OnBatchTakeDrawGuaranteeAward(err_code, activity_id, success_items, take_record)
  activity_id = tonumber(activity_id)
  log_format(bWriteLog and "LogicPHomeStore:OnBatchTakeDrawGuaranteeAward err_code:%s activity_id:%s", tostring(err_code), tostring(activity_id))
  if not activity_id then
    return
  end
  if not self.guraanteAwardProgress then
  ...
  self.guraanteAwardProgress[activity_id].take_record = take_record or {}
  if err_code ~= 0 then
    self:ShowErrNotice(err_code)
    EventSystem:postEvent(EVENTTYPE_SOCIAL_ISLAND, EVENTID_SOCIAL_ISLAND_DRAW_MANOR_DRAW_CHANGE)
    return
  end
  local aggregated = {}
  if success_items then
    for _, seg in ipairs(success_items) do
      if seg and seg.items then
        for item_id, cnt in pairs(seg.items) do
          aggregated[item_id] = (aggregated[item_id] or 0) + cnt
        end
      end
    end
  end
  local allData = {}
  for item_id, cnt in pairs(aggregated) do
    if cnt and 0 < cnt then
  ...
  log_tree("LogicPHomeStore:OnBatchTakeDrawGuaranteeAward allData", allData)
...
function LogicPHomeStore:initHomeStoreCfgMap()
  ...
  local shopCfgsByLabel = {}
  for shopId, cfg in pairs(tb) do
    local a = cfg.label_A_ID
    local b = cfg.label_B_ID
    local byB = shopCfgsByLabel[a]
    if not byB then
      byB = {}
      shopCfgsByLabel[a] = byB
    end
    local byId = byB[b]
    if not byId then
      byId = {}
  ...
  self._  local t2 = slua.getMicroseconds()
  local diff = t2 - t1
