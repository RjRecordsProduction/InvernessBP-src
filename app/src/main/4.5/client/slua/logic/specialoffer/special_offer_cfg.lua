...
Cfg.BaseModules = {
  [Cfg.recharge] = true,
  [Cfg.ACTIVITY_TYPE_CONSUME_UC] = true,
  [Cfg.CONSUME_UC] = true
}
function Cfg.IsSimpleLobby()
...
function Cfg.IsModuleReachable(id)
  if not Cfg.IsSimpleLobby() then
    return true
  end
  return Cfg.BaseModules and Cfg.BaseModules[id] or false
end
function Cfg.IsSpecialOfferBaseDownloaded()
  local tUICfg = UIManager.UI_Config and UIManager.UI_Config.SpecialOffer_DownloadTip_UIBP
  local sUIPath = tUICfg and tUICfg.path
  if not sUIPath then
    log_error("Cfg:IsSpecialOfferBaseDownloaded SpecialOffer_DownloadTip_UIBP path nil")
    return false
  end
  local PufferConst = require("client.slua.logic.download.puffer_const")
  local PufferManager = require("client.slua.logic.download.puffer.puffer_manager")
  local nState = PufferManager.GetState(PufferConst.ENUM_DownloadType.ODPAK, {sUIPath})
  return nState == PufferConst.ENUM_DownloadState.Done
end
local id2ActId = {
...
local recharge, golden, recharge_purchase, ACTIVITY_TYPE_CONSUME_UC, JkGiftSet, LimitedSet, MaterialsGift, ConditionsGift, OPTIONAL_RECHARGE, CONSUME_UC, DailyFortunePack, DailySpecialBundle, Financial, DiscountDirect, TEMU, NewGroupBuy, WOWCoinRecharge = Cfg.recharge, Cfg.golden, Cfg.recharge_purchase, Cfg.ACTIVITY_TYPE_CONSUME_UC, Cfg.JkGiftSet, Cfg.LimitedSet, Cfg.MaterialsGift, Cfg.ConditionsGift, Cfg.OPTIONAL_RECHARGE, Cfg.CONSUME_UC, Cfg.DailyFortunePack, Cfg.DailySpecialBundle, Cfg.Financial, Cfg.DiscountDirect, Cfg.TEMU, Cfg.NewGroupBuy, Cfg.WOWCoinRecharge
...
Cfg.uiCdg = {
  ...
  [WOWCoinRecharge] = "UGC_WOWCoin_Recharge_UIBP"
}
Cfg.id2Func = {
...
Cfg.id2CheckShow = {
  ...
  [NewGroupBuy] = function()
    ...
    return logic_group_buying:IsShow() or logic_bargain:IsShow()
  end,
  [Cfg.WOWCoinRecharge] = function()
...
if PublishRegionMacros.IsJapanOrKorea() then
  ...
  Cfg.uiCdg[WOWCoinRecharge] = "UGC_WOWCoin_Recharge_JK_UIBP"
