...
function ReportPlatformCrashKit:SendWithCustomData(error, category, customData)
  if not Client or self.isEditor then
    return false
  end
  if not self:IsCanReport() then
    return false
  end
  self:_ConformSendWithCustomData(error, category, customData)
end
function ReportPlatformCrashKit:_ConformSendWithCustomData(error, category, customData)
  category = self:GetReportCategory(category)
  if not Client or self.isEditor then
    return
  end
  local UScriptHelperClient = import("ScriptHelperClient")
  if UScriptHelperClient and UScriptHelperClient.UQMCrashPostExceptionWithCustomData then
    local safeCustomData = {}
    if customData then
      for k, v in pairs(customData) do
        safeCustomData["X_CUST_" .. tostring(k)] = tostring(v)
      end
    end
    UScriptHelperClient.UQMCrashPostExceptionWithCustomData(category, tostring(error or ""), safeCustomData, 0)
    self.lastSendTime = TimeUtil.GetServerTimeInSec()
    self:AddServerTimeToCrashContext(self.lastSendTime)
  end
end
