...
function logic_account_sensitive_aciton:CheckFaceSlap()
  ...
  if not PublishRegionMacros.IsGlobalVersion() then
    log(bWriteLog and "logic_account_sensitive_aciton:CheckFaceSlap.  ")
    return
  end
  local SettingAccount = require("client.logic.setting.logic_setting_account")
  if not SettingAccount.CanBindMore() then
    log(bWriteLog and "logic_account_sensitive_aciton:CheckFaceSlap. CanBindMore  not ")
    return
  end
  ...
  self:CancelFaceSlap()
  local bindType = setting_macro.AccountNewOperationType.BindSocialRemind
  local logic_chat_channel_chat_room = require("client.slua.logic.lobby_chat.chatroom.logic_chat_channel_chat_room")
  if logic_chat_channel_chat_room.IsGuest() then
    bindType = setting_macro.AccountNewOperationType.GuestGuide
  end
  self:ShowCommonPopupUI(bindType)
end
