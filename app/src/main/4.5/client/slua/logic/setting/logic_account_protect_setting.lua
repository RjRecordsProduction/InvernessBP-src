...
function logic_account_protect_setting:IsNoticeNeedSyncRead(noticeInfo)
  if not noticeInfo or not next(noticeInfo) then
    return false
  end
  if not noticeInfo.iNoticeType or noticeInfo.iNoticeType == 0 then
    return false
  end
  local hasChange = noticeInfo.AChangeNotices and next(noticeInfo.AChangeNotices) ~= nil
  local hasRisk = noticeInfo.ARiskLoginNotices and next(noticeInfo.ARiskLoginNotices) ~= nil
  return hasChange or hasRisk
end
...
function logic_account_protect_setting:SetIsClickGo(isClick, from)
  ...
  if logic_account_protect_setting:IsNoticeNeedSyncRead(noticeInfo) then
