...
function LuckybackActivitySystem.do_exchange_by_activity_id_rsp(rs, myData, award_info, activity_id)
  ...
  if LuckybackActivitySystem.activityId2ShowPanelFunc[activity_id] then
    LuckybackActivitySystem.activityId2ShowPanelFunc[activity_id](arrayItemList)
  else
    local time_ticker = require("common.time_ticker")
    local Logic_CommonItemGet = require("client.slua.logic.common.CommonItemGet.Logic_CommonItemGet")
    if LuckybackActivitySystem.exchangeTimer then
      time_ticker.RemoveTimer(LuckybackActivitySystem.exchangeTimer)
      LuckybackActivitySystem.exchangeTimer = nil
    end
    local logic_xsuit_activity = ModuleManager.GetModule(ModuleManager.JumpModuleConfig.logic_xsuit_activity)
    if logic_xsuit_activity:GetExchangeActivityID() == activity_id then
      LuckybackActivitySystem.exchangeTimer = time_ticker.AddTimer(0, function()
        local bXSuitBox = false
        local LogicXSuit = require("client.slua.logic.XSuit.logic_xsuit")
        for index, data in pairs(arrayItemList) do
          if LogicXSuit.IsSpecialBranchBox(data.res_id) then
            if LogicXSuit.CheckSpecialBranchBoxDownload(data.res_id) then
              local count = data.count
              local bSelecting = false
              for i = 1, count do
                bSelecting = true
                UIManager.ShowUI(UIManager.UI_Config.XSuit_Box_Select_UIBP, data.res_id, function()
                  bSelecting = false
                end, 1)
                while bSelecting do
                  coroutine.yield(1)
        ...
        if not bXSuitBox then
          Logic_CommonItemGet.ShowPanel_DefaultStyle(arrayItemList, nil, false)
        end
      end)
    else
