...
function LobbyLightLogic.OnLevelLoaded(levelName)
  ...
  LobbyLightLogic.CheckVolumetricLightmap(world, levelName)
end
local VLM_STATUS_ERROR = -1
function LobbyLightLogic.CheckVolumetricLightmap(world, levelName)
  if not slua.isValid(world) then
    return
  end
  local streamingLevel = LobbySceneManager.LobbySceneMgrHelper.GetStreamingLevel(levelName)
  if not streamingLevel or not slua.isValid(streamingLevel) then
    return
  end
  local packageFName = streamingLevel:GetWorldAssetPackageFName()
  local STExtraClientUtils = import("STExtraClientUtils")
  if not STExtraClientUtils.IsVolumetricLightmapRegistered then
    return
  end
  local vlmStatus = STExtraClientUtils.IsVolumetricLightmapRegistered(world, packageFName)
  if vlmStatus == VLM_STATUS_ERROR then
    log(bWriteLog and string.format("[LobbyLightLogic] VLM re-registered: level=%s pkg=%s", tostring(levelName), tostring(packageFName)))
    STExtraClientUtils.EnsureVolumetricLightmapRegistered(world, packageFName)
  end
end
