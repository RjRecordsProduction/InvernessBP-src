...
function lobby_scene_module:LoadStreamLevel(SceneName, CameraID, LightName, Extra)
  ...
  self.Loading  local resolvedScene = SceneName
  if not bSceneDownloaded then
    resolvedScene = self:GetDefaultScene(SceneName)
  end
  if (not bSceneDownloaded or not bLightDownloaded) and self:IsPufferInited() then
    self:DownloadLevelAndLoad(SceneName, LightName)
  end
  if resolvedScene ~= nil and resolvedScene ~= "" then
    LobbyLightLogic.LoadStreamLevel(true, resolvedScene, true, bAsync)
  end
  if LightName ~= nil and LightName ~= "" and (bLightDownloaded or not self:IsPufferInited()) then
