...
function MessageHandler:IsMessageExceedMaxLimit(Content)
  local MaxChar = Config_UGC_Copilot.GetMaxMsgCharCntChat()
...
function MessageHandler:AppendUIGenProgress(TraceID)
  ...
  if not stateMachine or not activeChat then
    return false
  end
  local aiMessage = activeChat.AIMessage
  if not aiMessage or aiMessage.TraceID ~= TraceID then
    for i = #(activeChat.Messages or {}), 1, -1 do
      local message = activeChat.Messages[i]
      if message and message.TraceID == TraceID then
        aiMessage = message
        break
      end
    end
  ...
  aiMessage.Content = aiMessage.Content or {}
  for _, segment in ipairs(aiMessage.Content) do
    local segmentContent = segment and segment.content
    local genPrecheck = type(segmentContent) == "table" and segmentContent.gen_precheck or nil
    if segment and segment.type == Config_UGC_Copilot.Enum_Copilot_MessageType.InGeneration and genPrecheck and genPrecheck.gen_type == Config_UGC_Copilot.Enum_Copilot_MessageType.ChatUIDsl then
      return false
    end
  end
  local currentState = stateMachine:GetCurrentState()
  if currentState ~= Config_UGC_Copilot.Enum_UGC_LLM_State.STREAMING and currentState ~= Config_UGC_Copilot.Enum_UGC_LLM_State.LONG_TASK_RUNNING then
    return false
  end
  local progressData = {
    type = Config_UGC_Copilot.Enum_Copilot_MessageType.GenPrecheck,
    subscene = Config_UGC_Copilot.Enum_ChatSceneId.pilot,
    StartTime = os.time(),
    gen_precheck = {
      precheck_result = 0,
      gen_type = Config_UGC_Copilot.Enum_Copilot_MessageType.ChatUIDsl,
      predict_duration = Config_UGC_Copilot.UIGenProgressDuration
    }
  }
  if not stateMachine:UpdateState(Config_UGC_Copilot.Enum_UGC_LLM_State.LONG_TASK_RUNNING, "UIGenInteractionProgress") then
    return false
  end
  stateMachine:SetCurrentLongTaskContext({precheck = progressData, trace_id = TraceID})
  local progressSegment = Config_UGC_Copilot.CreateContentSegment(Config_UGC_Copilot.Enum_Copilot_MessageType.InGeneration, progressData, Config_UGC_Copilot.Enum_Copilot_RoleType.Assistant)
  aiMessage.HideMessageBG = true
  aiMessage.HideFeedback = true
  table.insert(aiMessage.Content, progressSegment)
  EventSystem:postEvent(EVENTTYPE_UGC_COPILOT, EVENTID_UGC_COPILOT_MESSAGE_PARTIAL, aiMessage, {progressSegment}, 0, false)
  return true
end
function MessageHandler:AppendBlockyLuaProgress(TraceID)
  if not TraceID or TraceID == "" then
    return false
  end
  local stateMachine = self.Owner.StateMachine
  ...
  for _, segment in ipairs(aiMessage.Content) do
    local segmentContent = segment and segment.content
    local genPrecheck = type(segmentContent) == "table" and segmentContent.gen_precheck or nil
    if segment and segment.type == Config_UGC_Copilot.Enum_Copilot_MessageType.InGeneration and genPrecheck and genPrecheck.gen_type == Config_UGC_Copilot.Enum_Copilot_MessageType.BlockyEdit then
      return false
    end
  end
  local currentState = stateMachine:GetCurrentState()
  ...
  local progressData = {
    type = Config_UGC_Copilot.Enum_Copilot_MessageType.GenPrecheck,
    subscene = Config_UGC_Copilot.Enum_ChatSceneId.BlockyLuaEdit,
    StartTime = os.time(),
    gen_precheck = {
      precheck_result = 0,
      gen_type = Config_UGC_Copilot.Enum_Copilot_MessageType.BlockyEdit,
      predict_duration = 120
    }
  }
  if not stateMachine:UpdateState(Config_UGC_Copilot.Enum_UGC_LLM_State.LONG_TASK_RUNNING, "BlockyLuaProgress") then
    return false
  end
  stateMachine:SetCurrentLongTaskContext({precheck = progressData, trace_id = TraceID})
  local progressSegment = Config_UGC_Copilot.CreateContentSegment(Config_UGC_Copilot.Enum_Copilot_MessageType.InGeneration, progressData, Config_UGC_Copilot.Enum_Copilot_RoleType.Assistant)
...
function MessageHandler:WithdrawTrailingUIGenProgress(TraceID)
  if not TraceID or TraceID == "" then
    return 0
  end
  local sessionManager = self.Owner.SessionManager
  ...
  if not aiMessage or aiMessage.TraceID ~= TraceID or type(aiMessage.Content) ~= "table" then
    return 0
  end
  local lastIndex = #aiMessage.Content
  local segment = aiMessage.Content[lastIndex]
  local segmentContent = segment and segment.content
  local genPrecheck = type(segmentContent) == "table" and segmentContent.gen_precheck or nil
  if not (segment and segment.type == Config_UGC_Copilot.Enum_Copilot_MessageType.InGeneration and genPrecheck) or genPrecheck.gen_type ~= Config_UGC_Copilot.Enum_Copilot_MessageType.ChatUIDsl then
    return 0
  end
  table.remove(aiMessage.Content, lastIndex)
  aiMessage.HideMessageBG = false
  aiMessage.HideFeedback = false
  return 1
end
function MessageHandler:WithdrawTrailingBlockyLuaProgress(TraceID)
  if not TraceID or TraceID == "" then
    return 0
  end
  local sessionManager = self.Owner.SessionManager
  ...
  for i = #aiMessage.Content, 1, -1 do
    local segment = aiMessage.Content[i]
    local segmentContent = segment and segment.content
    local genPrecheck = type(segmentContent) == "table" and segmentContent.gen_precheck or nil
    if segment and segment.type == Config_UGC_Copilot.Enum_Copilot_MessageType.InGeneration and genPrecheck and genPrecheck.gen_type == Config_UGC_Copilot.Enum_Copilot_MessageType.BlockyEdit then
      table.remove(aiMessage.Content, i)
      aiMessage.HideMessageBG = false
      aiMessage.HideFeedback = false
      return 1
    end
...
function MessageHandler:OnInteraction(data, cb_data)
  ...
  if not data.seq_id then
    self._autoSeqId = (self._autoSeqId or 0) + 1
    data.seq_id = self._autoSeqId
  end
  local seq_id = data.seq_id
  local ttype = data.type or "none"
  print("[copilot_uigen_debug] OnInteraction: TraceID=" .. tostring(TraceID) .. ", seq_id=" .. tostring(data.seq_id) .. ", type=" .. tostring(ttype))
  for k, v in pairs(data) do
    if type(v) == "table" then
      print("[copilot_uigen_debug]   " .. tostring(k) .. " = (table)")
      for k2, v2 in pairs(v) do
        print("[copilot_uigen_debug]     " .. tostring(k2) .. " = " .. tostring(v2))
      end
    else
      print("[copilot_uigen_debug]   " .. tostring(k) .. " = " .. tostring(v))
    end
  end
  ...
  elseif InteractionConfig and InteractionConfig.bNotifyOnly then
