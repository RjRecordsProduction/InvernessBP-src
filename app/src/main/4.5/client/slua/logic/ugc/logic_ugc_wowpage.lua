...
function logic_ugc_wowpage:GetTabInformation(bForce)
  ...
  for k, Index in ipairs(ReqIndexList) do
...
function logic_ugc_wowpage:MatchSendReq()
  ...
  for k, Req in ipairs(PendingReqList) do
...
function logic_ugc_wowpage:GetModInfoReq(ModIDList, ModListType)
  ...
  if (not ReqList or not next(ReqList)) and 0 < CacheModInfoListCount then
    ...
    if 0 < ReqListCount then
      log(bWriteLog and "logic_ugc_wowpage:GetModInfoReq ModListType:" .. tostring(ModListType) .. " ReqListCount:" .. tostring(ReqListCount))
      self.pendingBatchRequests[ModListType] = math.ceil(ReqListCount / 20)
      log(bWriteLog and "logic_ugc_wowpage:GetModInfoReq ModListType:" .. tostring(ModListType) .. " self.pendingBatchRequests[ModListType]:" .. tostring(self.pendingBatchRequests[ModListType]))
...
function logic_ugc_wowpage:OnModInfoBatchRsp(_, _, ReqListType, bIsDirty, MetaList, ClientParam, filter_offline_mod_list)
  ...
  if ClientParam and (ClientParam == "logic_ugc_hall" or ClientParam == "logic_ugc_intention") then
    log(bWriteLog and "logic_ugc_wowpage:OnModInfoBatchRsp ClientParam:" .. tostring(ClientParam))
    return
  end
  local LogicUGC = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.LogicUGC)
  log(bWriteLog and "logic_ugc_wowpage:OnModInfoBatchRsp pre ReqListType:" .. tostring(ReqListType) .. " self.pendingBatchRequests[ReqListType]:" .. tostring(self.pendingBatchRequests[ReqListType]))
  if ReqListType and self.pendingBatchRequests[ReqListType] then
    ...
    log(bWriteLog and "logic_ugc_wowpage:OnModInfoBatchRsp next ReqListType:" .. tostring(ReqListType) .. " self.pendingBatchRequests[ReqListType]:" .. tostring(self.pendingBatchRequests[ReqListType]))
