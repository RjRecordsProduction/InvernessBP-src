...
function Logic_UGC_ThemePlay_ActivityTemplate:GetFirstValidActivityID(actType)
  actType = actType and actType ~= "" and actType or "wow_lobby"
  local Activity = self:GetThemeActData()
  local Logic_UGC = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.LogicUGC)
  local faceSlapActData = Logic_UGC:GetUGCFaceSlapData(true)
  if not Activity or not next(Activity) then
    log(bWriteLog and "Logic_UGC_ThemePlay_ActivityTemplate:GetFirstValidActivityID Activity is nil")
    return nil
  end
  if not faceSlapActData or not next(faceSlapActData) then
    log(bWriteLog and "Logic_UGC_ThemePlay_ActivityTemplate:GetFirstValidActivityID faceSlapAct is nil")
    return nil
  end
  local activityMap = {}
  ...
  local validActIDs = {}
  for _, cfg in ipairs(faceSlapActData) do
    local actID = cfg and tonumber(cfg.BackupParam1)
    if actID and activityMap[actID] then
      table.insert(validActIDs, actID)
    end
  end
  if #validActIDs == 0 then
    return nil
  end
  ...
  local typeShownMap = shownParams and type(shownParams[actType]) == "table" and shownParams[actType] or {}
  for _, actID in ipairs(validActIDs) do
    local firstShownTime = typeShownMap[actID]
...
function Logic_UGC_ThemePlay_ActivityTemplate:CheckMyReward(nActID)
  local ActivityNewSystem = require("client.slua.logic.activity.logic_activity_mgr")
  local tSerVerData = ActivityNewSystem.GetActivityByID(nActID)
  if not tSerVerData or not tSerVerData.List then
    return false
  end
