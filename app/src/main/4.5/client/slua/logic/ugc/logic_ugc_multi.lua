...
function LogicUGCMulti:StartMatch()
  ...
  if not LobbySystem.CheckOpen(BP_ENUM_MULTI_UGC_SWITCH) then
  ...
  UGCModHandler.send_ugc_multi_match_req(MatchModList, false, nil, false, false)
  local Params = {MultiModList = MatchModList}
  ...
  Logic_UGC_TLog:SendInteractionTLog(UGCMacros.ENU_UGC_TLOG_REACH_TYPE.START_MATCH, nil, Params)
end
...
function LogicUGCMulti:GetResState()
  ...
  return LogicUGCResManager:BatchGetResState(LoaderType, Mods), LogicUGCResManager:BatchGetResSize(LoaderType, Mods)
end
...
function LogicUGCMulti:DownloadMultiModList()
  ...
  local LogicUGCResManager = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.LogicUGCResManager)
  LogicUGCResManager:BatchDownloadRes(LogicUGCResManager.DownloaderType.ModList, Mods)
