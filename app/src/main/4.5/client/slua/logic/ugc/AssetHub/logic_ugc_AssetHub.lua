...
function Logic_UGC_AssetHub:GetModListAssetSize(ModList)
  local DependsDic = {}
  for k, ModInfo in pairs(ModList) do
    local ModDepends = self:_GetModDepends(ModInfo)
    if ModDepends then
      for i, Depend in ipairs(ModDepends) do
        DependsDic[Depend] = true
      end
    end
  end
  local TableUtil = require("common.table_util")
  local Depends = TableUtil.GetKeys(DependsDic)
...
function Logic_UGC_AssetHub:PauseAll()
  ...
  if self.DownloadSeqCaches then
    for _, Seq in pairs(self.DownloadSeqCaches) do
      if Seq then
        self:_PauseAssetOuter(Seq)
      end
    end
    self.DownloadSeqCaches = {}
  end
end
