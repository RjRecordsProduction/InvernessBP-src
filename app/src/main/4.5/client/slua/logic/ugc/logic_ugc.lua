  ...
  if bGetShowUGCMainUI then
    ...
    if curPage == ENUM_LobbyPageType.Right then
      ...
      self:AddTimer(0.5, function()
        ...
        logic_mode_selection:ShowMainUI({defaultTopMenuId = 900})
      end)
    end
  end
...
function Logic_UGC:NewReqSearchMod(keyword, rank_type, tag_list, is_ugc_season, feature_tag_list)
  ...
  local search_source = logic_ugc_search and logic_ugc_search:GetSearchSource() or 0
  local UGCSearchHandler = require("client.network.Protocol.UGCSearchHandler")
  UGCSearchHandler.send_ugc_combined_search_mod_req(keyword, rank_type, tag_list, is_ugc_season, feature_tag_list, search_source)
  if logic_ugc_search then
    logic_ugc_search:SetSearchSource(0)
  end
end
...
function Logic_UGC:GetExpiredAssetIds()
  ...
  local TimeUtil = require("client.common.time_util")
  local NowTime = TimeUtil.GetServerTimeInSec()
  print(bWriteLog and "Logic_UGC:GetExpiredAssetIds NowTime" .. tostring(NowTime))
  print(bWriteLog and "Logic_UGC:GetExpiredAssetIds NowTime Date: " .. os.date("%Y-%m-%d %H:%M:%S", NowTime))
...
function Logic_UGC:GetUGCFaceSlapData(HasSpecialType)
  ...
  for _, v in pairs(activity) do
    if currTime >= v.StartTime and currTime <= v.EndTime then
      ...
      local data = {
        ...
        List = v.List,
        BackupParam1 = v.BackupParam1
      }
      local isSpecial = data.Condition1 == self.EFaceSlapType.Special
      if HasSpecialType == nil then
        HasSpecialType = false
      end
      if HasSpecialType == isSpecial then
...
function Logic_UGC:IsCanShow()
  ...
  if not bInLobby then
    log(bWriteLog and "Logic_UGC:IsCanShow return false of gameStatus = " .. tostring(GameStatus.GetGameStatus()) .. " ,not in Login Status and MainCity")
    return false
  end
...
function Logic_UGC:ReqNewbieUGCRecommend()
  ...
  local Config_UGC = require("client.slua.logic.ugc.config_ugc")
  if not Config_UGC.IsUGCUnlock() then
    log(bWriteLog and "Logic_UGC:ReqNewbieUGCRecommend IsUGCUnlock is false")
    return
  end
  local logic_ugc_new_process = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.logic_ugc_new_process)
