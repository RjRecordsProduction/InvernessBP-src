...
function logic_ugc_WOWPass:_ShowBuyBuyPassGuide()
  ...
  if UIManager.IsUIShow(UIManager.UI_Config.NewUGCMainPanel) or UIManager.IsUIShow(UIManager.UI_Config.UGC_Hall_UIBP) then
...
function logic_ugc_WOWPass:ShowTaskPanel()
  ...
  if not self:IsSeasonActive() then
    ...
    logic_assembly_new_system:ShowMainUI()
  else
    local Config_UGC = require("client.slua.logic.ugc.config_ugc")
...
function logic_ugc_WOWPass:ReqPassRecordInfo()
  log(bWriteLog and "logic_ugc_WOWPass:ReqPassRecordInfo")
  local UGCPassHandler = require("client.network.Protocol.UGCPassHandler")
  UGCPassHandler.send_ugc_pass_record_info_req()
end
function logic_ugc_WOWPass:OnPassRecordInfoRsp(ErrorCode, RecordList)
  log(bWriteLog and "logic_ugc_WOWPass:OnPassRecordInfoRsp", ErrorCode)
  if ErrorCode == 0 then
    log_tree(bWriteLog and "logic_ugc_WOWPass:OnPassRecordInfoRsp RecordList", RecordList)
    self.Pass  else
    ShowNotice(ErrorCode)
  end
  EventSystem:postEvent(EVENTTYPE_UGC, EVENTID_UGC_WOW_PASS_RECORD_RSP, ErrorCode, RecordList)
end
function logic_ugc_WOWPass:GetPassRecordList()
  return self.PassRecordList
end
