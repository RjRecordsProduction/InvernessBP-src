...
function logic_ugc_inventory:OpenInventoryWithItem(itemId)
  if not itemId then
    return
  end
  local tabId = self:GetTabIDDigits(itemId)
  if not self:CheckTabIdIsInventory(tabId) then
    return
  end
  local ui_jump_manager = require("client.common.uibase.ui_jump_manager")
  ui_jump_manager.OpenJumpModule(BP_ENUM_MODULE_UGC_DEPOT, {targetItemId = itemId})
end
function logic_ugc_inventory:MarkTargetItem(sortedData, targetItemId)
  if not targetItemId or not sortedData then
    return nil
  end
  for idx, itemData in ipairs(sortedData) do
    if itemData.ID == targetItemId then
      itemData.NowSelect = true
      return idx
    end
  end
  return nil
...
function logic_ugc_inventory:IsItemVisibleInInventory(itemId)
  if not itemId then
    return false
  end
  local tabId = self:GetTabIDDigits(itemId)
  local subTabId = self:GetSubTabIDDigits(itemId)
  local tabIndex = self:FindTabIndexByTabId(tabId)
  if not tabIndex then
    return false
  end
  local subTabIndex = self:FindSubTabIndexBySubTabId(tabIndex, subTabId)
  if not subTabIndex then
    return false
  end
  local dataName = UGC_Inventory.InventoryList[tabIndex][subTabIndex].DataName
  if not dataName then
    return false
  end
  local dataList = CDataTable.GetTable(dataName)
  if not dataList then
    return false
  end
  local configData
  for k, v in pairs(dataList) do
    if type(k) == "number" and v.ID == itemId then
      configData = v
      break
    end
  end
  ...
  if configData.IsShow == 1 then
    return true
  end
  if self.personal_depot and self.personal_depot[itemId] then
    return true
  end
  return false
end
...
function logic_ugc_inventory:GetPlayerCommentData(ID)
  if not ID then
    return nil
  end
  return CDataTable.GetTableData(UGC_Inventory.DataNameList.Dress_PlayerComment, ID)
end
...
function logic_ugc_inventory:AutoDownloadResource(Data, callback, bForce)
  ...
  local onDownloadComplete = function()
    if callback then
      local state = PufferManager.GetState(PufferConst.ENUM_DownloadType.ODPAK, downloadList)
