...
function logic_ugc_work_detail_featured_comment:SetCommentList(modInfo)
  ...
  if self.AppreciationCommentList then
    for _, comment in ipairs(self.AppreciationCommentList) do
      ...
      comment1.is_appreciation_comment = true
      comment1.comment_score = comment.score
      comment1.comment_time = comment.timestamp
      comment1.comment_text = comment.comment
      comment1.is_set_top = comment.is_top
      comment1.author_reply_time = comment.author_reply_content and comment.author_reply_content.reply_time or 0
...
function logic_ugc_work_detail_featured_comment:GetCompRankData(data)
  ...
  local commentTime = data.comment.comment_time or 0
  local diffDateOri = math.abs(os.difftime(commentTime, serverTime))
  local diffDate = math.floor(diffDateOri / 86400)
  local W_time = 1 / (1 + math.log(1 + diffDate, 10))
  local W_likes = math.min(1.0, (data.comment.support_count or 0) / (5 + math.sqrt(diffDate)))
...
function logic_ugc_work_detail_featured_comment:CompareSpecialPriorityLevel(a, b)
  ...
  if appreciationA and appreciationB then
    local adminPriorityA = a.comment.admin_priority or 0
    local adminPriorityB = b.comment.admin_priority or 0
    if adminPriorityA ~= adminPriorityB then
      return adminPriorityA > adminPriorityB
    end
  end
  return nil
end
