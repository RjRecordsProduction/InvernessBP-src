...
function LogicUGCTemplate:DefineAndResetData()
  ...
  self.templateWhiteList = nil
  self.good_template_id_list = {}
  self.good_valid_id_list = {}
  self.temp_mod_id_list = {}
  self.TemplateModsList = {}
end
function LogicUGCTemplate:RegistEvents()
  ...
  self:AddCommonEvent(EVENTTYPE_UGC, EVENTID_UGC_GET_MOD_BATCH, self.OnGetModBatchInfo, self)
end
  ...
  for i = #template_list, 1, -1 do
    ...
    if template.LaunchTime then
      ...
      if shouldRemove then
        local whiteRemove = true
        if self.templateWhiteList and next(self.templateWhiteList) then
          for _, white_template in pairs(self.templateWhiteList) do
            if white_template == template.ID then
              whiteRemove = false
            end
          end
        end
...
function LogicUGCTemplate:UpdateTemplateTabRedDot()
  ...
  local ugc_center_reddot_data = require("client.slua.logic.ugc.center.ugc_center_reddot_data")
  local template_list = self:FilterTemplate(new_templates)
  local CheckGoodShowRedDot = function(templateID)
    for _, template_id in ipairs(self.good_template_id_list) do
      if templateID == template_id then
        for _, mod_id in ipairs(self.good_valid_id_list[template_id] or {}) do
          local state = self:CheckTemplateGoodModRedDot(template_id, mod_id)
          if not state then
            return true
          end
        end
  ...
  for k, v in pairs(template_list) do
    if v.Type then
      local good_show = CheckGoodShowRedDot(v.ID)
      if not (version_util.CompareVersionStandard(cur_version, v.Version) ~= 0 or Shown(v.ID)) or good_show then
...
function LogicUGCTemplate:RspGoodModOfTemplate(ret_list)
  ...
  for k, v in pairs(ret_list or {}) do
    ...
    for i, j in pairs(mod_list_s) do
      ...
      table.insert(self.temp_mod_id_list, tonumber(j))
    end
    self.good_mods_map[v.id] = mod_id_list
  end
  self:GetAllGoodMods()
end
function LogicUGCTemplate:GetAllGoodMods()
  local LogicUGC = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.LogicUGC)
  LogicUGC:BatchGetModInfo(self.temp_mod_id_list, LogicUGC.C_ModListTypes.template_mod, nil, {bSimple = true})
end
...
function LogicUGCTemplate:UpdateTemplateWhiteList(templateWhiteList)
  log_tree(bWriteLog and "LogicUGCTemplate:UpdateTemplateWhiteList templateWhiteList = ", templateWhiteList)
  self.  EventSystem:postEvent(EVENTTYPE_UGC, EVENTID_UGC_UPDATE_TEMPLATE_WHITE_LIST)
end
function LogicUGCTemplate:GetTemplateWhiteList()
  return self.templateWhiteList
end
function LogicUGCTemplate:CheckTemplateGoodModRedDot(template_id, mod_id)
  local PlayerPrefsSystem = require("client.logic.LogicPlayerPrefs.playerprefs")
  local LoadTable = PlayerPrefsSystem.LoadFileToTable_N(PlayerPrefsSystem.ePlayerPrefsType.eUGCTemplateModsPopup)
  if LoadTable ~= nil and LoadTable.GoodList and LoadTable.GoodList[template_id] then
    for k, ModID in ipairs(LoadTable.GoodList[template_id]) do
      if ModID == mod_id then
        return true
      end
    end
  end
  log(bWriteLog and "Logic_UGC:CheckTemplateGoodModRedDot can show New")
  return false
end
function LogicUGCTemplate:SaveTemplateGoodModRedDot(template_id, mod_id)
  local PlayerPrefsSystem = require("client.logic.LogicPlayerPrefs.playerprefs")
  local LoadTable = PlayerPrefsSystem.LoadFileToTable_N(PlayerPrefsSystem.ePlayerPrefsType.eUGCTemplateModsPopup) or {}
  if not LoadTable.GoodList then
    LoadTable.GoodList = {}
  end
  if not LoadTable.GoodList[template_id] then
    LoadTable.GoodList[template_id] = {}
  end
  local isExist = false
  for i, ModID in ipairs(LoadTable.GoodList[template_id]) do
    if ModID == mod_id then
      isExist = true
      break
    end
  ...
  if not isExist then
    table.insert(LoadTable.GoodList[template_id], mod_id)
  end
  log_tree("LogicUGCTemplate:SaveTemplateGoodModRedDot ", LoadTable.GoodList)
  PlayerPrefsSystem.SaveTableToFile_N(LoadTable, PlayerPrefsSystem.ePlayerPrefsType.eUGCTemplateModsPopup)
end
function LogicUGCTemplate:UpdateTemplateGoodModRedDot()
  if not (self.good_template_id_list and next(self.good_template_id_list) and self.good_valid_id_list) or not next(self.good_valid_id_list) then
    return
  end
  local ugc_center_reddot_data = require("client.slua.logic.ugc.center.ugc_center_reddot_data")
  for _, template_id in ipairs(self.good_template_id_list) do
    local TemplateInfo = self:GetTemplateByID(template_id)
    for i, mod_id in ipairs(self.good_valid_id_list[template_id]) do
      if not (TemplateInfo and next(TemplateInfo)) then
        break
      end
      if TemplateInfo.Type then
        local state = self:CheckTemplateGoodModRedDot(template_id, mod_id)
        if not state then
          ugc_center_reddot_data.UpdateCreatTemplateRedDotData(TemplateInfo.Type, TemplateInfo.SceneType, TemplateInfo.ID, true)
          break
        else
          ugc_center_reddot_data.UpdateCreatTemplateRedDotData(TemplateInfo.Type, TemplateInfo.SceneType, TemplateInfo.ID)
        end
      end
    end
  end
end
function LogicUGCTemplate:CompareGoodLists(template_id)
  if not template_id then
    log_error("LogicUGCTemplate:CompareGoodLists invalid template_id")
    return true
  end
  local PlayerPrefsSystem = require("client.logic.LogicPlayerPrefs.playerprefs")
  local LoadTable = PlayerPrefsSystem.LoadFileToTable_N(PlayerPrefsSystem.ePlayerPrefsType.eUGCTemplateModsPopup) or {}
  local saved_list = LoadTable.GoodList and LoadTable.GoodList[template_id] or {}
  local current_list = self.good_valid_id_list and self.good_valid_id_list[template_id] or {}
  ...
  for _, current_item in ipairs(current_list) do
    local found = false
    for _, saved_item in ipairs(saved_list) do
      if current_item == saved_item then
        found = true
        break
      end
    end
...
function LogicUGCTemplate:OnGetModBatchInfo(_, _, ListType, bIsDirty, MetaList, Param, FilterOfflineModList)
  local UGCMacros = require("client.slua.logic.ugc.ugc_macros")
  if not UGCMacros.CheckMetaType(ListType, UGCMacros.ENUM_MODE_TYPE.template_mod) then
    return
  end
  log(bWriteLog and "LogicUGCTemplate:OnGetModBatchInfo")
  self.good_valid_id_list = {}
  self.good_template_id_list = {}
  local LogicUGC = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.LogicUGC)
  local mod_info_list = LogicUGC:GetCacheByType(UGCMacros.ENUM_MODE_TYPE.template_mod) or {}
  self:TemplateModListToArray(mod_info_list)
  for template_id, mod_id_list in pairs(self.good_mods_map) do
    local bShow = false
    if type(mod_id_list) == "table" then
      for i, modid in ipairs(mod_id_list) do
        local mod_info = mod_info_list[modid]
        if mod_info then
          bShow = true
          if not self.good_valid_id_list[template_id] then
            self.good_valid_id_list[template_id] = {}
          end
          table.insert(self.good_valid_id_list[template_id], modid)
        end
      end
    end
  end
  for template_id, _ in pairs(self.good_valid_id_list) do
    table.insert(self.good_template_id_list, template_id)
  end
  log_tree("LogicUGCTemplate:OnGetModBatchInfo good_valid_id_list", self.good_valid_id_list)
  log_tree("LogicUGCTemplate:OnGetModBatchInfo good_template_id_list", self.good_template_id_list)
  local ExistList, ReqModList = LogicUGC:BatchGetModInfo(self.temp_mod_id_list, nil, nil, {bNotPostEvent = true})
  local bNeedReq = ReqModList and 0 < #ReqModList
  if not bNeedReq then
    log(bWriteLog and "LogicUGCTemplate:OnGetModBatchInfo bNeedReq is false")
    EventSystem:postEvent(EVENTTYPE_UGC, EVENTID_UGC_GET_GOOD_MOD_OF_TEMPLATE)
  end
end
function LogicUGCTemplate:TemplateModListToArray(modList)
  if not modList or not next(modList) then
    log(bWriteLog and "LogicUGCTemplate:TemplateModListToArray no modList")
    return nil
  end
  if not self.temp_mod_id_list or not next(self.temp_mod_id_list) then
    log(bWriteLog and "LogicUGCExposure:TemplateModListToArray no temp_mod_id_list")
    return nil
  end
  self.TemplateModsList = {}
  for _, modId in ipairs(self.temp_mod_id_list) do
    if modId and modList[modId] then
      self.TemplateModsList[modId] = modList[modId]
    end
  end
  return self.TemplateModsList
end
function LogicUGCTemplate:GetTemplateModsList()
  return self.TemplateModsList
end
