...
Config_UGC_Center.Config_UGC_Centerlocal _NoviceTeachingTab = {
  ID = _TeachingRoadTabsID.NoviceTeaching,
  NameID = 8971016,
  Module = "UGC_Center_Teaching_Page",
...
Config_UGC_Center.Clocal _CrystalIncentiveTab = {
  ...
  Open = function()
    ...
    local regionGroupConfig = CDataTable.GetTableData("RegionGroupConfig", FuncUtil.GetRegisterCountry())
    if regionGroupConfig and regionGroupConfig.CrystalIncentiveSwitch == 0 then
      return false
    end
    return true
  end
...
local _PromoteIncentiveTab = {
  ID = _TabID.PromoteIncentiveTab,
  NameID = 68683,
  Module = "UGC_Center_PromotionIncentive",
  GemReport = gem_report_utils.SubEventName_UGC_UGC_Center_RewardIncentives,
  Tlog = TLogEventDefine.UGC_Center_RewardIncentives,
  Open = function()
    if LobbySystem.CheckOpen(92114) then
      return true
    else
      return false
    end
