...
function Logic_UGC_Copilot:DefineAndResetData()
  ...
  if self.UIGenFeature then
    self.UIGenFeature:ResetData()
  end
end
function Logic_UGC_Copilot:OnInitialize()
  ...
  if self.UIGenFeature then
    self.UIGenFeature:OnInitialize()
  end
end
...
function Logic_UGC_Copilot:RefreshOpenBlockyluaEdit(bOpenLLMBlockyLua)
  print(bWriteLog and "Logic_UGC_Copilot:RefreshOpenBlockyluaEdit " .. tostring(bOpenLLMBlockyLua))
  self.bOpenLLMBlockyLua = bOpenLLMBlockyLua or false
  EventSystem:postEvent(EVENTTYPE_UGC_COPILOT, EVENTID_UGC_COPILOT_TOPICCARD_REFRESH)
end
function Logic_UGC_Copilot:IsLLMSceneEditEnabled()
  return self.bOpenLLMSceneEdit or false
end
function Logic_UGC_Copilot:IsBlockyluaEditEnabled()
  local CreativeUtility = require("GameLua.Mod.CreativeBase.Gameplay.Meta.CreativeModeUtility")
  if CreativeUtility:IsModAuthor() and CreativeUtility:CanOpenFunctionVS() then
    return self.bOpenLLMBlockyLua or false
  else
    return false
  end
end
function Logic_UGC_Copilot:RefreshOpenUIGen(bOpenUIGen)
  print(bWriteLog and "Logic_UGC_Copilot:RefreshOpenUIGen " .. tostring(bOpenUIGen))
  self.bOpenUIGen = bOpenUIGen or false
  self.bOpenUIGen = false
  if DataMgr and DataMgr.roleData then
    DataMgr.roleData.open_llm_pilot = bOpenUIGen
    self:OnSyncInfoUpdated()
  end
  EventSystem:postEvent(EVENTTYPE_UGC_COPILOT, EVENTID_UGC_COPILOT_TOPICCARD_REFRESH)
end
function Logic_UGC_Copilot:IsUIGenEnabled()
  if self.bOpenUIGen then
    return true
  end
  if DataMgr and DataMgr.roleData and DataMgr.roleData.open_llm_pilot then
    return true
  end
  return false
end
...
function Logic_UGC_Copilot:TryShowExitConfirmPopup(OnConfirm)
  local TeamUpNewSystem = require("client.slua.logic.teamup.logic_team_up")
  if not TeamUpNewSystem.IsTeamLeader() then
    return false
  end
  local activeTraceID = self.StateMachine and self.StateMachine:GetActiveTraceID()
  if not activeTraceID or activeTraceID == "" then
    print(bWriteLog and "Logic_UGC_Copilot:TryShowExitConfirmPopup - no active trace, skip popup")
    return false
  end
  local subsceneID = self:GetCurrentSubsceneID()
  local ExitConfirmCfg = Config_UGC_Copilot.GetExitConfirmConfig(subsceneID)
  if not ExitConfirmCfg then
    return false
  end
  print(bWriteLog and string.format("Logic_UGC_Copilot:TryShowExitConfirmPopup - show exit confirm popup, subsceneID: %s, activeTraceID: %s", tostring(subsceneID), tostring(activeTraceID)))
  local CommonMsgBoxMgr = require("client.slua.logic.common.logic_common_msg_box")
  CommonMsgBoxMgr.Show(CommonMsgBoxMgr.SHOW_TYPE_TWO, Util_UGC.GetLocalizeResStr(ExitConfirmCfg.TitleResID), Util_UGC.GetLocalizeResStr(ExitConfirmCfg.ContentResID), function()
    if OnConfirm then
      OnConfirm()
    end
  end)
...
function Logic_UGC_Copilot:IsCollectingContext()
  return self.StateMachine:IsCollectingContext()
end
...
function Logic_UGC_Copilot:HasCurrentChatID()
  return self.SessionManager:HasCurrentChatID()
end
...
function Logic_UGC_Copilot:HandleNetworkError(ErrCode, Reason)
  ...
  if self.OperationHandler and self.OperationHandler.EnterStopChatSendCooldown then
    self.OperationHandler:EnterStopChatSendCooldown("HandleNetworkError:" .. tostring(Reason) .. tostring(ErrCode))
  end
...
local NamedFeatures = {
  ...
  UIGenFeature = "client.slua.logic.ugc.copilot.uigen_feature"
}
