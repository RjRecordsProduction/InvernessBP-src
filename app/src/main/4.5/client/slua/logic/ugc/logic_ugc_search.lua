...
function Logic_ugc_search:DefineAndResetData()
  ...
  self.RelatedSearchWordList = {}
  self.RelatedSearchWordIndex = 1
end
...
function Logic_ugc_search:SetSearchText(Text)
  self.Searchend
function Logic_ugc_search:SetSearchSource(source)
  self.SearchSource = source or 0
end
function Logic_ugc_search:GetSearchSource()
  return self.SearchSource or 0
end
...
function Logic_ugc_search:UpdateRecommendSearchText()
  if self.RelatedSearchWordList and next(self.RelatedSearchWordList) then
    self.RelatedSearchWordIndex = self.RelatedSearchWordIndex % #self.RelatedSearchWordList + 1
  end
  local bHasHotWords = self.LanguageTab and next(self.LanguageTab)
  local bHasRelatedWords = self.RelatedSearchWordList and next(self.RelatedSearchWordList)
  if not bHasHotWords and not bHasRelatedWords then
    return
  end
  if bHasHotWords then
    local MaxNum = math.min(#self.LanguageTab, 10)
...
function Logic_ugc_search:GetRecommendSearchText()
  if self.RelatedSearchWordList and next(self.RelatedSearchWordList) then
    local Index = self.RelatedSearchWordIndex
    if not Index or Index < 1 or Index > #self.RelatedSearchWordList then
      Index = 1
    end
    return self.RelatedSearchWordList[Index] or ""
  end
  local RandomIndex = 1
  ...
  if self.LanguageTab and next(self.LanguageTab) and RandomIndex <= #self.LanguageTab then
...
function Logic_ugc_search:SetRelatedSearchWordList(ret_info)
  local PublishRegionMacros = require("client.slua.config.ClientMacros.PublishRegionMacros")
  if PublishRegionMacros.IsBLUEHOLE() then
    self.RelatedSearchWordList = {}
  else
    self.RelatedSearchWordList = ret_info or {}
  end
  self.RelatedSearchWordIndex = 1
  log(bWriteLog and "Logic_ugc_search:SetRelatedSearchWordList related_words_num = " .. tostring(#self.RelatedSearchWordList) .. ", first_word = " .. tostring(self.RelatedSearchWordList[1]))
  local RecommendWord = self:GetRecommendSearchText()
  log(bWriteLog and "Logic_ugc_search:SetRelatedSearchWordList RecommendWord = " .. tostring(RecommendWord))
  if RecommendWord and RecommendWord ~= "" then
    EventSystem:postEvent(EVENTTYPE_UGC, EVENTID_UGC_NEWSERCHHITTEXT_MOD, RecommendWord)
  end
end
...
function Logic_ugc_search:OnUgcGetSearchInfoRsp(HotSearch, Icon_search)
  ...
  if self.RecommendSearchTextIndex == 0 and self.LanguageTab and next(self.LanguageTab) then
    self.RecommendSearchTextIndex = 1
    self:InitRecommendSearchText()
  end
  local RecommendWord = self:GetRecommendSearchText()
...
function Logic_ugc_search:on_ugc_gallery_feeds_rsp(ModList, RecallSourceMap)
  if not ModList or not next(ModList) then
    log(bWriteLog and "Logic_ugc_search:on_ugc_gallery_feeds_rsp info is nil")
    EventSystem:postEvent(EVENTTYPE_UGC, EVENTID_UGC_INDIVIDUATION_THERESULTNIL)
    return
  end
  self.ComprehensiveModIdList = ModList
  self.RecallSource = RecallSourceMap
  local LogicUgcFilterTag = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.LogicUgcFilterTag)
  local tag_list = {}
...
function Logic_ugc_search:GetRecallSource(ModID)
  return self.RecallSource and self.RecallSource[ModID]
end
