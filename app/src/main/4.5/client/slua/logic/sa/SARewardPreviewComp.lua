...
function SARewardPreviewComp.AsyncGetRPTaskRewards(callback)
  local rewardDic = {}
    ...
    for index, value in pairs(UnknowPassMissionSystem.Week_Mission_List) do
      ...
      if bIsShowReward and value.status == 1 and value.reward_list and 1 <= #value.reward_list then
        for _, reward in ipairs(value.reward_list) do
          printf("SARewardPreviewComp.AsyncGetRPTaskRewards unknown pass mission reward.res_id:%s reward.res_num:%s", reward.res_id, reward.res_num)
        ...
        if i == PufferConst.Enum_BundleID.Recommend then
          ...
          if state == PufferConst.ENUM_DownloadState.Done then
            ...
            elseif i == PufferConst.BIND_PHONE_MAIL_REWARD_ID then
              log(bWriteLog and "SARewardPreviewComp.AsyncGetDownloadRewards. bind phone mail ignored")
            else
