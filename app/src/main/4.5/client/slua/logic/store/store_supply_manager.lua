...
function store_supply_manager:ResponseStoreData(store_version, info, back_user_buy_info, material_limit_info, rpPlusLimitInfo)
  ...
  if GlobalData.IsJapanOrKorea() then
    StoreConst.store_free_data[tab_id] = nil
    local itemList = info[StoreConst.label_market_index_market_list]
    if itemList then
      for shopId, itemData in pairs(itemList) do
        if itemData[StoreConst.label_item_index_zero_uc_mark] == 1 then
          local subTabId = itemData[StoreConst.label_item_index_page_id] or 0
          if not StoreConst.store_free_data[tab_id] then
            StoreConst.store_free_data[tab_id] = {}
          end
          if not StoreConst.store_free_data[tab_id][subTabId] then
            StoreConst.store_free_data[tab_id][subTabId] = {}
          end
          table.insert(StoreConst.store_free_data[tab_id][subTabId], {
            shopId = shopId,
            buyLimit = itemData[StoreConst.label_item_index_buy_limit] or {}
          })
        end
      end
    end
...
function store_supply_manager:CheckNeedShowCollect(market_id, info)
  ...
  local key = LadderCarDetailConfig.GetCurrentVersionNormalCarTypeName()
...
function store_supply_manager:GetJKTabFreeFlag(tabId, subTabId)
  local subFreeList = StoreConst.store_free_data and StoreConst.store_free_data[tabId] and StoreConst.store_free_data[tabId][subTabId]
  if not subFreeList then
    return false
  end
  local StoreUtils = require("client.slua.logic.store.utils.store_utils")
  for _, item in ipairs(subFreeList) do
    local limit = item.buyLimit
    if not StoreUtils.IsSoldOut(item.shopId, limit[StoreConst.label_buy_limit_type_daily], limit[StoreConst.label_buy_limit_type_week], limit[StoreConst.label_buy_limit_type_permanent]) then
      return true
    end
  end
  return false
