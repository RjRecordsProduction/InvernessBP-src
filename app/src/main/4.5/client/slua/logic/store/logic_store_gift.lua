...
local _IsCurrentSmallRPCardGift = function(itemId)
  local Logic_SmallRP = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.Logic_SmallRP)
  return Logic_SmallRP and Logic_SmallRP:GetSmallRPCardId() == tonumber(itemId)
end
...
function GiftSystem.PopCommonTip()
  ...
  if giftRecvList and next(giftRecvList) then
    for i, msginfo in ipairs(giftRecvList) do
      ...
      local bIsSmallRPCardGift = _IsCurrentSmallRPCardGift(msginfo.item)
      local bCanPopup = itemDataCfg and _IsPopGift(itemDataCfg.ItemSubType) or CDataTable.GetTableData("XSuitBranchChest", msginfo.item) or bIsSmallRPCardGift or false
      if itemDataCfg and not msginfo.isFetched and bCanPopup then
        ...
        if not hasExist then
          msginfo.bSmallRPCardGift = bIsSmallRPCardGift or nil
...
function GiftSystem.OnGetGift(index, tItemList, tDecItemList)
  ...
  if not StoreUtils.CheckIsChest(tmp.item) then
    ...
    elseif GiftSystem.CheckIsSmallCard(tmp.item) then
      ...
      local needShowItemGet = 0
      for k, v in pairs(tItemList) do
        if not tDecItemList[k] then
          local branchState = GiftSystem.OpenXSuitBranchBox(v)
          needShowItemGet = branchState ~= 0 and branchState or needShowItemGet
        end
      end
      if needShowItemGet ~= 2 then
        local Logic_CommonItemGet = require("client.slua.logic.common.CommonItemGet.Logic_CommonItemGet")
        Logic_CommonItemGet.ShowPanel_DecomposeStyle(tItemList, tDecItemList, {
          bCheckSpecialItem = needShowItemGet == 0
        })
      end
    end
  end
...
function GiftSystem.OnGetAllGift(indexList, tGetAllItem, tDecItemList)
  ...
  if GiftSystem.AllGiftTimer then
    time_ticker.RemoveTimer(GiftSystem.AllGiftTimer)
    GiftSystem.AllGiftTimer = nil
  end
  GiftSystem.bShowXSuitBox = false
  GiftSystem.AllGiftTimer = time_ticker.AddTimer(0, function()
    if 0 < #itemList then
      local needShowItemGet = 0
      for k, v in ipairs(itemList) do
        ...
        local branchState = GiftSystem.OpenXSuitBranchBox(v)
        if branchState == 2 then
          GiftSystem.bShowXSuitBox = true
          while GiftSystem.bShowXSuitBox do
            coroutine.yield(1)
          end
        end
        needShowItemGet = branchState ~= 0 and branchState or needShowItemGet
...
function GiftSystem.OpenGiftGetView()
  ...
  if GiftSystem.IsOpenCarGiftSendView(data) then
    return
  end
  local LogicFriend = require("client.slua.logic.friend.logic_new_friend")
...
function GiftSystem.OpenXSuitBranchBox(data)
  local itemId = data.item or data.resid
  local LogicXSuit = require("client.slua.logic.XSuit.logic_xsuit")
  if LogicXSuit.IsSpecialBranchBox(itemId) then
    if LogicXSuit.CheckSpecialBranchBoxDownload(itemId) then
      UIManager.ShowUI(UIManager.UI_Config.XSuit_Box_Select_UIBP, itemId, function()
        GiftSystem.bShowXSuitBox = false
      end, 2)
      return 2
    else
...
function GiftSystem.IsOpenCarGiftSendView(tItemData)
  ...
  if not LadderCarDetailConfig.IsCurrentVersionNormalCar(tItemData.item) and not LadderCarDetailConfig.IsCurrentVersionSpecialCar(tItemData.item) then
    log(bWriteLog and "GiftSystem.IsOpenCarGiftSendView not CurrentVersionCar " .. tostring(tItemData.item))
    return false
  end
