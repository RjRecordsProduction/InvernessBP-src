...
function EightDaySystem.ShowUI()
  ...
  logic_newbie_activity.ShowUIWithModCheck(UIManager.UI_Config.Flap_Newbie_EightDays)
end
...
function EightDaySystem.ShouldSlap()
  ...
  if not NewbieActivitySystem.IsModReady() then
    log_warning(bWriteLog and "EightDaySystem.ShouldSlap - not IsModReady")
    return false
  end
  if not NewbieActivitySystem.HasNewbieActivity() then
    log_warning(bWriteLog and "EightDaySystem.ShouldSlap - not HasNewbieActivity")
    return false
  end
  local errorCode = EightDaySystem.GetShowErrorCode()
  if errorCode ~= 0 then
    log_warning(bWriteLog and "EightDaySystem.ShouldSlap - GetShowErrorCode ~= 0. errorCode = " .. tostring(errorCode))
    return false
  end
  ...
  if LogicNewbie.IsNewbie() and not LogicNewbie.NeedShowNewbieGuide(20000) then
    log_warning(bWriteLog and "EightDaySystem.ShouldSlap - not NeedShowNewbieGuide")
    return false
  end
