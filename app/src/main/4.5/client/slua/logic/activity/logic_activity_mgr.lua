  ...
  if errorCode == NetErrorCode_NONE then
    ...
    if ActivityNewSystem.bCanUpdateOtherModuleData then
      ...
      elseif activityId == LuckyDoubleSystem.ActivityId then
        ...
        return
      elseif activityId == logic_crazy_weekend_luckydraw.activityId then
        logic_crazy_weekend_luckydraw:send_get_happy_weekend_ticket_req()
        return
...
function ActivityNewSystem.OnBatchTakeActivityAwardRsp(err_code, activity_id, index_awards)
  if err_code == 0 then
    ...
    EventSystem:postEvent(EVENTTYPE_ACTIVITY, EVEMTID_DATAMGR_ACTIVITY_REWARDS_COMMON, activity_id, item_data)
  else
    ShowNotice(err_code)
  end
end
...
local InitCond = function(subActivity, cond, extraCond)
  ...
  elseif activityType == ActivityType.LUCKYBACK then
    if extraCond and extraCond ~= "" then
      local args = StringUtil.SplitToNum(extraCond, ",")
  ...
  elseif activityType == ActivityType.PetBattle and condition[1] ~= 0 then
    subActivity.Total = condition[2]
  end
end
...
function ActivityNewSystem.UpdateOneActivityData(id, data)
  ...
  if activity then
    ...
    elseif actType == ActivityType.LoginPunchIn and ActivityNewSystem.IsSignInType(activity) then
      log(bWriteLog and "ActivityNewSystem.UpdateOneActivityData.")
      for index, subActivity in pairs(activity.List) do
        local status = tonumber(data.award[index].status) or ActivityProgressStatus.Not
        subActivity.Status = tonumber(status) or ActivityProgressStatus.Not
      end
    end
    if data.from then
      activity.    end
...
function ActivityNewSystem.HasActivityRedDotByID(activityId, fromBrother)
  ...
  local externalRedDotType = ActivityCenterModule:GetExternalImageRedDot(activityId)
  if externalRedDotType ~= nil then
    RedDotType = externalRedDotType
    return RedDotType ~= ActivityMacros.RedDotType.None, RedDotType
  end
  local ActivityCenterModule = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.ActivityCenterModule)
  if ActivityCenterModule:GetMergeBrotherId(activityId) and not fromBrother then
    return false, RedDotType
  end
  local map = ActivityNewSystem.dataMap
  if not map then
  ...
  local ActivityCenterModule = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.ActivityCenterModule)
  local isCenterRedPoint = ActivityCenterModule:CheckHasH5CenterRedPoint(activityId)
...
function ActivityNewSystem.ShowActivityUI(_, _, vars, ParamTable)
  ...
  if not ActivityNewSystem.CheckIsActivityCenterDownload() then
    log(bWriteLog and "[SY]ActivityNewSystem.ShowActivityUI.ShowActivityCenterDownloadTip")
    ActivityNewSystem.ShowActivityCenterDownloadTip(_, _, vars, ParamTable)
    return
  end
  if UIManager.IsUIShow(UIManager.UI_Config.ActivityCenter_Main_UIBP) then
    local UI = UIManager.GetUI(UIManager.UI_Config.ActivityCenter_Main_UIBP)
    if not UI:JumpTo(vars) then
    ...
    UIManager.ShowUI(UIManager.UI_Config.ActivityCenter_Main_UIBP, extraData, ParamTable)
  end
end
function ActivityNewSystem.OpenSuperCore(_, _, vars)
  ...
  LogicCustomerService.Open("kf9021")
end
...
function ActivityNewSystem.OpenPremiumHallSVIP(_, _, vars)
  ...
  local IntlHelper = import("IntlHelper")
  IntlHelper.HelpshiftClearUnreadMessgesCount()
  local sceneId = vars and vars.scene_id or "kfbcustomer"
  local LogicCustomerService = require("client.slua.logic.CustomerService.LogicCustomerService")
...
function ActivityNewSystem.GetActivityCenterDownloadList()
  local tResourcesList = {
    UIManager.UI_Config.ActivityCenter_Main_UIBP.path
  }
  return tResourcesList
end
function ActivityNewSystem.CheckIsActivityCenterDownload()
  local PufferManager = require("client.slua.logic.download.puffer.puffer_manager")
  local PufferConst = require("client.slua.logic.download.puffer_const")
  local tResourcesList = ActivityNewSystem.GetActivityCenterDownloadList()
  local state = PufferManager.GetState(PufferConst.ENUM_DownloadType.ODPAK, tResourcesList)
  log(bWriteLog and "[SY]ActivityNewSystem.CheckIsActivityCenterDownload.state " .. state)
  return state == PufferConst.ENUM_DownloadState.Done
end
function ActivityNewSystem.ShowActivityCenterDownloadTip(_, _, vars, ParamTable)
  local PufferManager = require("client.slua.logic.download.puffer.puffer_manager")
  local PufferConst = require("client.slua.logic.download.puffer_const")
  local tResourcesList = ActivityNewSystem.GetActivityCenterDownloadList()
  PufferManager.ShowDownloadTips(PufferConst.ENUM_DownloadType.ODPAK, tResourcesList, nil, nil, function()
    ActivityNewSystem.ShowActivityUI(_, _, vars, ParamTable)
  end)
end
...
function ActivityNewSystem.JumpUrl(url, total, actID, dontCloseActivityInJumping)
  ...
  local CloseActivityUI = function()
    if UIManager then
      UIManager.CloseUI(UIManager.UI_Config.ActivityCenter_Main_UIBP)
    end
  end
  ...
  elseif StringUtil.Starts(url, "http") or StringUtil.Starts(url, "www") then
    local ActivityCenterModule = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.ActivityCenterModule)
    ActivityCenterModule:UpdateRedPointInJumpWebUrl(actID)
...
function ActivityNewSystem.CheckActIsOpen(actId)
  log(bWriteLog and "ActivityNewSystem.CheckActIsOpen actId:" .. tostring(actId))
  local data = ActivityNewSystem.GetActivityByID(actId)
  if not data then
    log(bWriteLog and "ActivityNewSystem.CheckActIsOpen data is nil")
    return false
  end
  local serverTime = TimeUtil.GetServerTimeInSec()
  local beginTime, endTime = ActivityNewSystem.GetStartWithEndTime(data)
  log(bWriteLog and "ActivityNewSystem.CheckActIsOpen serverTime:" .. tostring(serverTime) .. " beginTime:" .. tostring(beginTime) .. " endTime:" .. tostring(endTime))
  if beginTime and endTime and serverTime > beginTime and serverTime < endTime then
    return true
  end
  return false
end
function ActivityNewSystem.CheckPandoraLinkIsReady(url)
  printf("ActivityNewSystem.CheckPandoraLinkIsReady - url=%s", tostring(url))
  local JumpUtils = require("client.logic.store.jump_utils")
  if JumpUtils.IsPanDoraJumpUrl(url) then
    local pandoraSystem = require("client.slua.logic.Pandora.pandora_system")
    if not pandoraSystem.CheckSysOpen() then
      printf("ActivityNewSystem.CheckPandoraLinkIsReady: pandoraSystem is not open")
      return false
    end
    local pandoraUtils = require("client.slua.logic.Pandora.pandora_utils")
    ...
    printf("ActivityNewSystem.CheckPandoraLinkIsReady - ActIsReady=%s, PakIsReady=%s", tostring(ActIsReady), tostring(PakIsReady))
