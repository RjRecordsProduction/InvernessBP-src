...
function SecuritySystem.ShowReportSucceedFace()
  ...
  if SlapFaceMailList[1].opt then
    local uiKey = SlapFaceMailList[1].opt.popface_type == 1 and "ReportSucceed_Slap_Pro_UIBP" or "ReportSucceed_Slap_UIBP"
    local uiConfig = UIManager.GetConfigByKey(uiKey)
    if not uiConfig then
      log_error(bWriteLog and "[chub]ShowReportSucceedFace " .. uiKey .. " UI config not found ")
      return
    end
    local pak_util = require("client.common.pak_util")
    if not pak_util.IsFileExist(uiConfig.path) then
      log_error(bWriteLog and "[chub]ShowReportSucceedFace " .. uiKey .. " uiConfig.path not exist ")
      return
    end
    UIManager.ShowUI(uiConfig, SlapFaceMailList[1])
  end
