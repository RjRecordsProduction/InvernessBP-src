...
function BasicDataAvatarWearInfo:on_get_avatar_show_rsp(res, target_uid, data)
  ...
  if data.wear_ext[ENUM_AVATAR_SHOW_TYPE.SHOW_POS_CLOTH] and data.wear_ext[ENUM_AVATAR_SHOW_TYPE.SHOW_POS_CLOTH][ENUM_AVATAR_DATA_TYPE.ItemID] then
    local clothItem = data.wear_ext[ENUM_AVATAR_SHOW_TYPE.SHOW_POS_CLOTH][ENUM_AVATAR_DATA_TYPE.ItemID]
    local period = LogicXSuit.GetPeriodByItemId(clothItem)
    if period then
      local branch = LogicXSuit.GetBranchByItemId(clothItem) or 0
      local newItemID = clothItem
      local Source = data.wear_ext[ENUM_AVATAR_SHOW_TYPE.SHOW_POS_CLOTH][ENUM_AVATAR_DATA_TYPE.Source] or 0
      if data.gold_dress_set_info_new and data.gold_dress_set_info_new.set_info and data.gold_dress_set_info_new.set_info[period] and data.gold_dress_set_info_new.set_info[period][branch] and Source ~= 1 then
        data.gold_dress_set_info_new.originalItem = clothItem
        newItemID = LogicXSuit.GetSwitchItemByItemAndSwitchLevel(newItemID, data.gold_dress_set_info_new.set_info[period][branch])
      end
      if data.gold_dress_set_info_all_new and data.gold_dress_set_info_all_new.set_info and data.gold_dress_set_info_all_new.set_info[period] and data.gold_dress_set_info_all_new.set_info[period][branch] then
        local state = data.gold_dress_set_info_all_new.set_info[period][branch].bicolor_state
        if state then
          data.gold_dress_set_info_all_new.originalItem = clothItem
          newItemID = LogicXSuit.ChangeItemIDByState(newItemID, state)
          data.gold_dress_set_info_all_new.displayItem = newItemID
        end
      end
    ...
    if DataMgr.roleData.uid ~= tostring(target_uid) then
      if data.wear_ext[ENUM_AVATAR_SHOW_TYPE.SHOW_POS_CLOTH][ENUM_AVATAR_DATA_TYPE.ShapeInfo] then
        logic_suit_multi_shape:SetSuitShapeInfo(target_uid, clothItem, 1, data.wear_ext[ENUM_AVATAR_SHOW_TYPE.SHOW_POS_CLOTH][6])
      else
        logic_suit_multi_shape:ClearSuitShapeInfo(target_uid, clothItem)
      end
    end
  end
  if data.wear and data.wear[ENUM_AVATAR_SHOW_TYPE.SHOW_POS_CLOTH] then
    local clothItem = data.wear[ENUM_AVATAR_SHOW_TYPE.SHOW_POS_CLOTH]
    local period = LogicXSuit.GetPeriodByItemId(clothItem)
    if period then
      local branch = LogicXSuit.GetBranchByItemId(clothItem) or 0
      local newItemID = clothItem
      if data.gold_dress_set_info_new and data.gold_dress_set_info_new.set_info and data.gold_dress_set_info_new.set_info[period] and data.gold_dress_set_info_new.set_info[period][branch] then
        data.gold_dress_set_info_new.originalItem = clothItem
        newItemID = LogicXSuit.GetSwitchItemByItemAndSwitchLevel(newItemID, data.gold_dress_set_info_new.set_info[period][branch])
      end
      if data.gold_dress_set_info_all_new and data.gold_dress_set_info_all_new.set_info and data.gold_dress_set_info_all_new.set_info[period] and data.gold_dress_set_info_all_new.set_info[period][branch] then
  ...
  if data.pspace_wear_ext and data.pspace_wear_ext[ENUM_AVATAR_SHOW_TYPE.SHOW_POS_CLOTH] and data.pspace_wear_ext[ENUM_AVATAR_SHOW_TYPE.SHOW_POS_CLOTH][1] then
    local clothItem = data.pspace_wear_ext[ENUM_AVATAR_SHOW_TYPE.SHOW_POS_CLOTH][1]
    local period = LogicXSuit.GetPeriodByItemId(clothItem)
    if period then
      local branch = LogicXSuit.GetBranchByItemId(clothItem) or 0
      local newItemID = clothItem
      ...
      if data.gold_dress_set_info_new and data.gold_dress_set_info_new.set_info and data.gold_dress_set_info_new.set_info[period] and data.gold_dress_set_info_new.set_info[period][branch] and Source ~= 1 then
        data.gold_dress_set_info_new.pOriginalItem = clothItem
        newItemID = LogicXSuit.GetSwitchItemByItemAndSwitchLevel(newItemID, data.gold_dress_set_info_new.set_info[period][branch])
      end
      if data.gold_dress_set_info_all_new and data.gold_dress_set_info_all_new.set_info and data.gold_dress_set_info_all_new.set_info[period] and data.gold_dress_set_info_all_new.set_info[period][branch] then
        local state = data.gold_dress_set_info_all_new.set_info[period][branch].bicolor_state
        if state then
          data.gold_dress_set_info_all_new.pOriginalItem = clothItem
          newItemID = LogicXSuit.ChangeItemIDByState(newItemID, state)
          data.gold_dress_set_info_all_new.displayItem = newItemID
        end
      end
    ...
    if DataMgr.roleData.uid ~= tostring(target_uid) then
      if clothItem and data.pspace_wear_ext[ENUM_AVATAR_SHOW_TYPE.SHOW_POS_CLOTH][ENUM_AVATAR_DATA_TYPE.ShapeInfo] then
...
function BasicDataAvatarWearInfo:GetVehicleBrakeCaliper(UID)
  local Data = self:GetCacheData(UID)
  if not (Data and Data.wear_ext) or not Data.wear_ext[ENUM_AVATAR_SHOW_TYPE.SHOW_POS_VST_SKIN] then
    return nil
  end
  return Data.wear_ext[ENUM_AVATAR_SHOW_TYPE.SHOW_POS_VST_SKIN][ENUM_AVATAR_VEHICLE_SUB_TYPE.BRAKE_CALIPER]
end
function BasicDataAvatarWearInfo:GetVehicleWheelHub(UID)
  local Data = self:GetCacheData(UID)
  if not (Data and Data.wear_ext) or not Data.wear_ext[ENUM_AVATAR_SHOW_TYPE.SHOW_POS_VST_SKIN] then
    return nil
  end
  return Data.wear_ext[ENUM_AVATAR_SHOW_TYPE.SHOW_POS_VST_SKIN][ENUM_AVATAR_VEHICLE_SUB_TYPE.WHEEL_HUB]
end
function BasicDataAvatarWearInfo:GetVehicleSunroof(UID)
  local Data = self:GetCacheData(UID)
  if not (Data and Data.wear_ext) or not Data.wear_ext[ENUM_AVATAR_SHOW_TYPE.SHOW_POS_VST_SKIN] then
    return nil
  end
  return Data.wear_ext[ENUM_AVATAR_SHOW_TYPE.SHOW_POS_VST_SKIN][ENUM_AVATAR_VEHICLE_SUB_TYPE.SUNROOF]
end
