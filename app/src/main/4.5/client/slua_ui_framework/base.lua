...
function UIBase:PreHandleChildCfg(config)
  if not config then
    return
  end
  if self.bIsInCombatState then
    return
  end
  if base_config_util.IsMainUI(config) and base_config_util.IsSingleton(config) and Client.IsDevelopment() then
    xpcallHandle("UIBase:PreHandleChildCfg shouldn't be MainUI. keyName:" .. self:_GetKeyName(config))
  end
end
...
function UIBase:Init(path, containerName, zOrder, loadFromPool, asy)
  if not assert(type(path) == "string", "UIBase:Init path should be string") then
    return false
  end
  if not assert(type(containerName) == "string", "UIBase:Init containerName should be string") then
    return false
  end
  if not assert(type(zOrder) == "number", "UIBase:Init zOrder should be number") then
    return false
  end
  if self.UIRoot then
  ...
  self._bpPath = path
  self._  if type(loadFromPool) ~= "number" then
...
function UIBase:MarkRefreshZOrderOnLoaded()
  log_format("UIBase:MarkRefreshZOrderOnLoaded keyName:%s", self:_GetKeyName(self._config))
  self._needRefreshZOrderOnLoaded = true
end
...
function UIBase:_HandleAsyncLoaded()
  ...
  if self._needRefreshZOrderOnLoaded then
    self._needRefreshZOrderOnLoaded = nil
    log_format("UIBase:_HandleAsyncLoaded refresh zOrder to top keyName:%s", self:_GetKeyName(self._config))
    UIManager._OnUIShowforDepthManager(self._config, self)
  end
...
function UIBase:AddSettingOptionEvent(OptionName, handleFunc, bInitialCall)
  if not Client then
    log_error("AddSettingOptionEvent is client-only")
    return
  end
  ...
  if self._settingOptionListeners[OptionName] then
    self:RemoveSettingOptionEvent(OptionName)
  end
  ...
  if bSuccess then
    self._settingOptionListeners[OptionName] = handleFunc
  end
  return bSuccess
end
function UIBase:RemoveSettingOptionEvent(OptionName)
  ...
  local handleFunc = self._settingOptionListeners[OptionName]
  self._settingOptionListeners[OptionName] = nil
  local SettingModule = ModuleManager_GetModule(CommonModuleConfig.SettingModule)
  SettingModule:RemoveOptionValueChangeEvent(OptionName, handleFunc)
end
function UIBase:RemoveAllSettingOptionEvent()
  ...
  local SettingModule = ModuleManager_GetModule(CommonModuleConfig.SettingModule)
  for OptionName, handleFunc in pairs(self._settingOptionListeners) do
    SettingModule:RemoveOptionValueChangeEvent(OptionName, handleFunc)
  end
...
function UIBase:_GetControlByName(controlName, control)
  ...
  for c in string_gmatch(controlName, "[^.]+") do
    control = control[c]
    if not control then
...
function UIBase:SetWidgetVisible(widget, visible, isButton)
  ...
  local SetWidgetVisibility = widget.SetVisibility_V2
  SetWidgetVisibility = SetWidgetVisibility or widget.SetWidgetVisibility
  local ESlateVisibility = UEnums.ESlateVisibility.Collapsed
  if visible then
    if isButton then
      ESlateVisibility = UEnums.ESlateVisibility.Visible
    else
      ESlateVisibility = UEnums.ESlateVisibility.SelfHitTestInvisible
    end
  end
  SetWidgetVisibility(widget, ESlateVisibility)
end
...
function UIBase:SetAdaptiveLayout(HorizontalType, VerticalType, OffsetX, OffsetY)
  if not self.UIRoot or not slua_isValid(self.UIRoot) then
    log_error("UIBase:SetAdaptiveLayout UIRoot is invalid")
    return
  end
  return UIUtil.SetAdaptiveLayout(self.UIRoot, HorizontalType, VerticalType, OffsetX, OffsetY)
end
...
function UIBase:ClearInvalidationBoxesCache()
  local ClientEVOConfig = require("client.logic.client_evo_config.client_evo_config")
  ClientEVOConfig.ToggleInvalidationPanels(false)
  self:AddTimer(0.1, function()
    ClientEVOConfig.ToggleInvalidationPanels(ClientEVOConfig.NeedInvalidationBoxThisGame or IsEditor)
  end)
end
