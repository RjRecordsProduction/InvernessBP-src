...
function UIManager._PreCreateUICheck(config, bpPath)
  ...
  if bCheck then
    if UIManager._ShowUIDownloadTips(config, bpPath) then
      return false
    end
    if PublishRegionMacros.IsFITVersion() then
      local blueprintPath = bpPath or config.path
      local bExist = FITBpPathExistCache[blueprintPath]
      if bExist == nil then
        bExist = pak_util.IsFileExist(blueprintPath)
        FITBpPathExistCache[blueprintPath] = bExist
      end
      if not bExist then
        if bWriteLog then
          local outputStr = string_format("%s: %s", LocUtil.GetLocalizeResStr(28002), blueprintPath)
          log_warning("UIManager._PreCreateUICheck - " .. outputStr)
          if Client.IsDevelopment() then
            ShowNotice(outputStr)
          end
        end
        return false
      end
...
function UIManager.Init()
  ...
  if PublishRegionMacros.IsFITVersion() then
    EventSystem:registEvent(EVENTTYPE_PUFFER, EVENTID_PUFFER_DOWNLOADFINISH, UIManager._OnPufferDownloadFinishForFITCache)
  end
end
function UIManager._OnPufferDownloadFinishForFITCache()
  FITBpPathExistCache = {}
end
...
function UIManager.AndroidBackToLobby()
  ...
  local ui_navigation_manager = ModuleManager_GetModule(CommonModuleConfig.ui_navigation_manager)
  if UIManager.IsUIInStack(UI_Config.xmission_main.keyName) then
    ui_navigation_manager:AndroidBackToUI(UI_Config.xmission_main.keyName)
  elseif GameStatus.IsInMainCity() then
    ui_navigation_manager:AndroidBackToUI(UI_Config.MainCity_Main_UIBP.keyName)
  elseif UIManager.IsCreativeEditMainUIInStack() then
    ui_navigation_manager:AndroidBackToUI(UIManager.UI_Config_InGame.CreativeModeEditMainUI.keyName)
  else
...
function UIManager.ForceBackTo2DLobby()
  ui_jump_manager.Clear()
  local LogicTxMissionMain = require("client.slua.logic.TxMission.logic_xmission_main")
  if LogicTxMissionMain.IsInXMission() or UIManager.IsUIInStack(UI_Config.xmission_main.keyName) then
    log(bWriteLog and "UIManager.ForceBackTo2DLobby - quit XMission first")
...
function UIManager.IsUIInStack(keyName)
  local ui_navigation_manager = ModuleManager_GetModule(CommonModuleConfig.ui_navigation_manager)
  return ui_navigation_manager:IsUIInStack(keyName)
end
function UIManager.IsCreativeEditMainUIInStack()
  local InGameConfig = UIManager.UI_Config_InGame
  return InGameConfig ~= nil and InGameConfig.CreativeModeEditMainUI ~= nil and UIManager.IsUIInStack(InGameConfig.CreativeModeEditMainUI.keyName)
end
