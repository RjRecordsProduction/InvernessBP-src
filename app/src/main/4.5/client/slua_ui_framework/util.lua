...
function util.SetImageParams(control, extraParams)
  extraParams = extraParams or {}
  local path = extraParams.ImagePath or ""
  local result = util.SetTexture(control, path, extraParams)
  if result == SetTextureConst.Error then
    log_error(string_format("util.SetImageParams SetTexture failed, path=%s", tostring(path)))
    return
  end
  if not slua_isValid(control) then
    return
  end
  local imageSize = extraParams.ImageSize
  if imageSize and control.Brush then
    local BrushRef = slua.IndexReference(control, "Brush")
    if BrushRef then
      local BrushClone = BrushRef:clone()
      BrushClone.ImageSize = imageSize
      control:SetBrush(BrushClone)
    end
  end
  local colorIndex = extraParams.ImageColorIndex
  if colorIndex ~= nil and control.SetActiveColorIndex then
    control:SetActiveColorIndex(colorIndex)
  end
  local angle = extraParams.ImageAngle
  if angle ~= nil and control.SetRenderAngle then
    control:SetRenderAngle(angle)
  end
  log_tree(bWriteLog and "util.SetImageParams done", extraParams)
end
