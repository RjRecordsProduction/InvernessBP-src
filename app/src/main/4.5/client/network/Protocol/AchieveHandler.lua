...
function AchieveHandler.on_get_achievement_rewards_res(id, result, timestamps, isRewardClick)
  log(bWriteLog and "AchieveHandler.on_get_achievement_rewards_res id = " .. tostring(id) .. ", result = " .. result .. ", timestamps = " .. timestamps)
  if result ~= 0 then
...
function AchieveHandler.send_get_achieve_extinct_req()
  log(bWriteLog and "AchieveHandler.send_get_achieve_extinct_req")
  NetManager.SendPkg(1906194579)
end
function AchieveHandler.on_get_achieve_extinct_rsp(err_code, extinct_achieve_ids, notified_extinct_ids, client_extinct_achieve_ids)
  log(bWriteLog and "AchieveHandler.on_get_achieve_extinct_rsp err_code = ", err_code)
  log_tree(bWriteLog and "AchieveHandler.on_get_achieve_extinct_rsp extinct_achieve_ids = ", extinct_achieve_ids)
  log_tree(bWriteLog and "AchieveHandler.on_get_achieve_extinct_rsp notified_extinct_ids = ", notified_extinct_ids)
  log_tree(bWriteLog and "AchieveHandler.on_get_achieve_extinct_rsp client_extinct_achieve_ids = ", client_extinct_achieve_ids)
  if err_code ~= 0 then
    return
  end
  AchieveHandler.bGetExtinctRsp = true
  AchieveHandler.resOutPrintInfo = extinct_achieve_ids or {}
  AchieveHandler.resExtinctInfo = notified_extinct_ids or {}
  AchieveHandler.resClientExtinctInfo = client_extinct_achieve_ids or {}
end
function AchieveHandler.IsExtinctByID(AchID, achConfig)
  if not AchID then
    log_error(bWriteLog and "AchieveHandler.IsExtinctByID AchID is nil")
    return false
  end
  local outTime = AchieveHandler.GetExtinctTimeStampWithCfg(AchID, achConfig)
  if not outTime then
    return false
  end
  local TimeUtil = require("client.common.time_util")
  return outTime <= TimeUtil.GetServerTimeInSec()
end
function AchieveHandler.GetExtinctTimeStampWithCfg(AchID, cfg)
  ...
  if AchieveHandler.bGetExtinctRsp then
    local timeType = "client"
    local clientMap = AchieveHandler.resClientExtinctInfo
    if clientMap then
      versionTime = clientMap[AchID]
    end
    if not versionTime or versionTime == 0 then
      timeType = "backend"
      local backendMap = AchieveHandler.resOutPrintInfo
      if backendMap then
        versionTime = backendMap[AchID]
      end
    end
    log(bWriteLog and string.format("AchieveHandler.GetExtinctTimeStampWithCfg fallback to backend extinct time, timeType = %s, AchID=%s, backendVersionTime=%s", tostring(timeType), tostring(AchID), tostring(versionTime)))
  else
    log(bWriteLog and string.format("AchieveHandler.GetExtinctTimeStampWithCfg, AchID=%s, OutPrintTimeForShowInUI=%s, OutPrintTime=%s", tostring(AchID), tostring(cfg.OutPrintTimeForShowInUI), tostring(cfg.OutPrintTime)))
    local TimeUtil = require("client.common.time_util")
    versionTime = TimeUtil.TimeStringToUnixstamp(cfg.OutPrintTimeForShowInUI)
    if not versionTime or versionTime == 0 then
      versionTime = TimeUtil.TimeStringToUnixstamp(cfg.OutPrintTime)
    end
  end
  return versionTime
end
