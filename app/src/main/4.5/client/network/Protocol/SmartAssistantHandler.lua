...
function SmartAssistantHandler.on_robot_assistant_get_soul_recommend_topic_rsp(err_code, info, ext_info)
  ...
  log_tree("SmartAssistantHandler.on_robot_assistant_get_soul_recommend_topic_rsp ext_info", ext_info)
  if err_code ~= 0 then
    ShowNotice(err_code)
    return true
  end
  return false, info, ext_info
end
...
function SmartAssistantHandler.send_get_highlight_reel_list_req()
  printf("SmartAssistantHandler.send_get_highlight_reel_list_req")
  NetManager.SendPkg(49681767)
end
function SmartAssistantHandler.on_get_highlight_reel_list_rsp(err_code, generating, finished)
  printf("SmartAssistantHandler.on_get_highlight_reel_list_rsp err_code:%s, generating:%s, finished:%s", err_code, generating and json.encode(generating) or "nil", finished and json.encode(finished) or "nil")
  if err_code ~= 0 then
    ShowNotice(err_code)
  end
  local logic_highlight_reel = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.logic_highlight_reel)
  logic_highlight_reel:_OnGetListRsp(err_code, generating, finished)
end
function SmartAssistantHandler.send_clear_highlight_reel_red_dot_req(battle_id)
  printf("SmartAssistantHandler.send_clear_highlight_reel_red_dot_req battle_id:%s", battle_id)
  NetManager.SendPkg(1664374951, battle_id)
end
function SmartAssistantHandler.on_clear_highlight_reel_red_dot_rsp(err_code, battle_id)
  printf("SmartAssistantHandler.on_clear_highlight_reel_red_dot_rsp err_code:%s, battle_id:%s", err_code, battle_id)
  local logic_highlight_reel = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.logic_highlight_reel)
  logic_highlight_reel:_OnClearRedDotRsp(err_code, battle_id)
end
local reqRsp = {
