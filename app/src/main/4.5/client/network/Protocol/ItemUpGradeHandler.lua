...
function ItemUpGradeHandler.send_upgrade_unlock_accessory_req(instid, acc_id, item_cost)
  ...
  acc_id = ItemUpgradeMgr:GetLevelBasePartId(acc_id)
...
function ItemUpGradeHandler.HasItem(itemId)
  ...
  local ItemUpgradeMgr = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.ItemUpgradeModule)
  local baseLevelPartId = ItemUpgradeMgr:GetLevelBasePartId(itemId)
  for _, group in pairs(ItemUpGradeHandler.data) do
    if group[baseLevelPartId] then
      return true
    end
  end
  return false
...
function ItemUpGradeHandler.IsUnLock(groupId, partId)
  ...
  local ItemUpgradeMgr = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.ItemUpgradeModule)
  local baseLevelPartId = ItemUpgradeMgr:GetLevelBasePartId(partId)
  return ItemUpGradeHandler.data[groupId][baseLevelPartId] ~= nil
end
function ItemUpGradeHandler.send_upgrade_item_req(instid, group_id, item_level, item_cost, form_choice)
  NetManager.SendPkg(104623399, instid, group_id, item_level, item_cost, form_choice)
end
