...
function MatchHandler.on_on_match_success(msg, team_info, antsvoice_url, deathmatch_teams, game_team_size, gameid, team_id, camp_id, sub_mode, feedback_flag, ugc_mod, promotion_layer)
  log(bWriteLog and "MatchHandler.on_match_success..submode id " .. tostring(sub_mode) .. " gameid=" .. tostring(gameid))
  LobbySystem.on_match_success(msg, team_info, antsvoice_url, deathmatch_teams, game_team_size, gameid, team_id, camp_id, sub_mode, feedback_flag, ugc_mod, promotion_layer)
end
...
function MatchHandler.send_batch_get_data_pass_to_ds_req(items)
  log(bWriteLog and "MatchHandler.send_batch_get_data_pass_to_ds_req item_cnt = " .. tostring(items and #items or 0))
  NetManager.SendPkg(2064178635, items)
end
function MatchHandler.on_batch_get_data_pass_to_ds_rsp(err_code, results)
  log(bWriteLog and "MatchHandler.on_batch_get_data_pass_to_ds_rsp err_code = " .. tostring(err_code))
  local logic_battle_data_transmission = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.logic_battle_data_transmission)
  logic_battle_data_transmission:proc_on_batch_get_data_pass_to_ds_rsp(err_code, results)
end
function MatchHandler.send_batch_set_data_pass_to_ds_req(items)
  log(bWriteLog and "MatchHandler.send_batch_set_data_pass_to_ds_req item_cnt = " .. tostring(items and #items or 0))
  NetManager.SendPkg(991643771, items)
end
function MatchHandler.on_batch_set_data_pass_to_ds_rsp(err_code, results)
  log(bWriteLog and "MatchHandler.on_batch_set_data_pass_to_ds_rsp err_code = " .. tostring(err_code))
  local logic_battle_data_transmission = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.logic_battle_data_transmission)
  logic_battle_data_transmission:proc_on_batch_set_data_pass_to_ds_rsp(err_code, results)
end
...
function MatchHandler.send_team_cross_bucket_req()
  NetManager.SendPkg(1316727344)
end
function MatchHandler.on_team_cross_bucket_res(err_code)
  log(bWriteLog and "MatchHandler.on_team_cross_bucket_res err_code = " .. tostring(err_code))
end
function MatchHandler.on_team_cross_bucket_notify()
  log(bWriteLog and "MatchHandler.on_team_cross_bucket_notify")
  local TeamUpNewSystem = require("client.slua.logic.teamup.logic_team_up")
  if TeamUpNewSystem.GetTeamNum() > 1 and not TeamUpNewSystem.IsTeamLeader() then
    log(bWriteLog and "MatchHandler.on_team_cross_bucket_notify -- not leader, ignore")
    return
  end
  local CommonMsgBoxMgr = require("client.slua.logic.common.logic_common_msg_box")
  local title = LocUtil.GetLocalizeResStr(101001)
  local nMsgLocId = 818405
  local msg = LocUtil.GetLocalizeResStr(nMsgLocId)
  CommonMsgBoxMgr.Show(2, title, msg, function()
    MatchHandler.send_team_cross_bucket_req()
  end, nil)
end
return MatchHandler