...
function ChatHandler.send_report_info(securityLog)
  log_tree("send_report_info", securityLog)
...
function ChatHandler.send_get_chat_collect_hall_list_req()
  NetManager.SendPkg(522408807)
end
function ChatHandler.on_get_chat_collect_hall_list_rsp(err, response)
  if err ~= 0 then
    ShowNotice(err)
    return
  end
  local Logic_PlanCHTopicModule = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.Logic_PlanCHTopicModule)
  Logic_PlanCHTopicModule:on_get_chat_collect_hall_list_rsp(response)
end
return ChatHandler