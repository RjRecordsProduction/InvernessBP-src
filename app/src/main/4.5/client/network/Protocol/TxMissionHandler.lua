...
function TxMissionHandler.send_metro_query_depot_items_req(item_ids)
  NetManager.SendPkg(1285442027, item_ids)
end
function TxMissionHandler.on_metro_query_depot_items_rsp(err_code, exist_item_ids)
end
function TxMissionHandler.on_lgd_metro_bonus_ntf(resid, count)
  log(bWriteLog and string.format("TxMissionHandler.on_lgd_metro_bonus_ntf, resid:%s count:%s", resid, count))
  local logic_xmission_info = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.logic_xmission_info)
  logic_xmission_info:proc_lgd_metro_bonus_ntf(resid, count)
end
local reqRsp = {
  send_query_tmode_room_battle_historys_req = "on_query_tmode_room_battle_historys_rsp",
  send_metro_query_depot_items_req = "on_metro_query_depot_items_rsp"
}
