...
local ErrSmallRPAlreadyHasGift = 18120021
...
function SmallRPHandler.send_small_rp_unlock_req(sourceType, if_by_unlock_card, force_buy)
  SmallRPHandler._lastUnlockSourceType = sourceType
  SmallRPHandler._lastUnlockByCard = if_by_unlock_card
  NetManager.SendPkg(1652991127, sourceType, if_by_unlock_card, force_buy)
end
function SmallRPHandler.on_small_rp_unlock_rsp(err_code)
  ...
  if err_code ~= 0 then
    if err_code == ErrSmallRPAlreadyHasGift then
      local Logic_SmallRP = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.Logic_SmallRP)
      Logic_SmallRP:ShowAlreadyHasGiftConfirm(SmallRPHandler._lastUnlockSourceType)
      return
    end
    ShowNotice(err_code)
    return
