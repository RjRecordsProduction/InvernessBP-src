...
function RegionHandler.on_sync_player_region_info(regionData, regionList, isBlockUser)
  ...
  logic_compliance.SDKQueryUserStatus(function(jsonData)
    if jsonData and jsonData.region then
      local compliance_util = require("client.slua.logic.gdpr.compliance_util")
      FuncUtil.RegisterCountry = compliance_util.GetRegionCodeByRegion(jsonData.region)
      log(bWriteLog and string.format("RegionHandler SDK registerCountry: %s", tostring(FuncUtil.RegisterCountry)))
    end
  end)
end
