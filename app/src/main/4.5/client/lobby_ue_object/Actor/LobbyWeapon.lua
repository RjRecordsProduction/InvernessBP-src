...
function LobbyWeapon:GetWeaponPlayActionID()
  local WeaponAvatarComp_BP = self.WeaponAvatarComp_BP
  return not slua.isValid(WeaponAvatarComp_BP) and 0 or WeaponAvatarComp_BP:GetLobbyWeaponPlayActionID()
end
...
function LobbyWeapon:IsWeaponAvatarFeatureSwitchOn(FeatureSwitchType, bCheckSelfOnly)
  local WeaponAvatarComp = self.WeaponAvatarComp_BP
  if not slua.isValid(WeaponAvatarComp) then
    log(bWriteLog and "LobbyWeapon:IsWeaponAvatarFeatureSwitchOn WeaponAvatarComp is nil")
    return true
  end
  local WeaponSkinDefineID = WeaponAvatarComp:GetEquippedItemDefineID(GUN_MASTER_SLOT)
  if not WeaponSkinDefineID or not WeaponSkinDefineID.TypeSpecificID then
    log(bWriteLog and "LobbyWeapon:IsWeaponAvatarFeatureSwitchOn WeaponSkinDefineID is nil")
    return true
  end
  local ItemUpgradeMgr = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.ItemUpgradeModule)
  local GroupID = ItemUpgradeMgr:GetNormalGroupIDOfWeaponID(WeaponSkinDefineID.TypeSpecificID)
  if GroupID <= 0 then
    return true
  end
  return DataMgr.IsWeaponSkinFeatureSwitchOn(GroupID, FeatureSwitchType, bCheckSelfOnly)
end
