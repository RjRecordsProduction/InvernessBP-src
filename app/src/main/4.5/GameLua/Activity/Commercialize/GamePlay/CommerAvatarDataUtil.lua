...
function CommerAvatarDataUtil:WeaponCanUsePendant(skinID)
  ...
  local IsPendant = ItemUpgradeMgr:CheckHasPendantByItemID(skinID)
  if IsPendant then
    return IsPendant == 1
  end
  return false
  ...
  if weapon_skin_list then
    for i, v in pairs(weapon_skin_list) do
      ...
      if weaponattach_skin_list and weaponattach_skin_list[v] ~= nil then
        ...
        local weaponLevel = ItemUpgradeMgr:GetLevel(WeaponID) or 0
        for _, attachId in pairs(weaponattach_skin_list[v]) do
          ...
          if attachCfg and attachCfg.GroupID and attachCfg.GroupID ~= GroupID then
          ...
          local levelMapCfg = CDataTable.GetTableData("ItemUpgradePartLevelMap", attachId)
          if levelMapCfg and levelMapCfg.BasePartId then
            local matched = ItemUpgradeMgr:GetLevelMatchedPartId(levelMapCfg.BasePartId, GroupID, weaponLevel)
            if matched then
              weaponattach_skin_list[v][_] = matched
            else
              print(bWriteLog and string.format("ConstructWeaponSkinList: no level-matched part, attachId=%s weaponId=%s level=%d", attachId, WeaponID, weaponLevel))
            end
          end
        end
...
function CommerAvatarDataUtil:GetTestExtendAttribute(attr)
      ...
      set_info = {
        [2] = {
          [0] = {
            unlock_info = {
              [2] = {
                [UnLockFeatureType.TeamEnterAction2] = {state = 1},
                [UnLockFeatureType.SprintShow] = {state = 1}
              }
            },
            flag_info = {
              [2] = {
                [XSuitFeatureSwitchType.RunAction] = {flag_state = 1}
              },
              [7] = {
                [XSuitFeatureSwitchType.Orbiter] = {flag_state = 1}
              }
            }
          }
        },
        ...
        [16] = {
          [0] = {
            flag_info = {
              ...
              [5] = {
                [XSuitFeatureSwitchType.Execute] = {flag_state = 1}
              }
            }
          }
        }
  ...
  elseif attr == ExtendAttribute.WeaponShowEmote then
    TestData = {12219541}
  elseif attr == ExtendAttribute.XSuitScheme then
    TestData = {
      [16] = {
        [0] = {
          [3] = 16000302
  ...
  elseif attr == ExtendAttribute.EliminationKingClothOverrideEnabled then
    return {flag = true}
...
function CommerAvatarDataUtil:_FillVehicleSkinList(PlayerInfo, uPlayerController)
  ...
  if uPlayerController.ShowVehicleSkin ~= 0 then
    local vehicleSkinId = uPlayerController.ShowVehicleSkin
    local ExtendAttribute = require("Server.config.ExtendAttribute")
    local PlayerDataMgr = require("Server.Data.ServerPlayerDataMgr")
    ...
    if VehicleFeatureTable then
      local caliperId = VehicleFeatureTable.brake_caliper_info and VehicleFeatureTable.brake_caliper_info[vehicleSkinId]
      local wheelHubId = VehicleFeatureTable.wheel_hub_info and VehicleFeatureTable.wheel_hub_info[vehicleSkinId]
      local sunroofId = VehicleFeatureTable.sunroof_info and VehicleFeatureTable.sunroof_info[vehicleSkinId]
      uPlayerController.VehiclePartData = {
        [1] = caliperId,
        [2] = wheelHubId,
        [3] = sunroofId
      }
...
function CommerAvatarDataUtil:FillGlideConsumable(id, tBornItems, tInfo)
  ...
  if not VehicleUseConfig then
    ...
    for _, specialID in pairs(LogicXSuit.GetAllSpecialGlideIDs(id) or {}) do
      if self:FillGlideConsumable(specialID, tBornItems, tInfo) then
        log(bWriteLog and "CommerAvatarDataUtil:_FillGlideConsumable base:%s matched special:%s", id, specialID)
        return true
      end
    end
    log(bWriteLog and "CommerAvatarDataUtil:_FillGlideConsumable no config for id:%s", id)
    return false
  end
  ...
  if wear_ext then
    ...
    for _, v in pairs(wear_ext) do
      ...
      XSuitAvatarDataUtil:ChangeXSuitSourceData(vCopy)
      local clothes = XSuitAvatarDataUtil:GetConvertXSuitItemIDByExtAttr(vCopy, nil, tInfo.ext_attr or {})
