...
GameMainConfig.DefaultPath = {
  ...
  Client = {
    ...
    SpecialLayoutConfig = {
      Suffix = "Config.SpecialLayoutConfig",
      bSuperposition = true,
      bNotAllowMerge = true
    },
    UserSettingInMod = {
      Suffix = "Config.UserSettingInMod",
      bSuperposition = true
    },
    NewbieGuideConfig = {
    ...
    FaceIntroUIConfig = {
      Suffix = "Config.FaceIntroUIConfig",
      bSuperposition = true
    },
    MapWidgetConfig = {
      Suffix = "Config.MapWidgetConfig",
      bSuperposition = true
    },
    ItemUIConfig = {
      Suffix = "Config.ItemUIConfig",
      bSuperposition = true
    }
  },
  DS = {
  ...
  GamePlay = {
    ...
    HighlightMomentConfig_Vehicle = {
      Suffix = "Config.HighlightMomentConfig_Vehicle",
      bSuperposition = true
    },
    HighlightMomentConfig_SceneShow = {
      Suffix = "Config.HighlightMomentConfig_SceneShow",
      bSuperposition = true
    },
    HighlightMomentSubsystem_DSChecker = {
      Suffix = "HighlightMoment.HighlightMomentSubsystem_DSChecker",
      bSuperposition = true
    },
    ...
    GameReportConfig = {
      Suffix = "Config.GameReportConfig",
      bSuperposition = true
    }
  }
}
...
function GameMainConfig.AddModeConfig(EvoBaseConfigPath, ModeConfigPath, DefaultPath, bFastCopyTable)
  ...
  if EvoBaseConfigPath ~= "" and GamePlayTools.CanRequireLuaFile(EvoBaseConfigPath) then
...
function GameMainConfig.GetAnimConfigInfo(Type, Key)
  ...
  if not GamePlayTools.LuaFileExits(EvoBasePath) then
    local BRModPath = string.format("GameLua.Mod.BRMod.%s.%s", Type, Value.Suffix)
    if GamePlayTools.LuaFileExits(BRModPath) then
      print(bWriteLog and string.format("GameMainConfig.GetAnimConfigInfo EvoBasePath fallback to BRMod: %s -> %s", EvoBasePath, BRModPath))
      EvoBasePath = BRModPath
    end
  end
  local ExtraPaths = {}
  ...
  local AppendConfig = function(Type)
    for Key, Value in pairs(GameMainConfig.DefaultPath[Type]) do
      ...
      if not GamePlayTools.LuaFileExits(EvoBasePath) then
        local BRModPath = string.format("GameLua.Mod.BRMod.%s.%s", Type, Value.Suffix)
        if GamePlayTools.LuaFileExits(BRModPath) then
          print(bWriteLog and string.format("GameMainConfig.GetConfig EvoBasePath fallback to BRMod: %s -> %s", EvoBasePath, BRModPath))
          EvoBasePath = BRModPath
        end
      end
      if GameMainConfig.CheckCanAddConfig(Key, Value.bSuperposition, Type == "Client") then
