...
function InputStateControl.ResetInputKeyState()
  print(bWriteLog and "InputStateControl_Debug_Msg: ResetInputKeyState")
  if not InputStateControl._InputStateTable then
    return
  end
  for _, StateInfo in pairs(InputStateControl._InputStateTable) do
    if StateInfo then
      StateInfo.State = _KeyState.None
      StateInfo.LastFrame = 0
    end
  end
end
