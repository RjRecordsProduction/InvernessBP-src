...
function SkillUtils.GetHitPointFromCharacterViewIncludePawn(uCharacter, tIgnoreActos)
  print(bWriteLog and "SkillUtils:GetHitPointFromCharacterViewIncludePawn")
  if not slua.isValid(uCharacter) then
    return FVector.ZeroVector, nil
  end
  local uPlayer = uCharacter:GetPlayerControllerSafety()
  ...
  if slua.isValid(uVehicle) then
    ...
    local uVehicleSeat = uVehicle:GetSeatComponent()
    if slua.isValid(uVehicleSeat) then
      for _, OtherCharacter in pairs(uVehicleSeat.SeatOccupiers) do
        if slua.isValid(OtherCharacter) and OtherCharacter ~= uCharacter then
          uIgnoreActorArray:Add(OtherCharacter)
        end
      end
  ...
  local uPlayerState = uCharacter.GetPlayerStateSafety and uCharacter:GetPlayerStateSafety() or nil
  if slua.isValid(uPlayerState) and uPlayerState.GetTeamMatePlayerStateList then
    ...
    if TeammateList then
      for _, uTeammatePS in pairs(TeammateList) do
        if slua.isValid(uTeammatePS) and uTeammatePS.GetPlayerCharacter then
          local uTeammateChar = uTeammatePS:GetPlayerCharacter()
          if slua.isValid(uTeammateChar) then
            uIgnoreActorArray:Add(uTeammateChar)
          end
        end
      end
    end
  ...
  if tIgnoreActos then
    for _, IgnoreActor in ipairs(tIgnoreActos) do
      if slua.isValid(IgnoreActor) then
        uIgnoreActorArray:Add(IgnoreActor)
      end
    end
  ...
  local Success = false
  Success, HitResult = USTExtraBlueprintFunctionLibrary.TraceBlockIncludePawn(uCharacter, StartLoc, EndLoc, HitResult, uIgnoreActorArray, false)
  if HitResult.bBlockingHit then
    return FVector(HitResult.ImpactPoint.X, HitResult.ImpactPoint.Y, HitResult.ImpactPoint.Z), HitResult.Actor
  end
  return FVector.ZeroVector, nil
