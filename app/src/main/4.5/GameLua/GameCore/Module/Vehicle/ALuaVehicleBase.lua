...
function LuaVehicleBase:_PostConstruct()
  ...
  if slua.isValid(VehicleAvatar) then
    ...
    self:AddControlEvent(VehicleAvatar, "VehicleSlotEquiped", self.HandleClientVehicleSlotEquiped, self)
...
function LuaVehicleBase:_PreDestruct()
  if self.VehicleUsersSkillTokenFeature then
    self.VehicleUsersSkillTokenFeature:_PreDestruct()
  end
  LuaVehicleBase.__super._PreDestruct(self)
end
...
function LuaVehicleBase:HandleClientVehicleSlotEquiped()
  local VehicleAvatar = self:GetAvatarComponent()
  if not slua.isValid(VehicleAvatar) then
    return
  end
  local netStruct = VehicleAvatar.NetHighTireStruct
  if netStruct and netStruct.bEnableHighTire ~= nil then
    print(bWriteLog and "LuaVehicleBase:HandleClientVehicleSlotEquiped reapply EnableHighTireLight, flag:", tostring(netStruct.bEnableHighTire))
    VehicleAvatar:EnableHighTireLight(netStruct.bEnableHighTire)
  end
end
...
function LuaVehicleBase:ServerResetToTransform(FLocation, FRotation, bClientSetTo)
  ...
  if bClientSetTo == true then
    ...
    self:MulticastResetAdnSetToTransform(FLocation, FRotation)
  else
    self:ClientResetToTransform()
  end
  self:ServerResetToPosition(FLocation, FRotation, true)
  if slua.isValid(self.BallMovement) then
    self.BallMovement.CurrentRotationYaw = FRotation.Yaw
    self.BallMovement.CameraRationYaw = FRotation.Yaw
    self.BallMovement.STCameraRationYaw = FRotation.Yaw
  end
...
LuaVehicleBase.MulticastRPC.MulticastResetAdnSetToTransform = {
  Reliable = true,
  Params = {
    import("/Script/CoreUObject.Vector"),
    import("/Script/CoreUObject.Rotator")
...
function LuaVehicleBase:MulticastResetAdnSetToTransform(InLoc, InRot)
  print(bWriteLog and "LuaVehicleBase:MulticastResetAdnSetToTransform")
  if InRot ~= nil and slua.isValid(self.BallMovement) then
    self.BallMovement.CurrentRotationYaw = InRot.Yaw
    self.BallMovement.CameraRationYaw = InRot.Yaw
    if self.BallMovement.ResetSimQuatFromRotator then
      self.BallMovement:ResetSimQuatFromRotator(InRot)
    end
    local vehicleSpringArm = self:GetVehicleSpringArm()
    if slua.isValid(vehicleSpringArm) then
      vehicleSpringArm:SetSteerCameraYaw(InRot.Yaw, false)
    end
    if self.DecorationCircle and slua.isValid(self.DecorationCircle.MeshComponent) then
      local CircleRot = FRotator()
      CircleRot.Pitch = self.DecorationCircle.DefaultTransform:Rotator().Pitch + InRot.Pitch
      CircleRot.Roll = self.DecorationCircle.DefaultTransform:Rotator().Roll + InRot.Roll
      CircleRot.Yaw = self.DecorationCircle.DefaultTransform:Rotator().Yaw + InRot.Yaw
      self.DecorationCircle.MeshComponent:K2_SetWorldRotation(CircleRot, false, nil, true)
    end
  end
end
...
function LuaVehicleBase:ReActivateExhaustParticle()
  if not self.bEnableUpdateExhuastEffectBP then
    self:LuaBroadcast("OnExhuastEffectRefresh")
    return
  end
