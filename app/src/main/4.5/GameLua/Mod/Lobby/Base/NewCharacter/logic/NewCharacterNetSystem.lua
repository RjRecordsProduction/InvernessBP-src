...
function NewCharacterNetSystem:EnsureSyncForLevelUp()
  local TimeUtil = require("client.common.time_util")
  local nowMs = TimeUtil.GetMiliseconds() or 0
  if self._lastSyncForLevelUpMs and nowMs - self._lastSyncForLevelUpMs < 3000 then
    log(bWriteLog and "NewCharacterNetSystem:EnsureSyncForLevelUp skip by debounce")
    return
  end
  self._lastSyncForLevelUpMs = nowMs
  local LobbyModUtils = require("GameLua.Mod.Lobby.Base.Common.LobbyModUtils")
  local CharacterHandler = require("client.network.Protocol.CharacterHandler")
  local ModName = LobbyModUtils.Enum_Mod_Name and LobbyModUtils.Enum_Mod_Name.EName_NewCharacter
  if not ModName then
    log_warning(bWriteLog and "NewCharacterNetSystem:EnsureSyncForLevelUp Enum_Mod_Name.EName_NewCharacter is nil")
    CharacterHandler.send_character_info_req()
    return
  end
  if LobbyModUtils.IsModDownloaded(ModName) then
    log(bWriteLog and "NewCharacterNetSystem:EnsureSyncForLevelUp mod downloaded, resend character_info_req")
    CharacterHandler.send_character_info_req()
    return
  end
  log(bWriteLog and "NewCharacterNetSystem:EnsureSyncForLevelUp mod not downloaded, trigger download")
  LobbyModUtils.DownloadMod(ModName, function()
    log(bWriteLog and "NewCharacterNetSystem:EnsureSyncForLevelUp mod download finished, resend character_info_req")
    CharacterHandler.send_character_info_req()
  end)
end
