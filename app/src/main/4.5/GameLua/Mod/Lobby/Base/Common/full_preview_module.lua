...
function full_preview_module:ToFullScreenCamera(avatarData)
  ...
  if isShowVideo then
    log(bWriteLog and "full_preview_module:ToFullScreenCamera.  isShowVideo")
    ShowNotice(87081)
    return false
  end
  ...
  self.nSceneType = avatarData.nSceneType
...
function full_preview_module:OnCameraChanged(nextPosY, nextPosZ, nextFov)
  ...
  local n  local n  local ConstAvatarDislay = require("client.slua.traits.DetailComponent.AvatarDisplay.ConstAvatarDislay")
  log(bWriteLog and "full_preview_module:OnCameraChanged. self.nSceneType: " .. tostring(self.nSceneType))
  if self.nSceneType and self.nSceneType == ConstAvatarDislay.ESceneType.RpPreview then
    nmoveZMaxNear = moveZMaxNear / 5
    nmoveZMaxFar = moveZMaxFar / 5
  end
  local ZLimit = nmoveZMaxNear - (nmoveZMaxNear - nmoveZMaxFar) * t
...
function full_preview_module:ResetSceneType()
  self.nSceneType = nil
end
