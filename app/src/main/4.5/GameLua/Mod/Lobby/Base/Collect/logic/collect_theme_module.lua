...
function collect_theme_module:GetLibrarySortData(quality)
  local collect_module = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.collect_module)
  local ownedIndex = collect_module:GetOwnedIndex()
  if not quality and ownedIndex and ownedIndex.ownedTabs then
    local result = {}
    for tab, _ in pairs(ownedIndex.ownedTabs) do
      result[#result + 1] = tab
    end
...
function collect_theme_module:GetLibrarySortGridData(tab)
  ...
  local collect_module = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.collect_module)
  local ownedIndex = collect_module:GetOwnedIndex()
  if ownedIndex and ownedIndex.tab2Items then
    local list = ownedIndex.tab2Items[tab]
    if not list then
      return {}
    end
    local result = {}
...
function collect_theme_module:GetClotheTotalCount()
  ...
  local ownedIndex = collect_module:GetOwnedIndex()
  if ownedIndex then
    return ownedIndex.clotheCount or 0
  end
  local itemTb = collect_module:GetItemTb()
  local LogicFusionModule = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.LogicFusionModule)
  local ENUM_WardrobePageType_Avatar = ENUM_WardrobePageTypeId.ENUM_WardrobePageType_Avatar
  local count = 0
  for itemId, _ in pairs(itemTb) do
    if not LogicFusionModule:IsFusionTargetItem(itemId) and self:ItemToPageType(itemId) == ENUM_WardrobePageType_Avatar then
      count = count + 1
    end
  end
  return count
