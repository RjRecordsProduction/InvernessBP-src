...
function collect_privacy_module:CanShowCollectLevelByProfile(profile)
  if not LobbySystem.CheckOpen(BP_ENUM_SWITCH_COLLECT) then
    log_warning(bWriteLog and "collect_privacy_module:CanShowCollectLevelByProfile.  CheckOpen() is not")
    return false
  end
  local myUid = DataMgr.roleData.uid
  if profile and tonumber(profile.uid) ~= tonumber(myUid) then
    return self:CanShowCollectLevel(profile and profile.collect_data and profile.collect_data.privacy)
  end
  return true
end
