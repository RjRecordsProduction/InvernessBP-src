...
local dragItemOffsetX = 20
local DRAG_SAFETY_TIMEOUT = 1.0
function UI_Lobby_Downloader_Btn:ctor()
  self.needRefreshPercent = true
  self.dragSafetyTimerHandle = nil
...
function UI_Lobby_Downloader_Btn:OnPostInitialize()
  ...
  self.UIRoot.TextBlock_1:SetText(LocUtil.GetLocalizeResStr(180106))
...
function UI_Lobby_Downloader_Btn:InitDrag()
  ...
  if saveData and saveData.X and saveData.Y and tonumber(saveData.X) and tonumber(saveData.Y) then
    ...
    self.UIRoot.CommonDragDropItem.Slot:SetPosition(FVector2D(saveData.X + dragItemOffsetX, saveData.Y - 20))
  end
end
...
function UI_Lobby_Downloader_Btn:UpdateButtonState()
  ...
  local pct = 1
  local btnWidth = 80
  if not LogicPufferBundle.IsFitLobbyResDownloaded() and PufferDownloader.PufferJsonDownloadReturn then
    switchIndex = 2
    image = self.UIRoot.Image_7
    local cSize, tSize = LogicPufferBundle.GetPackListSize({
      PufferConst.FIT_LOBBY_RES_KEY
    })
    if tSize ~= 0 then
      pct = cSize / tSize
    end
    ...
    if cfg and not cfg.is_got and LogicPufferBundle.GetBundleState(bundleID) ~= PufferConst.ENUM_DownloadState.Done and not PufferDownloader.RecommendReddot then
      ...
      image = self.UIRoot.Image_Progress_02
    end
  end
  if image then
  ...
  local UIUtil = require("client.common.ui_util")
  UIUtil.SetSize(self.UIRoot.CommonDragDropItem, btnWidth, 80)
end
function UI_Lobby_Downloader_Btn:NeedShowReddot()
  ...
  if PufferDownloader.DownloadRewardCfg then
    local data = PufferDownloader.DownloadRewardCfg[Enum_BundleID.Recommend]
    ...
    if not v.is_got and v.itemid1 and v.itemid1 > 0 then
      ...
      if cfg then
        if LogicPufferBundle.GetBundleState(bundleID) == PufferConst.ENUM_DownloadState.Done then
          ...
          elseif bundleID ~= Enum_BundleID.Recommend and bundleID ~= PufferConst.BIND_PHONE_MAIL_REWARD_ID then
            log_format("UI_Lobby_Downloader_Btn:NeedShowReddot. bundleID=%s", bundleID)
            return true
          end
        end
...
function UI_Lobby_Downloader_Btn:OnDrag(DragWidget, Index, GeneratedWidget, DragDropData)
  ...
  if self.dragSafetyTimerHandle then
    self:RemoveTimer(self.dragSafetyTimerHandle)
  end
  self.dragSafetyTimerHandle = self:AddTimerOnce(DRAG_SAFETY_TIMEOUT, function()
    self:_OnDragSafetyTimeout()
  end)
end
...
function UI_Lobby_Downloader_Btn:OnDragCancel(DragWidget, Index, DragDropData)
  ...
  self.UIRoot.CommonDragDropItem.Slot:SetPosition(FVector2D(viewportPosition.X + dragItemOffsetX, viewportPosition.Y - 20))
  self.UIRoot.Panel_Downloader:SetWidgetVisibility(UEnums.ESlateVisibility.SelfHitTestInvisible)
  local PlayerPrefsSystem = require("client.logic.LogicPlayerPrefs.playerprefs")
  local pos = {}
...
function UI_Lobby_Downloader_Btn:_ClearDragSafetyTimer()
  self.bIsDragging = false
  if self.dragSafetyTimerHandle then
    self:RemoveTimer(self.dragSafetyTimerHandle)
    self.dragSafetyTimerHandle = nil
...
function UI_Lobby_Downloader_Btn:_OnDragSafetyTimeout()
  log(bWriteLog and "UI_Lobby_Downloader_Btn:_OnDragSafetyTimeout - OnDragCancel lost, restoring panel")
  self.bIsDragging = false
  self.dragSafetyTimerHandle = nil
...
function UI_Lobby_Downloader_Btn:ResetDragPosition()
  ...
  self.UIRoot.CommonDragDropItem.Slot:SetPosition(FVector2D(initPos.X + dragItemOffsetX, initPos.Y - 20))
end
