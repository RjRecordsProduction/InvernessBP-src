...
function collect_module:DefineAndResetData()
  ...
  self.itemTb = {}
  self.ownedIndex = nil
  self.ownedIndexUid = nil
  self.itemTbFingerprints = nil
  self.item2Score = {}
...
local _FP_MOD = 2147483647
local _CalcItemTbFingerprint = function(itemTb)
  if not itemTb then
    return 0, 0, 0
  end
  local n, h1, h2 = 0, 0, 0
  for itemId, time in pairs(itemTb) do
    n = n + 1
    local t = type(time) == "number" and time or 1
    h1 = (h1 + itemId * 31 + t) % _FP_MOD
    h2 = (h2 + itemId + t * 131) % _FP_MOD
  end
  return n, h1, h2
end
function collect_module:OnGetItemData(uid, data)
  ...
  local n, h1, h2 = _CalcItemTbFingerprint(self.itemTb)
  local fp = self.itemTbFingerprints and self.itemTbFingerprints[uid]
  if self.ownedIndex and self.ownedIndexUid == uid and fp and fp.n == n and fp.h1 == h1 and fp.h2 == h2 then
    log(bWriteLog and string.format("collect_module:OnGetItemData skip _BuildOwnedIndex (cache hit) uid=%s n=%d", tostring(uid), n))
    return
  end
  self:_BuildOwnedIndex()
  self.ownedIndexUid = uid
  self.itemTbFingerprints = self.itemTbFingerprints or {}
  self.itemTbFingerprints[uid] = {
    n = n,
    h1 = h1,
      }
end
function collect_module:_BuildOwnedIndex()
  local itemTb = self.itemTb
  if not itemTb or not next(itemTb) then
    self.ownedIndex = nil
    return
  end
  ...
  local PACKAGE_SLOT = ENUM_ITEM_SUBTYPE.Package_Slot
  local idx = {
    quality2Items = {},
    item2Quality = {},
    car2SubCar = {},
  ...
  for q = ItemMacros.QUALITY_PURPLE, QUALITY_GOLDEN do
    idx.quality2Items[q] = {}
  end
  for itemId, v in pairs(itemTb) do
    local itemData = fieldMap[itemId]
    local quality, itemSubType
    if itemData then
      quality = itemData.ItemQuality
      itemSubType = itemData.ItemSubType
    end
    local flagX = AvatarCommon.IsXSuit(itemId) and true or false
    local flagG = quality == QUALITY_GOLDEN and itemSubType == PACKAGE_SLOT and true or false
    local flagU = ItemUpgradeMgr:IsUpdateGun(itemId) and true or false
    local flagT = collect_season_module:IsBetterCar(itemId) and true or false
    idx.itemFlags[itemId] = {
      x = flagX,
      g = flagG,
      u = flagU,
      t = flagT
    ...
    if flagX then
      idx.countXSuit = idx.countXSuit + 1
    elseif flagG then
      idx.countGolden = idx.countGolden + 1
    elseif flagU then
      idx.countUpgrade = idx.countUpgrade + 1
    elseif flagT then
      idx.countTarot = idx.countTarot + 1
    end
    if itemData then
      local itemType = itemData.ItemType
      local wardrobeTab = itemData.WardrobeTab
      if quality then
        idx.item2Quality[itemId] = quality
        local bucket = idx.quality2Items[quality]
        if bucket then
          bucket[itemId] = v
        end
      end
      if itemType == VehicleType then
        idx.car2SubCar[itemId] = itemSubType
      elseif itemType == BUDDY_TYPE then
        idx.petTb[itemId] = itemSubType
      elseif itemType == BUDDY_NEW_TYPE then
        idx.petClothesTb[itemId] = itemSubType
      end
      local pageType
      if itemType == Weapon then
        pageType = ENUM_WardrobePageType_Weapon
      else
        pageType = tab2Sort[wardrobeTab]
      end
      if pageType == ENUM_WardrobePageType_Avatar and not LogicFusionModule:IsFusionTargetItem(itemId) and wardrobeTab then
        local list = idx.tab2Items[wardrobeTab]
        if not list then
          list = {}
          idx.tab2Items[wardrobeTab] = list
          idx.ownedTabs[wardrobeTab] = true
        end
        list[#list + 1] = itemId
        idx.clotheCount = idx.clotheCount + 1
      end
    end
  end
  self.ownedIndex = idx
  local collect_theme_module = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.collect_theme_module)
  collect_theme_module.Quality2ItemTb = idx.quality2Items
  collect_theme_module.Item2Quality = collect_theme_module.Item2Quality or prealloctable(0, 3700)
  local i2q = collect_theme_module.Item2Quality
  for itemId, q in pairs(idx.item2Quality) do
    if not i2q[itemId] then
      i2q[itemId] = q
    end
  end
  local collect_vehicle_module = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.collect_vehicle_module)
  collect_vehicle_module.car2SubCar = idx.car2SubCar
  local collect_pet_module = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.collect_pet_module)
  collect_pet_module.petTb = idx.petTb
  collect_pet_module.petClothesTb = idx.petClothesTb
  log(bWriteLog and string.format("collect_module:_BuildOwnedIndex done clothe=%d xSuit=%d golden=%d upgrade=%d tarot=%d", idx.clotheCount, idx.countXSuit, idx.countGolden, idx.countUpgrade, idx.countTarot))
end
function collect_module:GetOwnedIndex()
  return self.ownedIndex
end
...
function collect_module:UpdateCollectStatsData()
  ...
  self.updateTimer = self:AddTimer(1, function()
    ...
    local ownedIndex = self.ownedIndex
    if ownedIndex then
      self.collect_stats_data.CollectTypeCount1 = ownedIndex.countXSuit or 0
      self.collect_stats_data.CollectTypeCount2 = ownedIndex.countGolden or 0
      self.collect_stats_data.CollectTypeCount3 = ownedIndex.countUpgrade or 0
      self.collect_stats_data.CollectTypeCount4 = ownedIndex.countTarot or 0
    else
      local collect_season_module = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.collect_season_module)
      local ItemUpgradeMgr = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.ItemUpgradeModule)
      local AvatarCommon = require("client.slua.logic.avatar.avatar_common")
...
function collect_module:GetCalSpecialItemCountTimer(fCalFinishCallback)
  local ownedIndex = self.ownedIndex
  if ownedIndex then
    local xSuit = ownedIndex.countXSuit or 0
    local golden = ownedIndex.countGolden or 0
    local upgrade = ownedIndex.countUpgrade or 0
    local tarot = ownedIndex.countTarot or 0
    local nTimer = self:AddTimer(0, function()
