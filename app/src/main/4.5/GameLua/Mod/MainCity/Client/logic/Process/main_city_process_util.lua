...
function main_city_process_util.IsMainCityEntryOpen(showTips, skipGrayCheck, skipFitLobbyResCheck)
  log_format("main_city_process_util.IsMainCityEntryOpen showTips = %s, skipGrayCheck = %s, skipFitLobbyResCheck = %s", showTips, skipGrayCheck, skipFitLobbyResCheck)
  if IsWoWEditor then
    return false
  end
  local LogicPufferBundle = require("client.slua.logic.download.bundle.logic_puffer_bundle")
  if not skipFitLobbyResCheck and not LogicPufferBundle.IsFitLobbyResDownloaded() then
    log(bWriteLog and "main_city_process_util.IsMainCityEntryOpen FitLobbyRes not downloaded")
    return false
  end
