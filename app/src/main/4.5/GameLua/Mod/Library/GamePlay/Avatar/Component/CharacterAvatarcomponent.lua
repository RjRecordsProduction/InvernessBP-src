local CharacterAvatarComponent = {
  ...
  MulticastRPC = {},
  LuaEventContainer = {
    "OnCharacterAvatarComponentColorChanged"
  }
}
...
function CharacterAvatarComponent:ctor()
  ...
  self.ClothSchemeIDList = slua.Array(UEnums.EPropertyClass.Int)
  self.ClothSchemeIDMap = {}
...
function CharacterAvatarComponent:_PostConstruct()
  ...
  if Client then
    ...
    self:AddControlEvent(self, "OnAvatarEquipped", self.OnAvatarEquipFinish, self)
...
function CharacterAvatarComponent:OnAvatarEquipFinish(slotType, isEquipped, itemID)
  if isEquipped == true and slotType == EAvatarSlotType.EAvatarSlotType_ClothesEquipemtSlot then
    ...
    if slua.isValid(ClothMeshComp) and ClothMeshComp.StartClothSimulationWithDelay then
      ClothMeshComp:StartClothSimulationWithDelay(2, 2)
    end
  end
end
...
function CharacterAvatarComponent:_DestroyManagers()
  if not Client then
    return
  end
...
function CharacterAvatarComponent:ReceiveEndPlay(EndPlayReason)
  ...
  self:_DestroyManagers()
  self.ColorDiyData = {}
  self.bInitGoldenSuitInfo = false
  CharacterAvatarComponent.__super.ReceiveEndPlay(self, EndPlayReason)
end
function CharacterAvatarComponent:_PreDestruct()
  self:_DestroyManagers()
  CharacterAvatarComponent.__super._PreDestruct(self)
end
...
function CharacterAvatarComponent:InitGoldenSuitData()
  ...
  if slua.isValid(uPawn) then
    ...
    if GoldenSuitStateInfo then
      for AvatarPeriod, branchInfo in pairs(GoldenSuitStateInfo) do
        local AvatarPeriodStateInfo = branchInfo and branchInfo[0]
...
function CharacterAvatarComponent:InitXSuitFeatureList()
  ...
  if unlockFeatureData and unlockFeatureData.set_info then
    for period, periodData in pairs(unlockFeatureData.set_info) do
      if periodData then
        for branch, branchData in pairs(periodData) do
          if type(branchData) == "table" then
            if branchData.unlock_info then
              for level, levelData in pairs(branchData.unlock_info) do
                for index, indexData in pairs(levelData) do
                  if indexData and indexData.state == 1 then
                    tempUnlockArray:Add(period)
                    tempUnlockArray:Add(branch)
                    tempUnlockArray:Add(level)
                    tempUnlockArray:Add(index)
                    print(bWriteLog and "CharacterAvatarComponent:InitXSuitFeatureList unlock added:", period, branch, level, index)
                  end
                end
              end
            end
            if branchData.flag_info then
              for level, levelData in pairs(branchData.flag_info) do
                for index, indexData in pairs(levelData) do
                  if indexData and indexData.flag_state == 0 then
                    tempFlagArray:Add(period)
                    tempFlagArray:Add(branch)
                    tempFlagArray:Add(level)
                    tempFlagArray:Add(index)
                    print(bWriteLog and "CharacterAvatarComponent:InitXSuitFeatureList flag added:", period, branch, level, index)
                  end
                end
              end
            end
...
function CharacterAvatarComponent:InitClothSchemeList()
  print(bWriteLog and "CharacterAvatarComponent:InitClothSchemeList")
  local uPawn = self:GetOwner()
  if not uPawn then
    print(bWriteLog and "CharacterAvatarComponent:InitClothSchemeList Can`t find Pawn")
    return
  end
  local PlayerController = uPawn:GetPlayerControllerSafety()
  if not PlayerController then
    print(bWriteLog and "CharacterAvatarComponent:InitClothSchemeList Can`t find PlayerController")
    return
  end
  local CommerAvatarDataUtil = require("GameLua.Activity.Commercialize.GamePlay.CommerAvatarDataUtil")
  local ExtendAttribute = require("Server.config.ExtendAttribute")
  local schemeData = CommerAvatarDataUtil:GetPlayerExtendAttributeAndTest(PlayerController.UID, ExtendAttribute.XSuitScheme)
  local tempArray = slua.Array(UEnums.EPropertyClass.Int)
  if schemeData then
    for period, periodData in pairs(schemeData) do
      if periodData then
        for branch, branchData in pairs(periodData) do
          if branchData then
            for featureID, schemeID in pairs(branchData) do
              if schemeID and 0 < schemeID then
                tempArray:Add(schemeID)
                print(bWriteLog and "CharacterAvatarComponent:InitClothSchemeList added:", period, branch, featureID, schemeID)
              end
            end
          end
        end
  ...
  self.ClothSchemeIDList = tempArray
  print(bWriteLog and "CharacterAvatarComponent:InitClothSchemeList count:", self.ClothSchemeIDList:Num())
end
...
function CharacterAvatarComponent:OnRep_XSuitUnlockFeatureList()
  ...
  if 0 < count and count % 4 == 0 then
    for i = 0, count - 1, 4 do
      local period = self.XSuitUnlockFeatureList:Get(i)
      local branch = self.XSuitUnlockFeatureList:Get(i + 1)
      local level = self.XSuitUnlockFeatureList:Get(i + 2)
      local index = self.XSuitUnlockFeatureList:Get(i + 3)
      if period and branch and level and index then
        if not self.XSuitUnlockFeatureMap[period] then
          self.XSuitUnlockFeatureMap[period] = {}
        end
        if not self.XSuitUnlockFeatureMap[period][branch] then
          self.XSuitUnlockFeatureMap[period][branch] = {}
        end
        if not self.XSuitUnlockFeatureMap[period][branch][level] then
          self.XSuitUnlockFeatureMap[period][branch][level] = {}
        end
        self.XSuitUnlockFeatureMap[period][branch][level][index] = {state = 1}
        print(bWriteLog and "CharacterAvatarComponent:OnRep_XSuitUnlockFeatureList added:", period, branch, level, index)
      end
    end
    print(bWriteLog and "CharacterAvatarComponent:OnRep_XSuitUnlockFeatureList updated, count:", count / 4)
    ...
    EventSystem:postEvent(EVENTTYPE_PLAYEREVENT_AVATAR, EVENTID_PLAYEREVENT_AVATAR_XSUIT_FEATURE_CHANGED, self.Object)
  else
    print(bWriteLog and "CharacterAvatarComponent:OnRep_XSuitUnlockFeatureList invalid count:", count, "should be multiple of 4")
  end
end
function CharacterAvatarComponent:OnRep_XSuitFeatureFlagList()
  ...
  if 0 < count and count % 4 == 0 then
    for i = 0, count - 1, 4 do
      ...
      local branch = self.XSuitFeatureFlagList:Get(i + 1)
      local level = self.XSuitFeatureFlagList:Get(i + 2)
      local index = self.XSuitFeatureFlagList:Get(i + 3)
      if period and branch and level and index then
        if not self.XSuitFeatureFlagMap[period] then
          self.XSuitFeatureFlagMap[period] = {}
        end
        if not self.XSuitFeatureFlagMap[period][branch] then
          self.XSuitFeatureFlagMap[period][branch] = {}
        end
        if not self.XSuitFeatureFlagMap[period][branch][level] then
          self.XSuitFeatureFlagMap[period][branch][level] = {}
        end
        self.XSuitFeatureFlagMap[period][branch][level][index] = {flag_state = 0}
        print(bWriteLog and "CharacterAvatarComponent:OnRep_XSuitFeatureFlagList added:", period, branch, level, index)
      end
    end
    print(bWriteLog and "CharacterAvatarComponent:OnRep_XSuitFeatureFlagList updated, count:", count / 4)
    local AvatarHandle = self:GetLoadedHandle(EAvatarSlotType.EAvatarSlotType_ClothesEquipemtSlot)
    if AvatarHandle then
      self:ReFreshAnimListOverride(AvatarHandle)
    end
    ...
    print(bWriteLog and "CharacterAvatarComponent:OnRep_XSuitFeatureFlagList invalid count:", count, "should be multiple of 4")
  end
end
function CharacterAvatarComponent:OnRep_ClothSchemeIDList()
  print(bWriteLog and "CharacterAvatarComponent:OnRep_ClothSchemeIDList", self.ClothSchemeIDList:Num())
  local newMap = {}
  local count = self.ClothSchemeIDList:Num()
  for i = 0, count - 1 do
    local schemeID = self.ClothSchemeIDList:Get(i)
    if schemeID and 0 < schemeID then
      newMap[schemeID] = true
      print(bWriteLog and "CharacterAvatarComponent:OnRep_ClothSchemeIDList added:", schemeID)
    end
  end
  if self:_IsClothSchemeMapEqual(newMap) then
    return
  end
  self.ClothSchemeIDMap = newMap
  self:_PostClothSchemeChanged()
end
function CharacterAvatarComponent:SetClothSchemeIDList(idList)
  local newMap = {}
  if idList then
...
function CharacterAvatarComponent:ClearClothSchemeIDList()
  if next(self.ClothSchemeIDMap) == nil then
    return
  end
  self.ClothSchemeIDMap = {}
  self:_PostClothSchemeChanged()
end
function CharacterAvatarComponent:AddClothSchemeID(schemeID)
  if not schemeID or schemeID <= 0 then
    return
  end
  if self.ClothSchemeIDMap[schemeID] then
    return
  end
  self.ClothSchemeIDMap[schemeID] = true
  self:_PostClothSchemeChanged()
end
function CharacterAvatarComponent:RemoveClothSchemeID(schemeID)
  if not schemeID or schemeID <= 0 then
    return
  end
  if not self.ClothSchemeIDMap[schemeID] then
    return
  end
  self.ClothSchemeIDMap[schemeID] = nil
  self:_PostClothSchemeChanged()
end
function CharacterAvatarComponent:_IsClothSchemeMapEqual(otherMap)
  local oldMap = self.ClothSchemeIDMap
  local oc, nc = 0, 0
  for k in pairs(oldMap) do
    oc = oc + 1
    if not otherMap[k] then
      return false
    end
  end
  for k in pairs(otherMap) do
    nc = nc + 1
  end
  return oc == nc
end
function CharacterAvatarComponent:_PostClothSchemeChanged()
  EventSystem:postEvent(EVENTTYPE_PLAYEREVENT_AVATAR, EVENTID_PLAYEREVENT_AVATAR_CLOTH_SCHEME_CHANGED, self.Object)
end
...
function CharacterAvatarComponent:HideHeadMesh(MasterBoneComp, OriginHeadMesh)
  ...
  local FakeHeadPath, FakeHeadPath
...
function CharacterAvatarComponent:DoChangeColor(AvatarSlotID, RealShowItemID, ColorDatas)
  ...
  self:LuaBroadcast("OnCharacterAvatarComponentColorChanged", AvatarSlotID, RealShowItemID)
end
...
function CharacterAvatarComponent:ProcessSpecialGlider()
  ...
  local ActualUsed  if LogicXSuit.IsXSuitGlide(GliderID) then
    ...
    if not ClotheLogicSlotDesc then
      print(bWriteLog and "CharacterAvatarComponent:ProcessSpecialGlider ClotheLogicSlotDesc is nil")
      return
    end
    local equippedClothID = ClotheLogicSlotDesc.ItemDefineID.TypeSpecificID
    local matchedCfg
    ...
    for _, cfg in pairs(allGlideRows or {}) do
      ...
      if cfg.XSuitID7 == equippedClothID or state2 == equippedClothID then
        matchedCfg = cfg
        break
      end
    end
    if matchedCfg and XSuitUtil.IsUserOpenSpecialGlideSetting(PlayerController.UID, NormalGlideID) then
      ActualUsedGliderID = matchedCfg.SpecialGlideID
    else
...
function CharacterAvatarComponent:HandleFaceHidden(CurrentClothID)
  ...
  local LogicUserBattleDataManager = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.LogicUserBattleDataManager)
  local AvatarHideSlotConfig = LogicUserBattleDataManager:GetHideSlotConfig(CurrentClothID)
  if AvatarHideSlotConfig and LogicUserBattleDataManager:GetHideSlotSetting(AvatarHideSlotConfig.GroupID) then
    print(bWriteLog and "CharacterAvatarComponent HandleFaceHidden CurrentClothID" .. tostring(CurrentClothID))
    self:ModifySlotFakeID(AvatarHideSlotConfig.SlotID, 0)
  end
end
...
function CharacterAvatarComponent:ChangeAllMeshToFeatureMaterial(material)
  if not Client then
    print(bWriteLog and "CharacterAvatarComponent:ChangeAllMeshToFeatureMaterial not Client")
    return
  end
...
function CharacterAvatarComponent:ClearAllFeatureMaterial()
  if not Client then
    print(bWriteLog and "CharacterAvatarComponent:ClearAllFeatureMaterial not Client")
    return
  end
  self.FeatureMaterial = nil
  local model_util = require("client.common.model_util")
...
function CharacterAvatarComponent:IsUnlockSpecialMove()
  ...
  local RunActionLevel = 2
  local level = XSuitUtil:GetLevelByItemId(XSuitItemID)
  if RunActionLevel > level then
    return false
  end
  local branchId = XSuitUtil:GetBranchIdByItemId(XSuitItemID)
  local XSuitFeatureSwitchType = require("GameLua.Activity.Commercialize.GamePlay.XSuit.XSuitFeatureSwitchType")
  if self.XSuitFeatureFlagMap and self.XSuitFeatureFlagMap[period] and self.XSuitFeatureFlagMap[period][branchId] and self.XSuitFeatureFlagMap[period][branchId][RunActionLevel] and self.XSuitFeatureFlagMap[period][branchId][RunActionLevel][XSuitFeatureSwitchType.RunAction] and self.XSuitFeatureFlagMap[period][branchId][RunActionLevel][XSuitFeatureSwitchType.RunAction].flag_state == 0 then
    return false
  end
  local needUnlock, unlockLevel, unlockIndex = XSuitUtil:IsUnlockedFeature(period, branchId, UnLockFeatureType.SprintShow)
  if not needUnlock then
    return true
  end
  if self.XSuitUnlockFeatureMap and self.XSuitUnlockFeatureMap[period] and self.XSuitUnlockFeatureMap[period][branchId] and self.XSuitUnlockFeatureMap[period][branchId][unlockLevel] and self.XSuitUnlockFeatureMap[period][branchId][unlockLevel][unlockIndex] and self.XSuitUnlockFeatureMap[period][branchId][unlockLevel][unlockIndex].state == 1 then
    return true
  end
  return false
end
...
function CharacterAvatarComponent:SwitchMeshToMinLOD(bSwitchToMinLOD)
  ...
  for SlotID, SlotMeshDesc in pairs(self.LoadedMeshComps) do
    if SlotID == EAvatarSlotType.EAvatarSlotType_ClothesEquipemtSlot then
      local AvatarHandle = self:GetLoadedHandle(EAvatarSlotType.EAvatarSlotType_ClothesEquipemtSlot)
      if slua.isValid(AvatarHandle) and AvatarHandle.SpecialType ~= EAvatarSpecialType.EAvatarSpecialType_MoreBoneCloth and AvatarHandle.SpecialType ~= EAvatarSpecialType.EAvatarSpecialType_ClothMidDeviceBiasLOD and AvatarHandle.SpecialType ~= EAvatarSpecialType.EAvatarSpecialType_ClothDeviceLevel6BiasLOD then
        goto lbl_59
      end
    end
