  ...
  if slua.isValid(uPlayerController) then
    ...
    if slua.isValid(uPlayerCharacter) then
      if self.bPhotoGrapherOpenState == true then
        ...
        uPlayerCharacter:LuaBroadcast("OnDSEnterSelfieMode")
      else
        EventSystem:postEvent(EVENTTYPE_INGAME_NORMAL, EVENTID_INGAME_DS_EXIT_SELFIE_MODE, uPlayerCharacter)
        uPlayerCharacter:LuaBroadcast("OnDSExitSelfieMode")
      end
    end
  end
  if self.bPhotoGrapherOpenState == true then
...
function PhotoGrapherFeature:RegisterPhotoGrapherExitEvent()
  ...
  local uPlayerCharacter = uPlayerController:GetPlayerCharacterSafety()
  if slua.isValid(uPlayerCharacter) then
    self:AddControlEvent(uPlayerCharacter, "OnCharacterEnterState", self.OnCharacterEnterState, self)
  end
...
function PhotoGrapherFeature:RemovePhotoGrapherExitEvent()
  ...
  local uPlayerCharacter = uPlayerController:GetPlayerCharacterSafety()
  if slua.isValid(uPlayerCharacter) and self:HasControlEventByControl(uPlayerCharacter, "OnCharacterEnterState") then
    self:RemoveControlEvent(uPlayerCharacter, "OnCharacterEnterState")
  end
  print(bWriteLog and "PhotoGrapherFeature:RemovePhotoGrapherExitEvent Success")
end
function PhotoGrapherFeature:OnCharacterEnterState(uCharacter, State)
  if Client then
    return
  end
  local EPawnState = import("EPawnState")
  if State == EPawnState.Dying or State == EPawnState.Dead then
    print(bWriteLog and "PhotoGrapherFeature:OnCharacterEnterState dying/dead exit PhotoGrapher State:" .. tostring(State))
    self:ExitPhotoGrapher()
  end
end
