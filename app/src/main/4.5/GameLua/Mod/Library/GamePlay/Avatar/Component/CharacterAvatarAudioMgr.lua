...
local s_EmoteAudioCD, s_EmoteAudioCDTimer
...
function CharacterAvatarAudioMgr:OnPlayEmote(_, _, AvatarComp, EmoteID)
  ...
  local bIsMainLobbyAvatar = false
  local TeamAvatarManager = require("client.logic.avatar.logic_team_avatar_manager")
  local allAvatars = TeamAvatarManager:GetAllAvatar()
  if allAvatars then
    for _, avatar in pairs(allAvatars) do
      if avatar:GetModelAvatarComp() == self.OwnerAvatarComp then
        bIsMainLobbyAvatar = true
        break
      end
    end
  end
  if bIsMainLobbyAvatar and s_EmoteAudioCD then
    log(bWriteLog and "CharacterAvatarAudioMgr:OnPlayEmote dedup skip")
    return
  end
  local playDuration = self:PlayRandomAudioByTriggerType(Enum_AudioTriggerCondition.ON_PLAY_EMOTE, EmoteID)
  if playDuration and bIsMainLobbyAvatar then
    s_EmoteAudioCD = true
    local time_ticker = require("common.time_ticker")
    s_EmoteAudioCDTimer = time_ticker.AddTimerOnce(playDuration, function()
      s_EmoteAudioCD = nil
      s_EmoteAudioCDTimer = nil
    end)
  end
end
