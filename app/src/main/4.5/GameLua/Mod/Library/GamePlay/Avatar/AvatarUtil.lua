...
function AvatarUtil.DoubleCheckIsDownLoadFinish(resID, pakName)
  ...
  PufferODPakManager:ResetStateByItemID(resID)
  return false
end
...
function AvatarUtil.IsWeaponAvatarFeatureSwitchOn(flag, featureSwitchType, bCheckSelfOnly)
  if not flag then
    return true
  end
  local GetControllingFeatureMask = require("client.logic.data.feature_control_mask")
  local selfBit = 1 << featureSwitchType
  local checkMask
  if bCheckSelfOnly then
    checkMask = selfBit
  else
    checkMask = selfBit | GetControllingFeatureMask(featureSwitchType)
  end
  return flag & checkMask == 0
end
function AvatarUtil._EnsureVoiceParamCache()
  if AvatarUtil.CacheVoiceParamMap then
    return
  end
  AvatarUtil.CacheVoiceParamMap = {}
  local AudioParamCfgs = CDataTable.GetTable("WeaponSkinVoiceParamCfg")
  if not AudioParamCfgs then
    return
  end
  local StringUtil = require("common.string_util")
  for _, Cfg in pairs(AudioParamCfgs) do
    if Cfg.RTPCKey and Cfg.RTPCKey ~= "" then
      local RTPCKeyList = {}
      local SplitResult = StringUtil.Split(Cfg.RTPCKey, ";")
      for _, Key in pairs(SplitResult) do
        if Key and Key ~= "" then
          table.insert(RTPCKeyList, Key)
        end
      end
      AvatarUtil.CacheVoiceParamMap[Cfg.AudioType] = {
        RTPCKeyList = RTPCKeyList,
        MinVolume = Cfg.MinVolume,
        MaxVolume = Cfg.MaxVolume,
        AudioType = Cfg.AudioType
      }
    end
  end
end
function AvatarUtil.GetWeaponSkinVoiceParamCache()
  AvatarUtil._EnsureVoiceParamCache()
  return AvatarUtil.CacheVoiceParamMap
end
function AvatarUtil.GetWeaponSkinVoiceParamByAudioType(AudioType)
  AvatarUtil._EnsureVoiceParamCache()
  return AvatarUtil.CacheVoiceParamMap[AudioType]
end
return AvatarUtil