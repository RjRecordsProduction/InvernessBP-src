...
function PlayEmoteFeature:ctor()
  ...
  self.QuickEmoteAndDecalList = slua.Array(UEnums.EPropertyClass.Int)
  if Client then
...
function PlayEmoteFeature:InitQuickEmoteAndDecalList(tQuickEmoteAndDecalList)
  log_tree(bWriteLog and "PlayEmoteFeature:InitQuickEmoteAndDecalList extAttr", tQuickEmoteAndDecalList)
  for i = 1, 12 do
    local EmoteID = tQuickEmoteAndDecalList[i]
    if EmoteID then
      self.QuickEmoteAndDecalList:Add(EmoteID)
    else
      self.QuickEmoteAndDecalList:Add(0)
    end
  end
end
