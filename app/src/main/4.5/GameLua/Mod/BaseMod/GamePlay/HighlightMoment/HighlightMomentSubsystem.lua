...
function HighlightMomentSubsystem:ctor()
  ...
  self.SceneShowPawnList = {}
  self.SceneShowLevelLoaded = false
  self.SceneShowSequenceActor = nil
  self.SceneShowLoadedAssetList = {}
  self.SceneShowForceEndTimer = nil
  self.SceneShowCreatePawnTimer = nil
  self.SceneShowPendingAvatarData = {}
  self.SceneShowPendingAvatarDataSnapshot = nil
  self.SceneShowAssetLoaded = false
  self.SceneShowPawnAllCreated = false
  self.SceneShowPawnClassHandle = nil
  self.SceneShowAssetArrayHandle = nil
  self.nDelayCreatePawnTime = 0.3
  self.nLowDeviceLevel = 0
  self.nSceneShowTypeID = 17
  self._bSceneShowFPSLimited = false
  self.bEnableClientTrigger = true
  if ClientGameMain.GetUIOtherSetting("bIgnoreHighLightTrigger") then
    print(bWriteLog and "HighlightMomentSubsystem:ctor bIgnoreHighLightTrigger")
    self.bEnableClientTrigger = false
  end
  self.SceneShowActiveScreenAppearanceMap = {}
  self.SceneShowScreenAppearanceLoadedClassMap = {}
end
function HighlightMomentSubsystem:OnInit()
  HighlightMomentSubsystem.__super.OnInit(self)
  Config = self:GetHightlightMomentConfig()
  ...
  if not Client then
    HighlightMomentSubsystem_DSChecker = GamePlayTools.GetCurrentConfig("HighlightMomentSubsystem_DSChecker")
    ...
    self.SceneShowCheckFuncRouter = {}
    if Config.SceneShowConfig then
      for nSceneID, tSceneConfig in pairs(Config.SceneShowConfig) do
        if type(nSceneID) == "number" and not tSceneConfig.bIgnoreCheck then
          local DamageType = tSceneConfig.DamageType or -1
          local DamageItemID = tSceneConfig.DamageItemID or -1
          if not self.SceneShowCheckFuncRouter[DamageType] then
            self.SceneShowCheckFuncRouter[DamageType] = {}
          end
          if not self.SceneShowCheckFuncRouter[DamageType][DamageItemID] then
            self.SceneShowCheckFuncRouter[DamageType][DamageItemID] = {}
          end
          if tSceneConfig.DSCheckFunc then
            local sFuncName = "CheckFunc" .. tSceneConfig.DSCheckFunc
            if type(HighlightMomentSubsystem_DSChecker[sFuncName]) == "function" then
              if not self[sFuncName] then
                self[sFuncName] = HighlightMomentSubsystem_DSChecker[sFuncName]
              end
              self.SceneShowCheckFuncRouter[DamageType][DamageItemID][nSceneID] = self[sFuncName]
              print(bWriteLog and string.format("HighlightMomentSubsystem:OnInit SceneShowCheckFuncRouter HighlightID:%d, DamageType:%d, DamageItemID:%d, Func:%s", nSceneID, DamageType, DamageItemID, sFuncName))
            end
          end
        end
      end
...
function HighlightMomentSubsystem:GetHightlightMomentConfig()
  if self.HighlightMomentConfig then
    return self.HighlightMomentConfig
  end
...
function HighlightMomentSubsystem:TriggerHighlightMoment(PlayerCharacter, Type, Param, AchievementParams)
  ...
  local uPlayerController = PlayerCharacter:GetPlayerControllerSafety()
  if not slua.isValid(uPlayerController) or not uPlayerController.HighlightShowFeature then
    print(bWriteLog and "HighlightMomentSubsystem:TriggerHighlightMoment uPlayerController is not valid")
    return
  end
  Param = Param or 0
  ...
  if HighlightConfig.IgnoreHighlight ~= true then
    uPlayerController.HighlightShowFeature:DSStartTriggerHighlightMoment(Type, Param)
  end
...
function HighlightMomentSubsystem:ClientOnTriggerHighlightMoment(_, _, Type, Param)
  ...
  if not self.bEnableClientTrigger then
    print(bWriteLog and "HighlightMomentSubsystem:ClientOnTriggerHighlightMoment bEnableClientTrigger is false, return")
    return
  end
  ...
  if bNeedCover then
    if HighlightConfig.bIsSceneShow then
      self:_SceneShowOnClientTrigger(Type, Param, HighlightConfig)
      return
    end
...
function HighlightMomentSubsystem:ClientOnTriggerWeaponHighlightMoment(Param)
  ...
  if not self:CacheVictimInfoOnClient(Param) or not self.bEnableClientTrigger then
    print(bWriteLog and "HighlightMomentSubsystem:ClientOnTriggerWeaponHighlightMoment - CacheVictimInfoOnClient failed")
    return
  end
...
function HighlightMomentSubsystem:OnPressPlayHighlight()
  ...
  local CompareFunc = function(a, b)
    return a.Param.TriggerOrder < b.Param.TriggerOrder
  end
  table.sort(OrderedHighlights, CompareFunc)
...
function HighlightMomentSubsystem:GetPendingShowIconPath()
  self:ClearConsumedHighlights()
  local uCharacter = GameplayData.GetPlayerCharacter()
  local bPlayerInDoor = Game:IsPlayer(uCharacter) and uCharacter.InDoor
  local bBlockPlayByInDoor = false
  ...
  table.sort(OrderedHighlights, CompareFunc)
  local sIconPath
  log_tree(bWriteLog and "HighlightMomentSubsystem:GetPendingShowIconPath OrderedHighlights", OrderedHighlights)
  if OrderedHighlights and 0 < #OrderedHighlights then
    for idx, Data in ipairs(OrderedHighlights) do
      local Type = Data.Type
      local HighlightConfig = Config[Type]
      if HighlightConfig and (not bPlayerInDoor or not not HighlightConfig.bCanPlayIndoor) and (Type ~= 6 or not not self:VehiclePlayCheck()) then
        print(bWriteLog and "HighlightMomentSubsystem:GetPendingShowIconPath vehicle play check failed!")
        if HighlightConfig.ButtonIconPath and HighlightConfig.ButtonIconPath ~= "" then
          sIconPath = HighlightConfig.ButtonIconPath
        end
        if HighlightConfig.bIsSceneShow and self.nCurSceneShowID and Config.SceneShowConfig[self.nCurSceneShowID] and Config.SceneShowConfig[self.nCurSceneShowID].ButtonIconPath ~= "" then
          print(bWriteLog and "HighlightMomentSubsystem:GetPendingShowIconPath Find Icon For Scene: " .. tostring(self.nCurSceneShowID))
          sIconPath = Config.SceneShowConfig[self.nCurSceneShowID].ButtonIconPath
        end
        break
      end
    end
...
function HighlightMomentSubsystem:OnSeqMsg(_, __, Player, sSeqEventMsg, ID, SceneID, sEventType)
  ...
  if sSeqEventMsg == "Highlight" and ID then
    ...
    if tHighlightConfig then
      do
        ...
        if nHighlightID == 17 and SceneID and tostring(self.nCurSceneShowID) == SceneID then
          if Config.SceneShowConfig[self.nCurSceneShowID] and Config.SceneShowConfig[self.nCurSceneShowID].SceneUIName then
            if UIManager.UI_Config_InGame[Config.SceneShowConfig[self.nCurSceneShowID].SceneUIName] then
              UIName = Config.SceneShowConfig[self.nCurSceneShowID].SceneUIName
            end
            if Config.SceneShowConfig[self.nCurSceneShowID].SceneUITextID and 0 < Config.SceneShowConfig[self.nCurSceneShowID].SceneUITextID then
              tHighlightConfig.SceneUITextID = Config.SceneShowConfig[self.nCurSceneShowID].SceneUITextID
            end
          end
          if sEventType == "Close" then
            print(bWriteLog and "HighlightMomentSubsystem:OnSeqMsg Close SceneUI SceneID:" .. tostring(SceneID) .. " UIName:" .. tostring(UIName))
            if UIManager.UI_Config_InGame[UIName] then
              UIManager.CloseUI(UIManager.UI_Config_InGame[UIName])
            end
            if self.HighlightPreloadHandleID then
              local asset_util = require("common.asset_util")
              asset_util.CancelAssetAsync(self.HighlightPreloadHandleID)
  ...
  elseif sSeqEventMsg == "PlayScreenParticle" and ID then
    if self.nCurSceneShowTypeID then
      local Index = tonumber(ID)
      print(bWriteLog and "HighlightMomentSubsystem:OnSeqMsg PlayScreenParticle Index:" .. tostring(Index))
      self:PlaySceneShowScreenParticleByIndex(Index)
    end
  elseif sSeqEventMsg == "StopScreenParticle" and ID and self.nCurSceneShowTypeID then
    local Index = tonumber(ID)
    print(bWriteLog and "HighlightMomentSubsystem:OnSeqMsg StopScreenParticle Index:" .. tostring(Index))
    if Index and 0 < Index then
      self:StopSceneShowScreenParticleByIndex(Index)
    else
      self:StopAllSceneShowScreenParticles()
    end
  end
end
...
function HighlightMomentSubsystem:IsInPlayingState()
  local bIsPlaying = self.nCurPlayingType and self.nCurPlayingType > 0 or false
  local bIsInTransition = bIsPlaying and self.bInSceneShowTransition == true
  return bIsPlaying, bIsInTransition
end
...
function HighlightMomentSubsystem:PlayVehicleHighlightInternal()
  ...
  local BindingData = {}
  local PlayTransform
  local Cur  local bUseLobbyVehicleSubActor = false
  local BackpackUtils = import("BackpackUtils")
  local itemDefineID = FItemDefineID(9, self.VehicleInfoCache.VehicleSkinItemID or 0)
  local bIsItemExist = self.VehicleInfoCache.VehicleSkinItemID and BackpackUtils.IsBattleItemHandleExist(itemDefineID, true, SkinVehicleConfig and SkinVehicleConfig.PossessLobbyVehicle or false, false) or false
  ...
  if bIsItemExist and SkinVehicleConfig and SkinVehicleConfig.LevelSeq and UEPathUtilityMethods.IsPathExist(SkinVehicleConfig.LevelSeq) then
    print(bWriteLog and "HighlightMomentSubsystem:PlayVehicleHighlightInternal levelseq: ", SkinVehicleConfig.LevelSeq)
    ...
    if SkinVehicleConfig.PossessLobbyVehicle then
      self:ShowLobbyVehicle(true)
      bUseLobbyVehicleSubActor = true
    end
  else
  ...
  if bUseLobbyVehicleSubActor then
    BindingData[ShowVehicleTrackPath] = slua.isValid(self.ShowVehicleActor) and self.ShowVehicleActor.SubActor or nil
  else
  ...
  local tSeqContext = {
    ...
    SequencePath = CurLevelSequencePath,
    PlayTransform = PlayTransform,
      }
  if bUseLobbyVehicleSubActor then
    self:_PlayVehicleHighlightSeqStart(tSeqContext)
  else
    self:_StartVehicleLevelSeq(tSeqContext)
  end
end
function HighlightMomentSubsystem:_DisableShowActorsURO()
  print(bWriteLog and "HighlightMomentSubsystem:_DisableShowActorsURO")
  if slua.isValid(self.ShowVehicleActor) and slua.isValid(self.ShowVehicleActor.SubActor) then
    local SubActorMesh = self.ShowVehicleActor.SubActor.Mesh
    if slua.isValid(SubActorMesh) then
      SubActorMesh.bEnableUpdateRateOptimization = false
    end
  end
  local USkeletalMeshComponent = import("SkeletalMeshComponent")
  for _, uShowPawn in pairs(self.ShowPawnList) do
    if slua.isValid(uShowPawn) then
      local MeshComps = uShowPawn:GetComponentsByClass(USkeletalMeshComponent)
      for _, comp in pairs(MeshComps) do
        if slua.isValid(comp) then
          comp.bEnableUpdateRateOptimization = false
        end
      end
    end
  end
end
function HighlightMomentSubsystem:_StartVehicleLevelSeq(tSeqContext)
  local PlayerController = tSeqContext.PlayerController
  local CurLevelSequencePath = tSeqContext.SequencePath
  local PlayTransform = tSeqContext.PlayTransform
  local BindingData = tSeqContext.BindingData
  ...
  if not slua.isValid(self.LevelSequenceActor) then
    print(bWriteLog and "HighlightMomentSubsystem:_StartVehicleLevelSeq not valid LevelSequenceActor!")
    self:SeqEndShow(false)
    return
  end
  print(bWriteLog and "HighlightMomentSubsystem:_StartVehicleLevelSeq", self.VehicleInfoCache.VehicleSkinItemID, CurLevelSequencePath)
...
function HighlightMomentSubsystem:_PlayVehicleHighlightSeqStart(tSeqContext)
  self:AddGameTimer(0.02, false, function()
    local sub = SubsystemMgr:Get("IngameSelfieSubsystem")
    if not sub or not sub.bIsIngameSelfieMode then
      print(bWriteLog and "HighlightMomentSubsystem:_PlayVehicleHighlightSeqStart selfie exited, abort")
      self:ShowLobbyPawn(false)
      self:ShowLobbyVehicle(false)
      return
    end
    self:_DisableShowActorsURO()
    self:_StartVehicleLevelSeq(tSeqContext)
  end)
end
...
function HighlightMomentSubsystem:ToggleVictimDeadBoxVisibility(bVisible)
  ...
  if not VictimPlayerKey or VictimPlayerKey < 0 then
    print(bWriteLog and "HighlightMomentSubsystem:ToggleVictimDeadBoxVisibility no VictimPlayerKey")
    return
  end
  local UPropsUtils = import("/Script/ShadowTrackerExtra.PropsUtils")
  local PlayerController = GameplayData.GetPlayerController()
  local VictimDeadBox = UPropsUtils.GetPlayerTombBoxByPlayerKey(PlayerController, VictimPlayerKey)
  if not slua.isValid(VictimDeadBox) then
    local UScriptGameplayStatics = import("/Script/ShadowTrackerExtra.ScriptGameplayStatics")
    local Character = UScriptGameplayStatics.GetCharacterByPlayerKey(PlayerController, VictimPlayerKey)
    print(bWriteLog and string.format("HighlightMomentSubsystem:ToggleVictimDeadBoxVisibility no VictimDeadBox %s, Name:%s", VictimPlayerKey, Character:GetPlayerNameSafety()))
    return
  end
...
function HighlightMomentSubsystem:CollectVehicleInfoOnClient()
  ...
  local bSunroofOpen = false
  local vehicleAvatarComp = uCurrentVehicle.VehicleAvatarComponent_BP
  if slua.isValid(vehicleAvatarComp) and vehicleAvatarComp.IsSunroofOpen then
    bSunroofOpen = vehicleAvatarComp:IsSunroofOpen()
  end
  self.VehicleInfoCache.  self.VehicleInfoCache.SeatInfo = {}
...
function HighlightMomentSubsystem:CacheVictimInfoOnClient(VictimPlayerKey)
  ...
  local uBoundingBox2D
  local uSrcBoundingBox2D = WeaponSequenceConfigure and WeaponSequenceConfigure.BoundingBox2D or nil
  if uSrcBoundingBox2D then
    uBoundingBox2D = {
      Min = FVector2D(uSrcBoundingBox2D.Min.X, uSrcBoundingBox2D.Min.Y),
      Max = FVector2D(uSrcBoundingBox2D.Max.X, uSrcBoundingBox2D.Max.Y)
    }
  end
  ...
  if bShow then
    ...
    local callOb, curDataTable = xpcall(function()
      if Config.VehicleSkinModelActorConfig and Config.VehicleSkinModelActorConfig[VehicleSkinItemID] then
        ...
        local ExtraTableData = {
          BrakeCaliper = LogicVehicleExtendedFeature:GetEquipedBrakeCaliperData(VehicleSkinItemID),
          WheelHub = LogicVehicleExtendedFeature:GetEquipedWheelHubData(VehicleSkinItemID),
          Canopy = LogicVehicleExtendedFeature:GetEquipedSunroofData(VehicleSkinItemID),
          bUseForHighLight = true,
          bSunroofOpen = self.VehicleInfoCache and self.VehicleInfoCache.bSunroofOpen or false
        }
        local uTransform = KismetMathLibrary.MakeTransform(self.VehicleInfoCache.Loc, self.VehicleInfoCache.Rot, FVector(1, 1, 1))
        self.ShowVehicleActor = SocialVehicleSystem.CreateRefitVehicle(VehicleSkinItemID, uTransform, VehicleRefitHandler.GetCarStyleList(VehicleSkinItemID, nil, nil, {}), 1, false, true, true, true, ExtraTableData)
...
function HighlightMomentSubsystem:VehiclePlayCheck()
  if Client.IsEditor() then
    ...
    self.HighlightMomentConfig = nil
    Config = self:GetHightlightMomentConfig()
  end
  local uCurrentVehicle = self:GetCurrentVehicle()
  local Pawn = GameplayData.GetPlayerCharacter()
...
function HighlightMomentSubsystem:_SceneShowOnClientTrigger(TypeID, Param, HighlightConfig)
  print(bWriteLog and "HighlightMomentSubsystem:_SceneShowOnClientTrigger TypeID:" .. tostring(TypeID) .. " Param:" .. tostring(Param))
  local SceneID = Param
  local SceneShowConfig = Config.SceneShowConfig and SceneID and Config.SceneShowConfig[SceneID] or nil
  if not SceneShowConfig then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowOnClientTrigger no SceneShowConfig SceneID:" .. tostring(SceneID))
    return
  end
  local SelfCharacter = GameplayData.GetPlayerCharacter()
  local uPlayerController = GameplayData.GetPlayerController()
  local HighlightShowFeature = slua.isValid(uPlayerController) and uPlayerController.HighlightShowFeature or nil
  if not HighlightShowFeature then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowOnClientTrigger no SceneShowFeature")
    return
  end
  local ConsumedSnapshots
  if HighlightShowFeature.ConsumePendingShowSnapshots then
    ConsumedSnapshots = HighlightShowFeature:ConsumePendingShowSnapshots()
  end
  self.SceneShowPendingAvatarData = {}
  local OtherPawnCount = SceneShowConfig.OtherParticipantCount or 0
  if 3 < OtherPawnCount then
    OtherPawnCount = 3
  end
  local bLocalHideHelmet = false
  local SettingConfig = slua_GameFrontendHUD:GetUserSettings()
  if SettingConfig and SettingConfig.LocalHideHelmet == true then
    bLocalHideHelmet = true
  end
  local SelfPlayerKey = slua.isValid(SelfCharacter) and SelfCharacter.PlayerKey or 0
  table.insert(self.SceneShowPendingAvatarData, {
    Index = 1,
    bIsSelf = true,
    PlayerKey = SelfPlayerKey
  })
  print(bWriteLog and "HighlightMomentSubsystem:_SceneShowOnClientTrigger OtherPawnCount:" .. tostring(OtherPawnCount))
  for i = 1, OtherPawnCount do
    local AvatarSyncData = ConsumedSnapshots and ConsumedSnapshots[i]
    if not (AvatarSyncData and AvatarSyncData.SlotSyncData) or 0 >= AvatarSyncData.SlotSyncData:Num() then
      print(bWriteLog and "HighlightMomentSubsystem:_SceneShowOnClientTrigger AvatarSyncData invalid, Index:" .. tostring(i))
      return
    end
    local Gender = AvatarSyncData and AvatarSyncData.Gender or 0
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowOnClientTrigger Other Pawn Count:" .. tostring(i) .. " UpdateFlag:" .. tostring(AvatarSyncData.UpdateFlag))
    table.insert(self.SceneShowPendingAvatarData, {
      Index = i + 1,
      bIsSelf = false,
      Gender = Gender,
  ...
  self.SceneShowPendingAvatarDataSnapshot = {}
  for _, Data in ipairs(self.SceneShowPendingAvatarData) do
    table.insert(self.SceneShowPendingAvatarDataSnapshot, Data)
  end
  self:_SceneShowInitState(TypeID, SceneID)
  ...
  if nDeviceLevel <= self.nLowDeviceLevel then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowOnClientTrigger LowEnd, show button directly TypeID:" .. tostring(TypeID))
    self:_SceneShowShowButtonDirectly()
  else
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowOnClientTrigger NonLowEnd, start preload TypeID:" .. tostring(TypeID))
    self:_SceneShowStartPreload(SceneID, HighlightConfig)
  end
end
function HighlightMomentSubsystem:_GetSceneShowConfigWithMapOverride(SceneID)
  local SceneShowConfig = Config.SceneShowConfig and SceneID and Config.SceneShowConfig[SceneID] or nil
  if not SceneShowConfig then
    return nil
  end
  local MapType = GameMainConfig.GetMapType()
  local MapOverride = SceneShowConfig.MapOverrideConfig and SceneShowConfig.MapOverrideConfig[MapType]
  if MapOverride then
    local OverrideConfig = TableUtil.LiteCopy(SceneShowConfig, true)
    for Key, Value in pairs(MapOverride) do
      OverrideConfig[Key] = Value
    end
    print(bWriteLog and "HighlightMomentSubsystem:_GetSceneShowConfigWithMapOverride apply map override SceneID:" .. tostring(SceneID) .. " MapType:" .. tostring(MapType))
    return OverrideConfig
  end
  return SceneShowConfig
end
function HighlightMomentSubsystem:PlayFuncSceneHighlight(Data, HighlightConfig)
  print(bWriteLog and "HighlightMomentSubsystem:PlayFuncSceneHighlight")
  if not self.nCurSceneShowTypeID then
    print(bWriteLog and "HighlightMomentSubsystem:PlayFuncSceneHighlight no nCurSceneShowTypeID")
    return false
  end
  local SceneID = self.nCurSceneShowID
  self.SceneShowConfig = self:_GetSceneShowConfigWithMapOverride(SceneID)
  if not self.SceneShowConfig then
    print(bWriteLog and "HighlightMomentSubsystem:PlayFuncSceneHighlight no SceneShowConfig SceneID:" .. tostring(SceneID))
    return false
  end
  if self.SceneShowConfig.TlogID then
    self:SentPlayerTLog(self.SceneShowConfig.TlogID)
    print(bWriteLog and "HighlightMomentSubsystem:PlayFuncSceneHighlight TlogID:" .. tostring(self.SceneShowConfig.TlogID))
  end
  self:_SceneShowPlayInternal()
  return false
end
function HighlightMomentSubsystem:_SceneShowInitState(TypeID, SceneID)
  self.nCurSceneShowID = SceneID
  self.nCurSceneShow  self.SceneShowAssetLoaded = false
  self.SceneShowPawnAllCreated = false
  self.SceneShowLoadedAssetList = {}
  self.SceneShowPawnList = self.SceneShowPawnList or {}
  self._SceneShowPreloadDoneCallback = nil
  self.SceneShowConfig = nil
end
function HighlightMomentSubsystem:_SceneShowStartPreload(HighlightConfig)
  print(bWriteLog and "HighlightMomentSubsystem:_SceneShowStartPreload TypeID:" .. tostring(self.nCurSceneShowTypeID) .. " SceneID:" .. tostring(self.nCurSceneShowID))
  local SceneShowConfig = self:_GetSceneShowConfigWithMapOverride(self.nCurSceneShowID)
  if not SceneShowConfig then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowStartPreload no SceneShowConfig")
    return
  end
  self.  local LoadingMaxTime = SceneShowConfig.LoadingMaxTime or 20
  self.SceneShowForceEndTimer = self:AddGameTimer(LoadingMaxTime, false, function()
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowStartPreload ForceEnd by timeout")
    self:_SceneShowCleanup()
  end)
  self:_SceneShowDoPreload()
end
function HighlightMomentSubsystem:_SceneShowDoPreload(OnDoneCallback)
  if not (self.nCurSceneShowTypeID and self.nCurSceneShowID) or not self.SceneShowConfig then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPreload no SceneShowID")
    return
  end
  print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPreload TypeID:" .. tostring(self.nCurSceneShowTypeID) .. " SceneID:" .. tostring(self.nCurSceneShowID))
  self.CurHighlightList[self.nCurSceneShowTypeID] = {
    Param = 0,
    TriggerOrder = self.CurTriggerOrder,
    bIsSceneShow = true,
    bLowEndPending = false
  }
  self._SceneShowPreloadDoneCallback = OnDoneCallback
  if not self.SceneShowConfig.LobbyPawnClassPath then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPreload no ShowPawnActorPath")
    return
  end
  self.SceneShowPawnClassHandle = self:AsyncLoadAsset(self.SceneShowConfig.LobbyPawnClassPath .. "_C", function(PawnClass)
    self.SceneShowPawnClassHandle = nil
    if not PawnClass then
      print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPreload LobbyPawnClass load failed")
      self:_SceneShowCleanup()
      return
    end
    self:_SceneShowCreatePawnEveryFrame()
    self.SceneShowCreatePawnTimer = self:AddGameTimer(self.nDelayCreatePawnTime, true, function()
      self:_SceneShowCreatePawnEveryFrame()
    end)
    local PreloadPaths = {
      self.SceneShowConfig.SequenceActorPath,
      self.SceneShowConfig.SequencePath,
      self.SceneShowConfig.LevelAssetPath
    }
    local ScreenParticlePathToIndex = {}
    local PlayerControllerForFeature = GameplayData.GetPlayerController()
    local HighlightShowFeatureRef = slua.isValid(PlayerControllerForFeature) and PlayerControllerForFeature.HighlightShowFeature or nil
    if HighlightShowFeatureRef and HighlightShowFeatureRef.CollectScreenParticlePreloadPaths then
      local ParticlePaths
      ParticlePaths, ScreenParticlePathToIndex = HighlightShowFeatureRef:CollectScreenParticlePreloadPaths(self.SceneShowConfig)
      if ParticlePaths then
        for _, AssetPath in ipairs(ParticlePaths) do
          table.insert(PreloadPaths, AssetPath)
        end
      end
    else
      print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPreload no HighlightShowFeature, skip screen particle preload")
    end
    self.SceneShowAssetArrayHandle = self:AsyncLoadAssetArray(PreloadPaths, function()
      self.SceneShowAssetArrayHandle = nil
      print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPreload AsyncLoadAsset Done")
      local BusinessHelper = import("BusinessHelper")
      for _, AssetPath in pairs(PreloadPaths) do
        local TAsset = BusinessHelper.LoadAssetFromPath(AssetPath)
        table.insert(self.SceneShowLoadedAssetList, TAsset)
      end
      if next(ScreenParticlePathToIndex) ~= nil then
        local PC = GameplayData.GetPlayerController()
        local FeatureRef = slua.isValid(PC) and PC.HighlightShowFeature or nil
        if FeatureRef and type(FeatureRef.BuildScreenParticleLoadedClassMap) == "function" then
          self.SceneShowScreenAppearanceLoadedClassMap = FeatureRef:BuildScreenParticleLoadedClassMap(ScreenParticlePathToIndex)
        end
      end
      self.SceneShowAssetLoaded = true
      self:_SceneShowCheckPreloadDone()
    end)
  end)
end
function HighlightMomentSubsystem:_SceneShowCheckPreloadDone()
  if not self.SceneShowAssetLoaded or not self.SceneShowPawnAllCreated then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowCheckPreloadDone not ready bAssetLoaded:" .. tostring(self.SceneShowAssetLoaded) .. " bPawnAllCreated:" .. tostring(self.SceneShowPawnAllCreated))
    return
  end
  print(bWriteLog and "HighlightMomentSubsystem:_SceneShowCheckPreloadDone all ready")
  if self._SceneShowPreloadDoneCallback then
    local Callback = self._SceneShowPreloadDoneCallback
    self._SceneShowPreloadDoneCallback = nil
    Callback()
  else
    self:_SceneShowCheckAndShowButton()
  end
end
function HighlightMomentSubsystem:_SceneShowCreatePawnEveryFrame()
  if #self.SceneShowPendingAvatarData == 0 then
    self.SceneShowPawnAllCreated = true
    if self.SceneShowCreatePawnTimer then
      self:RemoveGameTimer(self.SceneShowCreatePawnTimer)
      self.SceneShowCreatePawnTimer = nil
    end
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowCreatePawnEveryFrame all pawns created")
    self:_SceneShowCheckPreloadDone()
    return
  end
  local AvatarData = table.remove(self.SceneShowPendingAvatarData, 1)
  local PlayerController = GameplayData.GetPlayerController()
  if not slua.isValid(PlayerController) then
    return
  end
  if not self.SceneShowConfig.LobbyPawnClassPath then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowCreatePawnEveryFrame no ShowPawnActorPath")
    return
  end
  local uShowPawn = ActorTools.SpawnActor(PlayerController, self.SceneShowConfig.LobbyPawnClassPath, FVector(0, 0, -10000), FRotator(0, 0, 0), FVector(1, 1, 1))
  if not slua.isValid(uShowPawn) then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowCreatePawnEveryFrame spawn failed Index:" .. tostring(AvatarData.Index))
    return
  end
  if slua.isValid(uShowPawn.Mesh) then
    uShowPawn.Mesh:SetAnimationMode(2)
  end
  if AvatarData.bIsSelf then
    Game:SetShowPawnAvatarFromPlayerKey(uShowPawn, AvatarData.PlayerKey)
  else
    self:_SceneShowApplyAvatarToPawn(uShowPawn, AvatarData)
  end
  uShowPawn:SetActorHiddenInGame(true)
  uShowPawn:SetActorEnableCollision(false)
  self.SceneShowPawnList[AvatarData.Index] = uShowPawn
  print(bWriteLog and "HighlightMomentSubsystem:_SceneShowCreatePawnEveryFrame Index:" .. tostring(AvatarData.Index))
end
function HighlightMomentSubsystem:_SceneShowApplyAvatarToPawn(uPawn, AvatarData)
  local AvatarComp = uPawn.CharacterAvatarComp2_BP
  if not slua.isValid(AvatarComp) then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowApplyAvatarToPawn no AvatarComp Index:" .. tostring(AvatarData.Index))
    return
  end
  if not AvatarData.AvatarSyncData then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowApplyAvatarToPawn no AvatarSyncData Index:" .. tostring(AvatarData.Index))
    return
  end
  AvatarComp.bSyncAvatar = false
  AvatarComp.NetAvatarData = AvatarData.AvatarSyncData
  AvatarComp:SetAvatarGender(AvatarData.Gender)
  AvatarComp:OnRep_BodySlotStateChanged()
  if AvatarData.Gender == 0 then
    uPawn:SetMaleAnimClass()
  elseif AvatarData.Gender == 1 then
    uPawn:SetFemaleAnimClass()
  end
end
function HighlightMomentSubsystem:_SceneShowShowButtonDirectly()
  print(bWriteLog and "HighlightMomentSubsystem:_SceneShowShowButtonDirectly TypeID:" .. tostring(self.nCurSceneShowTypeID))
  if not self.nCurSceneShowTypeID then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowShowButtonDirectly no nCurSceneShowTypeID")
    return
  end
  self.CurHighlightList[self.nCurSceneShowTypeID] = {
    Param = 0,
  ...
  if self.FlauntBtnPanel then
    self.FlauntBtnPanel:RefreshUI(self:GetHighlightNum())
    self.FlauntBtnPanel:ResetShinningTime(self.nCurSceneShowTypeID)
  end
end
function HighlightMomentSubsystem:_SceneShowCheckAndShowButton()
  print(bWriteLog and "HighlightMomentSubsystem:_SceneShowCheckAndShowButton bAssetLoaded:" .. tostring(self.SceneShowAssetLoaded) .. " bPawnAllCreated:" .. tostring(self.SceneShowPawnAllCreated))
  if not self.SceneShowAssetLoaded or not self.SceneShowPawnAllCreated then
    return
  end
  if not self.nCurSceneShowTypeID then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowCheckAndShowButton no nCurSceneShowTypeID")
    return
  end
  print(bWriteLog and "HighlightMomentSubsystem:_SceneShowCheckAndShowButton bAssetLoaded:")
  if self.SceneShowForceEndTimer then
    self:RemoveGameTimer(self.SceneShowForceEndTimer)
    self.SceneShowForceEndTimer = nil
  end
  if self.FlauntBtnPanel then
    self.FlauntBtnPanel:RefreshUI(self:GetHighlightNum())
  ...
  print(bWriteLog and "HighlightMomentSubsystem:_SceneShowCheckAndShowButton ready TypeID:" .. tostring(self.nCurSceneShowTypeID))
end
function HighlightMomentSubsystem:_SceneShowPlayInternal()
  print(bWriteLog and "HighlightMomentSubsystem:_SceneShowPlayInternal TypeID:" .. tostring(self.nCurSceneShowTypeID))
  if not self.nCurSceneShowTypeID or not self.SceneShowConfig then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowPlayInternal no nCurSceneShowTypeID")
    return false
  end
  local PlayerController = GameplayData.GetPlayerController()
  if not slua.isValid(PlayerController) then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowPlayInternal invalid PlayerController")
    return false
  end
  local HighlightEntry = self.nCurSceneShowTypeID and self.CurHighlightList[self.nCurSceneShowTypeID] or nil
  if HighlightEntry and HighlightEntry.bLowEndPending then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowPlayInternal LowEnd pending, start preload with transition UI")
    HighlightEntry.bLowEndPending = false
    self.bInSceneShowTransition = true
    local IngameSelfieSubsystem = SubsystemMgr:Get("IngameSelfieSubsystem")
    if not IngameSelfieSubsystem then
      print(bWriteLog and "HighlightMomentSubsystem:_SceneShowPlayInternal no IngameSelfieSubsystem")
      return false
    end
    self.nCurPlayingType = self.nCurSceneShowTypeID or -1
    if not IngameSelfieSubsystem:EnterSelfie() then
      print(bWriteLog and "HighlightMomentSubsystem:_SceneShowPlayInternal EnterSelfie failed in transition")
      self.nCurPlayingType = -1
      self.bInSceneShowTransition = false
      return false
    end
    function self.EnterSelfieCallBack()
      ...
      if PhotoUI and PhotoUI.SetLoadingBGImage and self.SceneShowConfig and self.SceneShowConfig.TransitionBGImagePath then
        PhotoUI:SetLoadingBGImage(self.SceneShowConfig.TransitionBGImagePath)
      end
    end
    function self.ExitSelfieCallBack()
      if self.bInSceneShowTransition then
        print(bWriteLog and "HighlightMomentSubsystem:_SceneShowPlayInternal ExitSelfie during transition, keep trigger state")
        self:_SceneShowTransitionExit()
      end
    end
    local LoadingMaxTime = self.SceneShowConfig.LoadingMaxTime or 20
    self.SceneShowForceEndTimer = self:AddGameTimer(LoadingMaxTime, false, function()
      print(bWriteLog and "HighlightMomentSubsystem:_SceneShowPlayInternal LowEnd ForceEnd by timeout")
      self.SceneShowForceEndTimer = nil
      self.bInSceneShowTransition = false
      self:_SceneShowCleanup()
    end)
    self:_SceneShowDoPreload(function()
      print(bWriteLog and "HighlightMomentSubsystem:_SceneShowPlayInternal LowEnd preload done")
      if self.SceneShowForceEndTimer then
        self:RemoveGameTimer(self.SceneShowForceEndTimer)
        self.SceneShowForceEndTimer = nil
      end
      if not self.bInSceneShowTransition then
        print(bWriteLog and "HighlightMomentSubsystem:_SceneShowPlayInternal preload done but already exited, skip play")
        return
      end
      self.bInSceneShowTransition = false
      local PhotoUI = UIManager.UI_Config_InGame.Ingame_Photo_UIBP and UIManager.GetUI(UIManager.UI_Config_InGame.Ingame_Photo_UIBP) or nil
      ...
      self:_SceneShowDoPlayAfterSelfieReady()
    end)
    return true
  end
  return self:_SceneShowDoPlay()
end
function HighlightMomentSubsystem:_SceneShowDoPlay()
  print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPlay")
  local PlayerController = GameplayData.GetPlayerController()
  if not slua.isValid(PlayerController) then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPlay invalid PlayerController")
    return false
  end
  local IngameSelfieSubsystem = SubsystemMgr:Get("IngameSelfieSubsystem")
  if not IngameSelfieSubsystem then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPlay no IngameSelfieSubsystem")
    return false
  end
  self.nCurPlayingType = self.nCurSceneShowTypeID or -1
  if not IngameSelfieSubsystem:EnterSelfie() then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPlay EnterSelfie failed")
    self.nCurPlayingType = -1
    return false
  end
  function self.EnterSelfieCallBack()
    ...
    PhotoGrapherSubSystem:ModifyPetOpen(false)
    self:_SceneShowDoPlayInternal()
  end
  return true
end
function HighlightMomentSubsystem:_SceneShowDoPlayAfterSelfieReady()
  print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPlayAfterSelfieReady")
  local PhotoGrapherSubSystem = SubsystemMgr:Get("PhotoGrapherSubSystem")
  if not PhotoGrapherSubSystem then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPlayAfterSelfieReady no PhotoGrapherSubSystem")
    self:_SceneShowCleanup()
    return
  end
  PhotoGrapherSubSystem:ModifyPetOpen(false)
...
function HighlightMomentSubsystem:_SceneShowTransitionExit()
  print(bWriteLog and "HighlightMomentSubsystem:_SceneShowTransitionExit")
  self.bInSceneShowTransition = false
  if self.SceneShowForceEndTimer then
    self:RemoveGameTimer(self.SceneShowForceEndTimer)
    self.SceneShowForceEndTimer = nil
  ...
  if self.SceneShowPawnList then
    for _, uPawn in pairs(self.SceneShowPawnList) do
      if slua.isValid(uPawn) then
        uPawn:K2_DestroyActor()
      end
    end
  end
  self.SceneShowPawnList = {}
  self.SceneShowPendingAvatarData = {}
  if self.SceneShowPendingAvatarDataSnapshot then
    for _, Data in ipairs(self.SceneShowPendingAvatarDataSnapshot) do
      table.insert(self.SceneShowPendingAvatarData, Data)
    end
  end
  self.SceneShowLoadedAssetList = {}
  if self.SceneShowPawnClassHandle then
    self:CancelAsyncLoad(self.SceneShowPawnClassHandle)
    self.SceneShowPawnClassHandle = nil
  end
  if self.SceneShowAssetArrayHandle then
    self:CancelAsyncLoad(self.SceneShowAssetArrayHandle)
    self.SceneShowAssetArrayHandle = nil
  end
  self:StopAllSceneShowScreenParticles()
  self.SceneShowActiveScreenAppearanceMap = {}
  ...
  self.SceneShowPawnAllCreated = false
  self._SceneShowPreloadDoneCallback = nil
  if self.SceneShowLevelLoaded then
    local SceneShowConfig = self.SceneShowConfig
    if SceneShowConfig and SceneShowConfig.LevelAssetName then
      local LevelStreamingMgr = SubsystemMgr:Get("LevelStreamingMgr")
      if LevelStreamingMgr then
        LevelStreamingMgr:UnloadStreamLevelNoLatent(SceneShowConfig.LevelAssetName)
      end
    end
    self.SceneShowLevelLoaded = false
  end
  local TypeID = self.nCurSceneShowTypeID
  local HighlightEntry = TypeID and self.CurHighlightList and self.CurHighlightList[TypeID] or nil
  if HighlightEntry then
    HighlightEntry.bLowEndPending = true
  end
end
function HighlightMomentSubsystem:_SceneShowDoPlayInternal()
  print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPlayInternal")
  if not self.SceneShowConfig then
    return
  end
  local PlayerController = GameplayData.GetPlayerController()
  if not slua.isValid(PlayerController) then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPlayInternal invalid PlayerController")
    self:_SceneShowCleanup()
    return
  end
  local LevelStreamingMgr = SubsystemMgr:Get("LevelStreamingMgr")
  if LevelStreamingMgr and not self.SceneShowLevelHoldEnabled then
    local uPlayerPawn = PlayerController:GetPlayerCharacterSafety()
    if slua.isValid(uPlayerPawn) then
      local PlayerLocation = uPlayerPawn:K2_GetActorLocation()
      LevelStreamingMgr:EnableClientViewPointLevelHold(true, PlayerLocation)
      self.SceneShowLevelHoldEnabled = true
      print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPlayInternal EnableClientViewPointLevelHold at:" .. tostring(PlayerLocation))
    else
      print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPlayInternal skip level hold, invalid player pawn")
    end
  end
  if LevelStreamingMgr and self.SceneShowConfig.LevelAssetName then
    LevelStreamingMgr:LoadStreamLevelNoLatent(self.SceneShowConfig.LevelAssetName, true, true)
    self.SceneShowLevelLoaded = true
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPlayInternal LoadStreamLevel:" .. tostring(self.SceneShowConfig.LevelAssetName))
  end
  for _, uPawn in pairs(self.SceneShowPawnList) do
    if slua.isValid(uPawn) then
      uPawn:SetActorHiddenInGame(false)
      if slua.isValid(uPawn.Mesh) then
        uPawn.Mesh:SetAnimationMode(2)
      end
    end
  end
  local BindingList = {}
  local PawnTrackNames = self.SceneShowConfig.PawnTrackNames or {}
  for Index, uPawn in pairs(self.SceneShowPawnList) do
    local TrackName = PawnTrackNames[Index]
    if TrackName and slua.isValid(uPawn) then
      BindingList[TrackName] = uPawn
    end
  end
  self:_SceneShowApplyFPSLimit()
  local SequenceActorPos = self.SceneShowConfig.SequenceActorPos and FVector(self.SceneShowConfig.SequenceActorPos.X, self.SceneShowConfig.SequenceActorPos.Y, self.SceneShowConfig.SequenceActorPos.Z) or FVector(0, 0, 0)
  local SequenceActorRot = self.SceneShowConfig.SequenceActorRot and FRotator(self.SceneShowConfig.SequenceActorRot.Pitch, self.SceneShowConfig.SequenceActorRot.Yaw, self.SceneShowConfig.SequenceActorRot.Roll) or FRotator(0, 0, 0)
  local TempTran = KismetMathLibrary.MakeTransform(SequenceActorPos, SequenceActorRot, FVector(1, 1, 1))
  self.SceneShowSequenceActor = Game:PlayLevelSequence(PlayerController, self.SceneShowConfig.SequencePath, TempTran, self.SceneShowConfig.SequenceActorPath .. "_C", false, BindingList, PlayerController)
  if not slua.isValid(self.SceneShowSequenceActor) then
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPlayInternal invalid SequenceActor")
    self:_SceneShowCleanup()
    return
  end
  self.SceneShowSequenceActor:SetOnSequenceStopCallBack(function()
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPlayInternal SequenceStop")
    self:_SceneShowCleanup()
    EventSystem:postEvent(EVENTTYPE_PLAYEREVENT_CHARACTER, EVENTID_PLAYEREVENT_SMARTCAMERA_SEQUENCE_END)
  end, self)
  self.SceneShowSequenceActor:SetOnSequenceFinishedCallBack(function()
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPlayInternal SequenceFinished")
    self:_SceneShowCleanup()
    EventSystem:postEvent(EVENTTYPE_PLAYEREVENT_CHARACTER, EVENTID_PLAYEREVENT_SMARTCAMERA_SEQUENCE_END)
  end, self)
  self.SceneShowSequenceActor:Play(0)
  EventSystem:postEvent(EVENTTYPE_INGAME_NORMAL, EVENTID_INGAME_ENTER_SMART_CAMERA)
  function self.ExitSelfieCallBack()
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPlayInternal ExitSelfie triggered, stop show")
    self:_SceneShowCleanup()
  end
  local TotalShowTime = self.SceneShowConfig.TotalShowTime or 15
  self.SceneShowForceEndTimer = self:AddGameTimer(TotalShowTime, false, function()
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPlayInternal TotalShowTime reached")
    self.SceneShowForceEndTimer = nil
    self:_SceneShowCleanup()
  end)
  print(bWriteLog and "HighlightMomentSubsystem:_SceneShowDoPlayInternal started successfully")
end
function HighlightMomentSubsystem:IsSceneShowIdle()
  if self.nCurPlayingType and self.nCurPlayingType > 0 then
    return false
  end
  if self.CurHighlightList and self.CurHighlightList[self.nSceneShowTypeID] then
    print(bWriteLog and "HighlightMomentSubsystem:IsSceneShowIdle CurHighlightList Exist SceneShow")
    return false
  end
  return true
end
function HighlightMomentSubsystem:_SceneShowApplyFPSLimit()
  ...
  print(bWriteLog and "HighlightMomentSubsystem:_SceneShowApplyFPSLimit MaxFps:" .. tostring(MaxFps) .. " OverrideMaxFPS:" .. tostring(OverrideMaxFPS))
  local nTargetFPS
  if 60 < MaxFps then
    nTargetFPS = 60
  elseif 40 < MaxFps then
    nTargetFPS = 40
  elseif 30 < MaxFps then
    nTargetFPS = 30
  end
  if nTargetFPS then
    GameInstance:ExecuteCMD("t.OverrideMaxFPS", nTargetFPS)
    self._bSceneShowFPSLimited = true
    print(bWriteLog and "HighlightMomentSubsystem:_SceneShowApplyFPSLimit limited to " .. tostring(nTargetFPS))
  end
end
function HighlightMomentSubsystem:_SceneShowRestoreFPSLimit()
  if not self._bSceneShowFPSLimited then
    return
  end
  local UIUtil = require("client.common.ui_util")
  local GameInstance = UIUtil.GetGameInstance()
  ...
  print(bWriteLog and "HighlightMomentSubsystem:_SceneShowRestoreFPSLimit")
  GameInstance:ExecuteCMD("t.OverrideMaxFPS", 0)
  self._bSceneShowFPSLimited = false
end
function HighlightMomentSubsystem:_GetSceneShowScreenAppearanceKey(Index)
  if not self.SceneShowConfig or not Index then
    return nil
  end
  local EffectCfg = self.SceneShowConfig.ScreenParticleConfig and self.SceneShowConfig.ScreenParticleConfig[Index] or nil
  if EffectCfg and EffectCfg.AppearanceKey and EffectCfg.AppearanceKey ~= "" then
    return EffectCfg.AppearanceKey
  end
  return nil
end
function HighlightMomentSubsystem:PlaySceneShowScreenParticleByIndex(Index)
  print(bWriteLog and string.format("HighlightMomentSubsystem:PlaySceneShowScreenParticleByIndex Index:%s", tostring(Index)))
  if not Index or not self.SceneShowConfig then
    print(bWriteLog and "HighlightMomentSubsystem:PlaySceneShowScreenParticleByIndex Invalid Index or no SceneShowConfig")
    return
  end
  local PC = GameplayData.GetPlayerController()
  if not slua.isValid(PC) then
    print(bWriteLog and "HighlightMomentSubsystem:PlaySceneShowScreenParticleByIndex Invalid PlayerController")
    return
  end
  local EffectCfg = self.SceneShowConfig.ScreenParticleConfig and self.SceneShowConfig.ScreenParticleConfig[Index] or nil
  if not (EffectCfg and EffectCfg.AssetPath) or EffectCfg.AssetPath == "" then
    print(bWriteLog and "HighlightMomentSubsystem:PlaySceneShowScreenParticleByIndex Invalid EffectCfg for index = " .. tostring(Index))
    return
  end
  local AppearanceKey = self:_GetSceneShowScreenAppearanceKey(Index)
  if not AppearanceKey then
    print(bWriteLog and "HighlightMomentSubsystem:PlaySceneShowScreenParticleByIndex AppearanceKey not configured, skip. index = " .. tostring(Index))
    return
  end
  local ScreenAppearanceStatics = import("ScreenAppearanceStatics")
  if self.SceneShowActiveScreenAppearanceMap[Index] then
    print(bWriteLog and "HighlightMomentSubsystem:PlaySceneShowScreenParticleByIndex Already playing, restart. index = " .. tostring(Index))
    ScreenAppearanceStatics.StopScreenAppearanceByName(PC, PC, AppearanceKey)
    self.SceneShowActiveScreenAppearanceMap[Index] = nil
  end
  local DoPlay = function(ScreenAppearanceClass)
    if not slua.isValid(ScreenAppearanceClass) then
      print(bWriteLog and "HighlightMomentSubsystem:PlaySceneShowScreenParticleByIndex DoPlay invalid class, index = " .. tostring(Index))
      return
    end
    if not slua.isValid(self.SceneShowSequenceActor) then
      print(bWriteLog and "HighlightMomentSubsystem:PlaySceneShowScreenParticleByIndex DoPlay skip, SceneShowSequenceActor invalid, index = " .. tostring(Index))
      return
    end
    local CurPC = GameplayData.GetPlayerController()
    if not slua.isValid(CurPC) then
      print(bWriteLog and "HighlightMomentSubsystem:PlaySceneShowScreenParticleByIndex DoPlay invalid PC, index = " .. tostring(Index))
      return
    end
    ScreenAppearanceStatics.PlayDynamicScreenAppearance(CurPC, CurPC, AppearanceKey, ScreenAppearanceClass)
    self.SceneShowActiveScreenAppearanceMap[Index] = true
    print(bWriteLog and string.format("HighlightMomentSubsystem:PlaySceneShowScreenParticleByIndex Played index = %s key = %s", tostring(Index), tostring(AppearanceKey)))
  end
  local CachedClass = self.SceneShowScreenAppearanceLoadedClassMap[Index]
  if not slua.isValid(CachedClass) then
    print(bWriteLog and "HighlightMomentSubsystem:PlaySceneShowScreenParticleByIndex Preload missed, skip. index = " .. tostring(Index))
    return
  end
  DoPlay(CachedClass)
end
function HighlightMomentSubsystem:StopSceneShowScreenParticleByIndex(Index)
  print(bWriteLog and "HighlightMomentSubsystem:StopSceneShowScreenParticleByIndex Index = " .. tostring(Index))
  if not Index then
    return
  end
  if not self.SceneShowActiveScreenAppearanceMap[Index] then
    print(bWriteLog and "HighlightMomentSubsystem:StopSceneShowScreenParticleByIndex Not active, skip. index = " .. tostring(Index))
    return
  end
  local PC = GameplayData.GetPlayerController()
  if slua.isValid(PC) then
    ...
    if AppearanceKey then
      local ScreenAppearanceStatics = import("ScreenAppearanceStatics")
      ScreenAppearanceStatics.StopScreenAppearanceByName(PC, PC, AppearanceKey)
      print(bWriteLog and "HighlightMomentSubsystem:StopSceneShowScreenParticleByIndex Stopped index = " .. tostring(Index) .. " key = " .. tostring(AppearanceKey))
    else
      print(bWriteLog and "HighlightMomentSubsystem:StopSceneShowScreenParticleByIndex AppearanceKey not configured, skip. index = " .. tostring(Index))
    end
  end
  self.SceneShowActiveScreenAppearanceMap[Index] = nil
end
function HighlightMomentSubsystem:StopAllSceneShowScreenParticles()
  print(bWriteLog and "HighlightMomentSubsystem:StopAllSceneShowScreenParticles")
  if not self.SceneShowActiveScreenAppearanceMap then
    return
  end
  local IndexList = {}
  for Index, _ in pairs(self.SceneShowActiveScreenAppearanceMap) do
...
function HighlightMomentSubsystem:_SceneShowCleanup()
  print(bWriteLog and "HighlightMomentSubsystem:_SceneShowCleanup")
  self:StopAllSceneShowScreenParticles()
  self.SceneShowActiveScreenAppearanceMap = {}
  self.SceneShowScreenAppearanceLoadedClassMap = {}
  self:_SceneShowRestoreFPSLimit()
  self.bInSceneShowTransition = false
  if slua.isValid(self.SceneShowSequenceActor) then
    self.SceneShowSequenceActor:Stop()
  end
  self.SceneShowSequenceActor = nil
  for _, uPawn in pairs(self.SceneShowPawnList) do
    if slua.isValid(uPawn) then
  ...
  self.SceneShowPawnList = {}
  if self.SceneShowLevelLoaded then
    local SceneShowConfig = Config.SceneShowConfig and self.nCurSceneShowID and Config.SceneShowConfig[self.nCurSceneShowID] or nil
    if SceneShowConfig and SceneShowConfig.LevelAssetName then
      local LevelStreamingMgr = SubsystemMgr:Get("LevelStreamingMgr")
      if LevelStreamingMgr then
        LevelStreamingMgr:UnloadStreamLevelNoLatent(SceneShowConfig.LevelAssetName)
        print(bWriteLog and "HighlightMomentSubsystem:_SceneShowCleanup UnloadStreamLevel:" .. tostring(SceneShowConfig.LevelAssetName))
      end
    end
    self.SceneShowLevelLoaded = false
  end
  if self.SceneShowLevelHoldEnabled then
    local LevelStreamingMgr = SubsystemMgr:Get("LevelStreamingMgr")
    if LevelStreamingMgr then
      LevelStreamingMgr:EnableClientViewPointLevelHold(false)
      print(bWriteLog and "HighlightMomentSubsystem:_SceneShowCleanup DisableClientViewPointLevelHold")
    end
    self.SceneShowLevelHoldEnabled = false
  end
  if self.SceneShowForceEndTimer then
    self:RemoveGameTimer(self.SceneShowForceEndTimer)
    self.SceneShowForceEndTimer = nil
  ...
  self.SceneShowConfig = nil
  local TypeID = self.nCurSceneShowTypeID
  if TypeID and self.CurHighlightList[TypeID] then
    self.CurHighlightList[TypeID] = nil
    if self.FlauntBtnPanel then
      self.FlauntBtnPanel:RefreshUI(self:GetHighlightNum())
    end
  end
  self.nCurSceneShowID = nil
  self.nCurSceneShowTypeID = nil
  self.nCurPlayingType = -1
  self.ExitSelfieCallBack = nil
  self.EnterSelfieCallBack = nil
  self:AutoExitSelfieMode()
  local uPlayerController = GameplayData.GetPlayerController()
  local HighlightShowFeature = slua.isValid(uPlayerController) and uPlayerController.HighlightShowFeature or nil
  if HighlightShowFeature and type(HighlightShowFeature.OnSubsystemSceneShowIdle) == "function" then
    HighlightShowFeature:OnSubsystemSceneShowIdle()
  else
    print(bWriteLog and "HighlightMomentSubsystem:Cleanup HighlightShowFeature not found")
  end
end
