...
local uCoopVaultAnimBpPath = "/Game/Arts_Player/Characters/Animation/Base_AnimBP/Feature/CH_Base_AnimBP_Vault.CH_Base_AnimBP_Vault_C"
MantleComponent.ServerRPC.ServerRPC_RequestCooperationVaultItem = {Reliable = true}
function MantleComponent:ReceiveBeginPlay()
  ...
  if Client then
    ...
    if slua.isValid(uOwner) and uLocalCharacter == uOwner then
      self:RequestCooperationVaultItem()
    end
  end
end
...
function MantleComponent:ReceiveEndVault()
  ...
  self:PlayerStuckCheckAndResolve(60)
  self:DelayPlayerStuckCheckAndResolve()
  self:NotifyEndVault()
end
function MantleComponent:PlayerStuckCheckAndResolve(nHalfHeight)
  if not Client and slua.isValid(self.PlayerPawn) then
    ...
    local CheckHalfHeight = nHalfHeight
    ...
    if Game:IsClassOf(self.VaultingActor, STExtraHouseActor) and self.NowVaultData.IsJump then
      CheckHalfHeight = nHalfHeight + 15
    end
...
function MantleComponent:DelayPlayerStuckCheckAndResolve()
  ...
  self.DelayStuckCheckTimer = self:AddGameTimer(1.0, false, function()
    if not slua.isValid(self.PlayerPawn) then
      return
    end
    ...
    if self.RecordEndVaultLocation and (self.RecordEndVaultLocation - CurLocation):Size() < 100 then
      self:PlayerStuckCheckAndResolve(65)
    end
  end)
end
...
function MantleComponent:RequestCooperationVaultItem()
  local SettingModule = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.SettingModule)
  local nValue = SettingModule:GetOptionValue("CoopVault")
  if nValue then
    self:ServerRPC_RequestCooperationVaultItem()
  end
end
function MantleComponent:ServerRPC_RequestCooperationVaultItem()
...
function MantleComponent:AddCooperationVaultItem()
  ...
  if CGameState and slua.isValid(CGameState) then
    ...
    if PlayerNumPerTeam <= 1 then
      ...
      print(bWriteLog and "MantleComponent:AddCooperationVaultItem PlayerNumPerTeam <= 1")
      return
    end
  end
  ...
  local nItemNum = Game:GetItemNumByResID(uCharacterOwner, 602420)
  if nItemNum and 0 < nItemNum then
    print(bWriteLog and "MantleComponent:AddCooperationVaultItem nItemNum > 0")
    return
  end
...
function MantleComponent:LuaOnEnterCoopVault(Role, NewStage)
  ...
  if Client then
    print(bWriteLog and "MantleComponent:LuaOnEnterCoopVault AsyncLoadAniHandle")
    self.AsyncLoadAniHandle = self:AsyncLoadAsset(uCoopVaultAnimBpPath, function(uClass)
      if slua.isValid(uClass) then
        print(bWriteLog and "MantleComponent:LuaOnEnterCoopVault AsyncLoadAniHandle SetSubInstance")
        self.uCoopVaultAnimClass = uClass
        local uOwner = self:GetOwner()
        if not slua.isValid(uOwner) then
          return
        end
        if slua.isValid(self.uCoopVaultAnimClass) then
          uOwner.Mesh:SetSubInstance("Vault", self.uCoopVaultAnimClass)
        end
      end
    end)
  end
