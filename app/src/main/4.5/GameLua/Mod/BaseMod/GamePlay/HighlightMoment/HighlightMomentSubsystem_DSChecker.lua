...
function HighlightMomentSubsystem_DSChecker:OnAddKills(_, _, uKillerPS, uVictim)
  ...
  local Config = self:GetHightlightMomentConfig()
...
function HighlightMomentSubsystem_DSChecker:EliminateKillConfirm(uVictim, uCauser, nTypeID, uKillerCharacter, nAdditionalValue)
  ...
  for TypeIDKey, CheckFuncMapByType in pairs(self.SceneShowCheckFuncRouter) do
    if TypeIDKey == -1 or TypeIDKey == nTypeID then
      for AdditionalValueKey, CheckFuncsByAdditionalValue in pairs(CheckFuncMapByType) do
        if AdditionalValueKey == -1 or AdditionalValueKey == nAdditionalValue then
          for nSceneID, fCheckFunc in pairs(CheckFuncsByAdditionalValue) do
            if fCheckFunc then
              local Config = self:GetHightlightMomentConfig()
              if Config and Config.SceneShowConfig and Config.SceneShowConfig[nSceneID] and (not bVictimDying or Config.SceneShowConfig[nSceneID].bIgnoreVictimDying) then
                fCheckFunc(self, nSceneID, Config.SceneShowConfig[nSceneID], uVictim, uCauser, nTypeID, uKillerCharacter, nAdditionalValue)
              end
            end
          end
        end
