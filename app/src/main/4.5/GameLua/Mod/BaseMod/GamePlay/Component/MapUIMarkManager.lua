...
function MapUIMarkManager:AfterChangeMark(bIsShow, TypeID, Location, InstanceID, CustomState)
  if not bIsShow then
    ...
    EventSystem:postEvent(EVENTTYPE_INGAME_MAP, EVENTID_MAP_MARK_NAVIGATOR_MARK_REMOVED, TypeID, InstanceID)
    return
  end
  ...
  EventSystem:postEvent(EVENTTYPE_INGAME_MAP, EVENTID_MAP_MARK_NAVIGATOR_MARK_ADDED, TypeID, InstanceID)
end
