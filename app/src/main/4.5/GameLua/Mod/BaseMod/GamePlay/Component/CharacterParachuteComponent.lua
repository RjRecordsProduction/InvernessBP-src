...
local CharacterParachuteComponent = {
  PARACHUTE_AVG_SPEED_REPORT_ID = 7,
  AVG_SPEED_WINDOW = 2.0,
  AVG_SPEED_TOLERANCE = 1
}
...
function CharacterParachuteComponent:OnParachuteStateChanged(LastParachuteState, ParachuteState)
  ...
  if ParachuteState == EParachuteState.PS_FreeFall or ParachuteState == EParachuteState.PS_Opening then
    self:StartParachuteAvgSpeedMonitor()
  elseif ParachuteState == EParachuteState.PS_None then
    self:StopParachuteAvgSpeedMonitor()
  end
  local MyOwner = self:GetOwnerActor()
  if MyOwner ~= nil and slua.isValid(MyOwner) and MyOwner.Role == ENetRole.ROLE_AutonomousProxy then
...
function CharacterParachuteComponent:StartParachuteAvgSpeedMonitor()
  local MyOwner = self:GetOwnerActor()
  if not slua.isValid(MyOwner) then
    return
  end
  if MyOwner.Role ~= ENetRole.ROLE_AutonomousProxy and MyOwner.Role ~= ENetRole.ROLE_Authority then
    return
  end
  if self.ParachuteAvgSpeedTimer then
    return
  end
  self:ResetParachuteAvgSpeedSample()
  self.ParachuteAvgSpeedTimer = self:AddGameTimer(self.AVG_SPEED_WINDOW, true, function()
    self:TickParachuteAvgSpeedSample()
  end)
end
function CharacterParachuteComponent:StopParachuteAvgSpeedMonitor()
  if self.ParachuteAvgSpeedTimer then
    self:RemoveGameTimer(self.ParachuteAvgSpeedTimer)
    self.ParachuteAvgSpeedTimer = nil
  end
  self:ResetParachuteAvgSpeedSample()
end
function CharacterParachuteComponent:ResetParachuteAvgSpeedSample()
  self.AvgSpeedStartLoc = nil
  self.AvgSpeedStartTime = nil
  self.AvgSpeedSawFreeFall = false
  local MyOwner = self:GetOwnerActor()
  if slua.isValid(MyOwner) then
    self.AvgSpeedStartLoc = MyOwner:K2_GetActorLocation()
    local UGameplayStatics = import("GameplayStatics")
    self.AvgSpeedStartTime = UGameplayStatics.GetTimeSeconds(CGameWorld) or 0
    if MyOwner.ParachuteState == EParachuteState.PS_FreeFall then
      self.AvgSpeedSawFreeFall = true
    end
  end
end
function CharacterParachuteComponent:TickParachuteAvgSpeedSample()
  local MyOwner = self:GetOwnerActor()
  if not slua.isValid(MyOwner) then
    self:StopParachuteAvgSpeedMonitor()
    return
  end
  if MyOwner.ParachuteState ~= EParachuteState.PS_FreeFall and MyOwner.ParachuteState ~= EParachuteState.PS_Opening then
    return
  end
  if MyOwner.ParachuteState == EParachuteState.PS_FreeFall then
    self.AvgSpeedSawFreeFall = true
  end
  if not self.AvgSpeedStartLoc or not self.AvgSpeedStartTime then
    self:ResetParachuteAvgSpeedSample()
    return
  end
  local UGameplayStatics = import("GameplayStatics")
  local NowTime = UGameplayStatics.GetTimeSeconds(CGameWorld) or 0
  local Elapsed = NowTime - self.AvgSpeedStartTime
  local StartLoc = self.AvgSpeedStartLoc
  local EndLoc = MyOwner:K2_GetActorLocation()
  local bUseFreeFallLimit = self.AvgSpeedSawFreeFall == true
  if not (not (Elapsed <= 0) and StartLoc) or not EndLoc then
    self:ResetParachuteAvgSpeedSample()
    return
  end
  local AvgX = (EndLoc.X - StartLoc.X) / Elapsed
  local AvgY = (EndLoc.Y - StartLoc.Y) / Elapsed
  local AvgZ = (EndLoc.Z - StartLoc.Z) / Elapsed
  self:ResetParachuteAvgSpeedSample()
  local ParachuteComp = self.Object
  if not slua.isValid(ParachuteComp) then
    return
  end
  local TolerancePer = 1 + self.AVG_SPEED_TOLERANCE
  local LimitF, LimitR, LimitFall
  if bUseFreeFallLimit then
    LimitF = (ParachuteComp.FreeFall_MaxForwardSpeed or 0) * TolerancePer
    LimitR = (ParachuteComp.FreeFall_MaxForwardSpeed or 0) * TolerancePer
    LimitFall = ((ParachuteComp.FreeFall_MaxFreeFallSpeed or 0) + (ParachuteComp.FreeFall_MaxDiveExtraFallSpeed or 0)) * TolerancePer
  else
    LimitF = (ParachuteComp.Opened_MaxForwardSpeed or 0) * TolerancePer
    LimitR = (ParachuteComp.Opened_MaxForwardSpeed or 0) * TolerancePer
    LimitFall = ((ParachuteComp.Opened_MaxFreeFallSpeed or 0) + (ParachuteComp.Opened_MaxForwardExtraFallSpeed or 0)) * TolerancePer
  end
  local AvgSpeed = math.sqrt(AvgX * AvgX + AvgY * AvgY + AvgZ * AvgZ)
  local CurVelX, CurVelY, CurVelZ = 0, 0, 0
  local CurVelocity = ParachuteComp:GetCurVelocity()
  if CurVelocity then
    CurVelX, CurVelY, CurVelZ = CurVelocity.X, CurVelocity.Y, CurVelocity.Z
  end
  local CurSpeed = math.sqrt(CurVelX * CurVelX + CurVelY * CurVelY + CurVelZ * CurVelZ)
  local TimestampSeconds = NowTime
  local Side = MyOwner.Role == ENetRole.ROLE_Authority and 1 or 0
  print(bWriteLog and string.format("CharacterParachuteComponent:ParachuteAvgSpeed OverLimit Side:%d PlayerKey:%s State:%s AvgVel:(%.1f,%.1f,%.1f) AvgSpeed:%.1f CurVel:(%.1f,%.1f,%.1f) CurSpeed:%.1f Limit(F/R/Fall):%.1f/%.1f/%.1f", Side, tostring(MyOwner.PlayerKey), tostring(MyOwner.ParachuteState), AvgX, AvgY, AvgZ, AvgSpeed, CurVelX, CurVelY, CurVelZ, CurSpeed, LimitF, LimitR, LimitFall))
  if LimitF >= math.abs(AvgX) and LimitR >= math.abs(AvgY) then
    return
  end
  local ReportArray = {
    Side,
  ...
  if MyOwner.Role == ENetRole.ROLE_Authority then
    self:WriteParachuteAvgSpeedToReplayOnDS(MyOwner.PlayerKey, ReportArray)
  else
    local GameReportUtils = require("GameLua.Mod.BaseMod.GamePlay.GameReport.GameReportUtils")
    GameReportUtils.ReplayReportData(self.PARACHUTE_AVG_SPEED_REPORT_ID, ReportArray)
  end
end
function CharacterParachuteComponent:WriteParachuteAvgSpeedToReplayOnDS(PlayerKey, ReportArray)
  if Client then
    return
  end
  if not slua.isValid(CGameState) or not slua.isValid(CGameState.ReplayRecordManager) then
    return
  end
  local LuaArray = slua.Array(UEnums.EPropertyClass.Float)
  for _, Value in ipairs(ReportArray) do
    LuaArray:Add(Value)
  end
  CGameState.ReplayRecordManager:RPC_Replay_ReportFloatArray(PlayerKey, self.PARACHUTE_AVG_SPEED_REPORT_ID, LuaArray)
end
function CharacterParachuteComponent:OnReceiveEndPlay(EndReason, bClearTable)
  CharacterParachuteComponent.__super.OnReceiveEndPlay(self, EndReason, bClearTable)
  self:StopParachuteAvgSpeedMonitor()
end
