...
function TeammateTakeOverFeature:RPC_Client_RequestTeammateTakeOver()
  if Client then
    ...
    local bTeammateTakeOverSetting = SettingModule:GetOptionValue("TeammateTakeOver")
...
function TeammateTakeOverFeature:RPC_Server_ResponseTeammateTakeOver(bEnable, bOpenSwitch)
  ...
  if slua.isValid(uPlayerState) then
    if uPlayerState.bHasSendBattleResult == true then
      print(bWriteLog and string.format("TeammateTakeOverFeature:RPC_Server_ResponseTeammateTakeOver - player %s has already settled, reject", tostring(uPlayerState.PlayerKey)))
      return
    end
