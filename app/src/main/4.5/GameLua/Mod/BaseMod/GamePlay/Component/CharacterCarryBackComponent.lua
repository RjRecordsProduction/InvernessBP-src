...
function CharacterCarryBackComponent:LocalCompleteCarryBackState()
  ...
  local ENetRole = import("ENetRole")
  local uOwnerPawn = self:GetOwner()
  if slua.isValid(uOwnerPawn) and (Client or uOwnerPawn.Role ~= ENetRole.ROLE_Authority) then
...
function CharacterCarryBackComponent:LocalExitBeCarriedBackState(DetachMethod)
  ...
  local ECarryBackState = import("ECarryBackState")
  local ENetRole = import("ENetRole")
  if (Client or uOwnerPawn.Role ~= ENetRole.ROLE_Authority) and DetachMethod ~= EPutDownDetachMethod.StateInterrupted_OtherDead then
...
function CharacterCarryBackComponent:LocalPutDownFailed()
  ...
  local ENetRole = import("ENetRole")
  local uOwnerPawn = self:GetOwner()
  if slua.isValid(uOwnerPawn) and uOwnerPawn.Role == ENetRole.ROLE_AutonomousProxy then
  ...
  if slua.isValid(uAttachedCharacter) then
    if slua.isValid(uAttachedCharacter.CharacterMovement) then
      ...
      if uAttachedCharacter.Mesh and slua.isValid(uAttachedCharacter.Mesh) then
        ...
        if slua.isValid(uCharacter) and (uCharacter.Role == ENetRole.ROLE_AutonomousProxy or UKismetSystemLibrary.IsStandalone(uCharacter)) then
    ...
    if bAttach then
      ...
      local HeightDelta = -29
      if slua.isValid(uCharacter) and slua.isValid(uCharacter.CapsuleComponent) and slua.isValid(uAttachedCharacter.CapsuleComponent) then
        HeightDelta = uAttachedCharacter.CapsuleComponent:GetScaledCapsuleHalfHeight() - uCharacter.CapsuleComponent:GetScaledCapsuleHalfHeight()
      end
      local EPutDownDetachMethod = import("EPutDownDetachMethod")
      ...
      if self.DetachMethod == EPutDownDetachMethod.Skill_Front then
        local uPassWallCheckLocation = uLocation + uForwardVector * 72.76 + uRightVector * 48 + FVector(0, 0, HeightDelta + 14)
        local uNewLocation = uLocation + uForwardVector * 72.76 + uRightVector * 48 + FVector(0, 0, HeightDelta)
        ...
        if self.bDebugPutDownLocation and (uCharacter.Role == ENetRole.ROLE_AutonomousProxy or UKismetSystemLibrary.IsStandalone(uCharacter)) then
      ...
      elseif self.DetachMethod == EPutDownDetachMethod.Skill_Back then
        local uPassWallCheckLocation = uLocation + uForwardVector * -58 + uRightVector * -10 + FVector(0, 0, HeightDelta + 14)
        local uNewLocation = uLocation + uForwardVector * -58 + uRightVector * -10 + FVector(0, 0, HeightDelta)
...
function CharacterCarryBackComponent:OnHandleCameraModeChanged(CameraMode)
  ...
  local ENetRole = import("ENetRole")
  if uOwnerPawn.Role ~= ENetRole.ROLE_AutonomousProxy and not self.bIsStandAlone and not uOwnerPawn:IsLocalViewed() then
    return
  end
  if slua.isValid(self.BeCarriedBackCharacter) then
...
function CharacterCarryBackComponent:OnPerspectiveChanged(bFPP)
  ...
  local ENetRole = import("ENetRole")
  if uOwnerPawn.Role ~= ENetRole.ROLE_AutonomousProxy and not self.bIsStandAlone then
    return
  end
...
function CharacterCarryBackComponent:LocalExitBeCarriedToVehicle(DetachMethod)
  ...
  if Client or uOwnerPawn.Role ~= ENetRole.ROLE_Authority then
  ...
  if (uAttachedCharacter.Role == ENetRole.ROLE_AutonomousProxy or UKismetSystemLibrary.IsStandalone(uAttachedCharacter)) and slua.isValid(uCurPlayerController) and hasInVehicle then
