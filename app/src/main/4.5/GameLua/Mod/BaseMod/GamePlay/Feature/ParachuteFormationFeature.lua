...
function ParachuteFormationFeature:IsVehicleParachuteEnabled()
  ...
  if not slua.isValid(CGameMode) or not slua.isValid(CGameMode.TeamModeComponent) then
    local VehicleInfo = uPC.GetParachutingVehicleInfo and uPC:GetParachutingVehicleInfo() or nil
...
function ParachuteFormationFeature:ExecuteTacticalSpread()
  ...
  self:ReportTacticalSpreadClickTlog()
end
function ParachuteFormationFeature:ReportTacticalSpreadClickTlog()
  if not self:HasAuthority() then
    return
  end
  local DSCommonTLogSubsystem = SubsystemMgr:Get("DSCommonTLogSubsystem")
  if not DSCommonTLogSubsystem then
    print(bWriteLog and "ParachuteFormationFeature:ReportTacticalSpreadClickTlog - DSCommonTLogSubsystem invalid")
    return
  end
  DSCommonTLogSubsystem:AddCommonTLog(FormationConfig.TacticalSpreadClickTlogID, 1, false)
  print(bWriteLog and string.format("ParachuteFormationFeature:ReportTacticalSpreadClickTlog - AddCommonTLog ID=%d, +1", FormationConfig.TacticalSpreadClickTlogID))
end
