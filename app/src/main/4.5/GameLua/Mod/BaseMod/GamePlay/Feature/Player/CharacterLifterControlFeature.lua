...
function CharacterLifterControlFeature:ReceiveEndPlay(EndPlayReason)
  ...
  self:TryRemoveNamedGameTimer("FixSpectatorCameraModeTimer")
...
function CharacterLifterControlFeature:EnterLifter(Lifter)
  ...
  if uPlayerState.PhotoGrapherFeature and uPlayerState.PhotoGrapherFeature.bPhotoGrapherOpenState then
    print(bWriteLog and "CharacterLifterControlFeature:EnterLifter bPhotoGrapherOpenState TRUE, return")
    return
  end
...
function CharacterLifterControlFeature:ForceTPP(IsEnter)
  ...
  if IsFPPGameMode then
    ...
    elseif self.Owner:IsLocalViewed() and not self.Owner:IsLocallyControlled() then
      self:ForceFixSpectatorCameraMode(IsEnter)
    end
  elseif Client then
...
function CharacterLifterControlFeature:ForceFixSpectatorCameraMode(IsEnter)
  print(bWriteLog and string.format("CharacterLifterControlFeature:ForceFixSpectatorCameraMode IsEnter = %s", IsEnter))
  self:TryRemoveNamedGameTimer("FixSpectatorCameraModeTimer")
  if IsEnter then
    self.FixSpectatorCameraModeTimer = self:AddGameTimer(0.1, false, function()
      ...
      if uPlayerController.CurCameraMode ~= EPlayerCameraMode.PCM_Normal then
        print(bWriteLog and string.format("CharacterLifterControlFeature:ForceFixSpectatorCameraMode force switch to PCM_Normal"))
        uPlayerController:SwitchCameraMode(EPlayerCameraMode.PCM_Normal, uCharacter, false, true)
      end
    end)
  end
end
