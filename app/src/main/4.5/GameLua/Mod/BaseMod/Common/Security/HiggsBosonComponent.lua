...
local HiggsBosonSkipRules = require("GameLua.Mod.BaseMod.Common.Security.HiggsBosonSkipRules")
...
ClientVersion=%s]], sMessage, tostring(ToolReportUtil:GetPackageInfo()))
  ...
  function CHiggsBosonComponent._ReportChatRobot(sMessage, uHiggsBosonComponent)
    ...
    do
      local bRuleMatched, sHitRuleName = HiggsBosonSkipRules.IsMessageMatched(sMessage)
      if bRuleMatched then
        bSkipUploadNoschat = true
        FormatLog("hit skip rule: %s", tostring(sHitRuleName))
      end
    end
...
function CHiggsBosonComponent:GetBandaCountOffset()
  if not slua.isValid(self.CharacterOwner) then
    return 0
  end
  local JumpMaxCount = 1
  if 0 < Game:GetItemNumByResID(self.CharacterOwner, 4181010) then
    JumpMaxCount = 2
  end
  local nBandaCountOffset = 0
  if JumpMaxCount and 0 < JumpMaxCount then
    nBandaCountOffset = JumpMaxCount - 1
  end
  print(bWriteLog and string.format("[CHiggsBosonComponent:GetBandaCountOffset] CharacterOwner[%d][%s] CharacterOwner.JumpMaxCount[%d] JumpMaxCount[%d], nBandaCountOffset[%d]", self.CharacterOwner.PlayerKey, self.CharacterOwner.PlayerName, self.CharacterOwner.JumpMaxCount, JumpMaxCount, nBandaCountOffset))
  return nBandaCountOffset
end
