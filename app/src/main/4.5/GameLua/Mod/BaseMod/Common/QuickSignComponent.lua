...
function QuickSignComponent:ReceiveBeginPlay()
  ...
  if Client then
    ...
    self:AddSettingOptionEvent("bCanIntelligentSign", function(SettingValue)
      self:RefreshIntelligentSign(SettingValue)
    end, true)
  end
...
function QuickSignComponent:RefreshIntelligentSign(SettingValue)
...
function QuickSignComponent:ShowMsgTipsVoice(MsgItem, IsSelf, PlayerKey)
  ...
  print(bWriteLog and string.format("QuickSignComponent:ShowMsgTipsVoice - ActorID: %s, TextID: %s, AudioID: %s", tostring(ActorID), tostring(TextID), tostring(AudioID)))
