...
function VoiceTransformSubsystem:GetActorBankByID(ActorID)
  ...
  if BankName ~= "" then
    ...
    local bIsActorIDCache = self.ActorTableCache[ActorID]
    print(bWriteLog and string.format("VoiceTransformSubsystem:GetActorBankByID - ActorID: %s, IsActorIDCache: %s, BankName is nil", tostring(ActorID), tostring(bIsActorIDCache)))
  end
  return BankName
end
