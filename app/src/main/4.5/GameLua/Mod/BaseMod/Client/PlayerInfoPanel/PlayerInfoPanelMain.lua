...
function PlayerInfoPanelMain:_StartTick()
  if self.TickTimerHandle then
    return
  end
  ...
  self.TickTimerHandle = self:AddGameTimer(0.1, true, function()
    ...
    if not self:_HasPendingTickWork() then
      self:_StopTick()
    end
  end)
end
function PlayerInfoPanelMain:_StopTick()
  if self.TickTimerHandle then
    self:RemoveGameTimer(self.TickTimerHandle)
    self.TickTimerHandle = nil
  end
end
function PlayerInfoPanelMain:_HasPendingTickWork()
  ...
  if self.bShowEnergyAddition or self.bAnimationIsPlaying then
    return true
  end
...
function PlayerInfoPanelMain:PlayerInfo_UpdatePredictEnergy()
  ...
  if self.bShowEnergyAddition then
    self:_StartTick()
  end
end
