...
function VoiceChatSubsystem:_PostConstruct()
  ...
  self._TLog_MicphoneIns = nil
  self._TLog_SpeakerIndex = 0
  self._TLog_SpeakerTimeStamp = 0.0
  self._SetAllVoiceTLogTimer = nil
end
...
function VoiceChatSubsystem:SwitchMicMode(MicMode)
  print(bWriteLog and "VoiceChatSubsystem:SwitchMicMode ", MicMode)
  local ClientBanLogic = require("GameLua.Mod.BaseMod.Client.Ban.ClientBanLogic")
  if ClientBanLogic.IsBanned() then
    print(bWriteLog and "VoiceChatSubsystem:SwitchMicMode Banned")
    ClientBanLogic.PostBannedNotif()
    MicMode = EMicMode.OFF
  end
...
function VoiceChatSubsystem:OnPreTeamChatChange(_, _, PlayerID)
  ...
  for key, TeammateState in pairs(TeammatePlayerStateList) do
    if slua.isValid(TeammateState) and (TeammateState.PlayerId == PlayerID or bSelf) then
      local TeammatePreTeamID = TeammateState.PreTeamID
      local TeammatePlayerId = TeammateState.PlayerId
      ...
      if TeammatePreTeamID ~= MyPreTeamID then
        ...
        if MyPreTeamChatState == 1 or MyPreTeamChatState == 3 or 2 <= TeammateChatState then
          uAntsVoiceInterface:SetPlayerVolume(TeammateOpenID, 0)
          self.WannaShowVolume[TeammatePlayerId] = self.LastVolumn[TeammatePlayerId] or 100
        else
          local Volume = self.LastVolumn[TeammatePlayerId] or 100
          uAntsVoiceInterface:SetPlayerVolume(TeammateOpenID, Volume)
          self.WannaShowVolume[TeammatePlayerId] = nil
        end
      end
    end
  end
...
function VoiceChatSubsystem:OnRelease()
  ...
  if self._SetAllVoiceTLogTimer then
    self:RemoveGameTimer(self._SetAllVoiceTLogTimer)
    self._SetAllVoiceTLogTimer = nil
  end
  self._TLog_MicphoneIns = nil
end
function VoiceChatSubsystem:InitReceiverStatus()
  ...
  if ReceiverSetting == 0 then
    logic_antsvoice_interface:OpenAllSpeaker()
  elseif ReceiverSetting == 1 then
...
function VoiceChatSubsystem.GetCurrentLang()
  local GameplayData = require("GameLua.GameCore.Data.GameplayData")
  local PlayerController = GameplayData.GetPlayerController()
  if slua.isValid(PlayerController) then
    local ChatComponent = PlayerController:GetChatComponent()
...
function VoiceChatSubsystem.IsSTTSRegion()
  ...
  return region == "AE" or region == "KW" or region == "QA" or region == "SA" or region == "EG" or region == "IQ"
end
...
function VoiceChatSubsystem:StartSpeakerTLogTimer()
  if self._SetAllVoiceTLogTimer then
    return
  end
  self._SetAllVoiceTLogTimer = self:AddGameTimer(30.0, true, function()
    self:SetAllVoiceTLog()
  end)
end
function VoiceChatSubsystem:RecordMicphoneTlogToServer()
  local PlayerController = GameplayData.GetPlayerController()
  ...
  if not slua.isValid(self._TLog_MicphoneIns) then
    local TLog_Micphone = import("TLog_Micphone")
    self._TLog_MicphoneIns = TLog_Micphone()
  end
  self._TLog_MicphoneIns.TeammateSpeakerTime = PlayerState.TeammateSpeakerTime and PlayerState.TeammateSpeakerTime or self._TLog_MicphoneIns.TeammateSpeakerTime
  self._TLog_MicphoneIns.EnemySpeakerTime = PlayerState.EnemySpeakerTime and PlayerState.EnemySpeakerTime or self._TLog_MicphoneIns.EnemySpeakerTime
  if PlayerController.RPC_Server_SetMicphoneTLogToServer then
    PlayerController:RPC_Server_SetMicphoneTLogToServer(self._TLog_MicphoneIns)
  end
end
function VoiceChatSubsystem:SetVoiceTLogSpeaker(SetIndex)
  local GameState = GameplayData.GetGameState()
  if not slua.isValid(GameState) then
    return
  end
  local ServerWorldTimeSeconds = GameState:GetServerWorldTimeSeconds()
  local ThisDeltaTime = ServerWorldTimeSeconds - self._TLog_SpeakerTimeStamp
  local PlayerState = GameplayData.GetPlayerState()
  if slua.isValid(PlayerState) and SetIndex ~= self._TLog_SpeakerIndex then
    if self._TLog_SpeakerIndex == 1 and PlayerState.TeammateSpeakerTime then
      PlayerState.TeammateSpeakerTime = ThisDeltaTime + PlayerState.TeammateSpeakerTime
      self:RecordMicphoneTlogToServer()
    elseif self._TLog_SpeakerIndex == 2 and PlayerState.EnemySpeakerTime then
      PlayerState.EnemySpeakerTime = ThisDeltaTime + PlayerState.EnemySpeakerTime
      self:RecordMicphoneTlogToServer()
    end
    self._TLog_SpeakerTimeStamp = ServerWorldTimeSeconds
    self._TLog_SpeakerIndex = SetIndex
  end
end
function VoiceChatSubsystem:SetAllVoiceTLog()
  local GameState = GameplayData.GetGameState()
  if not slua.isValid(GameState) then
    return
  end
  local ServerWorldTimeSeconds = GameState:GetServerWorldTimeSeconds()
  local SpeakerDeltaTime = ServerWorldTimeSeconds - self._TLog_SpeakerTimeStamp
  local PlayerState = GameplayData.GetPlayerState()
