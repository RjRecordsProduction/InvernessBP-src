...
function QuickMenu:OnInitialize()
  ...
  self.ReuseListMultiSize_History = self:InitReuseListMultiSize(self.UIRoot.ReuseListMultiSize_History, "GameLua.Mod.BaseMod.Client.ChatHistoryItem")
  self.ReuseListMultiSize_History:SetStickToEnd(true)
end
...
function QuickMenu:RegistSetting()
  ...
  local SpectatingPetVisible = SettingModule:GetOptionValue("bSpectatingPetVisible")
  ...
  self:AddSettingOptionEvent("bSpectatingPetVisible", function(bSpectatingPetVisible)
...
function QuickMenu:HandleOnAddOneMark()
  ...
  self:AddChatToHistory(ChatComponent.MarkText, ChatComponent.IsMe, QuickSignComponent.currMsg, ChatComponent.CurrMsg)
end
function QuickMenu:SetPetSpectatingSetting()
  local SettingModule = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.SettingModule)
  local SpectatingPetVisible = SettingModule:GetOptionValue("bSpectatingPetVisible")
  SettingModule:SetOptionValue("bSpectatingPetVisible", not SpectatingPetVisible)
end
...
function QuickMenu:OnAddOneMsgtoUIInner()
  ...
  self:AddChatToHistory(ChatComponent.addToUIText, ChatComponent.IsMe, QuickSignComponent.currMsg, ChatComponent.CurrMsg)
end
...
function QuickMenu:AddChatToHistory(Text, bIsMe, QuickMsg, CurrMsg)
  self.UIRoot.ScrollBoxHistory:SetWidgetVisibility(UEnums.ESlateVisibility.Collapsed)
  self.ReuseListMultiSize_History:AppendItem({
    Text = Text,
    bIsMe = bIsMe,
    QuickMsg = QuickMsg:clone(),
    CurrMsg = CurrMsg:clone(),
    msgType = 3
  })
  self.ReuseListMultiSize_History:ScrollToEnd()
  self.ReuseListMultiSize_History:InvalidateLayoutCache()
end
