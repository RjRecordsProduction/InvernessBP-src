...
function LevelStreamingMgr:EnableClientViewPointLevelHold(bEnable, uPoint)
  local uWorld = slua_GameFrontendHUD:GetWorld()
  if not slua.isValid(uWorld) then
    return
  end
  print(bWriteLog and "LevelStreamingMgr EnableClientViewPointLevelHold: " .. tostring(bEnable))
  local uWorldComposition = uWorld.WorldComposition
  if uWorldComposition then
    if bEnable and uPoint then
      uWorldComposition.bClientUseInitViewpoint = true
      uWorldComposition.ClientInitViewPoint = uPoint
    else
      uWorldComposition.bClientUseInitViewpoint = false
    end
  end
end
