...
function BattlePopBottomKillTips:GetCauserPlayerName(messageData)
  ...
  elseif messageData.bIamCauser then
  elseif tonumber(causerName) then
    local loc = math.tointeger(tonumber(causerName))
    if loc then
      local localResCfg = CDataTable.GetTableData("LocalizeRes", loc)
      if localResCfg then
        causerName = LocUtil.GetLocalizeResStr(loc)
      end
    end
  end
...
function BattlePopBottomKillTips:SetAdditionalWeaponIcon(Type, AdditionalParam)
  ...
  if path and path ~= "" then
    ...
    if ModKillInfoCfg then
      if ModKillInfoCfg.FatalDamageCharacterType2WeaponIconMap and ModKillInfoCfg.FatalDamageCharacterType2WeaponIconMap[Type] then
        print(bWriteLog and string.format("BattlePopBottomKillTips:SetAdditionalWeaponIcon ModCfg CauserType:%s,path:%s", tostring(Type), tostring(ModKillInfoCfg.FatalDamageCharacterType2WeaponIconMap[Type])))
        path = ModKillInfoCfg.FatalDamageCharacterType2WeaponIconMap[Type]
        bModMatched = true
      elseif AdditionalParam and ModKillInfoCfg.FatalDamageMonsterID2WeaponIconMap and ModKillInfoCfg.FatalDamageMonsterID2WeaponIconMap[AdditionalParam] then
        print(bWriteLog and string.format("BattlePopBottomKillTips:SetAdditionalWeaponIcon ModCfg AdditionalParam:%s,path:%s", tostring(AdditionalParam), tostring(ModKillInfoCfg.FatalDamageMonsterID2WeaponIconMap[AdditionalParam])))
        path = ModKillInfoCfg.FatalDamageMonsterID2WeaponIconMap[AdditionalParam]
        bModMatched = true
      end
    end
    if not bModMatched and AdditionalParam and KillInfoCfg.FatalDamageMonsterID2WeaponIconMap[AdditionalParam] then
