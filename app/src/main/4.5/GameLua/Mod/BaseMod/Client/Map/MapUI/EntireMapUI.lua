...
function EntireMapUI:SetReshowAirplaneRouteBtnVisibility(bIsShow)
  ...
  if bIsShow then
    ...
    bIsOpen = SettingModule:GetOptionValue("ReshowAirlineRouteBtnChecked")
    if bIsOpen then
    ...
    if MapIconSubsystem and not MapIconSubsystem.bBaltic then
      ...
      SuperData.bAirlineRouteShow = false
...
function EntireMapUI:OnAirlineRouteShow()
  ...
  local SuperData = self:GetSuperData()
  SuperData.bAirlineRouteShow = true
end
...
function EntireMapUI:SaveReshowRouteSetting()
  ...
  local SettingModule = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.SettingModule)
  if not self.HoldWidget then
    return
  end
  local SettingValue = SettingModule:GetOptionValue("ReshowAirlineRouteBtnChecked")
  ...
  if SettingValue == nil or SettingValue ~= bIsReshowBtnChecked then
    ...
    SettingModule:SetOptionValue("ReshowAirlineRouteBtnChecked", bIsReshowBtnChecked)
  end
...
function EntireMapUI:SetReshowRouteEffectVisibility()
  ...
  if not self.HightLightReshowAirLineTimes then
    local SettingModule = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.SettingModule)
    self.HightLightReshowAirLineTimes = SettingModule:GetOptionValue("HightLightReshowAirLineTimes")
  end
...
function EntireMapUI:SaveHightLightTimes()
  ...
  local SettingModule = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.SettingModule)
  SettingModule:SetOptionValue("HightLightReshowAirLineTimes", self.HightLightReshowAirLineTimes)
end
...
function EntireMapUI:SetAdaptation()
  ...
  self.AdaptationBottomOffset = Margin.Bottom
...
function EntireMapUI:RepositionAllMapMark(Param)
  local TeamMates = MapUIUtils.GetTeamMateListFromPlayerState(false)
...
function EntireMapUI:ShowEntiremap()
  ...
  elseif self.HoldEntireMapWidget.CanvasPanel_AutoLock:GetVisibility() == UEnums.ESlateVisibility.SelfHitTestInvisible and MapUIUtils.GetCurAreaID() <= 0 and self.bAutoLock == true then
    self.HoldEntireMapWidget.WidgetSwitcher_AutoLock:SetActiveWidgetIndex(1)
    self:HandleAutoLockBtnClick()
  end
end
