...
function GameGuideUIUtil.GetTabNameByItemID(nItemID)
  if not nItemID then
    return nil
  end
  local GameGuideConfig = GameGuideUIUtil.GetGameGuideConfig()
  ...
  for _, Tabs in pairs(GameGuideConfig) do
    local TabsConfig = Tabs.TabsConfig or {}
    for _, Tab in pairs(TabsConfig) do
      if Tab.nItemID and Tab.nItemID == nItemID then
        return Tab.TabName
      end
    end
  end
...
function GameGuideUIUtil.GetGameGuideUIMainConfig()
  local GameGuideUIConfigSubsystem = SubsystemMgr:Get("GameGuideUIConfigSubsystem")
  if GameGuideUIConfigSubsystem then
    return GameGuideUIConfigSubsystem:GetGameGuideUIMainConfig()
  end
...
function GameGuideUIUtil.OpenGuideTargetTab(TargetTabName)
  ...
  if slua.isValid(MainControlBaseUI) then
    ...
    if EntireMapUI then
      ...
      local GameGuideUIConfig = GameGuideUIUtil.GetGameGuideUIMainConfig()
      local GameGuideUIMain = UIManager.GetUI(GameGuideUIConfig)
