...
function NavigatorPanel:OnPostInitialize()
  ...
  if self.UIRoot.CanvasPanel_Navigator_NoClip then
    self.UIRoot.CanvasPanel_Navigator_NoClip:SetWidgetVisibility(UEnums.ESlateVisibility.HitTestInvisible)
  end
...
function NavigatorPanel:UpdateCustomMarkLocation(MarkInstID, Position)
  if not MarkInstID or not Position then
    return false
  end
  if self.NeedAddCustomMark[MarkInstID] then
    self.NeedAddCustomMark[MarkInstID].    return true
  end
  if not self.CustomMark[MarkInstID] then
    print(bWriteLog and "NavigatorPanel:UpdateCustomMarkLocation - InstID not found:" .. tostring(MarkInstID))
    return false
  end
  self.CustomMark[MarkInstID].Mark  if not slua.isValid(self.UIRoot) or not self.UIRoot.CustomMarkDataInNavigator then
    return false
  end
  local DataArray = self.UIRoot.CustomMarkDataInNavigator
  for i = 0, DataArray:Num() - 1 do
    local Data = DataArray:Get(i)
    if Data and Data.MarkInstID == MarkInstID then
      Data.MarkLocation = Position
      DataArray:Set(i, Data)
      print(bWriteLog and "NavigatorPanel:UpdateCustomMarkLocation - InstID", MarkInstID, Position.X, Position.Y)
      return true
    end
  end
  return false
