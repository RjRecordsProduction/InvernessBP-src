...
function MiniMapWindow:CheckDisableInvalidationBoxes()
  if HDmpveRemote.HDmpveRemoteConfigGetBool("DisableInvalidationBox", false) == true then
    log_shipping_client("MiniMapWindow:CheckDisableInvalidationBoxes Disable Invalidation Boxes")
    if self.UIRoot.InvalidationBox_MarkSystem then
      self.UIRoot.InvalidationBox_MarkSystem:SetCanCache(false)
    end
  end
end
