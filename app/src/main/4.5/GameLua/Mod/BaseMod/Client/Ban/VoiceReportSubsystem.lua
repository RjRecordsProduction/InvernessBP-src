...
function VoiceReportSubsystem:ShowBlueToothNotice()
  ...
  if SettingModule:GetOptionValue("bHasShowBlueToothNotice") == true then
    return
  end
  SettingModule:SetOptionValue("bHasShowBlueToothNotice", true)
  local IngameTipsTools = require("GameLua.Mod.BaseMod.Common.UI.InGameTipsTools")
...
function VoiceReportSubsystem:OnGlobalMicReport()
  ...
  if ReceiverSetting == 0 and logic_antsvoice_interface:LbsSpeakerEnable() then
    ...
    SettingModule:SetOptionValue("ReceiverSetting", 1)
  end
end
