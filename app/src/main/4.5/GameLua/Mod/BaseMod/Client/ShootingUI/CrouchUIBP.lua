...
function CrouchUIBP:HandleCrouchVisibility()
  ...
  self:AddSettingOptionEvent("OneKeyProneAndCrouchSwitch", RefreshCrouchVisibility, true)
end
...
function CrouchUIBP:OnCrouchBtnPressed(MyGeometry, MouseEvent)
  ...
  EventSystem:postEvent(EVENTTYPE_INGAME_CREATIVE_MODE, EVENTID_UGC_NATIVE_BUTTON_PRESSED, "Crouch")
...
function CrouchUIBP:OnCrouchBtnReleased()
  ...
  EventSystem:postEvent(EVENTTYPE_INGAME_CREATIVE_MODE, EVENTID_UGC_NATIVE_BUTTON_RELEASED, "Crouch")
...
function CrouchUIBP:CheckFov()
  ...
  local SettingModule = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.SettingModule)
  local DefaultFov = SettingModule:GetOptionValue("TpViewValue")
  local FixedOffet = 5
