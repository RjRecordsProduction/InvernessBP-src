...
function BackPackItemUI:ctor(_, BackPackUI, Type)
  self.  self.ItemBP  local ItemUIConfig = GamePlayTools.GetCurrentConfig("ItemUIConfig")
  if ItemUIConfig then
    self.ListItemType = ItemUIConfig.ItemType.BackpackType
  end
end
...
function BackPackItemUI:InnerUpdateItemData(ItemTableData)
  ...
  self:UpdateGuideNewIcon(ItemTableData.ItemID)
end
...
function BackPackItemUI:UpdateGuideNewIcon(nItemID)
...
function BackPackItemUI:ShowHandleBtns()
  ...
  self:CheckUpgradeItemUseless()
  if self.ButtonMenu and self.ButtonMenu.UpdateGuideButtonState then
    local DefineID = slua.IndexReference(self.ItemData, "DefineID")
    self.ButtonMenu:UpdateGuideButtonState(DefineID.TypeSpecificID)
  end
