...
function FightingStateStateMachine:Enter()
  ...
  GameplayData.AddSelfPlayerCharacterEvent(self, "MovementModeChangedDelegate", self.OnMovementModeChanged, self)
...
function FightingStateStateMachine:OnMovementModeChanged()
  self:OnFightingStateChange()
end
...
function FightingStateStateMachine:CanGotoSwimState()
  ...
  local CharacterMovement = PlayerCharacter.STCharacterMovement
  if slua.isValid(CharacterMovement) then
    local EMovementMode = import("EMovementMode")
    if CharacterMovement.MovementMode == EMovementMode.MOVE_Swimming then
      return true
    end
  end
  return false
