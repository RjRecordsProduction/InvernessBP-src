...
function BattleResultUtil:GetClothSchemeList(result, UID)
  local clothSchemeList
  if result.gold_dress_personal_equipment_info and result.gold_dress_personal_equipment_info[UID] then
    clothSchemeList = {}
    for period, v in pairs(result.gold_dress_personal_equipment_info[UID]) do
      if v then
        for branch, vv in pairs(v) do
          if vv then
            for feature, schemeId in pairs(vv) do
              if schemeId then
                table.insert(clothSchemeList, schemeId)
              end
            end
          end
        end
