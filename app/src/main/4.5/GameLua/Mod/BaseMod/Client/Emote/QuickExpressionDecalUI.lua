...
function QuickExpressionDecalUI:OnClose()
  ...
  if self.StateInterruptedDelegate then
    local PlayerCharacter = GameplayData.GetPlayerCharacter()
    if slua.isValid(PlayerCharacter) and PlayerCharacter.StateInterruptedHandlerBP then
      PlayerCharacter.StateInterruptedHandlerBP:Remove(self.StateInterruptedDelegate)
    end
...
function QuickExpressionDecalUI:OnInitialize()
  ...
  self:AddSettingOptionEvent("QuickEmoteDecalSwitch", function(QuickEmoteDecalSwitch)
    print(bWriteLog and "QuickExpressionDecalUI:OnInitialize:QuickEmoteDecalSwitch", QuickEmoteDecalSwitch)
    self._bUseJoyStickMode = QuickEmoteDecalSwitch == 2
    if QuickEmoteDecalSwitch == 2 then
      self:CloseCommonPart()
      self.UIRoot.WidgetSwitcher_Button:SetActiveWidgetIndex(1)
      self:ShowWheelJoyStick()
    else
      self.UIRoot.WidgetSwitcher_Button:SetActiveWidgetIndex(0)
    end
  end, true)
  self:ShowWheelJoyStick()
end
...
function QuickExpressionDecalUI:ShowWheelJoyStick()
  if self.SprayPaintUI then
    return
  end
  self.SprayPaintUI = self:CreateChildWindow(self.UIRoot.CanvasPanel_JoyStick, UIManager.UI_Config_InGame.SprayPaintUI)
end
...
function QuickExpressionDecalUI:OpenSubPanel()
  print(bWriteLog and "QuickExpressionDecalUI:OpenSubPanel")
  ...
  if self._bUseJoyStickMode and slua.isValid(self.UIRoot.WidgetSwitcher_Button) then
    self.UIRoot.WidgetSwitcher_Button:SetActiveWidgetIndex(0)
  end
  self:OpenCommonPart()
end
