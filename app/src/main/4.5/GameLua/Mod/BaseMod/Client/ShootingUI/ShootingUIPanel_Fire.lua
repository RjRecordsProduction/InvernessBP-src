...
function ShootingUIPanelIMP:RegistEvents_Fire()
  ...
  self:AddControlEventByControl(self.UIRoot.OnFireBtn_ReleaseBtn, "OnReleased", self.OnNativeReleasedRightFireBtn, self, true)
  self:AddControlEventByControl(self.UIRoot.OnFireBtn_LReleaseBtn, "OnReleased", self.OnNativeReleasedLeftFireBtn, self, false)
  ...
  self:AddDataListener(SuperData, "CharacterDataReady", function()
    ...
    self:BindLuaObjEvent(PlayerController, "DoUITouchMoveEvent", self.OnDoUITouchMove, self)
  end)
  self:AddSettingOptionEvent("ShotGunShootMode", function(ShotGunShootMode)
    self.ShotGunReleaseFireType = ShotGunShootMode - 1
  end, true)
  self:AddSettingOptionEvent("SingleShotWeaponShootMode", function(SingleShotWeaponShootMode)
    self.SniperReleaseFireType = SingleShotWeaponShootMode - 1
  end, true)
end
...
function ShootingUIPanelIMP:OnDoUITouchMove(Vec2D)
  local Size = self.UIRoot.AttackBtnPanel_Rside.Slot:GetSize()
  local UIUtil = require("client.common.ui_util")
  local ScreenPadding = UIUtil.GetScreenPadding()
  local ViewSizeByScale = UIUtil.GetViewportSizebyScale()
  local Result = ViewSizeByScale - Vec2D + Size * 0.5
  self.UIRoot.AttackBtnPanel_Rside.Slot:SetPosition(FVector2D(ScreenPadding.Left - Result.X, ScreenPadding.Bottom - Result.Y))
end
...
function ShootingUIPanelIMP:OnPressFireBtn(FingerIndex, TouchForce, FireType, bIsRightSide)
  ...
  EventSystem:postEvent(EVENTTYPE_INGAME_CREATIVE_MODE, EVENTID_UGC_NATIVE_BUTTON_PRESSED, "Attack")
...
function ShootingUIPanelIMP:OnNativeReleasedLeftFireBtn()
  print(bWriteLog and "ShootingUIPanelUIBase:OnNativeReleasedLeftFireBtn")
  EventSystem:postEvent(EVENTTYPE_INGAME_CREATIVE_MODE, EVENTID_UGC_NATIVE_BUTTON_RELEASED, "Attack")
  self:OnReleasedFire(false)
end
function ShootingUIPanelIMP:OnNativeReleasedRightFireBtn()
  print(bWriteLog and "ShootingUIPanelUIBase:OnNativeReleasedRightFireBtn")
  EventSystem:postEvent(EVENTTYPE_INGAME_CREATIVE_MODE, EVENTID_UGC_NATIVE_BUTTON_RELEASED, "Attack")
  self:OnReleasedFire(true)
end
