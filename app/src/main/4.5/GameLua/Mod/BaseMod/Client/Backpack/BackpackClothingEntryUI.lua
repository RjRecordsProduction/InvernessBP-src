...
function BackpackClothingEntryUI:RegistEvents()
  ...
  self:AddCommonEvent(EVENTTYPE_INGAME_UI, EVENTID_QUICK_TWEAK_LAYOUT_STATE, function(_, __, bQuickTweakLayout)
    local bShow = not bQuickTweakLayout and self:IsWardrobeAvailable()
...
function BackpackClothingEntryUI:OnGameStateChange()
  local bShow = self:IsWardrobeAvailable()
  self:SetRolewearVisible(bShow)
end
function BackpackClothingEntryUI:IsWardrobeAvailable()
