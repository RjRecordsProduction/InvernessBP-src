...
function ConfigDrivePlaneShowSubsystem:GetRoleCacheData(PlayerState)
  ...
  if slua.isValid(UCharacterAvatarComponent) then
    ...
    local SchemeIDList = {}
    if UCharacterAvatarComponent.ClothSchemeIDMap then
      for SchemeID in pairs(UCharacterAvatarComponent.ClothSchemeIDMap) do
        if SchemeID and 0 < SchemeID then
          table.insert(SchemeIDList, SchemeID)
        end
      end
    end
    CacheData.Cloth    if RealPawn.PMGCPlayerCharacterFeature and RealPawn.PMGCPlayerCharacterFeature.CupReadyState and RealPawn.PMGCPlayerCharacterFeature.CupReadyState > 10000 then
      CacheData.SupportTeamID = RealPawn.PMGCPlayerCharacterFeature.CupReadyState
    end
    CacheData.NicknameColor = PlayerState.NicknameColor
    local NicknameColorManager = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.NicknameColorManager)
    NicknameColorManager:SetUserData(CacheData.UID, PlayerState.NicknameColor)
    return CacheData
  end
  return nil
end
