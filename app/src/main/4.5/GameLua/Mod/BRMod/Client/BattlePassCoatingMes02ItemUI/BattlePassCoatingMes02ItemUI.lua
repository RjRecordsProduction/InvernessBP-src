...
function BattlePassCoatingMes02ItemUI:LuaGetData()
  ...
  if slua.isValid(GameState) then
    ...
    for i = 0, self.ShowDataNum - 1 do
      ...
      self:_ApplyNicknameColor(self.BattlePassItemList:Get(i), self.UpassShowInfoList:Get(i))
...
function BattlePassCoatingMes02ItemUI:SetWingManInfo(number, Info, XSuitIconId, PlayerUID)
  ...
  if number == EWingMan.Card1 then
    if slua.isValid(self.BattlePass_Coating_Wingman_Item_0) then
      ...
      self:_ApplyNicknameColor(self.BattlePass_Coating_Wingman_Item_0, Info, PlayerUID)
  ...
  elseif number == EWingMan.Card2 and slua.isValid(self.BattlePass_Coating_Wingman_Item_1) then
    ...
    self:_ApplyNicknameColor(self.BattlePass_Coating_Wingman_Item_1, Info, PlayerUID)
...
function BattlePassCoatingMes02ItemUI:_SetBattlePass_MainItemAvatar(UpassInfoData)
  ...
  self:_ApplyNicknameColor(self.BattlePass_MainItem, UpassInfoData)
...
function BattlePassCoatingMes02ItemUI:_ApplyNicknameColor(widget, UpassInfoData, OverridePlayerUID)
  if not slua.isValid(widget) or not widget.Name then
    return
  end
  local PlayerUID = OverridePlayerUID
  if not PlayerUID and UpassInfoData then
    PlayerUID = UpassInfoData.PlayerUID
  end
  ...
  if not NicknameColorManager then
    return
  end
  local darkColor = NicknameColorManager:GetColorByUID(PlayerUID)
  widget.Name:SetColorAndOpacity(darkColor)
end
