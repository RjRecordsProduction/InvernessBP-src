...
function MLAIProcessUtil:SpawnItemForAIByItemList(uAIController, AIBagItemList, bNeedDropAtBegin, bNeedCheckEquipment, bCheckAIEquipping)
  if Game:IsValid(uAIController) and Game:IsAIController(uAIController) then
    local uItemDropMgr = CGameMode.BP_ItemDropMgr
    if Game:IsValid(uItemDropMgr) then
      if bNeedDropAtBegin == nil or bNeedDropAtBegin == true then
        if bCheckAIEquipping then
          local bFrameBattleItemNotEmpty = uAIController.FrameBattleItemPickUpInfoList:Num() > 0
          local bFrameRequiredNotEmpty = 0 < uAIController.FrameRequiredPickUpList:Num()
          local bFrameOptionNotEmpty = 0 < uAIController.FrameOptionPickUpList:Num()
          local bFrameOptionBattleNotEmpty = 0 < uAIController.FrameOptionBattleItemPickUpInfoList:Num()
          if bFrameBattleItemNotEmpty or bFrameRequiredNotEmpty or bFrameOptionNotEmpty or bFrameOptionBattleNotEmpty then
            print(bWriteLog and string.format("[MLAIProcessUtil] SpawnItemForAIByItemList AI %s is equipping items, skip spawn. FrameBattleItem:%s, FrameRequired:%s, FrameOption:%s, FrameOptionBattle:%s", tostring(uAIController.PlayerKey), tostring(bFrameBattleItemNotEmpty), tostring(bFrameRequiredNotEmpty), tostring(bFrameOptionNotEmpty), tostring(bFrameOptionBattleNotEmpty)))
            return
          end
        end
        uAIController:ForceDropItemsWithTypeList({
...
function MLAIProcessUtil:GetRealPlayerNum()
  ...
  for _, uPawn in pairs(uPawns) do
    if Game:IsPlayer(uPawn) and not uPawn:ActorHasTag("NotAffectPlayerCount") then
...
function MLAIProcessUtil:GotoNextRound(DelayTime, fCallback)
  ...
  self.RestartTimerHandle = Game:SetTimer(DelayTime, false, function()
    if MLAIProcessSubSystem then
      ...
      if MLAIProcessSubSystem.GameWasStoped and CGameMode.MlAIType then
        ...
        print(bWriteLog and "MLAIProcessUtil:GotoNextRound, GameWasStoped or CGameMode.MlAIType is nil")
      end
    end
    self.RestartTimerHandle = nil
    if fCallback then
...
function MLAIProcessUtil:IsValidPVEAIType()
  if ServerDataMgr and ServerDataMgr.SyncGameParams and ServerDataMgr.SyncGameParams.pve_super_ai_type then
    local LocalPVESuperAIType = ServerDataMgr.SyncGameParams.pve_super_ai_type
    if LocalPVESuperAIType ~= 0 then
      return true
    end
  end
  return false
