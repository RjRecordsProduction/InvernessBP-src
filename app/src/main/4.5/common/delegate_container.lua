...
local EventProxy
local local bUseSingleton = false
...
function DelegateContainer:_PreDestruct()
  self:Dispose()
end
...
function DelegateContainer:_UnRegistEvents()
  ...
  if self._nativeLuaListener then
    for control, ControlListener in pairs(self._nativeLuaListener) do
      for eventName, ListenerInfo in pairs(ControlListener) do
        if type(control) == "table" or slua_isValid(control) then
          self:RemoveControlEventSingleton(control, eventName)
        else
          local DelegateLuaEvents
          if type(ListenerInfo) == "table" then
            DelegateLuaEvents = ListenerInfo.DelegateLuaEvents
          end
          if DelegateLuaEvents then
            DelegateLuaEvents[self] = nil
          end
        end
        ControlListener[eventName] = nil
      end
      self._nativeLuaListener[control] = nil
    end
    self._nativeLuaListener = nil
  end
...
function DelegateContainer:AddControlEventSingletonWithLuaObj(control, LuaObj, eventName, handleFunc, ...)
  ...
  local controlListeners = self._nativeLuaListener[control]
  if not controlListeners then
    controlListeners = {}
    self._nativeLuaListener[control] = controlListeners
  end
  controlListeners[eventName] = {HandleFunc = handleFunc, DelegateLuaEvents = DelegateLuaEvents}
  return true
end
function DelegateContainer:RemoveControlEventSingletonWithLuaObj(control, LuaObj, eventName)
  ...
  local controlListeners = self._nativeLuaListener[control] or self._nativeLuaListener[LuaObj]
  if controlListeners then
    local ListenerInfo = controlListeners[eventName]
    if type(ListenerInfo) == "table" then
      ListenerInfo.DelegateLuaEvents = nil
      ListenerInfo.HandleFunc = nil
    end
...
function DelegateContainer:BindLuaObjEvent(LuaObj, eventName, handleFunc, ...)
  ...
  if luaEventContainer then
    ...
    if not eventContainerSet then
      ...
      eventContainerSet = LuaObj._LuaEventContainerSet
      if eventContainerSet then
        local LuaTable = LuaObj._GetRawClass and LuaObj:_GetRawClass() or LuaObj
        self:_UpdateLuaEventContainerSet(LuaObj, LuaTable)
      end
    end
    if eventContainerSet and not eventContainerSet[eventName] then
      return false
    end
  end
...
function DelegateContainer:LuaBroadcastCommonEventCpp(EventType, EventID, ...)
  ...
  if self.LuaEventContainer then
    local eventContainerSet = self._LuaEventContainerSet
    if not eventContainerSet then
      self._LuaEventContainerSet = {}
      eventContainerSet = self._LuaEventContainerSet
      if eventContainerSet then
        local LuaTable = self:_GetRawClass()
        self:_UpdateLuaEventContainerSet(self, LuaTable)
      end
    end
    if eventContainerSet and eventContainerSet[EventID] then
...
function DelegateContainer:RemoveSettingOptionEvent(OptionName)
  if not assert(OptionName, "DelegateContainer:RemoveSettingOptionEvent param invalid") then
    return false
  end
  if not self._settingOptionListeners then
    return
