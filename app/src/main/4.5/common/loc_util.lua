...
function LocUtil.GetLocalizeResStrV2(id)
  return _GetLocalizeResStrInner(id)
end
function LocUtil.LocalizeResFormatV2(id, ...)
  return LocUtil.LocalizeResFormat(id, ...)
end
...
function LocUtil.ResolveText(text)
  if type(text) == "number" then
    return LocUtil.GetLocalizeResStr(text)
...
function LocUtil.FormatDecimalForLocale(num, decimalPlaces)
  ...
  local formatted = math.floor(n * factor + 0.5) / factor
  local str = tostring(formatted)
  local LanguageMacros = require("client.slua.config.ClientMacros.LanguageMacros")
  local CommaDecimalLanguages = {
    [LanguageMacros.TR] = true,
    [LanguageMacros.AR] = true,
    [LanguageMacros.UR] = true,
    [LanguageMacros.RU] = true
  }
  local language = Client.GetCurrentLanguage()
  if CommaDecimalLanguages[language] then
    str = string_gsub(str, "%.", ",")
  end
  return str
end
function LocUtil.FormatDecimalStrForLocale(str)
  local LanguageMacros = require("client.slua.config.ClientMacros.LanguageMacros")
  local CommaDecimalLanguages = {
    [LanguageMacros.TR] = true,
    [LanguageMacros.AR] = true,
  ...
  local language = Client.GetCurrentLanguage()
  if CommaDecimalLanguages[language] then
    return string_gsub(tostring(str or ""), "%.", ",")
  end
