...
function CustomAssetImageManager:ctor(_)
  ...
  self.LoadHandler = {}
  self.RegistedIconID = {}
  self.ImageResourceReadyMap = {}
  self.ImageResourceReadyWaiters = {}
  self.ImageResourceReadyWaiterSeq = 0
end
...
function CustomAssetImageManager:DynamicUnregisterConfig(AssetKey)
  ...
  self.ImageResourceReadyMap[AssetKey] = nil
  self.ImageResourceReadyWaiters[AssetKey] = nil
...
function CustomAssetImageManager:IsImageResourceReady(AssetKey)
  return AssetKey ~= nil and self.ImageResourceReadyMap[AssetKey] == true
end
function CustomAssetImageManager:WaitImageResourceReady(AssetKey, Callback)
  if AssetKey == nil or type(Callback) ~= "function" then
    return nil
  end
  if self:IsImageResourceReady(AssetKey) then
    Callback(AssetKey, self.AssetKey2HashID[AssetKey])
    return nil
  end
  self.ImageResourceReadyWaiterSeq = (self.ImageResourceReadyWaiterSeq or 0) + 1
  local WaiterId = self.ImageResourceReadyWaiterSeq
  local Waiters = self.ImageResourceReadyWaiters[AssetKey]
  if Waiters == nil then
    Waiters = {}
    self.ImageResourceReadyWaiters[AssetKey] = Waiters
  end
  Waiters[WaiterId] = Callback
  return WaiterId
end
function CustomAssetImageManager:RemoveImageResourceReadyWaiter(AssetKey, WaiterId)
  if AssetKey == nil or WaiterId == nil then
    return
  end
  local Waiters = self.ImageResourceReadyWaiters[AssetKey]
  if Waiters ~= nil then
...
function CustomAssetImageManager:NotifyImageResourceReady(AssetKey, HashID)
  if AssetKey == nil then
    return
  end
  self.ImageResourceReadyMap[AssetKey] = true
  local Waiters = self.ImageResourceReadyWaiters[AssetKey]
  if Waiters == nil then
    return
  end
  ...
  for _, Callback in pairs(Waiters) do
    pcall(Callback, AssetKey, HashID)
  end
end
function CustomAssetImageManager:UpdateTextureResource(AssetKey, CompressedTexture_PB)
  ...
  self:NotifyImageResourceReady(AssetKey, HashID)
end
...
function CustomAssetImageManager:TestEncode(CompressedFileName, AssetKey)
  ...
  if CustomAssetTextureUtil.PlatformUseRowBinary(CompressedTexture) then
