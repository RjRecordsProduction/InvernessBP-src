...
function CustomAssetTextureUtil.LuaUpdateTextureResource2(Texture, TexGroup, Width, Height, MipIndex, PixelFormat, ImageBinaryData)
  ...
  if UCustomAssetImageManager.IsPngImageData(ImageBinaryData) == true then
    if UCustomAssetImageManager.LuaUpdateTextureResource2Png then
      UCustomAssetImageManager.LuaUpdateTextureResource2Png(Texture, TexGroup, Width, Height, MipIndex, ImageBinaryData)
    end
  else
...
function CustomAssetTextureUtil.LuaUpdateTextureResourceByPB(Texture, CompressedTexture_PB)
  ...
  if CustomAssetTextureUtil.PlatformUseRowBinary(CompressedTexture_PB) then
    ...
    local Format = 0
    local MipIndex = 0
    if CustomAssetTextureUtil.LuaUpdateTextureResource2 then
...
function CustomAssetTextureUtil.PlatformUseRowBinary(ImageBinaryData)
  if CustomAssetTextureUtil.IsWindows() then
    return true
  end
  if ImageBinaryData == nil or type(ImageBinaryData) == "table" then
    return false
  end
  if type(UCustomAssetImageManager.IsPngImageData) ~= "function" then
    return false
  end
  return UCustomAssetImageManager.IsPngImageData(ImageBinaryData) == true
end
