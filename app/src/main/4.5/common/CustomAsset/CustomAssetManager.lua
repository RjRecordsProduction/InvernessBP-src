...
local CustomAssetDownloadCompletedInfo = {
  ...
  DownloadSeq = 0,
  bIsTemp = false,
  DownloadPriority = CustomAssetDownloadPriority.LOW,
  bSuffixFallbackTried = false,
  SuffixFallbackTriedMap = nil,
  Callback = nil
}
...
function CustomAssetManager:ctor()
  ...
  self.DsMountableSizeLimit = nil
  self.AutoMountSizeLimit = nil
  self.CallInMountedHandleAssetMap = {}
  self.BatchMountWaitMap = {}
  self.BatchAssetKeyMap = {}
  self.AllocPackagePathMap = {}
  self._DsAutoMountReplicateCompleteCallBackMap = {}
  self._CurDsAutoMountCallBackID = 0
  self.CustomAssetKeyListHashToAssetMap = {}
  self.CustomAssetKeyListAssetToHashMap = {}
end
...
function CustomAssetManager:OnFightingStatusEnter()
  ...
  if not self:IsDedicatedServer() then
    self:BuildCustomAssetKeyListMapping()
  end
  ...
  if not self:IsDedicatedServer() then
    self:AddCommonEvent(EVENTTYPE_UGC, EVENTID_UGC_PREFAB_MALL_FIRST_META_COMPLETE, self.OnPrefabMallDataUpdateCompleted, self)
  end
end
function CustomAssetManager:BuildCustomAssetKeyListMapping()
  self.CustomAssetKeyListHashToAssetMap = {}
  self.CustomAssetKeyListAssetToHashMap = {}
  local LogicUGC = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.LogicUGC)
  if LogicUGC and LogicUGC:IsUGCEditMod() then
    return
  end
  local CustomAssetKeyList
  local LogicUGCMatch = ModuleManager.GetModule(ModuleManager.LobbyModuleConfig.LogicUGCMatch)
  local modID = LogicUGCMatch and LogicUGCMatch:GetMatchModID() or 0
  if modID == 0 then
    print(bWriteLog and "CustomAssetManager:BuildCustomAssetKeyListMapping no match modID")
    return
  end
  print(bWriteLog and "CustomAssetManager:BuildCustomAssetKeyListMapping modID:" .. tostring(modID))
  local ModInfo = LogicUGC and LogicUGC:GetModByAllCache(modID) or nil
  ...
  if not CustomAssetKeyList then
    print(bWriteLog and "CustomAssetManager:BuildCustomAssetKeyListMapping no custom_asset_key_list")
    return
  end
  for _, AssetKey in ipairs(CustomAssetKeyList) do
    local HashId = self:GenerateCustomAssetKeyHashID(AssetKey)
    if HashId and 0 < HashId then
      self.CustomAssetKeyListHashToAssetMap[HashId] = AssetKey
      self.CustomAssetKeyListAssetToHashMap[AssetKey] = HashId
    end
  end
  print(bWriteLog and "CustomAssetManager:BuildCustomAssetKeyListMapping done, count:" .. tostring(#CustomAssetKeyList))
end
function CustomAssetManager:OnFightingStatusPreExit()
  ...
  if not self:IsDedicatedServer() then
    self:RemoveCommonEvent(EVENTTYPE_UGC, EVENTID_UGC_PREFAB_MALL_FIRST_META_COMPLETE)
  end
end
...
function CustomAssetManager:GetCustomAssetInfo(InAssetKey)
  ...
  if CustomAssetInfo == nil then
    CustomAssetInfo = TableUtil.FastCopyTable(CustomAssetInfoDefine)
...
function CustomAssetManager:_GetCustomAssetUseSuffixType(AssetKey)
  ...
  if AssetInfo ~= nil then
    ...
    if CustomAssetConfig ~= nil then
      if CustomAssetConfig.DefaultSuffixType then
        return CustomAssetConfig.DefaultSuffixType
      end
      if CustomAssetConfig.PlatformAsset ~= nil then
...
function CustomAssetManager:_NewCustomAssetDownloadCompletedInfo()
  return TableUtil.FastCopyTable(CustomAssetDownloadCompletedInfo)
end
function CustomAssetManager:_NewCustomAssetBatchDownloadCompletedInfo()
  return TableUtil.FastCopyTable(CustomAssetBatchDownloadCompletedInfoDefine)
end
function CustomAssetManager:_NewCustomAssetUploaderInfo()
  return TableUtil.FastCopyTable(CustomAssetUploaderInfoDefine)
end
function CustomAssetManager:_GetFallbackSuffixType(AssetKey, SuffixType)
  ...
  if CustomAssetConfig == nil or CustomAssetConfig.SuffixFallbackMap == nil then
    return nil
  end
  local FallbackSuffixType = CustomAssetConfig.SuffixFallbackMap[SuffixType]
  if FallbackSuffixType == nil or SuffixType == FallbackSuffixType then
    return nil
  end
  return FallbackSuffixType
end
function CustomAssetManager:_TryFallbackCustomAssetDownload(DownloadCompletedInfo, DownloadSuc, AssetKey, SuffixType)
  print("[CustomAsset][SuffixFallback] Try AssetKey:" .. tostring(AssetKey) .. " DownloadSuc:" .. tostring(DownloadSuc) .. " SuffixType:" .. tostring(SuffixType) .. " HasWaitInfo:" .. tostring(DownloadCompletedInfo ~= nil) .. " Tried:" .. tostring(DownloadCompletedInfo and DownloadCompletedInfo.bSuffixFallbackTried))
  if DownloadSuc == true or DownloadCompletedInfo == nil or DownloadCompletedInfo.bSuffixFallbackTried == true then
    return false
  end
  DownloadCompletedInfo.SuffixFallbackTriedMap = DownloadCompletedInfo.SuffixFallbackTriedMap or {}
  if SuffixType ~= nil then
    if DownloadCompletedInfo.SuffixFallbackTriedMap[SuffixType] == true then
      print("[CustomAsset][SuffixFallback] Skip repeated suffix AssetKey:" .. tostring(AssetKey) .. " SuffixType:" .. tostring(SuffixType))
      return false
    end
    DownloadCompletedInfo.SuffixFallbackTriedMap[SuffixType] = true
  end
  local FallbackSuffixType = self:_GetFallbackSuffixType(AssetKey, SuffixType)
  print("[CustomAsset][SuffixFallback] Map AssetKey:" .. tostring(AssetKey) .. " FromSuffixType:" .. tostring(SuffixType) .. " ToSuffixType:" .. tostring(FallbackSuffixType))
  if FallbackSuffixType == nil then
    return false
  end
  if DownloadCompletedInfo.SuffixFallbackTriedMap[FallbackSuffixType] == true then
    print("[CustomAsset][SuffixFallback] Skip loop fallback AssetKey:" .. tostring(AssetKey) .. " FromSuffixType:" .. tostring(SuffixType) .. " ToSuffixType:" .. tostring(FallbackSuffixType))
    return false
  end
  DownloadCompletedInfo.SuffixFallbackTriedMap[FallbackSuffixType] = true
  DownloadCompletedInfo.bSuffixFallbackTried = true
  print(bWriteLog and "[CustomAsset]CustomAssetManager:_TryFallbackCustomAssetDownload AssetKey:" .. tostring(AssetKey) .. " FromSuffixType:" .. tostring(SuffixType) .. " ToSuffixType:" .. tostring(FallbackSuffixType))
  local UsePriority = DownloadCompletedInfo.DownloadPriority or CustomAssetDownloadPriority.LOW
  if self.uCustomAssetCacheManager:IsAssetCached(AssetKey, FallbackSuffixType) then
    local AssetBytes = self.uCustomAssetCacheManager:LoadCustomAssetFromFile(AssetKey, FallbackSuffixType, UsePriority)
    self:_CustomAssetDownloadHandle(DownloadCompletedInfo, AssetBytes ~= nil, AssetKey, FallbackSuffixType, AssetBytes)
  else
    DownloadCompletedInfo.DownloadSeq = self.uCustomAssetDownloader:RequestDownloadAsset(AssetKey, DownloadCompletedInfo.bIsTemp == true, FallbackSuffixType, UsePriority, self._CustomAssetDownloadHandle, self, DownloadCompletedInfo)
  end
  return true
end
function CustomAssetManager:_CustomAssetDownloadHandle(AsyncLoadSeqOrDownloadCompletedInfo, DownloadSuc, AssetKey, SuffixType, AssetBytes)
  ...
  print("[CustomAsset][SuffixFallback] DownloadHandle AssetKey:" .. tostring(AssetKey) .. " DownloadSuc:" .. tostring(DownloadSuc) .. " SuffixType:" .. tostring(SuffixType) .. " ArgType:" .. tostring(type(AsyncLoadSeqOrDownloadCompletedInfo)) .. " HasWaitInfo:" .. tostring(DownloadCompletedInfo ~= nil))
  if self:_TryFallbackCustomAssetDownload(DownloadCompletedInfo, DownloadSuc, AssetKey, SuffixType) then
    return
  end
...
function CustomAssetManager:_GetAuthorLevelSizeLimit(CacheFieldName, ConfigFieldName, DefaultLimit, LogTag)
  ...
  if GameStatus.IsInLobbyOrMainCity() then
    ...
    if LogicUGCAuthor then
      AuthorLevel = LogicUGCAuthor:GetMineAuthorLevel()
    end
    if AuthorLevel == 0 then
      print(bWriteLog and "CustomAssetManager:_GetAuthorLevelSizeLimit, " .. LogTag .. " AuthorLevel = ", AuthorLevel)
      return DefaultLimit
    end
  else
    AuthorLevel = CGameState.GetAuthorLevel and CGameState:GetAuthorLevel() or 0
    if not AuthorLevel or AuthorLevel == 0 then
      print(bWriteLog and "CustomAssetManager:_GetAuthorLevelSizeLimit, " .. LogTag .. " level still not sync = ", AuthorLevel)
      return DefaultLimit
    end
  end
  local UGCAuthorLevelConfig = CDataTable.GetTableData("UGCAuthorLevelConfig", AuthorLevel)
  local LimitMB = UGCAuthorLevelConfig and UGCAuthorLevelConfig[ConfigFieldName]
  if type(LimitMB) == "number" then
    self[CacheFieldName] = LimitMB * CustomAssetDefine.MB
    print(bWriteLog and "CustomAssetManager:_GetAuthorLevelSizeLimit, " .. LogTag .. ", level, Size = ", AuthorLevel, LimitMB)
    return self[CacheFieldName]
  end
  return DefaultLimit
end
function CustomAssetManager:_GetServerPrefabLimit(FieldNameInGame, FieldName, LogTag)
  if GameStatus.IsInLobbyOrMainCity() then
    ...
    if LogicUGCAuthor then
      ...
      local LimitMB = AuthorInfo and AuthorInfo[FieldName] or 0
      self[FieldName] = LimitMB * CustomAssetDefine.MB
      print(bWriteLog and "CustomAssetManager:_GetServerPrefabLimit, " .. LogTag .. " Size = ", LimitMB)
      return LimitMB
    end
  else
    if self[FieldNameInGame] then
      return self[FieldNameInGame]
    end
    if CGameState.CreativeStateCustomAssetFeature then
      local LimitMB = CGameState.CreativeStateCustomAssetFeature[FieldNameInGame]
      if LimitMB ~= 0 then
        self[FieldNameInGame] = LimitMB * CustomAssetDefine.MB
        print(bWriteLog and "CustomAssetManager:_GetServerPrefabLimit InGame, " .. LogTag .. " Size = ", LimitMB)
        return LimitMB
      end
    end
  end
...
function CustomAssetManager:GetMapDependencySizeLimit()
  local ServerLimit = self:_GetServerPrefabLimit("PrefabTotalUseLimit", "prefab_total_use_limit", "GetMapDependencySizeLimit")
  if 0 < ServerLimit then
    return ServerLimit
  end
  return self:_GetAuthorLevelSizeLimit("MapDependencySizeLimit", "AssetCapacityLimit", CustomAssetDefine.MapDependencySizeLimit, "GetMapDependencySizeLimit")
end
function CustomAssetManager:GetAutoMountSizeLimit()
  local ServerLimit = self:_GetServerPrefabLimit("PrefabPreloadLimit", "prefab_preload_limit", "GetAutoMountSizeLimit")
  if 0 < ServerLimit then
    return ServerLimit
  end
  return self:_GetAuthorLevelSizeLimit("AutoMountSizeLimit", "AssetLoadLimit", CustomAssetDefine.AutoMountSizeLimit, "GetAutoMountSizeLimit")
end
function CustomAssetManager:GetDsMountableSizeLimit()
  local ServerLimit = self:_GetServerPrefabLimit("PrefabLoadLimit", "prefab_load_limit", "GetDsMountableSizeLimit")
  if 0 < ServerLimit then
    return ServerLimit
  end
  return self:_GetAuthorLevelSizeLimit("DsMountableSizeLimit", "AssetMountLimit", CustomAssetDefine.DsMountableSizeLimit, "GetDsMountableSizeLimit")
end
...
function CustomAssetManager:AsyncLoadCustomAssetToBytesImplementation(AssetKey, OutHandleFunc, ...)
  ...
  if not self.uCustomAssetCacheManager:IsAssetCached(AssetKey, UseSuffixType) then
    ...
    DownloadWaitInfo.DownloadSeq = self.uCustomAssetDownloader:RequestDownloadAsset(AssetKey, false, UseSuffixType, CustomAssetDownloadPriority.LOW, self._CustomAssetDownloadHandle, self, AsyncLoadSeq)
    return AsyncLoadSeq
  else
...
function CustomAssetManager:AsyncLoadCustomAssetImplementation(AssetKey, OutHandleFunc, OutDefaultAssetHandleFunc, ...)
  ...
  if not self.uCustomAssetCacheManager:IsAssetCached(AssetKey, UseSuffixType) then
    ...
    DownloadWaitInfo.bIsTemp = false
    DownloadWaitInfo.DownloadPriority = CustomAssetDownloadPriority.VERY_HIGH
    self.LoadSeqToDownloadCompletedInfo[AsyncLoadSeq] = DownloadWaitInfo
    DownloadWaitInfo.DownloadSeq = self.uCustomAssetDownloader:RequestDownloadAsset(AssetKey, DownloadWaitInfo.bIsTemp, UseSuffixType, DownloadWaitInfo.DownloadPriority, self._CustomAssetDownloadHandle, self, AsyncLoadSeq)
    return AsyncLoadSeq
  else
...
function CustomAssetManager:DeserializeCustomAsset(AssetBytes, CustomAssetType, SuffixType, OutHandleFunc, ...)
  ...
  if SuffixType == nil then
    SuffixType = CustomAssetDefine.Enum_CustomAssetSuffixType.Source
    local CustomAssetConfig = CustomAssetConfigs[CustomAssetType]
    if CustomAssetConfig ~= nil then
      if CustomAssetConfig.DefaultSuffixType then
        SuffixType = CustomAssetConfig.DefaultSuffixType
      elseif CustomAssetConfig.PlatformAsset ~= nil then
        if self.CurPlatformSuffixType == nil then
          self.CurPlatformSuffixType = self:GetCurPlatformSuffixType()
        end
        if CustomAssetConfig.PlatformAsset[self.CurPlatformSuffixType] == true then
          SuffixType = self.CurPlatformSuffixType
        end
      end
    end
  end
...
function CustomAssetManager:CustomAssetIsCacheed(InAssetKey)
  ...
  local UseSuffixType = self:_GetCustomAssetUseSuffixType(AssetKey)
  if self.uCustomAssetCacheManager:IsAssetCached(AssetKey, UseSuffixType) then
    return true
  end
  local FallbackSuffixType = self:_GetFallbackSuffixType(AssetKey, UseSuffixType)
  if FallbackSuffixType ~= nil then
    return self.uCustomAssetCacheManager:IsAssetCached(AssetKey, FallbackSuffixType)
  end
  return false
end
...
function CustomAssetManager:DownloadCustomAsset(InAssetKey, bIsTemp, InOptions, OutHandleFunc, ...)
  local Options = type(InOptions) == "table" and InOptions or nil
  local args = table.pack(...)
  local AssetKey = InAssetKey
  local UseSuffixType = self:_GetCustomAssetUseSuffixType(AssetKey)
  print(bWriteLog and "[CustomAsset]CustomAssetManager:DownloadCustomAsset UseSuffixType:" .. tostring(UseSuffixType))
  local UseDownloadType = Options and Options.DownloadType or CustomAssetDefine.CustomAssetDownloadType.DownloadOnly
  local UsePriority = Options and Options.Priority or CustomAssetDownloadPriority.LOW
  ...
  if not self:IsCustomAssetKey(AssetKey, false) then
    ...
    if OutHandleFunc ~= nil then
      ...
      if UseDownloadType == CustomAssetDefine.CustomAssetDownloadType.DownloadAndLoad then
        local AssetBytes = self.uCustomAssetCacheManager:LoadCustomAssetFromFile(AssetKey, UseSuffixType, UsePriority)
        handle(AssetBytes)
      else
  ...
  if not self.uCustomAssetCacheManager:IsAssetCached(AssetKey, UseSuffixType) then
    ...
    DownloadWaitInfo.DownloadType = UseDownloadType
    DownloadWaitInfo.bIsTemp = bIsTemp == true
    DownloadWaitInfo.DownloadPriority = UsePriority
    DownloadWaitInfo.DownloadSeq = self.uCustomAssetDownloader:RequestDownloadAsset(AssetKey, DownloadWaitInfo.bIsTemp, UseSuffixType, DownloadWaitInfo.DownloadPriority, self._CustomAssetDownloadHandle, self, DownloadWaitInfo)
...
function CustomAssetManager:DownloadCustomAssetList(AssetKeyList, OutHandleFunc, ...)
  ...
  if 0 < #NeedDownloadAssetKeyList then
    ...
    for i = 1, #NeedDownloadAssetKeyList do
      ...
      BatchDownloadWaitInfo.DownloadMap[AssetKey] = self:DownloadCustomAsset(AssetKey, false, nil, self._CustomAssetBatchDownloadHandle, self, BatchDownloadSeq)
    end
...
function CustomAssetManager:IsDsAutoMountReplicateComplete()
  if slua.isValid(CGameState) and CGameState.GetCustomAssetMountStateComponent ~= nil then
    local uMountStateComponent = CGameState:GetCustomAssetMountStateComponent()
    if slua.isValid(uMountStateComponent) and uMountStateComponent.IsDsAutoMountAllReplicated ~= nil then
      return uMountStateComponent:IsDsAutoMountAllReplicated()
    end
  end
  return false
end
function CustomAssetManager:_GenerateDsAutoMountCallBackID()
  local NewCallBackID = self._CurDsAutoMountCallBackID
  self._CurDsAutoMountCallBackID = self._CurDsAutoMountCallBackID + 1
  return NewCallBackID
end
function CustomAssetManager:AddDsAutoMountReplicateCompleteCallBack(handleFunc, ...)
  print(bWriteLog and "[CustomAsset]CustomAssetManager:AddDsAutoMountReplicateCompleteCallBack")
  local common = require("client.slua_ui_framework.common")
  local args = table.pack(...)
  local handle = function(...)
    return common.CallCombinationArgs(handleFunc, args, ...)
  end
  if self:IsDsAutoMountReplicateComplete() then
    handle()
    return -1
  end
  local CallBackID = self:_GenerateDsAutoMountCallBackID()
  self._DsAutoMountReplicateCompleteCallBackMap[CallBackID] = handle
  return CallBackID
end
function CustomAssetManager:RemoveDsAutoMountReplicateCompleteCallBack(CallBackID)
  print(bWriteLog and "[CustomAsset]CustomAssetManager:RemoveDsAutoMountReplicateCompleteCallBack CallBackID:" .. tostring(CallBackID))
  if self._DsAutoMountReplicateCompleteCallBackMap[CallBackID] then
    self._DsAutoMountReplicateCompleteCallBackMap[CallBackID] = nil
  end
end
function CustomAssetManager:OnDsAutoMountAllReplicated()
  print(bWriteLog and "[CustomAsset]CustomAssetManager:OnDsAutoMountAllReplicated")
  if next(self._DsAutoMountReplicateCompleteCallBackMap) == nil then
    return
  end
  local _CallBackMap = self._DsAutoMountReplicateCompleteCallBackMap
  self._DsAutoMountReplicateCompleteCallBackMap = {}
  local CommonUtility = require("common.utility")
  for CallBackID, handle in pairs(_CallBackMap) do
    local CallSuc = xpcall(handle, CommonUtility.ErrorMessageHandler)
    if not CallSuc then
      print(bWriteLog and "[CustomAsset]CustomAssetManager:OnDsAutoMountAllReplicated CallHandle Xpcall failed CallBackID:" .. tostring(CallBackID))
    end
  end
end
function CustomAssetManager:OnPrefabMallDataUpdateCompleted(_, _)
  print(bWriteLog and "[CustomAsset]CustomAssetManager:OnPrefabMallDataUpdateCompleted")
  for AssetKey, CustomAssetConfig in pairs(CustomAssetConfigs) do
    if CustomAssetConfig.OnPrefabMallDataUpdateCompletedHandle ~= nil then
      CustomAssetConfig:OnPrefabMallDataUpdateCompletedHandle(self:IsDedicatedServer())
    end
  end
end
function CustomAssetManager:OnMountStateChange(_, _, AssetKey)
  ...
  if MountInfo ~= nil then
    if MountInfo.MountStatus == EMountStatus.Mounted and self.CallInMountedHandleAssetMap[AssetKey] ~= true then
      local CustomAssetInfo = self:GetCustomAssetInfo(AssetKey)
      if CustomAssetInfo ~= nil then
        local CustomAssetConfig = CustomAssetConfigs[CustomAssetInfo.AssetType]
        if CustomAssetConfig ~= nil and CustomAssetConfig.InMountedHandle ~= nil then
          self.CallInMountedHandleAssetMap[AssetKey] = true
          CustomAssetConfig:InMountedHandle(AssetKey, self:IsDedicatedServer())
        end
      end
    end
  elseif self.CallInMountedHandleAssetMap[AssetKey] == true then
    self.CallInMountedHandleAssetMap[AssetKey] = nil
    local CustomAssetInfo = self:GetCustomAssetInfo(AssetKey)
    if CustomAssetInfo ~= nil then
      local CustomAssetConfig = CustomAssetConfigs[CustomAssetInfo.AssetType]
      if CustomAssetConfig ~= nil and CustomAssetConfig.OutMountedHandle ~= nil then
        CustomAssetConfig:OutMountedHandle(AssetKey, self:IsDedicatedServer())
      end
    end
  end
  self:_AssetBatchMountStatusCheck(AssetKey)
end
function CustomAssetManager:_AssetBatchMountStatusCheck(AssetKey)
  if not Client then
    return
  end
  print(bWriteLog and "[CustomAsset]_AssetBatchMountStatusCheck AssetKey:" .. tostring(AssetKey))
  local MountInfo = self:GetCustomAssetMountedInfo(AssetKey)
  if MountInfo == nil then
    return
  end
  ...
  if self:IsCustomAssetKeyHashID(AssetKeyOrHashID) then
    ...
    if CustomAssetKey ~= nil then
      ...
      if WaitCustomAssetHashIDInfo == nil then
        WaitCustomAssetHashIDInfo = TableUtil.FastCopyTable(WaitCustomAssetHashIDInfoDefine)
...
function CustomAssetManager:GetCustomAssetKeyByHashID(HashID)
  ...
  if not self:IsDedicatedServer() and self.CustomAssetKeyListHashToAssetMap and self.CustomAssetKeyListHashToAssetMap[HashID] then
    return self.CustomAssetKeyListHashToAssetMap[HashID]
  end
  return nil
end
function CustomAssetManager:GetHashIDByCustomAssetKey(AssetKey)
  ...
  if not self:IsDedicatedServer() and self.CustomAssetKeyListAssetToHashMap and self.CustomAssetKeyListAssetToHashMap[AssetKey] then
    return self.CustomAssetKeyListAssetToHashMap[AssetKey]
  end
  return 0
end
...
function CustomAssetManager:GetCustomAssetKeyByHashIDInLobby(HashId, custom_asset_key_list)
  local HashToAssetMap = self:GenerateHashToAssetMap(custom_asset_key_list)
  return HashToAssetMap and HashToAssetMap[HashId] or nil
end
function CustomAssetManager:GenerateHashToAssetMap(custom_asset_key_list)
  if not custom_asset_key_list then
    return nil
  end
  local HashToAssetMap = {}
  for _, AssetKey in ipairs(custom_asset_key_list) do
    local HashId = self:GenerateCustomAssetKeyHashID(AssetKey)
    HashToAssetMap[HashId] = AssetKey
  end
  return HashToAssetMap
end
function CustomAssetManager:AsyncLoadAndAllocPackagePath(CustomAssetKey, OutHandleFunc, ...)
  print(bWriteLog and "CustomAssetManager:AsyncLoadAndAllocPackagePath CustomAssetKey:" .. tostring(CustomAssetKey))
  local common = require("client.slua_ui_framework.common")
  local args = table.pack(...)
  local handle = function(...)
    return common.CallCombinationArgs(OutHandleFunc, args, ...)
  end
  if not self:IsCustomAssetKey(CustomAssetKey) then
    handle(nil)
    return
  end
  local AssetInfo = self:GetCustomAssetInfo(CustomAssetKey)
  if self.AllocPackagePathMap[AssetInfo.AssetKey] ~= nil then
    handle(self.AllocPackagePathMap[AssetInfo.AssetKey].PackagePath)
    return
  end
  self:AsyncLoadCustomAsset(CustomAssetKey, function(uCustomAsset)
    if not slua.isValid(uCustomAsset) then
      handle(nil)
      return
    end
    slua.addRef(uCustomAsset)
    local CustomAssetTextureUtil = require("common.CustomAsset.CustomImage.CustomAssetTextureUtil")
    local AssetPackagePath = CustomAssetTextureUtil.GetObjectPath(uCustomAsset)
    local AssetPackagePathInfo = {PackagePath = AssetPackagePath, uCustomAsset = uCustomAsset}
    self.AllocPackagePathMap[AssetInfo.AssetKey] = AssetPackagePathInfo
    handle(AssetPackagePath)
  end)
end
