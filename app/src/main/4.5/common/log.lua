...
local safe_format = function(formatStr, ...)
  if type(formatStr) ~= "string" then
    local args = {
      ...
    }
    local parts = {
      "[log_format_type_error: fmt=" .. tostring(formatStr)
    }
    for i = 1, #args do
      parts[#parts + 1] = " arg" .. i .. "=" .. tostring(args[i])
    end
    parts[#parts + 1] = "]"
    return table.concat(parts)
  end
  return string_format(formatStr, ...)
end
function _G.assert_format(condition, formatStr, ...)
  if not condition then
    local msg = safe_format(formatStr, ...)
    local utility = require("common.utility")
...
function _G.log_format(formatStr, ...)
  ...
  log(safe_format(formatStr, ...))
end
function _G.log_warning_format(formatStr, ...)
  log_warning(safe_format(formatStr, ...))
end
function _G.log_error_format(formatStr, ...)
  log_error(safe_format(formatStr, ...))
end
